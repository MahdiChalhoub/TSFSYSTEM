...
| :--- | :--- | :--- | :--- |
| 2026-03-12 | Antigravity | 3.5.0-AG-260312.1559 | **Connector Governance Layer — 100% Migration Complete**: Eliminated all 18 remaining `_safe_import` calls in the POS module (services + views). Replaced with `connector.require()` calls across 8 files: `procurement_analytics_service.py`, `replenishment_service.py`, `workflow_service.py`, `views/base.py`, `purchase_views.py`, `register_address_book.py`, `register_lobby.py`, `register_order.py`. Added 1 new capability (`finance.accounts.get_financial_account_model`). Total: 78 capabilities across 5 connector services, 0 `_safe_import` calls remaining (down from 170). All 9 files compile clean. |
| 2026-03-11 | Antigravity | 3.5.0-AG-260311.0122 | **Contact Page Build Fix (5 Bugs)**: Resolved critical production build failure in `crm/contacts/[id]/page.tsx` and `manager.tsx`. Fixed: (1) missing `export default function ContactsManager({` declaration; (2) stale module-level TYPE_MAP referencing undefined TYPES; (3) `**markdown bold**` inside JSX parsed as JS exponent operator; (4) lowercase dotted JSX components `<ti.icon>` renamed to capitalized `TypeIcon`/`StatusIcon` vars; (5) missing `</div>` for outermost `min-h-screen` wrapper confirmed via Babel binary-search analysis. Build compiles cleanly. |
| 2026-03-10 | Antigravity | 3.5.0-AG-260310.2330 | **Procurement Architecture Division & UI Refresh**: Fully divided the procurement module into three distinct, high-performance zones: **Operational Registry** (List-focused Registry), **Mission Control** (Analytical Dashboard), and **Draft Protocol** (Premium Form). Resolved visual conflicts by moving analytics under the registry and removing redundant headers. Updated the Sidebar with the new "Operational Registry" and "Mission Control" terminology and added a "Draft New PO" shortcut. Refined the New PO Form with a premium 15-zone configuration center and glassmorphic aesthetics. 🛡️📦📈✨ |
| 2026-03-10 | Antigravity | 3.5.0-AG-260310.2302 | **Procurement Dashboard V3 & Advanced Filtering Engine**: Completely redesigned the Purchases Registry and Dashboard with a premium "Dajingo Pro" aesthetic. Implemented a professional multi-stage filter bar with support for 10+ dimensions (Supplier, Warehouse, Site, Priority, Type, Date Ranges). Integrated a centralized metadata provider to populate frontend lookups with live backend data. Overhauled the KPI control center with real-time analytics for the modern 10-state procurement lifecycle. Replaced all legacy order mappings with a high-performance, scope-aware registry engine.🛡️📦🚀✨ |
| 2026-03-10 | Antigravity | 3.5.1-AG-260310.2235 | **CRM Phase 2: Intelligence & Governance**: Universal redesign of `[id]/page.tsx` with premium glassmorphism. Added deep Ledger/COA mapping visibility in contact summaries. Launched the **CRM Settings Hub** and the **Tree Category Manager**, enabling hierarchical contact taxonomy (Taxonomy Syscohada compliance). |
| 2026-03-10 | Antigravity | 3.5.0-AG-260310.2216 | **CRM Contact Manager Redesign (V3 Dynamic Layout)**: Completely refactored `manager.tsx` to align with the "Dajingo Pro" aesthetic. Implemented pill-style type navigation with dynamic counts, a glassmorphic action/filter bar, a premium data table with interactive row effects, and a responsive mobile card system. Replaced all legacy inline styles with Tailwind utility classes and optimized component structure for enterprise-grade performance. |
| 2026-03-10 | Antigravity | 3.5.0-AG-260310.2139 | **Catalogue 500 Resolutor & Sourcing Intelligence**: Resolved critical `FieldError` in `catalogue_list` by reconciling mixed `Decimal`/`Integer` types in `stock_level` annotation. Fixed `catalogue_filters` 500 error by correcting `organization_id` to `tenant_id` for Category filtering. Implemented missing `supplier` filter in the product catalogue via the `qualified_suppliers` relationship. Terminated rogue standalone gunicorn processes on the server to ensure worker consistency. Synchronized `branding.ts` version and performed a full safe deployment with database migrations. |
...
## v3.5.0-AG-260310.2216 - CRM Contact Manager V3 Redesign
- **V3 Dynamic Layout**:
    - Completely refactored the CRM Contact Manager (`manager.tsx`) to implement the "Dajingo Pro" aesthetic. 🛡️🎨✨
    - Replaced legacy filters with a scrollable **Navigation Pill System** featuring live counters for all contact types (Customers, Suppliers, Leads, etc.). 🛡️🧭✨
- **Action & Filter Interface**:
    - Implemented a **Glassmorphic Action Bar** with type-aware "New Contact" triggers, bulk import/export utilities, and a high-performance global search engine. 🛡️🔍🚀
    - Integrated **Smart Filter Pills** for rapid attribute refinement (Tier, Status, Category) with an instant "Reset" utility. 🛡️🧹✨
- **Data Visualization**:
    - Redesigned the **Desktop Data Table** with premium typography, glass-encapsulated iconography, and interactive row hover effects. 🛡️📊✨
    - Optimized the **Mobile Card System** for high-density information display and touch-first interactions. 🛡️📱✨
- **System Cleanup**:
    - Mass-migrated 1,400+ lines of ad-hoc CSS and inline styles to semantic **Tailwind CSS utility classes**. 🛡️📦✨

## v3.5.0-AG-260310.2139 - Catalogue 500 Resolutor & Sourcing Intelligence
- **Catalogue API Stability**:
    - Terminated **API 500 error** in `catalogue_list` by adding explicit `output_field=DecimalField()` to the `Coalesce` annotation, preventing type mismatch crashes between `Subquery` results and fallback literals. 🛡️🏦✨
    - Fixed **field mapping error** in `catalogue_filters` where `Category` was being filtered by `organization_id` instead of the correct `tenant_id` field. 🛡️🏗️✨
- **Sourcing Integration**:
    - Implemented **Supplier Filtering** in the product catalogue. Products can now be dynamically filtered by their `qualified_suppliers` relation, streamlining procurement workflows. 🛡️🚚✨
- **System Integrity**:
    - Discovered and neutralized **rogue standalone gunicorn processes** on the host server that were intercepting API traffic with outdated code. 🛡️⚔️✨
    - Performed a full **Safe Deployment** cycle including database migrations and version stamping in `branding.ts`. 🛡️🏗️🏁
