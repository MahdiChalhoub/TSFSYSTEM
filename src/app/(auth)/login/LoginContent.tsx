...
export function LoginContent({ initialSubdomain = '' }: { initialSubdomain?: string }) {
    ...
    // Subdomain is known at SSR time (passed as prop) — no flash, no useEffect needed
    const [subdomain] = useState(initialSubdomain);

    useEffect(() => {
        getPublicConfig().then(setConfig).catch(() => { });

        // Already logged in check — redirect to dashboard if valid session
                        ...
                        {state?.two_factor_required ? (
                            <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                                ...
                                <div className="text-center">
                                    <button type="button" onClick={() => window.location.reload()} className="text-xs font-bold text-slate-500 hover:text-white uppercase tracking-widest transition-colors">
                                        Cancel & Restart
                                    </button>
                                </div>
                            </div>
                            ...
                            <>
                                <div className="space-y-4">
                                    {/* Workspace field — only shown on root domain (no subdomain) */}
                                    {isRoot && (
                                        <div className="space-y-2">
