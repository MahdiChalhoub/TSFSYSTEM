            ...
            <div className="max-w-4xl mx-auto relative z-10 space-y-6">
                ...
                {!loading && wallet && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in duration-500">
                        <div className="p-6 bg-gradient-to-br from-amber-600/20 to-amber-900/20 border border-app-warning/20 rounded-3xl space-y-2">
                            <div className="flex items-center gap-2">
                                <Wallet size={20} className="text-app-warning" />
                                <p className="text-app-warning/70 text-xs font-bold uppercase tracking-widest">Wallet Balance</p>
                            </div>
                            <p className="text-4xl font-black text-white">${parseFloat(wallet.balance).toFixed(2)}</p>
                        </div>
                        <div className="p-6 bg-gradient-to-br from-purple-600/20 to-purple-900/20 border border-app-accent/20 rounded-3xl space-y-2">
                            <div className="flex items-center gap-2">
                                <Sparkles size={20} className="text-app-accent" />
                                <p className="text-app-accent/70 text-xs font-bold uppercase tracking-widest">Loyalty Points</p>
                            </div>
                            <p className="text-4xl font-black text-white">{wallet.loyalty_points.toLocaleString()}</p>
                            <p className="text-xs text-app-accent/50">Lifetime: {wallet.lifetime_points.toLocaleString()} pts</p>
                        </div>
                    </div>
                )}
                ...
                {loading && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[1, 2].map(i => <div key={i} className="h-32 bg-app-surface/60 rounded-3xl animate-pulse" />)}
                    </div>
                )}
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-app-muted-foreground text-[10px] uppercase tracking-widest">Current Rank</p>
                                </div>
                            </div>
                            {tierInfo.next && (
                                <div className="text-right">
                                    <p className={`text-xs font-bold ${tierInfo.next.accent}`}>{tierInfo.next.name}</p>
                                    <p className="text-app-muted-foreground text-[10px]">{tierInfo.next.min - (wallet.lifetime_points || 0)} pts to go</p>
                                </div>
                            )}
                        </div>
                        <div className="space-y-2">
                            <div className="w-full h-3 bg-app-surface-2 rounded-full overflow-hidden">
                                <div className={`h-full ${tierInfo.current.bar} rounded-full transition-all duration-1000`}
                                    style={{ width: `${tierInfo.progress}%` }} />
                            </div>
                            <div className="flex justify-between text-[10px] text-app-faint font-medium">
                    ...
                    <div className="p-6 bg-app-surface/60 border border-white/5 rounded-3xl space-y-4">
                                ...
                                <button key={amt} onClick={() => setTopUpAmount(amt)}
                                    className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all border
                                        ${topUpAmount === amt
                                            ...
                                            : 'bg-white/5 text-app-muted-foreground border-white/5 hover:border-white/20'
                                        }`}>
                                    ${amt}
                                </button>
                            ))}
                        ...
                        <div className="flex gap-3">
                            <input type="number" placeholder="Custom amount"
                                ...
                                className="flex-1 bg-app-surface-2/60 border border-white/5 text-white px-4 py-3 rounded-xl outline-none focus:border-app-success/30 placeholder:text-app-faint" />
                            <button onClick={handleTopUpRequest} disabled={topUpSent || !topUpAmount}
                                className="px-6 py-3 bg-app-primary-dark hover:bg-app-primary text-white rounded-xl font-bold transition-all disabled:opacity-40 flex items-center gap-2">
                                {topUpSent ? <><CheckCircle2 size={16} /> Requested!</> : 'Request'}
                            </button>
                        </div>
                        <p className="text-app-faint text-xs">Top-up requests are processed by the store administrator</p>
                    </div>
                )}
                ...
                <div className="space-y-3">
                    ...
                    {transactions.length === 0 ? (
                        <div className="py-12 text-center text-app-muted-foreground">No transactions yet</div>
                    ) : (
                        <div className="space-y-2">
                            {transactions.map(txn => {
                                ...
                                return (
                                    <div key={txn.id}
                                        className="p-4 bg-app-surface/60 border border-white/5 rounded-xl flex items-center gap-4 hover:border-white/10 transition-all">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-app-muted-foreground text-[11px]">{new Date(txn.created_at).toLocaleString()}</p>
                                        </div>
                                        <p className={`font-bold text-lg ${isCredit ? 'text-app-success' : 'text-app-error'}`}>
