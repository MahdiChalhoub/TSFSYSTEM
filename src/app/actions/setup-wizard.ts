...
export async function getWizardConfig() {
    try {
        const [config, currencies, refCountries, businessTypes, coaTemplates, warehouses, modules, coaItems, posAccounts] = await Promise.all([
            erpFetch('auth/config/').catch(() => ({})),
            erpFetch('currencies/').catch(() => []),
            erpFetch('reference/countries/').catch(() => []),
        ...
        return {
            ...
            countries: Array.isArray(refCountries) ? refCountries : refCountries?.results || [],
        ...
        // Expected when at SaaS root with no tenant context — safe fallback
        return {
            currencies: [], countries: [], businessTypes: [], coaTemplates: [], warehouses: [], modules: [], coaItems: [], posAccounts: [], tenant: {},
        }
    }
}
...
export async function checkSetupReadiness(): Promise<{
    ...
    try {
        ...
        // Safe fallback for legacy orgs, SaaS root, or total API failure
        return {
            ready: true,
            hasCompanyType: true, hasCurrency: true, hasFiscalYear: true,
            hasCOA: true, hasWarehouse: true, hasPOSAccount: true
