...
export async function loginAction(prevState: any, formData: FormData) {
    ...
    if (data.slug && data.slug.toString().trim() !== '') {
        ...
        // Strip port for comparison (e.g., "saas.developos.shop:3000" → "saas.developos.shop")
        const host = rawHost.split(':')[0];
        const hostPort = rawHost.includes(':') ? rawHost.split(':')[1] : '';

        // CHECK IF HOST IS AN IP ADDRESS (IPv4)
        const isIp = /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/.test(host);

        // Dynamically extract root domain from hostname or use env fallback
        const envRoot = process.env.NEXT_PUBLIC_ROOT_DOMAIN || 'localhost';
        const hostParts = host.split('.');
        let baseDomain = envRoot;

        // If explicitly on a multi-part domain that is NOT our root domain,
        // try to extract the base (e.g. customer.their-domain.com → their-domain.com)
        if (hostParts.length >= 3 && !host.endsWith(envRoot)) {
            baseDomain = hostParts.slice(1).join('.');
        }

        // If already on the correct subdomain, skip redirect entirely
        const alreadyOnSlug = host === `${slug}.${baseDomain}` || host.startsWith(`${slug}.`);

        if (isIp || alreadyOnSlug) {
            // IP address or already on correct subdomain — proceed with login, no redirect
        }
        else {
            // Domain-based logic
            let protocol = "http";
            const isLocalhost = host.includes('localhost');
            if (headersList.get('x-forwarded-proto') === 'https' || (!isLocalhost && !isIp)) {
                protocol = "https";
            }

            let newHost = "";
            if (isLocalhost) {
                newHost = `${slug}.localhost${hostPort ? ':' + hostPort : ':3000'}`;
            } else {
                newHost = `${slug}.${baseDomain}`;
            }

            // Only redirect if we are not already on that host
            if (host !== newHost && host !== newHost.split(':')[0]) {
