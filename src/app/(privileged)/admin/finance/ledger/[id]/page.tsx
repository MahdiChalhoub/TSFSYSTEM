    ...
    return (
        <div className="p-8 max-w-4xl mx-auto">
            ...
            <div className="flex justify-between items-center mb-8">
                <Link href="/admin/finance/ledger" className="flex items-center gap-2 text-stone-500 hover:text-stone-900 transition-colors text-sm font-medium">
                    <ArrowLeft size={16} /> Back to Ledger
                </Link>
                <div className="flex gap-3">
                    <button className="flex items-center gap-2 px-4 py-2 border border-stone-200 rounded-lg text-sm font-bold text-stone-600 hover:bg-stone-50 transition-all">
                        <Printer size={16} /> Print Voucher
                    </button>
                    {entry.status !== 'REVERSED' && (
                        <Link
                            href={`/admin/finance/ledger/${entry.id}/edit`}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden mb-8">
                <div className="p-8 border-b border-stone-100 flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-stone-500 text-sm">JV Number: <span className="font-mono font-bold text-stone-900">#{entry.id}</span></p>
                    </div>
                    <div className="text-right">
                        <div className="text-sm text-stone-400 font-bold uppercase tracking-widest mb-1">Transaction Date</div>
                        <div className="text-xl font-bold text-stone-900">{entry.transactionDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</div>
                    </div>
                </div>

                <div className="grid grid-cols-2 p-8 gap-12 bg-stone-50/50">
                    <div>
                        <div className="text-[10px] font-bold uppercase text-stone-400 tracking-widest mb-2 flex items-center gap-1.5">
                            <FileText size={12} /> Description
                        </div>
                        <p className="text-stone-800 font-medium leading-relaxed">{entry.description || 'No description provided'}</p>
                    </div>
                    <div>
                        <div className="text-[10px] font-bold uppercase text-stone-400 tracking-widest mb-2">Reference</div>
                        <p className="text-stone-800 font-mono font-bold">{entry.reference || 'N/A'}</p>
                    </div>
                </div>

                {/* Reversal Metadata Footer */}
                {(entry.reversalOf || entry.reversedBy) && (
                    <div className="px-8 py-4 border-t border-stone-100 bg-white space-y-2">
                        {entry.reversalOf && (
                            <div className="flex items-center gap-2 text-rose-600 text-xs font-bold bg-rose-50 px-3 py-2 rounded-lg border border-rose-100 w-fit">
                                ...
                                <Link href={`/admin/finance/ledger/${entry.reversalOf.id}`} className="underline ml-2">View Original</Link>
                            </div>
                        )}
                        {entry.reversedBy && (
                            <div className="flex items-center gap-2 text-amber-600 text-xs font-bold bg-amber-50 px-3 py-2 rounded-lg border border-amber-100 w-fit">
                                ...
                                <Link href={`/admin/finance/ledger/${entry.reversedBy.id}`} className="underline ml-2">View Reversal</Link>
                            </div>
                        )}
                    </div>
                )}
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-stone-50 border-b border-stone-200">
                            <th className="px-6 py-4 text-[10px] font-bold uppercase text-stone-500 tracking-widest">Account</th>
                            <th className="px-6 py-4 text-[10px] font-bold uppercase text-stone-500 tracking-widest">Description</th>
                            <th className="px-6 py-4 text-[10px] font-bold uppercase text-stone-500 tracking-widest text-right">Debit</th>
                            <th className="px-6 py-4 text-[10px] font-bold uppercase text-stone-500 tracking-widest text-right">Credit</th>
                        </tr>
                    </thead>
                    ...
                    <tfoot>
                        <tr className="bg-stone-50/80 font-bold border-t-2 border-stone-200">
                            <td colSpan={2} className="px-6 py-5 text-sm text-stone-900">Total Voucher Value</td>
                            <td className="px-6 py-5 text-right font-mono text-stone-900 h-6">
            ...
            <div className="mt-6 flex flex-col items-center gap-2">
                ...
                <p className="text-stone-400 text-[10px] italic">Verified by Trial Balance Guard</p>
            </div>
        </div>
    )
}
...
function getStatusStyle(status: string) {
    switch (status) {
        ...
        case 'DRAFT': return 'bg-stone-50 text-stone-600 border-stone-100'
