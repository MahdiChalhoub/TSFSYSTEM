                ...
                {filtered.length === 0 ? (
                    ...
                    <div key={s.id} className="flex items-center gap-3 px-4 py-3 border-b border-app-border/20 hover:bg-app-background/50 transition-colors">
                        ...
                        <div className="w-[120px] flex items-center justify-center gap-1">
                            ...
                            {!['COMPLETED', 'CANCELLED'].includes(s.status) && <button onClick={() => doAction('cancel', s.id)} disabled={isPending} className="p-1.5 rounded-lg hover:bg-rose-500/10" title="Cancel"><XCircle size={13} className="text-rose-500" /></button>}
                        </div>
                    </div>
                ))}
            </div>
