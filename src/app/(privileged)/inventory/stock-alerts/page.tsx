...
type StockAlerts = Record<string, any>

const ALL_COLUMNS: ColumnDef<StockAlerts>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function StockAlertsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<StockAlerts[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_stock-alerts', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/stock-alerts/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<StockAlerts>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/stock-alerts/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/stock-alerts/${r.id}`),
          onEdit: (r) => router.push(`/inventory/stock-alerts/${r.id}/edit`),
        }}
      >
        <TypicalFilter
