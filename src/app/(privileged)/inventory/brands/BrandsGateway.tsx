...
const MobileClient = dynamic(
    () => import('./mobile/MobileBrandsClient').then(m => m.MobileBrandsClient),
    { ssr: false, loading: () => <Skeleton /> }
)

const DesktopClient = dynamic(
    () => import('@/components/admin/BrandManager').then(m => m.BrandManager),
    { ssr: false, loading: () => <Skeleton /> }
)
    ...
    return (
        <div className="flex flex-col p-4 gap-3 animate-pulse" style={{ height: 'calc(100dvh - 6rem)' }}>
            ...
            <div className="flex-1 space-y-2">
                {[...Array(5)].map((_, i) => (
                    <div key={i} className="h-20 rounded-2xl bg-app-surface/20" />
                ))}
            </div>
        </div>
    )
...
export function BrandsGateway(props: any) {
