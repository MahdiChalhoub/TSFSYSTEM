...
/**
 * Audit Tab — last 50 changes for this brand (create / update / delete).
 * Diff-style rendering: for UPDATE, shows field-level before → after pairs.
 * Queries /api/inventory/brands/<id>/audit/ (generic AuditLog table).
 */
...
export function AuditTab({ brand }: { brand: Brand }) {
    ...
    useEffect(() => {
        let cancelled = false
        setLoading(true)
        erpFetch(`inventory/brands/${brand.id}/audit/`)
            .then((res: any) => {
                if (cancelled) return
    ...
    if (entries.length === 0) {
        return (
            <div className="rounded-xl py-8 px-4 text-center m-3"
                ...
                <p className="text-tp-xs" style={{ color: 'var(--app-muted-foreground)' }}>
                    Changes to this brand will appear here once audit logging records them.
                </p>
            </div>
        )
    }
                ...
                return (
                    <div key={e.id} className="p-3 rounded-xl"
                        ...
                        <div className="flex items-center gap-2 mb-1.5">
                            ...
                            <span className="text-tp-xxs font-bold uppercase tracking-widest" style={{ color: tint }}>{e.action}</span>
                            {e.actor && <span className="text-tp-xs font-medium" style={{ color: 'var(--app-foreground)' }}>{e.actor}</span>}
                            <span className="text-tp-xxs ml-auto" style={{ color: 'var(--app-muted-foreground)' }}>
                                {new Date(e.timestamp).toLocaleString()}
                            </span>
                        </div>
                        {e.description && <p className="text-tp-sm" style={{ color: 'var(--app-foreground)' }}>{e.description}</p>}
                        {diff.length > 0 && (
                            <div className="mt-2 space-y-0.5 text-tp-xs font-mono">
                                {diff.map(d => (
                                    <div key={d.key} className="flex items-start gap-2">
                                        <span className="font-bold uppercase tracking-widest" style={{ color: 'var(--app-muted-foreground)', minWidth: 90 }}>{d.key}</span>
                                        <span style={{ color: 'var(--app-error, #ef4444)', textDecoration: 'line-through' }}>{fmtVal(d.before)}</span>
                                        <span style={{ color: 'var(--app-muted-foreground)' }}>→</span>
                                        <span style={{ color: 'var(--app-success, #22c55e)' }}>{fmtVal(d.after)}</span>
                                    </div>
                                ))}
                            </div>
                        )}
