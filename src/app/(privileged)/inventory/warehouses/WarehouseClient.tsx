...
const TYPE_CONFIG: Record<string, { icon: any; color: string; label: string; bg: string }> = {
 BRANCH: { icon: Building2, color: 'text-app-primary', label: 'Branch / Site', bg: 'bg-app-primary/5' },
 STORE: { icon: Store, color: 'text-app-primary', label: 'Store', bg: 'bg-app-primary-light' },
 WAREHOUSE: { icon: Warehouse, color: 'text-app-muted-foreground', label: 'Warehouse', bg: 'bg-app-background' },
 VIRTUAL: { icon: Cloud, color: 'text-app-primary', label: 'Virtual', bg: 'bg-violet-50' },
};
...
export function WarehouseClient({ initialWarehouses }: { initialWarehouses: any[] }) {
 ...
 <div>
 <p className="font-black text-app-foreground tracking-tight">{row.name}</p>
 <div className="flex items-center gap-2">
 <span className="text-[10px] text-app-muted-foreground font-mono uppercase tracking-wider">{row.code || `LOC-${row.id}`}</span>
 ...
 label: 'Parent',
 render: (row: any) => (
 <div className="flex items-center gap-1.5 text-app-muted-foreground font-medium">
 ...
 render: (row: any) => (
 <Badge className="bg-app-surface text-app-foreground border-none font-mono">
 ...
 >
 {!row.is_active && (
 <div className="absolute inset-0 bg-app-foreground/60 backdrop-blur-[2px] z-10 flex items-center justify-center">
 <Badge className="bg-app-surface-2 text-app-foreground font-black text-[10px] tracking-[0.2em] px-4 py-2 rounded-xl">INACTIVE</Badge>
 </div>
 )}
 ...
 <h3 className="text-2xl font-black text-app-foreground mb-2 truncate group-hover:text-app-primary transition-colors">{row.name}</h3>
 <div className="flex items-center gap-2 text-xs text-app-muted-foreground font-bold uppercase tracking-tight mb-1">
 <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black ${cfg.bg} ${cfg.color}`}>{cfg.label}</span>
 {row.parent_name && (
 <>
 <MapPin size={12} className="text-app-primary" />
 <span className="truncate">{row.parent_name}</span>
 </>
 )}
 </div>
 {(row.city || row.phone) && (
 <div className="flex items-center gap-3 text-[11px] text-app-muted-foreground mt-2">
 ...
 <div>
 <div className="text-[9px] text-app-muted-foreground font-black uppercase tracking-widest mb-1">Inventory</div>
 <div className="flex items-baseline gap-1">
 <span className="text-3xl font-black text-app-foreground">{row.inventory_count || 0}</span>
 <span className="text-[10px] font-black text-app-muted-foreground uppercase">SKUs</span>
 </div>
 </div>
 {row.can_sell ? (
 <div className="px-3 py-1.5 bg-app-primary text-app-foreground text-[9px] font-black uppercase rounded-lg shadow-lg shadow-app-primary/20 tracking-widest">
 Point of Sale
 </div>
 ) : (
 <div className="px-3 py-1.5 bg-app-background text-app-muted-foreground text-[9px] font-black uppercase rounded-lg border border-app-border tracking-widest">
 No Sales
 </div>
 )}
 </div>
 ...
 <div>
 <p className="text-[10px] font-black text-app-primary uppercase tracking-widest">Branches</p>
 ...
 <div>
 <p className="text-[10px] font-black text-app-primary uppercase tracking-widest">Retail Points</p>
 <h2 className="text-2xl font-black text-app-foreground mt-0.5 tracking-tighter">{retailActive}</h2>
 </div>
 </CardContent>
 </Card>

 <Card className="rounded-[2rem] border-0 shadow-sm bg-gradient-to-br from-slate-900 to-slate-800 text-app-foreground overflow-hidden group hover:shadow-xl transition-all">
 <CardContent className="p-5 flex items-center gap-4">
 <div className="w-12 h-12 rounded-2xl bg-app-foreground/10 text-app-foreground flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm backdrop-blur-sm">
 <BarChart3 size={24} />
 </div>
 <div>
 <p className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest">Global SKUs</p>
 <h2 className="text-2xl font-black text-app-foreground mt-0.5 tracking-tighter">{globalSKUCount}</h2>
 </div>
 </CardContent>
 </Card>
 </div>
