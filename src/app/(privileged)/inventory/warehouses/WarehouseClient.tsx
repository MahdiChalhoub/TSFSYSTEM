...
const TYPE_CONFIG: Record<string, { icon: any; color: string; label: string; bg: string }> = {
    ...
    WAREHOUSE: { icon: Warehouse, color: 'text-slate-600', label: 'Warehouse', bg: 'bg-slate-50' },
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] text-gray-400 font-mono uppercase tracking-wider">{row.code || `LOC-${row.id}`}</span>
        ...
        return (
            ...
            >
                ...
                <div className="mb-6">
                    ...
                    <h3 className="text-2xl font-black text-gray-900 mb-2 truncate group-hover:text-indigo-600 transition-colors">{row.name}</h3>
                    <div className="flex items-center gap-2 text-xs text-stone-500 font-bold uppercase tracking-tight mb-1">
                ...
                <div className="pt-6 border-t border-stone-50 flex justify-between items-center">
                    <div>
                        <div className="text-[9px] text-stone-400 font-black uppercase tracking-widest mb-1">Inventory</div>
                    ...
                    {row.can_sell ? (
                        ...
                        <div className="px-3 py-1.5 bg-slate-50 text-slate-400 text-[9px] font-black uppercase rounded-lg border border-slate-100 tracking-widest">
                            No Sales
                        </div>
                    )}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
                <Card className="rounded-[2rem] border-0 shadow-sm bg-gradient-to-br from-indigo-50 to-white overflow-hidden group hover:shadow-md transition-all">
                    <CardContent className="p-5 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h2 className="text-2xl font-black text-gray-900 mt-0.5 tracking-tighter">{branchCount}</h2>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="rounded-[2rem] border-0 shadow-sm bg-gradient-to-br from-slate-50 to-white overflow-hidden group hover:shadow-md transition-all">
                    <CardContent className="p-5 flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">All Locations</p>
                            <h2 className="text-2xl font-black text-gray-900 mt-0.5 tracking-tighter">{totalNodes}</h2>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="rounded-[2rem] border-0 shadow-sm bg-gradient-to-br from-emerald-50 to-white overflow-hidden group hover:shadow-md transition-all">
                    <CardContent className="p-5 flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-white text-emerald-600 flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm">
                            <Store size={24} />
                        </div>
                        <div>
                            <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Retail Points</p>
                            <h2 className="text-2xl font-black text-gray-900 mt-0.5 tracking-tighter">{retailActive}</h2>
                        </div>
                    </CardContent>
                </Card>
