...
import { getWeightPolicy, updateWeightPolicy } from '@/app/actions/plm-governance'
import { Scale, Save, RefreshCw, Settings, AlertTriangle } from 'lucide-react'
...
export default function WeightPolicyPage() {
    const [policy, setPolicy] = useState<WeightPolicy>({})
    ...
    async function loadData() {
        setLoading(true)
        const res = await getWeightPolicy()
        if (res.success) setPolicy((res.data as WeightPolicy) || {})
        else setPolicy({})
        setLoading(false)
    }
    ...
    async function handleSave() {
        ...
        const res = await updateWeightPolicy(payload)
        if (res.success) { toast.success('Weight policy updated'); loadData() }
            ...
            <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center"
                        style={{ background: 'linear-gradient(135deg, var(--app-warning), var(--app-accent))', boxShadow: '0 4px 15px color-mix(in srgb, var(--app-warning) 30%, transparent)' }}>
            ...
            <div className="space-y-4">
                <div className="rounded-xl p-5" style={{ background: 'var(--app-surface)', border: '1px solid var(--app-border)' }}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-[10px] font-bold text-app-muted-foreground mb-1">Default Weight Unit</label>
                            <select value={policy?.default_weight_unit || ''} onChange={e => setPolicy({ ...policy, default_weight_unit: e.target.value })}
                        ...
                        <div>
                            <label className="block text-[10px] font-bold text-app-muted-foreground mb-1">Tolerance %</label>
                            <input type="number" step="0.1" value={policy?.weight_tolerance_pct || ''} onChange={e => setPolicy({ ...policy, weight_tolerance_pct: parseFloat(e.target.value) || 0 })}
                                placeholder="e.g. 2.0" className="w-full px-3 py-2.5 rounded-xl text-sm outline-none"
                                style={{ background: 'var(--app-bg)', border: '1px solid var(--app-border)', color: 'var(--app-foreground)' }} />
                        </div>
                    </div>
                </div>
