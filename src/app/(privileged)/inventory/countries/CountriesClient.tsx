...
export function CountriesClient({ initialCountries, categories }: { initialCountries: any[], categories: any[] }) {
        ...
        },
        {
            ...
            render: (row: any) => (
                <div>
                    ...
                    <p className="text-[10px] text-gray-400 font-medium">Origin Node</p>
                </div>
            )
        },
        {
    ...
    const renderCard = (row: any) => (
        <div
            className="group p-6 rounded-[2.5rem] bg-white border border-gray-100 shadow-sm hover:shadow-2xl hover:shadow-indigo-900/5 transition-all relative overflow-hidden h-full flex flex-col justify-between cursor-pointer"
    ...
    return (
        <div className="p-6 space-y-6 bg-gray-50/50">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {data.map(brand => (
                    <div key={brand.id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="divide-y divide-gray-50">
                            {brand.products.map((p: any) => (
                                <div key={p.id} className="px-4 py-3 flex justify-between items-center text-sm hover:bg-gray-50 transition-colors">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200 p-4">
            <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="px-8 py-6 border-b border-gray-100">
                    ...
                    <p className="text-gray-400 text-sm font-medium">Manufacturing & Origin Details</p>
                </div>
                ...
                >
                    <div className="space-y-1.5">
                        ...
                        <input name="name" defaultValue={country?.name} className="w-full h-12 px-4 rounded-xl border border-gray-100 bg-gray-50/50 focus:bg-white focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all font-semibold" placeholder="e.g. France" required />
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">ISO Alpha-2 Code</label>
                        <input name="code" defaultValue={country?.code} className="w-full h-12 px-4 rounded-xl border border-gray-100 bg-gray-50/50 focus:bg-white focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all font-mono font-bold uppercase" placeholder="FR" required maxLength={3} />
                    </div>
