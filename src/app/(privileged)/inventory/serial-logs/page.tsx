...
type SerialLogs = Record<string, any>

const ALL_COLUMNS: ColumnDef<SerialLogs>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function SerialLogsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<SerialLogs[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_serial-logs', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/serial-logs/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load serial-logs:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<SerialLogs>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/serial-logs/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/serial-logs/${r.id}`),
          onEdit: (r) => router.push(`/inventory/serial-logs/${r.id}/edit`),
        }}
      >
        <TypicalFilter
