...
export default function SerialLogsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`inventory/serial-logs/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`inventory/serial-logs/${id}/`, {
        method: 'DELETE'
      })
      router.push('/inventory/serial-logs')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Serial-Logs #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage serial logs details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/inventory/serial-logs/${id}/edit`)}
            >
