'use client'
/**
 * Internal Consumption Events — Management Page
 * ================================================
 * Dajingo Pro V2 Design Language
 * Tracks auto-livraison / internal consumption of inventory.
...
import {
  ...
  RefreshCw, Package, DollarSign, CheckCircle2, Clock
} from 'lucide-react'

type Evt = Record<string, any>

const TRIGGERS = ['INTERNAL_USE','EMPLOYEE_BENEFIT','CROSS_BRANCH','EXEMPT_PROJECT','MARKETING','OTHER']
const TRIGGER_LABELS: Record<string, string> = {
  INTERNAL_USE: 'Internal Use', EMPLOYEE_BENEFIT: 'Employee Benefit',
  CROSS_BRANCH: 'Cross-Branch', EXEMPT_PROJECT: 'Exempt Project',
...
function EvtRow({ item, onEdit, onAssess }: { item: Evt; onEdit: () => void; onAssess: () => void }) {
    ...
    <div className="group flex items-center gap-2 md:gap-3 transition-all duration-150 cursor-pointer border-b border-app-border/30 hover:bg-app-surface/40 py-2.5 px-3"
      ...
      <div className="flex-1 min-w-0">
        ...
        <span className="text-[10px] font-bold text-app-muted-foreground truncate block">
          {item.description || `Event #${item.id}`}
        </span>
      </div>
      <div className="hidden sm:block w-24 flex-shrink-0 text-right">
        <div className="text-[10px] font-bold text-app-muted-foreground">FMV</div>
        <div className="font-mono text-[13px] font-black text-app-foreground tabular-nums">
          {parseFloat(item.fair_market_value || 0).toLocaleString()}
        </div>
      </div>
      <div className="hidden md:block w-24 flex-shrink-0 text-right">
        <div className="text-[10px] font-bold text-app-muted-foreground">VAT Due</div>
...
function EvtEditor({ item, onSave, onCancel }: { item: Evt | null; onSave: (d: Evt) => void; onCancel: () => void }) {
  const [form, setForm] = useState<Evt>(item || {
    event_date: new Date().toISOString().split('T')[0], trigger_type: 'INTERNAL_USE',
    description: '', cost_value: 0, fair_market_value: 0, vat_rate: 0.18,
    vat_amount: 0, currency_code: 'XOF', status: 'PENDING', notes: '',
  })
  const [saving, setSaving] = useState(false)
  const upd = (k: string, v: any) => setForm(p => ({ ...p, [k]: v }))
  const vatCalc = parseFloat(form.fair_market_value || 0) * parseFloat(form.vat_rate || 0)
  ...
  return (
    <div className="flex flex-col h-full p-4 md:p-6 animate-in fade-in duration-300">
      <div className="flex items-center gap-3 mb-4 flex-shrink-0">
        ...
        <button onClick={async () => { setSaving(true); await onSave({ ...form, vat_amount: vatCalc }); setSaving(false) }} disabled={saving}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4">
        ...
        <div className="rounded-2xl p-4" style={{ background: 'linear-gradient(135deg, color-mix(in srgb, var(--app-primary) 6%, transparent), color-mix(in srgb, var(--app-surface) 80%, transparent))', border: '1px solid color-mix(in srgb, var(--app-primary) 15%, transparent)' }}>
          <div className="text-[9px] font-black text-app-muted-foreground uppercase tracking-widest mb-2">LIVE VAT CALCULATION</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px' }}>
            {[
              { label: 'Cost', value: parseFloat(form.cost_value || 0), color: 'var(--app-muted-foreground)' },
              { label: 'Fair Market Value', value: parseFloat(form.fair_market_value || 0), color: 'var(--app-info, #3b82f6)' },
              { label: `VAT (${(parseFloat(form.vat_rate || 0) * 100).toFixed(0)}%)`, value: vatCalc, color: 'var(--app-warning, #f59e0b)' },
            ].map(c => (
        ...
        <div className="rounded-2xl p-4" style={{ background: 'color-mix(in srgb, var(--app-surface) 50%, transparent)', border: '1px solid color-mix(in srgb, var(--app-border) 50%, transparent)' }}>
          <div className="text-[9px] font-black text-app-muted-foreground uppercase tracking-widest mb-3">EVENT DETAILS</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>
            <div><label className={labelCls}>Event Date</label><input className={inputCls} type="date" value={form.event_date} onChange={e => upd('event_date', e.target.value)} /></div>
            <div><label className={labelCls}>Trigger Type</label>
              <select className={inputCls} value={form.trigger_type} onChange={e => upd('trigger_type', e.target.value)}>
        ...
        <div className="rounded-2xl p-4" style={{ background: 'color-mix(in srgb, var(--app-surface) 50%, transparent)', border: '1px solid color-mix(in srgb, var(--app-border) 50%, transparent)' }}>
          <div className="text-[9px] font-black text-app-muted-foreground uppercase tracking-widest mb-3">VALUATION</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>
            ...
            <div><label className={labelCls}>Fair Market Value</label><input className={inputCls} type="number" min={0} step={0.01} value={form.fair_market_value} onChange={e => upd('fair_market_value', e.target.value)} /></div>
            <div><label className={labelCls}>VAT Rate</label><input className={inputCls} type="number" min={0} max={1} step={0.0001} value={form.vat_rate} onChange={e => upd('vat_rate', e.target.value)} /><p className="text-[9px] font-bold text-app-muted-foreground mt-0.5">Decimal (0.18 = 18%)</p></div>
          </div>
        </div>
        <div className="rounded-2xl p-4" style={{ background: 'color-mix(in srgb, var(--app-surface) 50%, transparent)', border: '1px solid color-mix(in srgb, var(--app-border) 50%, transparent)' }}>
          <div className="text-[9px] font-black text-app-muted-foreground uppercase tracking-widest mb-3">DESCRIPTION & NOTES</div>
          <div className="space-y-3">
            <div><label className={labelCls}>Description</label><input className={inputCls} value={form.description} onChange={e => upd('description', e.target.value)} placeholder="e.g. 50 units of Product X used for office supplies" /></div>
...
export default function SelfSupplyVATPage() {
  ...
  async function load() {
    try {
      ...
      const [d, s] = await Promise.all([
        erpFetch('finance/self-supply-vat/'),
        erpFetch('finance/self-supply-vat/dashboard/'),
      ])
  ...
  async function handleSave(data: Evt) {
    try {
      if (data.id) {
        await erpFetch(`finance/self-supply-vat/${data.id}/`, { method: 'PATCH', body: JSON.stringify(data) })
      } else {
        await erpFetch('finance/self-supply-vat/', { method: 'POST', body: JSON.stringify(data) })
      }
  ...
  async function handleAssess(id: number) {
    try {
      await erpFetch(`finance/self-supply-vat/${id}/assess/`, { method: 'POST' })
      load()
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(i =>
      i.trigger_type?.toLowerCase().includes(q) ||
  ...
  const kpis = [
    { label: 'Total Events', value: dashboard.total_events ?? items.length, icon: <Package size={11} />, color: 'var(--app-primary)' },
    { label: 'Pending', value: dashboard.pending ?? 0, icon: <Clock size={11} />, color: 'var(--app-warning, #f59e0b)' },
    { label: 'Assessed', value: dashboard.assessed ?? 0, icon: <CheckCircle2 size={11} />, color: 'var(--app-info, #3b82f6)' },
    { label: 'Total VAT Due', value: (dashboard.total_vat_due ?? 0).toLocaleString(), icon: <DollarSign size={11} />, color: 'var(--app-error, #ef4444)' },
  ]

  if (editing) {
    return <EvtEditor item={editing === 'new' ? null : editing} onSave={handleSave} onCancel={() => setEditing(null)} />
  }
    ...
    <div className={`flex flex-col h-full p-4 md:p-6 animate-in fade-in duration-300 transition-all ${focusMode ? 'max-h-[calc(100vh-4rem)]' : 'max-h-[calc(100vh-8rem)]'}`}>
        ...
        <div className="flex items-center gap-2">
          {focusMode && (
            <div className="flex items-center gap-2 flex-shrink-0">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'var(--app-primary)' }}><Package size={14} style={{ color: '#fff' }} /></div>
              <span className="text-[12px] font-black text-app-foreground hidden sm:inline">Self-Supply</span>
            </div>
          )}
          <div className="flex-1 relative">
            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-app-muted-foreground" />
            <input ref={searchRef} type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              placeholder="Search events... (Ctrl+K)" className="w-full pl-9 pr-3 py-2 text-[12px] md:text-[13px] bg-app-surface/50 border border-app-border/50 rounded-xl text-app-foreground placeholder:text-app-muted-foreground focus:bg-app-surface focus:border-app-border focus:ring-2 focus:ring-app-primary/10 outline-none transition-all" />
          </div>
          {focusMode && <>
      ...
      <div className="flex-1 min-h-0 bg-app-surface/30 border border-app-border/50 rounded-2xl overflow-hidden flex flex-col">
        <div className="flex-shrink-0 flex items-center gap-2 md:gap-3 px-3 py-2 bg-app-surface/60 border-b border-app-border/50 text-[10px] font-black text-app-muted-foreground uppercase tracking-wider">
          ...
          <div className="flex-1 min-w-0">Trigger / Description</div>
          <div className="hidden sm:block w-24 flex-shrink-0 text-right">FMV</div>
          <div className="hidden md:block w-24 flex-shrink-0 text-right">VAT Due</div>
          <div className="hidden lg:block w-20 flex-shrink-0">Status</div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {loading ? (
            ...
            filtered.map(item => <EvtRow key={item.id} item={item} onEdit={() => setEditing(item)} onAssess={() => handleAssess(item.id)} />)
          ) : (
            <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
              <Package size={36} className="text-app-muted-foreground mb-3 opacity-40" />
              <p className="text-sm font-bold text-app-muted-foreground">No self-supply events</p>
              <p className="text-[11px] text-app-muted-foreground mt-1">Track internal consumption VAT by creating a new event.</p>
            </div>
          )}
        </div>
      </div>
