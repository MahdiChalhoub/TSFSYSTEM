...
export default function ProductLocationsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`inventory/product-locations/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`inventory/product-locations/${id}/`, {
        method: 'DELETE'
      })
      router.push('/inventory/product-locations')
    } catch (error) {
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `Product-Locations #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage product locations details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/inventory/product-locations/${id}/edit`)}
            >
