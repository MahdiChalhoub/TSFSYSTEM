...
type ProductLocations = Record<string, any>

const ALL_COLUMNS: ColumnDef<ProductLocations>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function ProductLocationsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<ProductLocations[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_product-locations', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/product-locations/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<ProductLocations>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/product-locations/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/product-locations/${r.id}`),
          onEdit: (r) => router.push(`/inventory/product-locations/${r.id}/edit`),
        }}
      >
        <TypicalFilter
