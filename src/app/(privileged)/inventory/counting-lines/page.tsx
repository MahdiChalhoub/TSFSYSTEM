...
type CountingLines = Record<string, any>

const ALL_COLUMNS: ColumnDef<CountingLines>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function CountingLinesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<CountingLines[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_counting-lines', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/counting-lines/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load counting-lines:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<CountingLines>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/counting-lines/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/counting-lines/${r.id}`),
          onEdit: (r) => router.push(`/inventory/counting-lines/${r.id}/edit`),
        }}
      >
        <TypicalFilter
