...
export default function CountingLinesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`inventory/counting-lines/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`inventory/counting-lines/${id}/`, {
        method: 'DELETE'
      })
      router.push('/inventory/counting-lines')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Counting-Lines #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage counting lines details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/inventory/counting-lines/${id}/edit`)}
            >
