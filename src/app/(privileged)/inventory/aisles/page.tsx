...
type Aisles = Record<string, any>

const ALL_COLUMNS: ColumnDef<Aisles>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function AislesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<Aisles[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_aisles', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/aisles/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Aisles>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/aisles/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/aisles/${r.id}`),
          onEdit: (r) => router.push(`/inventory/aisles/${r.id}/edit`),
        }}
      >
        <TypicalFilter
