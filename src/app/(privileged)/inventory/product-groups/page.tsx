...
type ProductGroups = Record<string, any>

const ALL_COLUMNS: ColumnDef<ProductGroups>[] = [
...
export default function ProductGroupsListPage() {
  ...
  const settings = useListViewSettings('inventory_product-groups', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/product-groups/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<ProductGroups>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/product-groups/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/product-groups/${r.id}`),
          onEdit: (r) => router.push(`/inventory/product-groups/${r.id}/edit`),
        }}
      >
        <TypicalFilter
