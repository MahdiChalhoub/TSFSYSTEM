...
export default function ProductGroupsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`inventory/product-groups/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`inventory/product-groups/${id}/`, {
        method: 'DELETE'
      })
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Product-Groups #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage product groups details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/inventory/product-groups/${id}/edit`)}
            >
