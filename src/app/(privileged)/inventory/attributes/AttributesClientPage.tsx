                                            ...
                                            <div onClick={() => toggleExpand(group.id)} className="group flex items-center gap-3 hover:bg-app-surface py-3 px-4 transition-all cursor-pointer border-b border-app-border/20" style={{ borderLeft: '3px solid var(--app-primary)', background: 'color-mix(in srgb, var(--app-primary) 3%, var(--app-surface))' }}>
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                                                        ...
                                                        {group.show_in_name && <span className="text-[9px] font-bold uppercase tracking-wider bg-amber-500/10 text-amber-600 px-1.5 py-0.5 rounded" title={`Position: ${group.name_position}`}>📝 In Name{group.short_label ? ` (${group.short_label})` : ''}</span>}
                                                        {group.is_required && <span className="text-[9px] font-bold uppercase tracking-wider bg-red-500/10 text-red-500 px-1.5 py-0.5 rounded">⭐ Required</span>}
                                                        {group.requires_barcode && <span className="text-[9px] font-bold uppercase tracking-wider bg-orange-500/10 text-orange-600 px-1.5 py-0.5 rounded">🔖 Barcode</span>}
                                                ...
                                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    <button onClick={e => { e.stopPropagation(); handleDeleteGroup(group.id) }} className="p-1.5 hover:bg-red-500/10 rounded-lg text-red-500"><Trash2 size={12} /></button>
                                                </div>
                                            </div>
                                                    ...
                                                    {group.children.map(child => (
                                                        <div key={child.id} className="group flex items-center gap-3 py-2 px-4 hover:bg-app-surface/50 border-b border-app-border/10 ml-6 border-l border-app-border/30">
                                                            ...
                                                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100">
                                                                ...
                                                                <button onClick={() => handleDeleteValue(child.id)} className="p-1.5 hover:bg-red-500/10 rounded-lg text-red-500"><Trash2 size={12} /></button>
                                                            </div>
                                                        </div>
                                                    ))}
