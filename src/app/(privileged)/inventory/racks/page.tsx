...
type Racks = Record<string, any>

const ALL_COLUMNS: ColumnDef<Racks>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function RacksListPage() {
  const router = useRouter()
  const [items, setItems] = useState<Racks[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_racks', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/racks/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Racks>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/racks/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/racks/${r.id}`),
          onEdit: (r) => router.push(`/inventory/racks/${r.id}/edit`),
        }}
      >
        <TypicalFilter
