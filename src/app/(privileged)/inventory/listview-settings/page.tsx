    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden transition-all duration-200 hover:shadow-sm">
            ...
            >
                ...
                <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-400">{visibleCount}/{totalCount} columns</span>
                    <span className="text-xs text-gray-400">{settings.pageSize}/page</span>
                    {expanded ? <ChevronUp className="h-4 w-4 text-gray-400" /> : <ChevronDown className="h-4 w-4 text-gray-400" />}
                </div>
            </button>

            {expanded && (
                <div className="border-t border-gray-100 px-5 py-5 space-y-6 animate-in slide-in-from-top-2 duration-200">
                        ...
                        <div className="space-y-1">
                            {settings.columnOrder
                                ...
                                .map((key, idx) => {
                                    ...
                                    return (
                                        <div key={key}
                                            className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${isVisible ? 'bg-white border border-gray-100' : 'bg-gray-50/50 border border-transparent'
                                                }`}>
                                            <Checkbox
                                                checked={isVisible}
                                            ...
                                            />
                                            <span className={`flex-1 text-sm ${isVisible ? 'text-gray-900' : 'text-gray-400 line-through'}`}>
                                                {col.label}
                                            </span>
                                            {col.alwaysVisible && (
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <Label className="text-xs text-gray-500">Default Sort</Label>
                        ...
                        {settings.filterPresets.length > 0 ? (
                            <div className="space-y-1 mb-3">
                                {settings.filterPresets.map(p => (
                                    <div key={p.name} className="flex items-center justify-between px-3 py-2 bg-gray-50 rounded-lg">
                            ...
                            <p className="text-xs text-gray-400 mb-3">No presets saved yet. Save filter combinations for quick access.</p>
                        )}
