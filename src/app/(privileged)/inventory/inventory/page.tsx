...
const ALL_COLUMNS: ColumnDef<Inventory>[] = [
...
export default function InventoryListPage() {
  ...
  const settings = useListViewSettings('inventory_inventory', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/inventory/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Inventory>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/inventory/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/inventory/${r.id}`),
          onEdit: (r) => router.push(`/inventory/inventory/${r.id}/edit`),
        }}
      >
        <TypicalFilter
