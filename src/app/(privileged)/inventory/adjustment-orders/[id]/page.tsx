...
export default function AdjustmentOrdersDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`inventory/adjustment-orders/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`inventory/adjustment-orders/${id}/`, {
        method: 'DELETE'
      })
      router.push('/inventory/adjustment-orders')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Adjustment-Orders #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage adjustment orders details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/inventory/adjustment-orders/${id}/edit`)}
            >
