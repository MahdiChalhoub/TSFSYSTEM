...
export default function DiscrepancyStrategyPage() {
    ...
    const columns: ColumnDef<any>[] = ALL_COLUMNS.map(c => {
        const renderers: Record<string, (r: any) => React.ReactNode> = {
            ...
            reference: r => <span className="font-mono font-bold text-gray-900">STRAT-{r.reference || r.id}</span>,
                ...
                expandable={{
                    ...
                    renderActions: (row) => (
                        <div className="flex gap-2">
                            ...
                            <Button size="sm" variant="ghost" className="h-7 text-[10px] font-bold text-gray-400 flex items-center gap-1"><Info size={12} /> VIEW AUDIT TRAIL</Button>
                        </div>
                    ),
                }}
                lifecycle={{
                ...
                <DialogContent className="rounded-[2rem] border-0 shadow-2xl">
                    ...
                    <div className="space-y-4 pt-4">
                        <div className="space-y-2">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Target Terminal Location</Label>
                        ...
                        <div className="space-y-2">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Governance Reason</Label>
                            <Input className="rounded-xl border-gray-100 h-12" placeholder="Describe the discrepancy analysis..." value={newReason} onChange={e => setNewReason(e.target.value)} />
                        </div>
                    </div>
                    <DialogFooter className="pt-6">
                        <Button variant="ghost" onClick={() => setShowCreate(false)} className="rounded-xl font-bold">Cancel</Button>
                        <Button onClick={handleCreate} disabled={isPending} className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl px-8 font-bold h-12 shadow-lg shadow-emerald-200">Initialize Discussion</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
