...
export default function AdjustmentOrdersPage() {
    ...
    async function loadData() {
        try {
            ...
            setOrders(asArr(ordersRes) as AdjustmentOrder[])
                                                                ...
                                                                <Table>
                                                                    ...
                                                                    <TableBody>
                                                                        {(order.lines ?? []).map((line) => {
                                                                            ...
                                                                            const qty = Number(ln.qty_adjustment ?? 0)
                                                                            const amount = ln.amount_adjustment as number | string | undefined
                                                                            ...
                                                                            <TableRow key={id}>
                                                                                ...
                                                                                <TableCell className={`text-sm text-right font-semibold ${qty >= 0 ? 'text-app-success' : 'text-app-error'}`}>
                                                                                    {qty > 0 ? '+' : ''}{qty}
                                                                                </TableCell>
                                                                                <TableCell className="text-sm text-right">
                                                                                    {amount ? `$${parseFloat(String(amount)).toLocaleString('en', { minimumFractionDigits: 2 })}` : '—'}
                                                                                </TableCell>
                            ...
                            <div key={i} className="flex items-start gap-3 border-l-2 border-muted pl-4 py-2">
                                <div className="flex-1">
                                    ...
                                    <p className="text-xs text-muted-foreground mt-1">
                                        {by || 'System'} · {at ? new Date(at).toLocaleString() : '—'}
                                    </p>
