...
type Bins = Record<string, any>

const ALL_COLUMNS: ColumnDef<Bins>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function BinsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<Bins[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_bins', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/bins/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Bins>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/bins/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/bins/${r.id}`),
          onEdit: (r) => router.push(`/inventory/bins/${r.id}/edit`),
        }}
      >
        <TypicalFilter
