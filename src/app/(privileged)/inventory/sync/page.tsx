...
const ALL_COLUMNS: ColumnDef<Sync>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function SyncListPage() {
  const router = useRouter()
  const [items, setItems] = useState<Sync[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_sync', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/sync/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Sync>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/sync/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/sync/${r.id}`),
          onEdit: (r) => router.push(`/inventory/sync/${r.id}/edit`),
        }}
      >
        <TypicalFilter
