...
type Parfums = Record<string, any>

const ALL_COLUMNS: ColumnDef<Parfums>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function ParfumsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<Parfums[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_parfums', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/parfums/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load parfums:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Parfums>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/parfums/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/parfums/${r.id}`),
          onEdit: (r) => router.push(`/inventory/parfums/${r.id}/edit`),
        }}
      >
        <TypicalFilter
