...
type Shelves = Record<string, any>

const ALL_COLUMNS: ColumnDef<Shelves>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function ShelvesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<Shelves[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_shelves', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/shelves/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Shelves>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/shelves/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/shelves/${r.id}`),
          onEdit: (r) => router.push(`/inventory/shelves/${r.id}/edit`),
        }}
      >
        <TypicalFilter
