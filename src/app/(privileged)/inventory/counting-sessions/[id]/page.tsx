...
export default function CountingSessionsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`inventory/counting-sessions/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`inventory/counting-sessions/${id}/`, {
        method: 'DELETE'
      })
      router.push('/inventory/counting-sessions')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Counting-Sessions #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage counting sessions details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/inventory/counting-sessions/${id}/edit`)}
            >
