...
export default function StockMovesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`inventory/stock-moves/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`inventory/stock-moves/${id}/`, {
        method: 'DELETE'
      })
      router.push('/inventory/stock-moves')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Stock-Moves #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage stock moves details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/inventory/stock-moves/${id}/edit`)}
            >
