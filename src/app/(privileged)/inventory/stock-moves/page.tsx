...
type StockMoves = Record<string, any>

const ALL_COLUMNS: ColumnDef<StockMoves>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function StockMovesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<StockMoves[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_stock-moves', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/stock-moves/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<StockMoves>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/stock-moves/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/stock-moves/${r.id}`),
          onEdit: (r) => router.push(`/inventory/stock-moves/${r.id}/edit`),
        }}
      >
        <TypicalFilter
