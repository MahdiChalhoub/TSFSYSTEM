...
const ALL_COLUMNS: ColumnDef<Zones>[] = [
...
export default function ZonesListPage() {
  ...
  const settings = useListViewSettings('inventory_zones', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('inventory/zones/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Zones>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/zones/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/zones/${r.id}`),
          onEdit: (r) => router.push(`/inventory/zones/${r.id}/edit`),
        }}
      >
        <TypicalFilter
