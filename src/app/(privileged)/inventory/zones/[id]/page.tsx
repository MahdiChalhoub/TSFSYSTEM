...
export default function ZonesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`inventory/zones/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`inventory/zones/${id}/`, {
        method: 'DELETE'
      })
      router.push('/inventory/zones')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Zones #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage zones details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/inventory/zones/${id}/edit`)}
            >
