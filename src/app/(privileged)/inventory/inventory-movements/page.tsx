...
type InventoryMovements = Record<string, any>

const ALL_COLUMNS: ColumnDef<InventoryMovements>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function InventoryMovementsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<InventoryMovements[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('inventory_inventory-movements', {
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<InventoryMovements>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/inventory/inventory-movements/new')}
        ...
        actions={{
          onView: (r) => router.push(`/inventory/inventory-movements/${r.id}`),
          onEdit: (r) => router.push(`/inventory/inventory-movements/${r.id}/edit`),
        }}
      >
        <TypicalFilter
