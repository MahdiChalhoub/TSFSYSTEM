...
export default function InventoryMovementsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`inventory/inventory-movements/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`inventory/inventory-movements/${id}/`, {
        method: 'DELETE'
      })
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `Inventory-Movements #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage inventory movements details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/inventory/inventory-movements/${id}/edit`)}
            >
