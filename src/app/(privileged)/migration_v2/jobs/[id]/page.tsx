...
export default function JobsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`migration_v2/jobs/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`migration_v2/jobs/${id}/`, {
        method: 'DELETE'
      })
      router.push('/migration_v2/jobs')
    } catch (error) {
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `Jobs #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage jobs details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/migration_v2/jobs/${id}/edit`)}
            >
