        ...
        <tr className="hover:bg-gray-50/60 transition-colors group">
            <td className="py-6 px-8">
                <div className="flex flex-col gap-1">
                    <span className="font-bold text-gray-900 group-hover:text-emerald-700 transition-colors">{product.name}</span>
            ...
            <td className="py-6 px-8">
                <div className="flex flex-col gap-1">
                    ...
                    {product.barcode ? (
                        <span className="text-[10px] text-gray-400 font-bold flex items-center gap-1 mt-0.5 tracking-tight">
            ...
            <td className="py-6 px-8">
                <div className="flex flex-col gap-0.5">
                    <span className={`text-lg font-black tracking-tighter ${totalStock > 0 ? 'text-gray-900 group-hover:text-emerald-800' : 'text-red-500'}`}>
                        {totalStock.toLocaleString()}
                    </span>
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1.5 py-0.5 bg-gray-50 rounded-lg w-fit">
            ...
            <td className="py-6 px-8 text-right">
                <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all scale-95 group-hover:scale-100">
                    ...
                    <Link
                        ...
                        className="p-2.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all border border-transparent hover:border-blue-100 shadow-sm hover:shadow-md"
...
export function GroupRow({ group }: { group: Record<string, any> }) {
    ...
    return (
        <tr className="hover:bg-emerald-50/20 transition-all group bg-gray-50/20">
            <td className="py-6 px-8">
                <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="font-black text-gray-900 text-lg tracking-tighter group-hover:text-emerald-700 transition-colors">{group.name}</span>
                    </div>
                    <span className="text-[10px] font-black text-gray-400 pl-11 uppercase tracking-widest">{group.brand?.name} ΓÇó {group.category?.name}</span>
                </div>
            </td>
            <td className="py-6 px-8">
                <div className="flex flex-col gap-2 pl-2 border-l-2 border-emerald-100 group-hover:border-emerald-400 transition-colors">
                    ...
                    <div className="flex gap-1 flex-wrap">
                        {uniqueCountries.map((c: any) => (
                            <span key={c} className="text-[9px] font-black bg-white border border-gray-100 px-2 py-0.5 rounded shadow-sm text-gray-600 uppercase">
                                {c}
                            </span>
                        ))}
                    </div>
            ...
            <td className="py-6 px-8">
                <div className="flex p-2 bg-white/50 rounded-xl border border-dashed border-gray-200 w-fit">
                    <span className="text-[10px] font-bold text-gray-400 italic">Hierarchical Master</span>
                </div>
            </td>
            <td className="py-6 px-8">
                <div className="flex flex-col gap-0.5">
            ...
            <td className="py-6 px-8 text-right">
                <Link
                    ...
                    className="inline-flex items-center justify-center gap-2 bg-white px-4 py-2 rounded-xl text-xs font-black text-emerald-600 border border-emerald-100 shadow-sm hover:shadow-md hover:bg-emerald-600 hover:text-white transition-all transform hover:-translate-y-0.5 active:translate-y-0 uppercase tracking-wider"
                >
