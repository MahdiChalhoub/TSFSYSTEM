...
export default function CreateWebhooksPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await erpFetch('integrations/webhooks/', {
        method: 'POST',
      ...
      setSuccess(true)
      setTimeout(() => router.push('/integrations/webhooks'), 500)
    } catch (err: any) {
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div>
          ...
          <p className="text-app-muted-foreground mt-1">
            Add a new webhooks to the system
          </p>
        </div>
      </div>
      ...
      <div className="max-w-4xl">
        <div className="bg-app-surface rounded-[32px] shadow-lg border border-app-border p-6 md:p-8">
          {error && (
            <div className="mb-6 p-4 rounded-2xl bg-app-error-soft text-app-error border border-app-error">
              {error}
            </div>
          )}

          {success && (
            <div className="mb-6 p-4 rounded-2xl bg-app-success-soft text-app-success border border-app-success">
