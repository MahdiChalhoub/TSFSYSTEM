...
type Webhooks = Record<string, any>

const ALL_COLUMNS: ColumnDef<Webhooks>[] = [
...
export default function WebhooksListPage() {
  ...
  const settings = useListViewSettings('integrations_webhooks', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('integrations/webhooks/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Webhooks>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/integrations/webhooks/new')}
        ...
        actions={{
          onView: (r) => router.push(`/integrations/webhooks/${r.id}`),
          onEdit: (r) => router.push(`/integrations/webhooks/${r.id}/edit`),
        }}
      >
        <TypicalFilter
