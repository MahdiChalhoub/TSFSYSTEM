...
const ALL_COLUMNS: ColumnDef<Files>[] = [
...
export default function FilesListPage() {
  ...
  const settings = useListViewSettings('storage_files', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('storage/files/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Files>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/storage/files/new')}
        ...
        actions={{
          onView: (r) => router.push(`/storage/files/${r.id}`),
          onEdit: (r) => router.push(`/storage/files/${r.id}/edit`),
        }}
      >
        <TypicalFilter
