...
const ALL_COLUMNS: ColumnDef<Requests>[] = [
...
export default function RequestsListPage() {
  ...
  const settings = useListViewSettings('workspace_requests', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('workspace/requests/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Requests>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/workspace/requests/new')}
        ...
        actions={{
          onView: (r) => router.push(`/workspace/requests/${r.id}`),
          onEdit: (r) => router.push(`/workspace/requests/${r.id}/edit`),
        }}
      >
        <TypicalFilter
