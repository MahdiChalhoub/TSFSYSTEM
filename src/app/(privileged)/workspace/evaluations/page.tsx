...
type Evaluations = Record<string, any>

const ALL_COLUMNS: ColumnDef<Evaluations>[] = [
...
export default function EvaluationsListPage() {
  ...
  const settings = useListViewSettings('workspace_evaluations', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('workspace/evaluations/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Evaluations>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/workspace/evaluations/new')}
        ...
        actions={{
          onView: (r) => router.push(`/workspace/evaluations/${r.id}`),
          onEdit: (r) => router.push(`/workspace/evaluations/${r.id}/edit`),
        }}
      >
        <TypicalFilter
