...
type Questionnaires = Record<string, any>

const ALL_COLUMNS: ColumnDef<Questionnaires>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function QuestionnairesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<Questionnaires[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('workspace_questionnaires', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('workspace/questionnaires/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Questionnaires>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/workspace/questionnaires/new')}
        ...
        actions={{
          onView: (r) => router.push(`/workspace/questionnaires/${r.id}`),
          onEdit: (r) => router.push(`/workspace/questionnaires/${r.id}/edit`),
        }}
      >
        <TypicalFilter
