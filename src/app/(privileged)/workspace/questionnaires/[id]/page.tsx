...
export default function QuestionnairesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`workspace/questionnaires/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`workspace/questionnaires/${id}/`, {
        method: 'DELETE'
      })
      router.push('/workspace/questionnaires')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Questionnaires #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage questionnaires details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/workspace/questionnaires/${id}/edit`)}
            >
