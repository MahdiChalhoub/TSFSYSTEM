...
const PRIORITY_COLORS: Record<string, string> = {
 ...
 LOW: 'bg-app-surface-2 text-app-text-muted border-app-border',
};
const STATUS_COLORS: Record<string, string> = {
 ...
 CANCELLED: 'bg-app-bg text-app-text-muted',
...
export default function TasksClient({ tasks: initialTasks, categories, users }: Props) {
 ...
 className="w-full pl-10 pr-4 py-2.5 bg-app-bg rounded-2xl border border-app-border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 transition-all"
 />
 </div>
 <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
 className="px-4 py-2.5 bg-app-bg rounded-2xl border border-app-border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200">
 ...
 <h3 className="text-lg font-bold text-app-text">Create New Task</h3>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <input placeholder="Task title *" value={newTitle} onChange={e => setNewTitle(e.target.value)}
 className="col-span-full px-4 py-3 bg-app-bg rounded-2xl border border-app-border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200" />
 <textarea placeholder="Description" value={newDescription} onChange={e => setNewDescription(e.target.value)} rows={3}
 className="col-span-full px-4 py-3 bg-app-bg rounded-2xl border border-app-border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 resize-none" />
 ...
 <p className="text-app-text-faint font-medium text-lg">No tasks found</p>
 ...
 {/* Status Icon */}
 <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${STATUS_COLORS[task.status] || 'bg-app-bg'}`}>
 <StatusIcon size={18} />
 </div>
 {/* Main Content */}
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2 mb-1">
 <h3 className="font-bold text-app-text truncate">{task.title}</h3>
 {task.subtask_count > 0 && (
 <span className="text-xs text-app-text-faint bg-app-bg px-2 py-0.5 rounded-full">{task.subtask_count} sub</span>
 )}
 </div>
 <div className="flex items-center gap-3 text-xs text-app-text-faint">
 {task.category_name && (
 <span className="bg-app-bg px-2 py-0.5 rounded-full">{task.category_name}</span>
 )}
