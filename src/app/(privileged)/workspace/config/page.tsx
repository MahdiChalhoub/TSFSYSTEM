...
export default function ConfigListPage() {
  ...
  const settings = useListViewSettings('workspace_config', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('workspace/config/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Config>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/workspace/config/new')}
        ...
        actions={{
          onView: (r) => router.push(`/workspace/config/${r.id}`),
          onEdit: (r) => router.push(`/workspace/config/${r.id}/edit`),
        }}
      >
        <TypicalFilter
