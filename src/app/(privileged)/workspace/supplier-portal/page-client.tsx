...
export default function SupplierPortalAdminPage() {
 ...
 <p className="text-[11px] font-black text-app-text-faint uppercase tracking-widest leading-none mb-1">Total Proformas</p>
 <h2 className="text-3xl font-black text-app-text">{proformas.length}</h2>
 </div>
 </div>
 {/* Tabs */}
 ...
 tab === 'pricing' ? (
 priceReqs.length === 0 ? <div className="text-sm text-app-text-muted py-8 text-center">No price change requests.</div> :
 priceReqs.map(r => {
 const pct = ((Number(r.requested_price) - Number(r.current_price)) / Number(r.current_price || 1)) * 100
 const isIncrease = pct > 0
 return (
 <div key={r.id} className="flex items-start gap-6 p-6 rounded-[2.5rem] bg-app-surface shadow-sm border border-slate-50 transition-all hover:shadow-md group">
 <div className="w-12 h-12 rounded-2xl bg-rose-50 text-rose-600 flex items-center justify-center shrink-0">
 <Package size={24} />
 </div>
 <div className="flex-1">
 <div className="flex items-center gap-3 mb-1">
 <span className="font-black text-sm text-app-text uppercase italic">{r.product?.name || r.product_name || '—'}</span>
 <Badge className={`${STATUS_BADGE[r.status] || 'bg-app-surface-2 text-app-text-faint'} border-0 text-[8px] font-black px-3 py-0.5 rounded-full uppercase tracking-widest`}>{r.status}</Badge>
 </div>
 ...
 {r.product?.sku && <span className="text-[10px] font-bold text-app-text-faint font-mono italic">{r.product.sku}</span>}
 </div>
 <div className="flex items-center gap-6 p-4 bg-app-bg rounded-2xl border border-slate-100/50">
 <div className="flex flex-col">
 <span className="text-[9px] font-black text-app-text-faint uppercase tracking-widest mb-1">Current</span>
 <span className="text-sm font-black text-app-text-muted line-through">${Number(r.current_price).toFixed(2)}</span>
 </div>
 ...
 <span className="text-xl font-black text-app-text">${Number(r.requested_price).toFixed(2)}</span>
 </div>
 ...
 proformas.length === 0 ? <div className="text-sm text-app-text-muted py-8 text-center">No proformas received.</div> :
 proformas.map(p => (
