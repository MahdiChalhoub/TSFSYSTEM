...
const STATUS_BADGE: Record<string, string> = {
 PENDING: 'bg-app-warning/40 text-app-warning border-app-warning/30',
 APPROVED: 'bg-app-success/40 text-app-primary border-app-success/30',
 REJECTED: 'bg-app-error/40 text-app-error border-app-error/30',
 DRAFT: 'bg-app-surface-2 text-app-muted-foreground border-app-border',
 SENT: 'bg-app-info/40 text-app-info border-app-info/30',
 ACCEPTED: 'bg-app-success/40 text-app-primary border-app-success/30',
}
export default function SupplierPortalAdminPage() {
 ...
 <Badge className="bg-app-primary-light text-app-primary border-app-success/30 font-black text-[10px] uppercase tracking-widest px-3 py-1">
 Active
 </Badge>
 ...
 Supplier <span className="text-app-primary">Ops</span>
 </h1>
 </div>
 ...
 <button className="h-12 px-6 rounded-2xl bg-app-primary text-app-foreground font-bold flex items-center gap-2 hover:bg-app-success transition-all shadow-lg shadow-emerald-200">
 ...
 <div className="flex justify-between items-start mb-4">
 <div className="w-12 h-12 rounded-2xl bg-app-warning/50 text-amber-100 flex items-center justify-center">
 <Tag size={24} />
 </div>
 <Badge variant="outline" className="text-amber-200 bg-app-warning/30 border-0 font-black text-[10px]">
 ...
 <Badge variant="outline" className="text-app-primary bg-app-primary-light border-0 font-black text-[10px]">
 REGISTRY
 </Badge>
 </div>
 ...
 tab === 'pricing' ? (
 priceReqs.length === 0 ? <div className="text-sm text-app-muted-foreground py-8 text-center">No price change requests.</div> :
 priceReqs.map(r => {
 ...
 <span className="font-black text-sm text-app-foreground uppercase italic">{r.product?.name || r.product_name || '—'}</span>
 <Badge className={`${STATUS_BADGE[r.status] || 'bg-app-surface-2 text-app-muted-foreground'} border-0 text-[8px] font-black px-3 py-0.5 rounded-full uppercase tracking-widest`}>{r.status}</Badge>
 </div>
 <div className="flex items-center gap-2 mb-3">
 <span className="text-[10px] font-black text-app-primary uppercase tracking-widest">{r.supplier?.name || r.supplier_name || '—'}</span>
 {r.product?.sku && <span className="text-[10px] text-app-muted-foreground">·</span>}
 {r.product?.sku && <span className="text-[10px] font-bold text-app-muted-foreground font-mono italic">{r.product.sku}</span>}
 </div>
 <div className="flex items-center gap-6 p-4 bg-app-background rounded-2xl border border-app-border/50">
 <div className="flex flex-col">
 <span className="text-[9px] font-black text-app-muted-foreground uppercase tracking-widest mb-1">Current</span>
 <span className="text-sm font-black text-app-muted-foreground line-through">${Number(r.current_price).toFixed(2)}</span>
 </div>
 <ChevronRight size={16} className="text-app-muted-foreground mt-4" />
 <div className="flex flex-col">
 ...
 {r.status === 'PENDING' && (
 <button onClick={() => handleApprove(r.id)} className="h-12 px-6 rounded-2xl bg-app-primary text-app-foreground font-black text-xs uppercase tracking-widest shadow-lg shadow-emerald-200 hover:bg-app-success transition-all self-center">
 Authorize
 </button>
 )}
 </div>
 ...
 <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${STATUS_BADGE[p.status] || 'bg-app-surface-2 text-app-muted-foreground border-app-border'}`}>{p.status}</span>
 </div>
 <p className="text-xs text-app-muted-foreground mt-0.5">{p.supplier?.name || p.supplier_name || '—'}{p.created_at ? ` · ${new Date(p.created_at).toLocaleDateString()}` : ''}</p>
 </div>
 <div className="font-mono font-bold text-sm text-app-foreground">${Number(p.total_amount || 0).toFixed(2)}</div>
 </div>
 ))
 )
 }
