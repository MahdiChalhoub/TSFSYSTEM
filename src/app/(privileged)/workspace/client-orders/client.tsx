...
const STATUS_FLOW: Record<string, { next: string; action: string; color: string; icon: any }> = {
 ...
 'PROCESSING': { next: 'SHIPPED', action: 'Ship', color: 'var(--app-info)', icon: Truck },
 'SHIPPED': { next: 'DELIVERED', action: 'Deliver', color: 'var(--app-success)', icon: Package },
};

const STATUS_COLORS: Record<string, string> = {
 CART: 'var(--app-muted-foreground)', PLACED: 'var(--app-warning)', CONFIRMED: '#6366f1', PROCESSING: 'var(--app-primary)',
 SHIPPED: 'var(--app-info)', DELIVERED: 'var(--app-success)', CANCELLED: '#ef4444', RETURNED: '#f97316',
};
...
export default function ClientOrdersClient({ orders: init }: { orders: any[] }) {
 ...
 padding: '0.5rem', background: 'var(--app-background)', border: '1px solid var(--app-surface)',
 ...
 style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--app-muted-foreground)' }}>
 {expanded === o.id ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
 </button>
 <div>
 <div style={{ fontWeight: 600 }}>{o.order_number}</div>
 <div style={{ color: 'var(--app-muted-foreground)', fontSize: '0.85rem' }}>{o.contact_name}</div>
 </div>
 </div>
 <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
 ...
 {expanded === o.id && (
 <div style={{
 marginTop: 12, padding: 12, background: 'var(--app-background)', borderRadius: 8,
 fontSize: '0.85rem', color: 'var(--app-muted-foreground)',
 }}>
