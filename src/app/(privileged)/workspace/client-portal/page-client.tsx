...
const TICKET_STATUS: Record<string, string> = {
 ...
 CLOSED: 'bg-gray-800 text-app-text-muted border-gray-700',
}
const PRIORITY: Record<string, string> = {
 HIGH: 'text-red-400',
 MEDIUM: 'text-amber-400',
 LOW: 'text-app-text-faint',
}
export default function ClientPortalAdminPage() {
 ...
 <p className="text-[11px] font-black text-app-text-faint uppercase tracking-widest leading-none mb-1">Quote Requests</p>
 <h2 className="text-3xl font-black text-app-text">{quotes.length}</h2>
 </div>
 </div>
 {/* Tabs */}
 ...
 tab === 'wallets' ? (
 wallets.length === 0 ? <div className="text-sm text-app-text-muted py-8 text-center">No client wallets yet.</div> :
 wallets.map(w => (
 ...
 tickets.length === 0 ? <div className="text-sm text-app-text-muted py-8 text-center">No tickets.</div> :
 tickets.map(t => (
 <div key={t.id} className="flex items-center gap-6 p-6 rounded-[2.5rem] bg-app-surface shadow-sm border border-slate-50 transition-all hover:shadow-md group text-app-text">
 ...
 <Badge className={`${TICKET_STATUS[t.status] || 'bg-app-surface-2 text-app-text-faint'} border-0 text-[8px] font-black px-3 py-0.5 rounded-full uppercase tracking-widest`}>{t.status}</Badge>
 {t.priority && <span className={`text-[10px] font-black uppercase tracking-widest ${PRIORITY[t.priority] || 'text-app-text-faint'}`}>{t.priority}</span>}
 </div>
 ...
 quotes.length === 0 ? <div className="text-sm text-app-text-muted py-8 text-center">No quote requests.</div> :
 quotes.map(q => (
