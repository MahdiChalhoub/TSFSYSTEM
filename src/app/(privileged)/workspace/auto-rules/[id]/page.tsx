...
export default function AutoRulesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`workspace/auto-rules/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`workspace/auto-rules/${id}/`, {
        method: 'DELETE'
      })
      router.push('/workspace/auto-rules')
    } catch (error) {
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `Auto-Rules #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage auto rules details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/workspace/auto-rules/${id}/edit`)}
            >
