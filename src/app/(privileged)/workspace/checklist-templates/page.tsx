...
type ChecklistTemplates = Record<string, any>

const ALL_COLUMNS: ColumnDef<ChecklistTemplates>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function ChecklistTemplatesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<ChecklistTemplates[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('workspace_checklist-templates', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('workspace/checklist-templates/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<ChecklistTemplates>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/workspace/checklist-templates/new')}
        ...
        actions={{
          onView: (r) => router.push(`/workspace/checklist-templates/${r.id}`),
          onEdit: (r) => router.push(`/workspace/checklist-templates/${r.id}/edit`),
        }}
      >
        <TypicalFilter
