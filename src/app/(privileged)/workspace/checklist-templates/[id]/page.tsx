...
export default function ChecklistTemplatesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`workspace/checklist-templates/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`workspace/checklist-templates/${id}/`, {
        method: 'DELETE'
      })
      router.push('/workspace/checklist-templates')
    } catch (error) {
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `Checklist-Templates #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage checklist templates details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/workspace/checklist-templates/${id}/edit`)}
            >
