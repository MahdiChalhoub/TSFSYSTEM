...
type Comments = Record<string, any>

const ALL_COLUMNS: ColumnDef<Comments>[] = [
...
export default function CommentsListPage() {
  ...
  const settings = useListViewSettings('workspace_comments', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('workspace/comments/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Comments>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/workspace/comments/new')}
        ...
        actions={{
          onView: (r) => router.push(`/workspace/comments/${r.id}`),
          onEdit: (r) => router.push(`/workspace/comments/${r.id}/edit`),
        }}
      >
        <TypicalFilter
