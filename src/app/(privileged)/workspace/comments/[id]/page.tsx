...
export default function CommentsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`workspace/comments/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`workspace/comments/${id}/`, {
        method: 'DELETE'
      })
      router.push('/workspace/comments')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Comments #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage comments details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/workspace/comments/${id}/edit`)}
            >
