...
export default function KpiConfigDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`workspace/kpi-config/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`workspace/kpi-config/${id}/`, {
        method: 'DELETE'
      })
      router.push('/workspace/kpi-config')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Kpi-Config #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage kpi config details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/workspace/kpi-config/${id}/edit`)}
            >
