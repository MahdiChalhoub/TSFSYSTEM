...
type KpiConfig = Record<string, any>

const ALL_COLUMNS: ColumnDef<KpiConfig>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function KpiConfigListPage() {
  const router = useRouter()
  const [items, setItems] = useState<KpiConfig[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('workspace_kpi-config', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('workspace/kpi-config/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<KpiConfig>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/workspace/kpi-config/new')}
        ...
        actions={{
          onView: (r) => router.push(`/workspace/kpi-config/${r.id}`),
          onEdit: (r) => router.push(`/workspace/kpi-config/${r.id}/edit`),
        }}
      >
        <TypicalFilter
