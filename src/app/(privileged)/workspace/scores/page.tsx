...
const ALL_COLUMNS: ColumnDef<Scores>[] = [
...
export default function ScoresListPage() {
  ...
  const settings = useListViewSettings('workspace_scores', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('workspace/scores/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Scores>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/workspace/scores/new')}
        ...
        actions={{
          onView: (r) => router.push(`/workspace/scores/${r.id}`),
          onEdit: (r) => router.push(`/workspace/scores/${r.id}/edit`),
        }}
      >
        <TypicalFilter
