...
export default function ScoresDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`workspace/scores/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`workspace/scores/${id}/`, {
        method: 'DELETE'
      })
      router.push('/workspace/scores')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Scores #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage scores details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/workspace/scores/${id}/edit`)}
            >
