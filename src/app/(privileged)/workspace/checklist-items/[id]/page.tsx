...
export default function ChecklistItemsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`workspace/checklist-items/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`workspace/checklist-items/${id}/`, {
        method: 'DELETE'
      })
      router.push('/workspace/checklist-items')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Checklist-Items #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage checklist items details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/workspace/checklist-items/${id}/edit`)}
            >
