...
type ChecklistItems = Record<string, any>

const ALL_COLUMNS: ColumnDef<ChecklistItems>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function ChecklistItemsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<ChecklistItems[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('workspace_checklist-items', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('workspace/checklist-items/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<ChecklistItems>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/workspace/checklist-items/new')}
        ...
        actions={{
          onView: (r) => router.push(`/workspace/checklist-items/${r.id}`),
          onEdit: (r) => router.push(`/workspace/checklist-items/${r.id}/edit`),
        }}
      >
        <TypicalFilter
