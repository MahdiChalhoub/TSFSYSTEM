...
type PriceGroups = Record<string, any>

const ALL_COLUMNS: ColumnDef<PriceGroups>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function PriceGroupsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<PriceGroups[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('crm_price-groups', {
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<PriceGroups>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/crm/price-groups/new')}
        ...
        actions={{
          ...
          onEdit: (r) => router.push(`/crm/price-groups/${r.id}/edit`),
        }}
      >
        <TypicalFilter
