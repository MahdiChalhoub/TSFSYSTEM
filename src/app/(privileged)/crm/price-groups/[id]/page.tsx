...
export default function PriceGroupsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`crm/price-groups/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`crm/price-groups/${id}/`, {
        method: 'DELETE'
      })
      router.push('/crm/price-groups')
    } catch (error) {
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `Price-Groups #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage price groups details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/crm/price-groups/${id}/edit`)}
            >
