...
type PriceRules = Record<string, any>

const ALL_COLUMNS: ColumnDef<PriceRules>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function PriceRulesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<PriceRules[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('crm_price-rules', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('crm/price-rules/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<PriceRules>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/crm/price-rules/new')}
        ...
        actions={{
          ...
          onEdit: (r) => router.push(`/crm/price-rules/${r.id}/edit`),
        }}
      >
        <TypicalFilter
