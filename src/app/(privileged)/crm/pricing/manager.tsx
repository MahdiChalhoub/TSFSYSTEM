...
export default function PricingManager({
 ...
 <div className="flex gap-2 bg-app-bg p-1.5 rounded-2xl">
 ...
 tab === t ? "bg-app-surface text-app-text shadow-sm" : "text-app-text-faint hover:text-app-text-muted"
 )}
 >
 ...
 <input
 className="pl-12 pr-6 py-3 rounded-2xl bg-app-bg border-none focus:ring-4 focus:ring-violet-100 outline-none font-bold text-gray-700 placeholder:text-gray-300 w-64"
 placeholder="Search..."
 value={search}
 ...
 <div>
 <h3 className="text-xl font-black text-app-text">{group.name}</h3>
 <p className="text-xs text-app-text-faint font-bold">Priority: {group.priority}</p>
 </div>
 </div>
 {group.description && (
 <p className="text-sm text-app-text-muted font-medium">{group.description}</p>
 )}
 </div>
 <button
 ...
 <div className="bg-app-bg rounded-2xl p-4 flex-1 text-center">
 <div className="text-2xl font-black text-app-text">{group.member_count || 0}</div>
 <div className="text-[10px] font-black text-app-text-faint uppercase tracking-widest">Members</div>
 </div>
 <div className="bg-app-bg rounded-2xl p-4 flex-1 text-center">
 <div className="text-2xl font-black text-app-text">{group.rule_count || 0}</div>
 <div className="text-[10px] font-black text-app-text-faint uppercase tracking-widest">Rules</div>
 </div>
 </div>

 {(group.valid_from || group.valid_until) && (
 <div className="mt-4 flex items-center gap-2 text-xs text-app-text-faint font-bold">
 <Calendar size={12} />
 ...
 {rule.min_quantity > 1 && (
 <span className="text-xs text-app-text-faint font-bold">Min qty: {rule.min_quantity}</span>
 )}
 </div>
 ...
 <h2 className="text-2xl font-black text-app-text">New Price Group</h2>
 <div className="space-y-4">
 <div>
 <label className="block text-xs font-black text-app-text-faint uppercase tracking-widest mb-2">Group Name</label>
 <input
 value={groupName} onChange={(e) => setGroupName(e.target.value)}
 className="w-full px-4 py-3 rounded-2xl bg-app-bg border-none focus:ring-4 focus:ring-violet-100 outline-none font-bold text-app-text"
 placeholder="e.g., VIP Clients"
 />
 </div>
 <div>
 <label className="block text-xs font-black text-app-text-faint uppercase tracking-widest mb-2">Description</label>
 <textarea
 value={groupDesc} onChange={(e) => setGroupDesc(e.target.value)}
 className="w-full px-4 py-3 rounded-2xl bg-app-bg border-none focus:ring-4 focus:ring-violet-100 outline-none font-bold text-app-text resize-none"
 rows={2} placeholder="Describe this pricing tier..."
 />
 </div>
 <div>
 <label className="block text-xs font-black text-app-text-faint uppercase tracking-widest mb-2">Priority (higher = wins)</label>
 <input
 ...
 <h2 className="text-2xl font-black text-app-text">New Price Rule</h2>
 <div className="space-y-4">
 <div className="grid grid-cols-2 gap-4">
 <div>
 <label className="block text-xs font-black text-app-text-faint uppercase tracking-widest mb-2">Discount Type</label>
 <select value={ruleType} onChange={(e) => setRuleType(e.target.value)}
 className="w-full px-4 py-3 rounded-2xl bg-app-bg border-none focus:ring-4 focus:ring-violet-100 outline-none font-bold text-app-text appearance-none">
 ...
 <div>
 <label className="block text-xs font-black text-app-text-faint uppercase tracking-widest mb-2">
 ...
 <div>
 <label className="block text-xs font-bold text-app-text-muted mb-1">Specific Contact</label>
 <select value={ruleContactId} onChange={(e) => { setRuleContactId(e.target.value); if (e.target.value) setRuleGroupId(''); }}
 className="w-full px-4 py-2.5 rounded-xl bg-app-surface border-none focus:ring-2 focus:ring-violet-200 outline-none font-bold text-app-text text-sm appearance-none">
 <option value="">— None —</option>
 {contacts.map((c: Record<string, any>) => <option key={c.id} value={c.id}>{c.name} ({c.type})</option>)}
 </select>
 </div>
 <div>
 <label className="block text-xs font-bold text-app-text-muted mb-1">Or Price Group</label>
 ...
 <p className="text-xs font-black text-app-text-faint uppercase tracking-widest">Scope (optional)</p>
 <div>
 <label className="block text-xs font-bold text-app-text-muted mb-1">Product</label>
 <select value={ruleProductId} onChange={(e) => setRuleProductId(e.target.value)}
 className="w-full px-4 py-2.5 rounded-xl bg-app-surface border-none focus:ring-2 focus:ring-gray-200 outline-none font-bold text-app-text text-sm appearance-none">
 <option value="">All Products</option>
 {products.map((p: Record<string, any>) => <option key={p.id} value={p.id}>{p.name}</option>)}
 </select>
 </div>
 <div>
 <label className="block text-xs font-bold text-app-text-muted mb-1">Or Category</label>
 ...
 <div>
 <label className="block text-xs font-black text-app-text-faint uppercase tracking-widest mb-2">Min Quantity</label>
