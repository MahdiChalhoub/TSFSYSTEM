    ...
    return (
        <div className="fixed inset-0 bg-app-background/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-app-surface rounded-[40px] shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-500 border border-app-border max-h-[90vh] overflow-y-auto">
                {/* Header */}
                <div className="px-8 py-6 border-b border-app-border flex justify-between items-center bg-app-surface-2/50 sticky top-0 z-10">
                    <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg ${type === 'CUSTOMER' ? 'bg-app-info shadow-blue-200' : 'bg-app-warning shadow-amber-200'} text-app-foreground`}>
                            {type === 'CUSTOMER' ? <User size={24} /> : <Briefcase size={24} />}
                        </div>
                        <div>
                            <h2 className="text-2xl font-black text-app-foreground leading-tight">
                                New {type === 'CUSTOMER' ? 'Customer' : type === 'SUPPLIER' ? 'Supplier' : type === 'BOTH' ? 'Client + Supplier' : type === 'SERVICE' ? 'Service Provider' : type === 'CREDITOR' ? 'Creditor' : type === 'DEBTOR' ? 'Debtor' : type === 'LEAD' ? 'Lead' : 'Contact'}
                            </h2>
                ...
                <form action={action} className="p-8 space-y-6">
                    ...
                    {/* Entity Type Toggle */}
                    <div>
                        <label className="block text-xs font-black text-app-muted-foreground uppercase tracking-widest mb-2 ml-1">Entity Type</label>
                        <div className="flex gap-2">
                            <button
                                type="button"
                                onClick={() => setEntityType('INDIVIDUAL')}
                                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl font-bold text-sm transition-all ${entityType === 'INDIVIDUAL'
                                        ? 'bg-app-primary text-white shadow-lg'
                                        : 'bg-app-background text-app-muted-foreground hover:bg-app-surface-2'
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl font-bold text-sm transition-all ${entityType === 'BUSINESS'
                                        ? 'bg-app-primary text-white shadow-lg'
                                        : 'bg-app-background text-app-muted-foreground hover:bg-app-surface-2'
                                    }`}
                            >
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        {type === 'SUPPLIER' && (
                            <div className="col-span-2 md:col-span-1">
                                ...
                                <div className="relative">
                                    <Tag className="absolute left-4 top-1/2 -translate-y-1/2 text-app-muted-foreground" size={18} />
                                    <select
                                        name="supplierCategory"
                                        className="w-full pl-12 pr-6 py-4 rounded-2xl bg-app-background border-none focus:ring-4 focus:ring-amber-100 outline-none transition-all font-bold text-app-foreground appearance-none"
                                    >
                        ...
                        {type === 'CUSTOMER' && (
                            <div className="col-span-2 md:col-span-1">
                                ...
                                <div className="relative">
                                    ...
                                    <select
                                        name="customerTier"
                                        className="w-full pl-12 pr-6 py-4 rounded-2xl bg-app-background border-none focus:ring-4 focus:ring-blue-100 outline-none transition-all font-bold text-app-foreground appearance-none"
                                    >
                        ...
                        {type === 'CUSTOMER' && (
                            <div className="col-span-2 md:col-span-1">
                                <label className="block text-xs font-black text-app-muted-foreground uppercase tracking-widest mb-2 ml-1">Home Delivery Zone</label>
                                <div className="relative">
                                    <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-app-muted-foreground" size={18} />
                                    <select
                                        name="homeZoneId"
                                        className="w-full pl-12 pr-6 py-4 rounded-2xl bg-app-background border-none focus:ring-4 focus:ring-blue-100 outline-none transition-all font-bold text-app-foreground appearance-none"
                                    >
                        ...
                        {/* WhatsApp Group ID */}
                        <div className="col-span-2 md:col-span-1">
                            <label className="block text-xs font-black text-app-muted-foreground uppercase tracking-widest mb-2 ml-1">WhatsApp Group ID</label>
                            <div className="relative">
                                <MessageCircle className="absolute left-4 top-1/2 -translate-y-1/2 text-app-primary" size={18} />
                                <input
                                    name="whatsappGroupId"
                                    className="w-full pl-12 pr-6 py-4 rounded-2xl bg-app-primary-light/30 border-none focus:ring-4 focus:ring-emerald-100 outline-none transition-all font-bold text-app-success placeholder-emerald-800/20"
                        ...
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <div className="relative">
                                <Clock className="absolute left-4 top-1/2 -translate-y-1/2 text-app-muted-foreground" size={18} />
                                <input
                        ...
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <div className="relative">
                                <Shield className="absolute left-4 top-1/2 -translate-y-1/2 text-app-primary" size={18} />
                                <select
                                    name="taxProfileId"
                                    className="w-full pl-12 pr-6 py-4 rounded-2xl bg-app-primary/5/40 border-none focus:ring-4 focus:ring-app-primary outline-none transition-all font-bold text-app-foreground appearance-none"
                                >
                    ...
                    <div className="p-6 bg-app-primary/5/50 rounded-[32px] border-2 border-dashed border-app-primary/30/50">
                        <div className="flex items-center gap-3 mb-4">
                            <CreditCard size={18} className="text-app-primary" />
                            <span className="text-sm font-black text-app-primary uppercase tracking-widest">Autonomous Ledger Automation</span>
                        </div>
                        <p className="text-xs text-app-primary/70 font-medium leading-relaxed">
                    ...
                    <div className="pt-6 border-t border-app-border flex gap-4">
                        <button
                            type="button"
                            onClick={onClose}
                            className="flex-1 px-6 py-4 rounded-2xl font-black text-app-muted-foreground hover:text-app-muted-foreground hover:bg-app-background transition-all text-sm uppercase tracking-widest"
                        >
                            Abort
                        </button>
                        <button
                            type="submit"
                            disabled={isPending}
                            className={`flex-[2] text-app-foreground px-6 py-4 rounded-2xl font-black shadow-xl transition-all text-sm uppercase tracking-widest disabled:opacity-50 ${type === 'CUSTOMER' ? 'bg-app-info shadow-blue-200 hover:bg-app-info' : 'bg-app-warning shadow-amber-200 hover:bg-app-warning'} hover:-translate-y-1 active:translate-y-0`}
                        >
