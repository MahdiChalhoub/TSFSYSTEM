            ...
            {taxProfiles.length > 0 && (
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <select name="taxProfileId" className={fieldCls} style={{ appearance: 'none' }} defaultValue={contact?.tax_profile || ''}>
                    <option value="">Auto-detect</option>
                    {taxProfiles.map((p: any) => <option key={p.id} value={p.id}>{p.name}{p.is_system_preset ? ' ★' : ''}</option>)}
                  </select>
                </div>
              </div>
            )}
