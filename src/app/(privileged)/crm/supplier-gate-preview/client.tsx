...
const PO_STATUS_STYLES: Record<string, { bg: string; text: string; icon: any }> = {
    DRAFT: { bg: 'bg-gray-500/10', text: 'text-gray-400', icon: FileText },
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Select Supplier</span>
                </div>
                <div className="relative">
                    <button
                        onClick={() => setDropdownOpen(!dropdownOpen)}
                        className="w-full flex items-center justify-between px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm hover:bg-gray-100 transition-colors"
                    >
                        <span className={selectedSupplier ? 'text-gray-900 font-bold' : 'text-gray-400'}>
                            {selectedSupplier ? selectedSupplier.name : 'Choose a supplier to preview...'}
                        </span>
                        <ChevronDown size={16} className={`text-gray-400 transition-transform ${dropdownOpen ? 'rotate-180' : ''}`} />
                    </button>
                    {dropdownOpen && (
                        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-2xl z-50 max-h-72 overflow-hidden">
                            ...
                            <div className="overflow-y-auto max-h-56">
                                {filteredSuppliers.length === 0 ? (
                                    <p className="text-center py-4 text-sm text-gray-400">No suppliers found</p>
            ...
            {selectedSupplier && (
                <div className="bg-[#020617] rounded-2xl border border-gray-800 overflow-hidden shadow-2xl">
                        ...
                        <div className="max-w-6xl mx-auto">
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">Welcome, <span className="text-indigo-400 font-bold">{selectedSupplier.name}</span></p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-3 mb-6">
                                <div className="px-5 py-3 bg-white/5 border border-white/5 rounded-2xl">
                                    <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Active Orders</span>
                                    <div className="text-2xl font-black text-white flex items-center gap-2">
                                        <TrendingUp size={14} className="text-emerald-400" />{activeCount}
                                    </div>
                                </div>
                                <div className="px-5 py-3 bg-white/5 border border-white/5 rounded-2xl">
                                    <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Fulfilled</span>
                                    <div className="text-2xl font-black text-white flex items-center gap-2">
                                        <CheckCircle2 size={14} className="text-indigo-400" />{receivedCount}
                                    </div>
                                </div>
                                <div className="px-5 py-3 bg-white/5 border border-white/5 rounded-2xl">
                                    <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Total Value</span>
                                    <div className="text-2xl font-black text-white flex items-center gap-2">
                    ...
                    <div className="bg-[#0a0f1e] p-8 pt-4">
                        <div className="max-w-6xl mx-auto">
                            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4">Purchase Orders</h3>
                            {loadingPOs ? (
                                ...
                                <div className="text-center py-16">
                                    ...
                                    <p className="text-gray-500 font-medium">No purchase orders with this supplier</p>
                                </div>
                            ) : (
                                            ...
                                            <div key={po.id} className="flex items-center gap-4 p-4 bg-white/5 border border-white/5 rounded-xl hover:border-indigo-500/20 transition-all">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-3 mt-0.5">
                                                        {po.order_date && <span className="text-[10px] text-gray-500">{new Date(po.order_date).toLocaleDateString()}</span>}
                                                        {po.line_count !== undefined && <span className="text-[10px] text-gray-600">{po.line_count} items</span>}
                                                    </div>
                                                </div>
            ...
            {!selectedSupplier && (
                <div className="bg-gray-50 rounded-2xl border border-dashed border-gray-200 p-16 text-center">
                    ...
                    <p className="text-gray-500 font-semibold">Select a supplier above</p>
                    <p className="text-xs text-gray-400 mt-1">You&apos;ll see exactly what they see when they access their portal</p>
                </div>
            )}
        </div>
    )
