...
export default function POSSettingsPage() {
  ...
  // Branch options for create form
    ...
    <div className={`flex flex-col h-full animate-in fade-in duration-300 transition-all ${focusMode ? 'max-h-[calc(100vh-4rem)]' : 'max-h-[calc(100vh-8rem)]'}`}>
        ...
        <div className="flex-shrink-0 mb-3 p-4 border rounded-2xl animate-in slide-in-from-top-2 duration-200"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2 items-end">
            ...
            <div>
              ...
              <select value={createForm.siteId} onChange={e => setCreateForm(f => ({ ...f, siteId: +e.target.value }))}
      ...
      {showSettings && (
        <>
          ...
          <GlobalSettingsPanel onClose={() => setShowSettings(false)} />
        </>
      )}
      {showUsers && (
        <>
          <div className="fixed inset-0 bg-black/30 z-40 animate-in fade-in duration-200" onClick={() => setShowUsers(false)} />
          <UsersPinsPanel users={users} onRefresh={load} onClose={() => setShowUsers(false)} />
        </>
      )}
    </div>
  )
