      ...
      <div className="app-page flex items-center justify-center h-screen bg-app-background">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6 fade-in-up">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0" style={{ background: 'var(--app-warning-bg)', border: '1px solid var(--app-warning)' }}>
            ...
            <div>
              ...
              <p className="text-sm text-app-muted-foreground mt-0.5">Point of sale terminal configuration</p>
            </div>
          </div>
        </header>
    ...
    <div className="min-h-screen bg-app-background">
      ...
      <div className="bg-app-surface border-b border-app-border px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/sales" className="p-2 rounded-lg hover:bg-app-surface-2 transition-colors">
    ...
    <div className="space-y-4">
      ...
      {(showCreate || editId) && (
        <div className="bg-app-surface rounded-2xl border border-app-primary/30 p-5 space-y-4 shadow-sm animate-in slide-in-from-top-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {!editId && (
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <label className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest">Branch / Location *</label>
                  <Link href="/inventory/warehouses" target="_blank" className="text-[10px] text-app-primary hover:text-app-primary flex items-center gap-0.5"><ExternalLink size={8} /> Create</Link>
                </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <label className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest">Warehouse</label>
                <Link href="/inventory/warehouses" target="_blank" className="text-[10px] text-app-primary hover:text-app-primary flex items-center gap-0.5"><ExternalLink size={8} /> Create</Link>
              </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <label className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest">Cash Account</label>
                <Link href="/finance" target="_blank" className="text-[10px] text-app-primary hover:text-app-primary flex items-center gap-0.5"><ExternalLink size={8} /> Create</Link>
              </div>
            ...
            <div>
              <label className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest block mb-1">Opening Mode</label>
          ...
          {form.openingMode === 'ADVANCED' && (
            <div className="flex items-center gap-3 p-3 bg-app-warning-bg rounded-xl border border-app-warning">
              <button
                ...
                className={clsx(
                  ...
                  form.cashierCanSeeSoftware ? "bg-app-warning justify-end" : "bg-app-surface-hover justify-start"
                )}
              >
                <div className="w-4 h-4 bg-app-surface rounded-full shadow" />
              </button>
              <div>
                <p className="text-xs font-bold text-app-warning">Cashier can see software values</p>
                <p className="text-[10px] text-app-warning">{form.cashierCanSeeSoftware ? 'Cashier sees full details' : 'Manager PIN required to reveal amounts'}</p>
              </div>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest block">Allowed Payment Accounts</label>
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setForm(f => ({ ...f, allowedAccountIds: accounts.filter(a => a.type !== 'SAVINGS' && a.type !== 'INVESTMENT').map(a => a.id) }))} className="text-[10px] text-app-primary font-bold hover:underline">Select All</button>
                <button type="button" onClick={() => setForm(f => ({ ...f, allowedAccountIds: [] }))} className="text-[10px] text-app-muted-foreground font-bold hover:underline">Clear</button>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-40 overflow-y-auto p-2 bg-app-background rounded-xl border border-app-border">
              {accounts.filter(a => a.type !== 'SAVINGS' && a.type !== 'INVESTMENT').map(acc => (
                <button key={acc.id} type="button"
                  ...
                  {form.allowedAccountIds.includes(acc.id) ? <Check size={14} /> : <div className="w-3.5 h-3.5 rounded border border-app-border" />}
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest block">Authorized Cashiers</label>
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setForm(f => ({ ...f, authorizedUserIds: users.map(u => u.id) }))} className="text-[10px] text-app-primary font-bold hover:underline">Select All</button>
                <button type="button" onClick={() => setForm(f => ({ ...f, authorizedUserIds: [] }))} className="text-[10px] text-app-muted-foreground font-bold hover:underline">Clear</button>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 max-h-48 overflow-y-auto p-2 bg-app-background rounded-xl border border-app-border">
              {users.map(u => (
                <button key={u.id} type="button"
                  ...
                  <div className={clsx("w-6 h-6 rounded flex items-center justify-center shrink-0",
                    form.authorizedUserIds.includes(u.id) ? "bg-app-primary/10" : "bg-app-surface-2"
                  )}>
                    <Users size={12} className={form.authorizedUserIds.includes(u.id) ? "text-app-primary" : "text-app-muted-foreground"} />
                  </div>
                  <div className="flex-1 text-left truncate">
                    {u.first_name ? `${u.first_name} ${u.last_name}`.trim() : u.username}
                  </div>
                  {form.authorizedUserIds.includes(u.id) ? <Check size={14} /> : <div className="w-3.5 h-3.5 rounded border border-app-border shrink-0" />}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="border border-app-primary/30 rounded-2xl p-4 bg-app-primary/5/30 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-black text-app-muted-foreground">Payment Methods</p>
                <p className="text-[10px] text-app-muted-foreground">Buttons shown in the POS for this register</p>
              </div>
              <div className="flex gap-1 flex-wrap justify-end">
                {['CASH', 'CARD', 'WAVE', 'OM', 'WALLET', 'MULTI', 'DELIVERY', 'CREDIT', 'CHECK', 'TRANSFER'].filter(
                  ...
                  <button key={k}
                    ...
                    className="px-2 py-1 text-[9px] font-bold rounded-lg bg-app-surface border border-app-primary/30 text-app-primary hover:bg-app-primary/5 transition-all"
                  >+ {k}</button>
                ))}
                <button type="button"
                  onClick={() => {
                  ...
                  className="px-2 py-1 text-[9px] font-bold rounded-lg bg-app-surface border border-app-border text-app-muted-foreground hover:bg-app-background transition-all"
                >+ Custom</button>
              </div>
            </div>
            {form.paymentMethods.length === 0 && (
              <p className="text-[10px] text-app-muted-foreground text-center py-2">No methods configured — will use global defaults</p>
            )}
            {form.paymentMethods.map((m, idx) => (
              <div key={idx} className="flex items-center gap-2 bg-app-surface rounded-xl border border-app-border px-3 py-2">
                <span className="text-app-muted-foreground cursor-grab text-xs">⠿</span>
                <input
                  type="text"
                  value={m.label}
                  ...
                  className="flex-1 text-xs font-bold text-app-foreground outline-none min-w-0"
                  placeholder="Label"
                />
                <span className="text-[9px] text-app-muted-foreground font-mono">{m.key}</span>
                <select
                  value={m.accountId || ''}
                  onChange={e => setForm(f => {
                    const arr = [...f.paymentMethods];
                  ...
                  className={clsx(
                    ...
                    m.accountId ? "bg-app-primary-light border-app-success text-app-success" : "bg-app-warning-bg border-app-warning text-app-warning"
                  )}
                >
                ...
                <button type="button"
                  ...
                  className="text-app-error hover:text-app-error transition-all p-1"
                ><X size={12} /></button>
              </div>
            ))}
          </div>
      ...
      {sites.length === 0 && (
        <div className="bg-app-warning-bg border border-app-warning/30 rounded-xl p-6 text-center">
          ...
          <p className="text-sm font-bold text-app-warning">No branches loaded</p>
          ...
          <button onClick={onRefresh} className="mt-3 px-4 py-2 bg-app-warning text-app-foreground rounded-lg text-xs font-bold hover:bg-app-warning transition-all inline-flex items-center gap-2">
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="bg-app-surface rounded-2xl border border-app-border overflow-hidden shadow-sm">
          ...
          {users.length === 0 && (
            <div className="p-6 text-center text-sm text-app-muted-foreground">
              ...
              <div key={user.id} className="grid grid-cols-[1fr_120px_200px] gap-3 px-4 py-2.5 border-b border-app-border items-center hover:bg-app-surface-2/50">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[9px] text-app-muted-foreground">{user.role_name || user.role || 'User'}</p>
                  </div>
                </div>
                <div>
                  {hasOverride ? (
    ...
    <div className="space-y-4">
      ...
      <div className="bg-app-surface rounded-2xl border border-violet-100 p-5 shadow-sm">
        ...
        <div className="bg-violet-50 rounded-xl p-3 mb-3 text-xs text-app-primary font-medium space-y-1">
          ...
          <p>3. After calibration, cash in hand + account book balance = expected cash software</p>
...
// ── Register Cash Account Isolation Section (own state) ──────────────
function RegisterCashIsolationSection() {
  ...
  useEffect(() => {
    erpFetch('pos-registers/pos-settings/')
      .then((data: any) => {
        if (data && typeof data.restrict_unique_cash_account === 'boolean') {
          setEnabled(data.restrict_unique_cash_account);
        }
      })
      .catch(() => { })
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await erpFetch('pos-registers/pos-settings/', {
        method: 'PATCH',
        body: JSON.stringify({ restrict_unique_cash_account: enabled }),
      });
      toast.success('Cash account isolation setting saved!');
    } catch { toast.error('Failed to save'); }
    setSaving(false);
  };

  return (
    <div className="bg-app-surface rounded-2xl border border-amber-200 p-5 shadow-sm">
      <h3 className="text-xs font-black text-amber-600 uppercase tracking-widest mb-1 flex items-center gap-2">
        <Banknote size={12} /> Register Cash Account Isolation
      </h3>
      <p className="text-[10px] text-app-muted-foreground mb-4">
        When enabled (recommended), each register must have its own unique cash account.
        A new COA account is auto-created under <b>RegisterCash</b> when a register is created without an account.
      </p>
      {loading ? (
        <div className="flex items-center gap-2 text-xs text-app-muted-foreground"><Loader2 size={12} className="animate-spin" /> Loading...</div>
      ) : (
        <div className="flex items-center justify-between py-2">
          <div>
            <p className="text-sm font-bold text-app-foreground">Enforce Unique Cash Account Per Register</p>
            <p className="text-[10px] text-app-muted-foreground">
              {enabled
                ? '✅ ON — each register has its own isolated cash ledger account'
                : '⚠ OFF — registers may share cash accounts (legacy mode)'}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => setEnabled(e => !e)}
              className={clsx('w-11 h-6 rounded-full transition-all relative shrink-0', enabled ? 'bg-app-primary' : 'bg-app-border')}
            >
              <div className={clsx('w-4 h-4 rounded-full bg-app-surface shadow absolute top-1 transition-all', enabled ? 'left-6' : 'left-1')} />
            </button>
            <button onClick={handleSave} disabled={saving}
              className="flex items-center gap-1 px-3 py-1.5 bg-amber-500 text-white rounded-lg font-bold text-xs hover:bg-amber-600 disabled:opacity-50 transition-all">
...
const SMS_PROVIDERS = [
  {
    ...
    active: 'border-app-border bg-app-surface-2',
    fields: [],
  },
  {
    key: 'twilio',
  ...
  },
  {
    ...
    bg: 'bg-app-primary-light',
    border: 'border-app-success',
    active: 'border-app-primary bg-app-primary-light ring-2 ring-emerald-100',
    fields: [
  ...
  },
  {
    ...
    color: 'text-app-primary',
    bg: 'bg-app-primary/5',
    border: 'border-app-primary/30',
    active: 'border-app-primary/30 bg-app-primary/5 ring-2 ring-app-primary',
    fields: [
    ...
    <div className="space-y-6">
      ...
      <div className="bg-app-surface rounded-2xl border border-app-border shadow-sm overflow-hidden">
        <div className="bg-gradient-to-r from-slate-900 to-indigo-950 px-5 py-4">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-xl bg-app-warning/20 flex items-center justify-center">
              <Hash size={15} className="text-app-warning" />
            </div>
            <div>
              <h2 className="text-app-foreground text-sm font-black">Confirmation Codes</h2>
              <p className="text-app-foreground/40 text-[10px]">Chain-of-custody protection for deliveries</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className={clsx(
            ...
            settings.require_driver_pos_code ? "border-app-primary/30 bg-app-primary/5" : "border-app-border bg-app-background"
          )}>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <div className="w-5 h-5 rounded-full bg-app-primary/10 flex items-center justify-center">
                  <span className="text-[9px] font-black text-app-primary">1</span>
                </div>
                <p className="text-sm font-black text-app-foreground">Register ↔ Driver Code</p>
              </div>
              <p className="text-xs text-app-muted-foreground ml-7">
            ...
            >
              {settings.require_driver_pos_code
                ? <ToggleRight size={32} className="text-app-primary" />
                : <ToggleLeft size={32} className="text-app-muted-foreground" />}
            </button>
          </div>
          ...
          <div className={clsx(
            "flex items-start justify-between gap-4 p-4 rounded-2xl border-2 transition-all",
            settings.require_client_delivery_code ? "border-app-warning bg-app-warning-bg" : "border-app-border bg-app-background"
          )}>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <div className="w-5 h-5 rounded-full bg-app-warning-bg flex items-center justify-center">
                  <span className="text-[9px] font-black text-app-warning">2</span>
                </div>
                <p className="text-sm font-black text-app-foreground">Driver ↔ Client Code</p>
              </div>
              <p className="text-xs text-app-muted-foreground ml-7">
            ...
            >
              {settings.require_client_delivery_code
                ? <ToggleRight size={32} className="text-app-warning" />
                : <ToggleLeft size={32} className="text-app-muted-foreground" />}
            </button>
          </div>
        </div>
      ...
      <div className="bg-app-surface rounded-2xl border border-app-border shadow-sm overflow-hidden">
        <div className="bg-gradient-to-r from-emerald-700 to-teal-800 px-5 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-xl bg-app-foreground/15 flex items-center justify-center">
                <MessageSquare size={15} className="text-app-foreground" />
              </div>
              <div>
                <h2 className="text-app-foreground text-sm font-black">SMS Gateway</h2>
                <p className="text-app-foreground/50 text-[10px]">Auto-send confirmation codes to clients</p>
              </div>
            </div>
            {/* Master SMS toggle */}
            <button
            ...
            >
              {settings.sms_delivery_code_enabled
                ? <ToggleRight size={28} className="text-app-success" />
                : <ToggleLeft size={28} className="text-app-foreground/30" />}
              <span className="text-app-foreground/70 text-xs font-bold">
        ...
        <div className="p-5 space-y-5">
          {/* Provider selector */}
          <div>
            <p className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest mb-3">Select Provider</p>
            <div className="grid grid-cols-5 gap-2">
              {SMS_PROVIDERS.map(p => {
                ...
                return (
                  ...
                  >
                    <div className={clsx("w-8 h-8 rounded-xl flex items-center justify-center", isActive ? 'bg-app-foreground/70' : 'bg-app-surface')}>
                      <Icon size={16} className={p.color} />
                    </div>
                    <span className={clsx("text-[10px] font-black leading-tight", isActive ? 'text-app-foreground' : 'text-app-muted-foreground')}>
                      {p.label}
                    </span>
                    {isActive && <Check size={10} className="text-app-primary" />}
                  </button>
                );
              })}
            </div>
            {activeProvider.key !== 'none' && (
              <p className="text-xs text-app-muted-foreground mt-2">{activeProvider.desc}</p>
            )}
          </div>
          ...
          {activeProvider.fields.length > 0 && (
            <div className="space-y-3">
              <p className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest">Credentials</p>
              {activeProvider.fields.map(f => {
                const isSecret = f.secret;
                const shown = showSecrets[f.key];
                return (
                  <div key={f.key}>
                    <label className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest block mb-1.5">
                      {f.label}
                    </label>
                    <div className="relative">
                      <input
                        ...
                        className="w-full px-4 py-3 bg-app-background border border-app-border rounded-2xl text-sm font-mono outline-none focus:border-app-primary/30 focus:ring-2 focus:ring-indigo-50 pr-10 placeholder:text-app-muted-foreground"
                      />
                      {isSecret && (
                        <button
                          onClick={() => setShowSecrets(s => ({ ...s, [f.key]: !s[f.key] }))}
                          className="absolute right-3 top-1/2 -translate-y-1/2 text-app-muted-foreground hover:text-app-muted-foreground"
                        >
          ...
          {settings.sms_delivery_code_enabled && activeProvider.key !== 'none' && (
            <div className="bg-app-background rounded-2xl p-4 border border-app-border">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-app-surface border border-app-border rounded-xl text-sm outline-none focus:border-app-primary/30"
                />
                <button
                  onClick={handleTestSMS}
                  disabled={testing || !testPhone}
                  className="px-4 py-2 bg-app-primary text-app-foreground rounded-xl text-xs font-black hover:bg-app-primary disabled:opacity-40 transition-all flex items-center gap-1.5"
                >
                  {testing ? <Loader2 size={12} className="animate-spin" /> : <TestTube size={12} />}
                  Send Test
                </button>
              </div>
              <p className="text-[10px] text-app-muted-foreground mt-1.5">Sends a sample confirmation code to verify your integration</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* ── Loyalty Program ── */}
      <div className="p-6 rounded-3xl border border-app-warning/30 bg-app-warning-bg/40 space-y-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-app-warning-bg flex items-center justify-center text-app-warning">
            ⭐
          </div>
          <div>
            <h3 className="text-sm font-black text-app-foreground">Loyalty Program</h3>
            <p className="text-[10px] text-app-muted-foreground">Configure how points are earned and what they are worth</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest block mb-1">
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-app-surface border border-app-warning rounded-xl text-sm font-bold text-app-foreground outline-none focus:ring-2 focus:ring-amber-300"
            />
            <p className="text-[9px] text-app-muted-foreground mt-1">Monetary value of 1 point (e.g. 500 = 1pt is worth 500 FCFA)</p>
          </div>
          <div>
            <label className="text-[10px] font-black text-app-muted-foreground uppercase tracking-widest block mb-1">
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-app-surface border border-app-warning rounded-xl text-sm font-bold text-app-foreground outline-none focus:ring-2 focus:ring-amber-300"
            />
            <p className="text-[9px] text-app-muted-foreground mt-1">Currency spent to earn 1 point (e.g. 10000 = spend 10,000 to get 1pt)</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-end">
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-indigo-500 to-indigo-600 text-app-foreground rounded-2xl text-sm font-black hover:from-indigo-400 hover:to-indigo-500 disabled:opacity-50 transition-all shadow-lg shadow-indigo-200"
        >
          {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}
