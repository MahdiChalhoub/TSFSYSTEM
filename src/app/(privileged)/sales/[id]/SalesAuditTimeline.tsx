...
const ACTION_ACCENT: Record<string, string> = {
 ...
 ORDER_CLOSED: 'var(--app-text-muted)',
 ORDER_CANCELLED: '#f43f5e',
 DELIVERY_DELIVERED: '#3b82f6',
 ...
 INVOICE_SENT: '#a5b4fc',
 STOCK_RESERVED: '#f59e0b',
 STOCK_RELEASED: '#fcd34d',
 STOCK_DEDUCTED: '#d97706',
 FIELD_CHANGE: 'var(--app-text-faint)',
 WORKFLOW_TRANSITION: 'var(--app-text-muted)',
}
...
export function SalesAuditTimeline({ orderId }: { orderId: string | number }) {
 ...
 if (loading) {
 return (
 <div className="flex items-center gap-2 py-4 text-sm" style={{ color: 'var(--app-text-muted)' }}>
 ...
 <ShieldCheck size={22} style={{ color: 'var(--app-text-faint)' }} />
 <p className="text-xs italic" style={{ color: 'var(--app-text-faint)' }}>
 ...
 {entries.map((entry) => {
 const accent = ACTION_ACCENT[entry.action_type] ?? 'var(--app-text-faint)'
 ...
 <div>
 <span className="text-xs font-black" style={{ color: 'var(--app-text)' }}>
 {formatLabel(entry.action_type)}
 </span>
 {entry.actor_name && (
 <span className="text-[10px] ml-2" style={{ color: 'var(--app-text-muted)' }}>
 by {entry.actor_name}
 </span>
 )}
 </div>
 <time className="text-[10px] flex-shrink-0 whitespace-nowrap" style={{ color: 'var(--app-text-faint)' }}>
 ...
 {entry.summary && (
 <p className="text-[11px] mt-1 leading-relaxed" style={{ color: 'var(--app-text-muted)' }}>
 {entry.summary}
 </p>
 )}
 ...
 >
 <span className="font-mono" style={{ color: 'var(--app-text-muted)' }}>{field}</span>
 ...
 style={{ background: 'var(--app-bg)', color: 'var(--app-text-faint)', border: '1px solid var(--app-border)' }}
 >
 {label}: {val}
 </span>
 ))}
