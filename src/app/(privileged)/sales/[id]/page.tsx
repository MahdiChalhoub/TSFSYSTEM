    ...
    if (!order) {
        return (
            <div className="flex flex-col items-center justify-center p-20 gap-4">
                ...
                <p className="text-app-text-faint text-sm max-w-md text-center">
                ...
                <div className="flex gap-4 mt-6">
                    <Link href="/sales/history" className="bg-emerald-500 text-app-text px-6 py-2 rounded-xl font-bold shadow-lg shadow-emerald-100">Return to History</Link>
                </div>
                <div className="text-[10px] text-app-text-faint font-mono mt-20 p-4 border border-dashed border-app-border rounded-lg">
                    Diagnostic Info:<br />
                    Attempted ID: {id}<br />
                    Path: /sales/[id]<br />
                    Mode: Production
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <Link href="/sales/history" className="flex items-center gap-2 text-xs font-bold text-app-text-faint hover:text-emerald-500 transition-all mb-4">
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    <button className="p-3.5 bg-app-surface border border-app-border rounded-2xl text-app-text-faint hover:text-emerald-600 hover:border-emerald-100 transition-all shadow-sm flex items-center gap-2">
                        <Printer size={20} />
            ...
            <div className="grid md:grid-cols-4 gap-4">
                {[
                    { label: 'Order Status', value: order.order_status || order.status || 'DRAFT', color: 'text-app-text' },
                    ...
                    <div key={label} className="bg-app-surface p-5 rounded-[2rem] border border-app-border shadow-sm">
                        <div className="text-[9px] font-black text-app-text-faint uppercase tracking-widest mb-1">{label}</div>
            ...
            <div className="grid md:grid-cols-3 gap-4">
                <div className="bg-app-surface p-5 rounded-[2rem] border border-app-border shadow-sm">
                    <div className="text-[9px] font-black text-app-text-faint uppercase tracking-widest mb-1">Date</div>
                    <div className="text-base font-black text-app-text">{order.created_at ? new Date(order.created_at).toLocaleDateString('fr-FR') : '—'}</div>
                </div>
                <div className="bg-app-surface p-5 rounded-[2rem] border border-app-border shadow-sm">
                    <div className="text-[9px] font-black text-app-text-faint uppercase tracking-widest mb-1">Customer</div>
                    <div className="text-base font-black text-app-text truncate">{order.contact_name || 'Walking Customer'}</div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                            ...
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {(order.lines || []).map((line: Record<string, any>) => (
                                        <tr key={line.id} className="text-sm">
                                            <td className="p-6">
                                                ...
                                                <div className="text-[10px] text-app-text-faint font-mono">SKU: {line.product_sku || 'N/A'}</div>
                                            </td>
                                            ...
                                            <td className="p-6 text-right text-app-text-faint">
                                    ...
                                    {(!order.lines || order.lines.length === 0) && (
                                        <tr>
                                            <td colSpan={5} className="p-10 text-center text-app-text-faint italic">
                    ...
                    <div className="bg-gray-900 text-app-text p-10 rounded-[3rem] shadow-2xl flex flex-col md:flex-row justify-between items-center gap-8">
                        <div className="flex gap-8">
                            <div>
                                <div className="text-[10px] font-black text-app-text-muted uppercase tracking-widest mb-1">Subtotal HT</div>
                                <div className="text-xl font-bold">{fmt(totalAmount - taxAmount)}</div>
                            </div>
                            <div>
                                <div className="text-[10px] font-black text-app-text-muted uppercase tracking-widest mb-1">Total Tax</div>
                                <div className="text-xl font-bold">{fmt(taxAmount)}</div>
                            </div>
                        </div>
                        <div className="text-right">
                            <div className="text-[10px] font-black text-app-text-muted uppercase tracking-widest mb-1">Grand Total TTC</div>
                ...
                <div className="space-y-6">
                    <div className="bg-app-surface p-6 rounded-[2rem] border border-app-border shadow-sm space-y-6">
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                <div className="text-[10px] font-black text-app-text-faint uppercase tracking-widest">Payment Info</div>
                                <div className="text-sm font-black text-app-text">{order.payment_method || 'N/A'}</div>
                            </div>
                        </div>
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                <div className="text-[10px] font-black text-app-text-faint uppercase tracking-widest">Transaction Time</div>
                                <div className="text-sm font-black text-app-text">{order.created_at ? new Date(order.created_at).toLocaleTimeString('fr-FR') : '—'}</div>
                            </div>
                        </div>
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                <div className="text-[10px] font-black text-app-text-faint uppercase tracking-widest">Processing Site</div>
                                <div className="text-sm font-black text-app-text">{order.site_name || 'Main Warehouse'}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-app-surface p-6 rounded-[2rem] border border-app-border shadow-sm">
                        <div className="flex items-center gap-2 mb-4">
                            <HandCoins size={16} className="text-app-text-faint" />
                            <h3 className="text-xs font-black text-app-text-muted uppercase tracking-widest">Payment Reconciliation</h3>
                        </div>
                    ...
                    <div className="bg-app-surface p-6 rounded-[2rem] border border-app-border shadow-sm">
                        <div className="flex items-center gap-2 mb-4">
                            <History size={16} className="text-app-text-faint" />
                            <h3 className="text-xs font-black text-app-text-muted uppercase tracking-widest">Audit Trail</h3>
                        </div>
