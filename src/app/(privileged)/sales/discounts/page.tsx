...
export default function DiscountRulesPage() {
 ...
 <span className="font-black text-app-text leading-tight">{rule.name}</span>
 ...
 render: (rule) => (
 <span className="text-sm font-black text-app-text tracking-tighter">
 ...
 <Icon size={12} className="text-app-text-faint" />
 ...
 render: (rule) => (
 <div className="flex flex-col">
 <div className="text-xs font-bold text-app-text">{rule.times_used ?? 0} <span className="text-stone-300 font-medium">/ {rule.usage_limit || '\u221E'}</span></div>
 ...
 <Badge className="bg-app-bg text-app-text-faint border-app-border text-[9px] font-black uppercase tracking-widest">Paused</Badge>
 )}
 ...
 <p className="text-sm font-medium text-app-text-faint mt-2 uppercase tracking-widest">Promotions & Discounts</p>
 </div>
 ...
 <div>
 <p className="text-[10px] font-black uppercase tracking-widest text-app-text-faint">Live Rules</p>
 <p className="text-3xl font-black mt-1 tracking-tighter text-app-text">{rules.filter(r => r.is_active).length}</p>
 ...
 <div>
 <p className="text-[10px] font-black uppercase tracking-widest text-app-text-faint">Total Redemptions</p>
 <p className="text-3xl font-black mt-1 tracking-tighter text-app-text">{rules.reduce((s, r) => s + (Number(r.times_used) || 0), 0)}</p>
 ...
 <div>
 <p className="text-[10px] font-black uppercase tracking-widest text-app-text-faint">Auto-Apply</p>
 <p className="text-3xl font-black mt-1 tracking-tighter text-app-text">{rules.filter(r => r.auto_apply).length}</p>
 ...
 <div>
 <p className="text-[10px] font-black uppercase tracking-widest text-app-text-faint">Scheduled</p>
 <p className="text-3xl font-black mt-1 tracking-tighter text-app-text">{rules.filter(r => r.start_date || r.end_date).length}</p>
 ...
 <div>
 <CardTitle className="text-2xl font-black tracking-tight text-app-text">
 {editId ? 'Edit Rule' : 'New Rule'}
 </CardTitle>
 <p className="text-[10px] font-black uppercase tracking-[0.2em] text-app-text-faint mt-1">Promotion Settings</p>
 </div>
 <Button onClick={() => setShowForm(false)} variant="ghost" size="icon" className="rounded-2xl hover:bg-app-bg text-stone-300 hover:text-app-text">
 <X size={24} />
 </Button>
 </CardHeader>
 ...
 <div className="flex items-center gap-2 text-app-text font-black text-xs uppercase tracking-widest">
 <Tag size={14} className="text-amber-500" /> Identity
 </div>
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Rule Designation *</label>
 <Input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Black Friday 40% Off" className="h-10 rounded-xl bg-app-bg border-app-border" />
 </div>
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Trigger Code</label>
 ...
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Benefit Logic</label>
 <Select value={form.discount_type} onValueChange={v => setForm({ ...form, discount_type: v })}>
 <SelectTrigger className="h-10 rounded-xl bg-app-bg border-app-border text-xs font-bold"><SelectValue /></SelectTrigger>
 ...
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Value</label>
 ...
 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Rule Domain</label>
 ...
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Priority SEQ</label>
 <Input type="number" value={form.priority} onChange={e => setForm({ ...form, priority: e.target.value })} className="h-10 rounded-xl" />
 </div>
 </div>
 {form.scope === 'PRODUCT' && (
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Target Entity</label>
 <Select value={form.product} onValueChange={v => setForm({ ...form, product: v })}>
 <SelectTrigger className="h-10 rounded-xl bg-app-bg border-app-border text-xs font-bold"><SelectValue placeholder="Select entity..." /></SelectTrigger>
 ...
 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Threshold Amnt</label>
 ...
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Threshold Qty</label>
 ...
 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Activation</label>
 ...
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Expiry</label>
 <Input type="date" value={form.end_date} onChange={e => setForm({ ...form, end_date: e.target.value })} className="h-10 rounded-xl text-xs" />
 </div>
 </div>
 <div>
 <label className="text-[10px] font-black uppercase text-app-text-faint mb-1.5 block">Global Usage Cap</label>
 ...
 <p className="text-[10px] font-black uppercase tracking-widest text-app-text-faint mt-1">Transaction Linkage Monitor</p>
 </div>
 <Button onClick={() => setUsageRuleId(null)} variant="ghost" size="icon" className="rounded-2xl text-stone-300 hover:text-app-text">
 <X size={24} />
 </Button>
 </header>
 ...
 <p className="text-sm font-bold text-app-text-faint">Neutral Data: No transactions found</p>
 </div>
 ) : (
 <div className="space-y-4">
 {usageLogs.map(log => (
 <div key={log.id} className="p-5 border border-app-border rounded-[2rem] hover:bg-app-bg hover:border-indigo-100 transition-all group/log">
 <div className="flex items-center justify-between">
 <div className="space-y-1">
 <p className="text-sm font-black text-app-text group-hover/log:text-indigo-600 transition-colors">Order #{String(log.order_ref || log.order)}</p>
 <p className="text-[10px] font-mono text-app-text-faint font-bold uppercase tracking-tight">
