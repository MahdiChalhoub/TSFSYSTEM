...
function CreateUserModal({ roles, onClose, onCreated }: {
 ...
 <h2 className="text-lg font-black text-app-text">Create New User</h2>
 <p className="text-xs text-app-text-faint">Add a team member or driver to your organization</p>
 </div>
 <button onClick={onClose} className="w-8 h-8 rounded-xl bg-app-surface-2 hover:bg-gray-200 text-app-text-muted flex items-center justify-center transition-all">
 <X size={14} />
 </button>
 </div>
 ...
 <div>
 <label className="block text-xs font-bold text-app-text-muted uppercase tracking-wider mb-1">First Name *</label>
 <input value={form.first_name} onChange={set('first_name')} placeholder="Adham"
 className={clsx("w-full px-3 py-2.5 border rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all", errors.first_name ? 'border-rose-300 bg-rose-50' : 'border-app-border bg-app-bg')}
 />
 {errors.first_name && <p className="text-rose-500 text-[10px] mt-0.5">{errors.first_name}</p>}
 </div>
 <div>
 <label className="block text-xs font-bold text-app-text-muted uppercase tracking-wider mb-1">Last Name</label>
 <input value={form.last_name} onChange={set('last_name')} placeholder="Chalhoub"
 className="w-full px-3 py-2.5 border border-app-border bg-app-bg rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all"
 />
 </div>
 </div>
 ...
 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="block text-xs font-bold text-app-text-muted uppercase tracking-wider mb-1">Username *</label>
 <input value={form.username} onChange={set('username')} placeholder="adham.c" autoComplete="off"
 className={clsx("w-full px-3 py-2.5 border rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all", errors.username ? 'border-rose-300 bg-rose-50' : 'border-app-border bg-app-bg')}
 />
 {errors.username && <p className="text-rose-500 text-[10px] mt-0.5">{errors.username}</p>}
 </div>
 <div>
 <label className="block text-xs font-bold text-app-text-muted uppercase tracking-wider mb-1">Email</label>
 <div className="relative">
 <Mail size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-app-text-faint" />
 <input type="email" value={form.email} onChange={set('email')} placeholder="name@company.com"
 className="w-full pl-8 pr-3 py-2.5 border border-app-border bg-app-bg rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all"
 />
 </div>
 </div>
 </div>
 ...
 {/* Password */}
 <div>
 <label className="block text-xs font-bold text-app-text-muted uppercase tracking-wider mb-1">Password *</label>
 <div className="relative">
 ...
 className={clsx("w-full px-3 py-2.5 pr-10 border rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all", errors.password ? 'border-rose-300 bg-rose-50' : 'border-app-border bg-app-bg')}
 />
 <button type="button" onClick={() => setShowPassword(s => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-app-text-faint hover:text-app-text-muted">
 ...
 <p className={clsx("font-black text-sm", form.is_driver ? "text-amber-800" : "text-app-text-muted")}>
 Tag as Delivery Driver
 </p>
 <p className={clsx("text-xs mt-0.5", form.is_driver ? "text-amber-600" : "text-app-text-faint")}>
 {form.is_driver
 ...
 className="flex-1 py-3 border border-app-border rounded-2xl text-app-text-muted font-bold text-sm hover:bg-app-bg transition-all">
 Cancel
 </button>
...
export default function DriverManagementPage() {
 ...
 <Link href="/sales" className="flex items-center gap-1.5 text-app-text-faint hover:text-gray-700 text-xs font-bold transition-all">
 ...
 <button onClick={load}
 className="bg-app-surface border border-app-border text-app-text-muted h-12 px-4 rounded-2xl font-bold hover:text-emerald-600 hover:border-emerald-100 transition-all flex items-center gap-2 shadow-sm">
 ...
 {[
 { label: 'Total Users', value: users.length, icon: User, bg: 'bg-app-surface-2', color: 'text-app-text-muted', vcol: 'text-slate-700' },
 ...
 <div>
 <p className="text-xs font-bold text-app-text-faint uppercase tracking-wider">{stat.label}</p>
 ...
 <Search size={15} className="absolute left-4 top-1/2 -translate-y-1/2 text-app-text-faint" />
 <input
 value={search}
 onChange={e => setSearch(e.target.value)}
 placeholder="Search by name or email…"
 className="w-full pl-10 pr-4 py-3 border border-app-border bg-app-surface rounded-2xl text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-300 transition-all shadow-sm"
 />
 </div>
 ...
 tab === t ? "bg-app-surface shadow-sm text-app-text" : "text-app-text-faint hover:text-gray-700"
 )}>
 ...
 {/* Table header */}
 <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-app-bg border-b border-app-border">
 <div className="col-span-4 text-xs font-black text-app-text-faint uppercase tracking-wider">User</div>
 <div className="col-span-3 text-xs font-black text-app-text-faint uppercase tracking-wider">Email</div>
 <div className="col-span-2 text-xs font-black text-app-text-faint uppercase tracking-wider">Status</div>
 <div className="col-span-2 text-xs font-black text-app-text-faint uppercase tracking-wider">Driver Tag</div>
 <div className="col-span-1 text-xs font-black text-app-text-faint uppercase tracking-wider"></div>
 </div>

 {loading ? (
 <div className="flex flex-col items-center justify-center py-20 gap-3">
 <Loader2 size={24} className="animate-spin text-gray-300" />
 <p className="text-app-text-faint text-sm">Loading users…</p>
 </div>
 ) : filtered.length === 0 ? (
 <div className="flex flex-col items-center justify-center py-20 gap-3 text-app-text-faint">
 ...
 <p className="font-bold text-app-text text-sm truncate">{name}</p>
 <p className="text-xs text-app-text-faint font-mono truncate">@{user.username}</p>
 </div>
 </div>
 {/* Email */}
 ...
 user.is_active ? "bg-emerald-50 text-emerald-700" : "bg-app-surface-2 text-app-text-faint"
 )}>
 ...
 : "bg-app-surface-2 text-app-text-muted hover:bg-amber-50 hover:text-amber-600 hover:border-amber-200 border border-app-border"
 )}
 >
