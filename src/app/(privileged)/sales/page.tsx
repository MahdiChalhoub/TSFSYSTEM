...
import { POSLayoutArcticGlass } from '@/components/pos/layouts/POSLayoutArcticGlass';
            ...
            return (
                <div className="min-h-[60vh] flex items-center justify-center p-8">
                    <div className="text-center max-w-md">
                        <div className="w-16 h-16 rounded-full bg-app-error/20 flex items-center justify-center mx-auto mb-4">
                            <AlertCircle size={32} className="text-rose-400" />
                        </div>
                        <h2 className="text-xl font-black text-app-foreground mb-2">POS Error</h2>
                        <p className="text-app-muted-foreground text-sm mb-4">{this.state.error || 'An unexpected error occurred. This is usually caused by a stale browser cache after a deployment.'}</p>
                        <button
                            onClick={() => { window.location.reload(); }}
                            className="px-6 py-3 bg-app-primary text-app-foreground rounded-xl font-bold hover:bg-app-primary transition-all flex items-center gap-2 mx-auto"
                        >
                            <RefreshCw size={16} /> Hard Reload
                        </button>
                        <p className="text-app-muted-foreground text-xs mt-4">If this persists, try Ctrl+Shift+R or clear your browser cache.</p>
                    </div>
                </div>
            );
        }
...
function POSTerminalInner() {
    ...
    useEffect(() => {
        ...
        if (saved && ['classic', 'modern', 'compact', 'original', 'intelligence', 'arctic-glass'].includes(saved)) {
    ...
    return (
        <>
            ...
            {currentLayout === 'arctic-glass' && <POSLayoutArcticGlass {...layoutProps as any} />}
            ...
            {terminal.showCreditWarning && (
                <div className="fixed inset-0 z-[200] bg-app-background/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-150">
                    <div className="bg-app-surface rounded-3xl shadow-2xl w-full max-w-md animate-in zoom-in-95 duration-200 overflow-hidden">
                        <div className="bg-app-warning px-6 py-5 flex items-center gap-4">
                            <div className="w-12 h-12 rounded-2xl bg-app-foreground/20 flex items-center justify-center shrink-0">
                        ...
                        <div className="px-6 py-6 space-y-4">
                            <div className="bg-app-warning-bg border border-app-warning rounded-2xl p-4 text-center">
                                <p className="text-xs font-bold text-app-warning uppercase tracking-widest mb-1">Amount to be credited</p>
                                <p className="text-4xl font-black text-app-warning tabular-nums">{terminal.currency}{terminal.creditWarningAmount.toFixed(2)}</p>
                            </div>
                            <div className="app-page space-y-2 text-sm text-app-muted-foreground">
                            ...
                            <div className="flex gap-3 pt-2">
                                <button onClick={() => terminal.setShowCreditWarning(false)}
                                    className="flex-1 py-3.5 border-2 border-app-border rounded-2xl text-app-muted-foreground font-bold text-sm hover:bg-app-background transition-all">
                                    Cancel
                                </button>
                                <button onClick={() => { terminal.setShowCreditWarning(false); terminal.handleCharge(true); }}
                                    className="flex-1 py-3.5 bg-app-warning hover:bg-app-warning text-app-foreground rounded-2xl font-black text-sm shadow-lg shadow-amber-200 transition-all">
                                    ✓ Confirm Credit Sale
                                </button>
                            </div>
                        </div>
