 ...
 * Advanced: { register_id, cashier_id, opening_mode: 'advanced', account_reconciliations, cash_counted, cash_software, account_book_balance, notes }
 */
export async function openRegisterSession(
    ...
    try {
        ...
        if (advancedData) {
            ...
            body.account_book_balance = advancedData.account_book_balance;
        } else {
...
/**
 * Get the live Account Book balance for a specific register.
 * Returns the net balance of all CashierAddressBook entries for the register's current session.
 */
export async function getAccountBookBalance(registerId: number): Promise<{
    success: boolean;
    balance?: number;
    entryCount?: number;
    pendingCount?: number;
    ...
    try {
        const data = await erpFetch(`pos-registers/account-book-balance/?register_id=${registerId}`);
        return { success: true, ...data };
    } catch (error: any) {
        return { success: false, error: error?.message || 'Failed to load account book balance' };
    }
}

/**
 * Get ledger balances for all payment accounts linked to a register.
 * Returns software balance (from COA ledger) + account metadata.
 */
export async function getRegisterAccountBalances(registerId: number): Promise<{
    success: boolean;
    data?: Array<{
        accountId: number;
        name: string;
    ...
    try {
        const data = await erpFetch(`pos-registers/account-balances/?register_id=${registerId}`);
        return { success: true, data: Array.isArray(data) ? data : [] };
    } catch (error: any) {
        return { success: false, error: error?.message || 'Failed to load account balances' };
    }
}
