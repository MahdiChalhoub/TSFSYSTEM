                ...
                <div className="w-1/2 flex flex-col gap-2">
                    {loading ? Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-16 bg-gray-800/50 rounded-xl animate-pulse" />) :
                        settlements.length === 0 ? (
                            <div className="bg-[#0F1729] rounded-2xl border border-gray-800 p-12 text-center text-gray-500 text-sm">No consignment settlements yet.</div>
                        ) : settlements.map(s => (
                            <button key={s.id} onClick={() => openDetail(s)} className={`w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${selected?.id === s.id ? 'bg-orange-900/20 border-orange-700' : 'bg-[#0F1729] border-gray-800 hover:border-gray-700'}`}>
                                <div className="flex-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${STATUS[s.status as keyof typeof STATUS] || 'bg-gray-800 text-gray-400 border-gray-700'}`}>{s.status}</span>
                                    </div>
                        ...
                        <>
                            <div className="flex items-start justify-between">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-400">{selected.supplier?.name || selected.supplier_name} · {selected.settlement_date}</p>
                                    {selected.performed_by && (
                                        <p className="text-xs text-gray-500 mt-0.5">By: {selected.performed_by.first_name} {selected.performed_by.last_name}</p>
                                    )}
                                </div>
                            ...
                            {detail?.lines && detail.lines.length > 0 && (
                                <div>
                                    <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Lines</h3>
