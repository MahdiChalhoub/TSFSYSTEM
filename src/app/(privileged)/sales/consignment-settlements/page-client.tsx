...
export default function ConsignmentPage() {
 ...
 Consignment <span className="text-app-primary">Settlements</span>
 </h1>
 <p className="text-sm font-medium text-app-muted-foreground mt-2 uppercase tracking-widest">Partner Reconciliation</p>
 </div>
 </div>
 ...
 <div key={s.label} className="bg-[#0F1729] rounded-2xl border border-app-border p-5">
 <div className="flex items-center gap-2 text-app-muted-foreground text-xs mb-2"><s.icon size={14} />{s.label}</div>
 ...
 {loading ? Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-16 bg-app-surface-2/50 rounded-xl animate-pulse" />) :
 settlements.length === 0 ? (
 <div className="bg-[#0F1729] rounded-2xl border border-app-border p-12 text-center text-app-muted-foreground text-sm">No consignment settlements yet.</div>
 ) : settlements.map(s => (
 <button key={s.id} onClick={() => openDetail(s)} className={`w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${selected?.id === s.id ? 'bg-orange-900/20 border-orange-700' : 'bg-[#0F1729] border-app-border hover:border-app-border'}`}>
 <div className="flex-1">
 <div className="flex items-center gap-2">
 <span className="font-medium text-sm text-app-foreground">{s.settlement_number || `#${s.id}`}</span>
 <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${STATUS[s.status as keyof typeof STATUS] || 'bg-app-surface-2 text-app-muted-foreground border-app-border'}`}>{s.status}</span>
 </div>
 <div className="flex items-center gap-2 text-xs text-app-muted-foreground mt-0.5">
 <User size={10} />
 ...
 <div className="text-sm font-semibold text-app-foreground">${Number(s.total_amount || 0).toFixed(2)}</div>
 ...
 <div className="text-xl font-bold text-app-foreground">${Number(selected.total_amount || 0).toFixed(2)}</div>
 </div>
 ...
 <div className="flex-1">
 <div className="text-sm text-app-foreground">{line.product?.name || line.product_name || '—'}</div>
 </div>
 <div className="text-right text-xs text-app-muted-foreground">
 <div>{line.quantity} × ${Number(line.unit_price || 0).toFixed(2)}</div>
 <div className="font-semibold text-app-foreground">${Number(line.subtotal || 0).toFixed(2)}</div>
 </div>
 </div>
 ))}
 </div>
