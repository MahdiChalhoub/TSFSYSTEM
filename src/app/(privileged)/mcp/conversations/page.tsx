...
type Conversations = Record<string, any>

const ALL_COLUMNS: ColumnDef<Conversations>[] = [
...
export default function ConversationsListPage() {
  ...
  const settings = useListViewSettings('mcp_conversations', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('mcp/conversations/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Conversations>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/mcp/conversations/new')}
        ...
        actions={{
          onView: (r) => router.push(`/mcp/conversations/${r.id}`),
          onEdit: (r) => router.push(`/mcp/conversations/${r.id}/edit`),
        }}
      >
        <TypicalFilter
