...
type AgentLogs = Record<string, any>

const ALL_COLUMNS: ColumnDef<AgentLogs>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function AgentLogsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<AgentLogs[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('mcp_agent-logs', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('mcp/agent-logs/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<AgentLogs>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/mcp/agent-logs/new')}
        ...
        actions={{
          onView: (r) => router.push(`/mcp/agent-logs/${r.id}`),
          onEdit: (r) => router.push(`/mcp/agent-logs/${r.id}/edit`),
        }}
      >
        <TypicalFilter
