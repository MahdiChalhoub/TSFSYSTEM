...
export default function AgentsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`mcp/agents/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`mcp/agents/${id}/`, {
        method: 'DELETE'
      })
      router.push('/mcp/agents')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Agents #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage agents details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/mcp/agents/${id}/edit`)}
            >
