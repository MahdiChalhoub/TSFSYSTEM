...
export default function ToolsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`mcp/tools/${id}/`)
      setItem(data)
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Tools #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage tools details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/mcp/tools/${id}/edit`)}
            >
