...
const ALL_COLUMNS: ColumnDef<Tools>[] = [
...
export default function ToolsListPage() {
  ...
  const settings = useListViewSettings('mcp_tools', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('mcp/tools/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Tools>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/mcp/tools/new')}
        ...
        actions={{
          onView: (r) => router.push(`/mcp/tools/${r.id}`),
          onEdit: (r) => router.push(`/mcp/tools/${r.id}/edit`),
        }}
      >
        <TypicalFilter
