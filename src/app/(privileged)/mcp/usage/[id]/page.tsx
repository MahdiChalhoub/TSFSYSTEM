...
export default function UsageDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`mcp/usage/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`mcp/usage/${id}/`, {
        method: 'DELETE'
      })
      router.push('/mcp/usage')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Usage #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage usage details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/mcp/usage/${id}/edit`)}
            >
