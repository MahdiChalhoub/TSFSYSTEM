...
const ALL_COLUMNS: ColumnDef<Usage>[] = [
...
export default function UsageListPage() {
  ...
  const settings = useListViewSettings('mcp_usage', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('mcp/usage/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Usage>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/mcp/usage/new')}
        ...
        actions={{
          onView: (r) => router.push(`/mcp/usage/${r.id}`),
          onEdit: (r) => router.push(`/mcp/usage/${r.id}/edit`),
        }}
      >
        <TypicalFilter
