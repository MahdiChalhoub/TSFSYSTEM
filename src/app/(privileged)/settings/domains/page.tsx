 ...
 ${type === 'SHOP' ? 'bg-violet-50 text-app-primary border border-violet-200' : 'bg-app-primary/5 text-app-primary border border-app-primary/30'}`}>
    ...
    return (
        <SettingsPageShell
            ...
            subtitle="Link your own domains to your storefront and control panel"
            icon={<Globe size={20} className="text-white" />}
            configKey="domains"
            config={domains.length > 0 ? Object.fromEntries(domains.map(d => [d.domain, d])) : null}
            hasChanges={false}
            onReload={refresh}
        >
                ...
                <div className="bg-app-surface border border-app-border rounded-2xl p-6 shadow-sm">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        <Input
                            ...
                            className="flex-1 h-12 rounded-xl border-app-border font-mono"
            ...
            {loading ? (
                <div className="space-y-4">
                    {[1, 2].map(i => (
                        <div key={i} className="bg-app-surface border border-app-border rounded-2xl p-6 animate-pulse">
                                ...
                                {domain.is_verified && !domain.is_active && (
                                    <div className="border-t border-app-border bg-app-info-bg/30 p-6 space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            ...
                                            <DnsCard label="Target / Points To" value={domain.cname_target || `saas${PLATFORM_CONFIG.suffix}`} onCopy={() => copyToClipboard(domain.cname_target || `saas${PLATFORM_CONFIG.suffix}`)} />
                                        </div>
