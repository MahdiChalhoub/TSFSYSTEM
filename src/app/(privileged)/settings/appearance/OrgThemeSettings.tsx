...
export function OrgThemeSettings({ currentOrgDefault }: { currentOrgDefault: AppThemeName | null }) {
 const { theme: userTheme, themes, setTheme } = useAppTheme();
 ...
 if (result.ok) {
 setOrgDefault(name);
 setFeedback({ type: 'success', msg: `"${themes.find(t => t.name === name)?.label}" is now the org default and your active theme.` });
 } else {
