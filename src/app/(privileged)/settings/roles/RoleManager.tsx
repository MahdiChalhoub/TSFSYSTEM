    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-[calc(100vh-280px)] min-h-[600px]">
            ...
            <div className="lg:col-span-4 flex flex-col gap-4">
                <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-4 h-full flex flex-col">
                    <div className="flex items-center justify-between mb-4 px-2">
                        <h2 className="text-xs font-black uppercase text-gray-400 tracking-widest">Roles List</h2>
                        <Button
                            variant="ghost"
                            size="sm"
                    ...
                    <div className="relative mb-4 px-2">
                        ...
                        <Input
                            ...
                            className="pl-9 h-10 rounded-xl bg-gray-50 border-none text-xs font-medium"
                            value={searchQuery}
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={cn(
                                            ...
                                            selectedRole?.id === role.id ? "bg-white/20" : "bg-gray-100"
                                        )}>
                                            <Shield size={16} className={selectedRole?.id === role.id ? "text-white" : "text-gray-500"} />
                                        </div>
                                        <div>
                                            <p className="text-sm font-black tracking-tight">{role.name}</p>
                                            <p className={cn(
                                                "text-[10px] font-bold uppercase",
                                                selectedRole?.id === role.id ? "text-gray-300" : "text-gray-400"
                                            )}>
            ...
            <div className="lg:col-span-8 h-full">
                {selectedRole ? (
                    <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm flex flex-col h-full overflow-hidden">
                        <div className="p-6 border-b border-gray-50 flex items-center justify-between bg-gray-50/30">
                            <div>
                                ...
                                <h1 className="text-2xl font-black text-gray-900 uppercase tracking-tighter">{selectedRole.name}</h1>
                                <p className="text-xs text-gray-500 font-medium italic mt-0.5">{selectedRole.description || 'No description provided'}</p>
                            </div>
                                ...
                                {Object.entries(groupedPermissions).map(([module, perms]) => (
                                    <div key={module} className="space-y-4">
                                        <div className="flex items-center gap-4">
                                            <h3 className="text-[10px] font-black uppercase text-gray-400 tracking-widest bg-gray-100 px-3 py-1 rounded-full">{module}</h3>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="min-w-0">
                                                            <p className={cn("text-xs font-black uppercase tracking-tight mb-1", isActive ? "text-emerald-700" : "text-gray-800")}>
                                                                {perm.name}
                                                            </p>
                                                        ...
                                                        <div className={cn(
                                                            ...
                                                            isActive ? "bg-emerald-600 text-white" : "bg-gray-100 text-gray-300 group-hover:bg-gray-200"
                                                        )}>
                    ...
                    <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm flex flex-col items-center justify-center p-12 text-center h-full">
                        ...
                        <p className="text-sm text-gray-400 font-bold uppercase tracking-widest mt-1">Select a role from the list to manage its permissions</p>
                    </div>
                )}
            </div>
