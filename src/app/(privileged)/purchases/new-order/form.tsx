                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-app-border">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p className="text-[8px] text-app-muted-foreground">{totalCount} products{supplierId > 0 ? ' • Supplier filtered' : ''}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <div className="w-48 shrink-0 border-r border-app-border/40 bg-app-background/20 overflow-y-auto p-3 space-y-3">
                            ...
                            <div><div className={fLbl}>Rotation</div>
                                <select className={fCls} value={rotationFilter} onChange={e => setRotationFilter(e.target.value)}>
                                    <option value="">All</option>
                                    <option value="fast">🚀 Fast (≥5/day)</option>
                    ...
                    <div ref={scrollRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-3">
                        {displayed.length === 0 && !loading ? (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                {displayed.map((p: any) => {
                                    const daysOfStock = p.daily_sales > 0 ? Math.round(p.stock / p.daily_sales) : p.stock > 0 ? 999 : 0;
                                    const proposedQty = Math.max(0, Math.round((p.daily_sales || 0) * 14 - (p.stock || 0)));
                                    return (
                                        <button key={p.id} type="button" onClick={() => onSelect(p)}
                                            ...
                                            <div className="grid grid-cols-4 gap-1 mt-2">
                                                <div className="text-center"><div className={`text-[9px] font-black ${p.stock > 0 ? 'text-emerald-500' : 'text-rose-500'}`}>{p.stock}</div><div className="text-[7px] text-app-muted-foreground">Avail</div></div>
                                                ...
                                                <div className="text-center"><div className={`text-[9px] font-black ${p.margin_pct >= 20 ? 'text-emerald-500' : p.margin_pct >= 10 ? 'text-amber-500' : 'text-app-muted-foreground'}`}>{p.margin_pct > 0 ? `${p.margin_pct}%` : '—'}</div><div className="text-[7px] text-app-muted-foreground">Margin</div></div>
                                            ...
                                            <div className="flex items-center justify-between mt-1.5 pt-1.5 border-t border-app-border/20">
                                                <span className="text-[8px] text-app-muted-foreground">Cost: {p.cost_price} → Sell: {p.selling_price}</span>
                                                <div className="flex items-center gap-1">
                                                    <span className="text-[8px] text-app-muted-foreground">{p.daily_sales}/day</span>
