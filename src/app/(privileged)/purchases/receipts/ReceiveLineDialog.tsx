...
export default function ReceiveLineDialog({ po, line, onClose, onSuccess }: ReceiveLineDialogProps) {
 ...
 <label className="text-[10px] font-black text-app-text-faint uppercase tracking-widest block mb-2">Item Context</label>
 <div className="font-bold text-app-text">{line.product_name}</div>
 <div className="text-xs text-app-text-muted font-mono mt-0.5">{line.product_sku}</div>
 </div>
 ...
 <label className="text-xs font-black text-app-text-muted uppercase tracking-widest ml-1 mb-2 block">Quantity to Receive</label>
 <div className="relative flex items-center">
 <Truck className="absolute left-4 text-app-text-faint group-focus-within:text-emerald-500 transition-colors" size={20} />
 <input
 type="number"
 autoFocus
 step="any"
 ...
 className="w-full pl-12 pr-6 h-16 bg-app-bg rounded-2xl border-2 border-transparent focus:border-emerald-500/20 focus:bg-app-surface focus:ring-8 focus:ring-emerald-500/5 outline-none transition-all font-black text-xl text-app-text"
 placeholder="0.00"
 />
 <button
 type="button"
 ...
 type="button"
 onClick={onClose}
 className="flex-1 h-14 rounded-2xl font-black text-xs uppercase tracking-widest text-app-text-faint hover:text-app-text-muted hover:bg-app-bg transition-all"
 >
 Cancel
 </button>
 <button
