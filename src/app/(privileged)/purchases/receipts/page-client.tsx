...
const STATUS_STYLES: Record<string, { bg: string, text: string, border: string, icon: any }> = {
 DRAFT: { bg: 'bg-app-bg', text: 'text-app-text-faint', border: 'border-app-border', icon: Clock },
...
export default function ReceiptsPage() {
 ...
 <h1 className="text-5xl font-black tracking-tighter text-app-text leading-tight">
 ...
 <p className="text-sm font-bold text-app-text-faint mt-2 uppercase tracking-[0.2em] flex items-center gap-2">
 ...
 <button onClick={load} className="h-14 px-8 rounded-2xl bg-app-surface border-2 border-gray-50 text-app-text-muted font-black text-xs uppercase tracking-widest hover:bg-app-bg hover:border-app-border shadow-sm transition-all flex items-center gap-2 active:scale-95 group">
 ...
 <div className="text-[10px] font-black text-app-text-faint uppercase tracking-widest">Total Committed Pipeline</div>
 </div>
 </div>
 </div>
 ...
 <div key={s.label} className="bg-app-surface p-6 rounded-[2.5rem] border border-app-border shadow-2xl shadow-gray-200/5 hover:-translate-y-2 transition-all group overflow-hidden relative">
 ...
 <div>
 <p className="text-[10px] font-black text-app-text-faint uppercase tracking-widest">{s.label}</p>
 <p className={`text-3xl font-black text-app-text mt-0.5 tracking-tight`}>{s.value}</p>
 </div>
 </div>
 <p className="text-[11px] font-bold text-app-text-faint mt-4 px-1">{s.desc}</p>
 </div>
 ))}
 </div>
 ...
 <div className="bg-app-surface p-4 rounded-[2rem] border border-app-border shadow-xl shadow-gray-200/10 flex items-center gap-3">
 ...
 />
 <button className="p-2.5 rounded-xl bg-app-bg text-app-text-faint hover:text-emerald-600 transition-colors">
 <Filter size={16} />
 </button>
 </div>
 ...
 <div className="text-lg font-black text-app-text tracking-tighter">${po.total_amount.toLocaleString()}</div>
 <div className="text-[9px] font-black text-app-text-faint uppercase tracking-[0.2em] mt-0.5">Grand Total</div>
 </div>
 </div>

 <div className="font-mono font-black text-lg text-app-text tracking-tighter group-hover:text-emerald-700 transition-colors">
 ...
 <div className="flex items-center gap-2 text-[10px] font-bold text-app-text-muted truncate">
 <Building2 size={12} className="text-gray-300" />
 {po.supplier?.name || po.supplier_name}
 </div>
 <div className="flex items-center gap-2 text-[10px] font-bold text-app-text-muted justify-end">
 ...
 <div className="flex-1 bg-app-surface rounded-[3.5rem] border border-app-border shadow-2xl shadow-gray-200/10 overflow-hidden flex flex-col min-h-[800px]">
 {!selected ? (
 <div className="flex-1 flex flex-col items-center justify-center text-gray-300 gap-6 opacity-40">
 <div className="w-32 h-32 rounded-[3.5rem] bg-app-bg flex items-center justify-center">
 <ListRestart size={48} />
 </div>
 <div>
 ...
 <h2 className="text-4xl font-black text-app-text tracking-tighter">{selected.po_number || `PO-${selected.id}`}</h2>
 ...
 <Building2 size={18} className="text-app-text-faint" />
 </div>
 <div>
 <div className="text-[10px] font-black text-app-text-faint uppercase tracking-widest">Supplier</div>
 <div className="text-sm font-black text-app-text">{selected.supplier?.name || selected.supplier_name}</div>
 </div>
 </div>
 <div className="flex items-center gap-3">
 <button onClick={() => window.location.href = '/purchases/new'} className="h-14 px-8 rounded-2xl bg-indigo-600 text-app-text font-black text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-500/20 flex items-center gap-2">
 ...
 <div className="w-10 h-10 rounded-xl bg-app-surface border border-app-border flex items-center justify-center shadow-sm">
 <Calendar size={18} className="text-app-text-faint" />
 </div>
 <div>
 <div className="text-[10px] font-black text-app-text-faint uppercase tracking-widest">Shipment Logic</div>
 <div className="text-sm font-black text-app-text">{selected.expected_date || 'STANDARD ETA'}</div>
 </div>
 </div>
 </div>
 </div>
 <div className="text-right">
 <div className="text-5xl font-black text-app-text tracking-tighter tabular-nums">${selected.total_amount.toLocaleString()}</div>
 ...
 <p className="text-sm font-bold text-app-text-muted leading-relaxed italic">{selected.notes}</p>
 </div>
 )}
 </div>
 ...
 <h3 className="text-[11px] font-black text-app-text-faint uppercase tracking-widest mb-6 flex items-center gap-2">
 <Package size={14} /> Shipment Documentation Check-in
 </h3>

 <div className="space-y-4">
 {!detail ? Array.from({ length: 3 }).map((_, i) => (
 <div key={i} className="h-28 bg-app-bg rounded-3xl animate-pulse" />
 )) : detail.lines?.length === 0 ? (
 <div className="p-12 bg-app-bg rounded-3xl text-center font-bold text-app-text-faint">
 ...
 return (
 <div key={line.id} className="group bg-app-surface p-6 rounded-[2.5rem] border border-app-border hover:border-emerald-200 hover:shadow-2xl hover:shadow-gray-200/20 transition-all flex flex-col md:flex-row items-center gap-8 relative overflow-hidden">
 <div className="flex items-center gap-6 flex-1 min-w-0">
 <div className="w-16 h-16 rounded-2xl bg-app-bg group-hover:bg-emerald-50 flex items-center justify-center transition-colors shrink-0">
 <Package size={28} className="text-gray-300 group-hover:text-emerald-500 transition-colors" />
 </div>
 <div className="flex-1 min-w-0">
 <div className="text-lg font-black text-app-text truncate group-hover:text-emerald-700 transition-colors">{line.product_name}</div>
 <div className="flex items-center gap-4 mt-1.5">
 <div className="text-xs font-mono font-black text-app-text-faint bg-app-bg px-2 py-0.5 rounded-lg border border-app-border">{line.product?.sku}</div>
 <div className="text-xs font-bold text-app-text-muted">Unit Basis: ${Number(line.unit_price).toFixed(2)}</div>
 </div>
 </div>
 </div>
 ...
 <span className="text-[9px] font-black text-app-text-faint uppercase tracking-widest">Progress</span>
 <span className="text-sm font-black text-app-text">{received} <span className="text-gray-300 font-medium">/</span> {ordered}</span>
 </div>
 ...
 <div className="h-3 bg-app-bg rounded-full overflow-hidden border border-app-border relative">
 <div
 ...
 <div className="text-lg font-black text-app-text">${Number(line.line_total).toLocaleString()}</div>
 <div className="text-[10px] font-bold text-app-text-faint uppercase tracking-widest">Post-Tax Value</div>
 </div>
 ...
 <div className="h-14 px-8 bg-app-bg text-app-text-faint rounded-2xl font-black text-xs uppercase tracking-widest border border-app-border flex items-center gap-2">
 ...
 <div className="p-8 bg-app-surface border-t border-gray-50 flex items-center justify-between">
 <div className="flex items-center gap-4">
 <div className="w-12 h-12 rounded-2xl bg-app-bg flex items-center justify-center text-app-text-faint border border-app-border">
 <AlertCircle size={24} />
 </div>
 <div>
 <div className="text-[10px] font-black text-app-text-faint uppercase tracking-widest leading-none">Status</div>
 <div className="text-sm font-black text-gray-700 mt-1 uppercase tracking-tighter">Awaiting Review</div>
 </div>
 </div>
 <div className="flex gap-4">
 <button className="h-14 w-14 rounded-2xl border-2 border-gray-50 flex items-center justify-center text-app-text-faint hover:text-app-text-muted hover:bg-app-bg transition-all">
