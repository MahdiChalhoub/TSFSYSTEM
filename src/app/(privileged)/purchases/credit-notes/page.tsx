...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
 DRAFT: { label: 'Draft', color: 'var(--app-muted-foreground)' }, ISSUED: { label: 'Issued', color: 'var(--app-info)' },
 APPLIED: { label: 'Applied', color: 'var(--app-success)' }, CANCELLED: { label: 'Cancelled', color: 'var(--app-error)' },
}
const ALL_COLUMNS = [
  ...
  <div className="flex flex-col h-[calc(100vh-8rem)] animate-in fade-in duration-300 transition-all">
    ...
    {focusMode ? (
     ...
     <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '8px' }}>
       {[{ label: 'Total Credits', value: fmt(totalValue), icon: <DollarSign size={11} />, color: 'var(--app-error)' },{ label: 'Draft', value: draftCount, icon: <Clock size={11} />, color: 'var(--app-warning)' },{ label: 'Applied', value: appliedCount, icon: <CheckCircle size={11} />, color: 'var(--app-success)' }].map(s => (
   ...
   <div className="flex-1 min-h-0 bg-app-surface/30 border border-app-border/50 rounded-2xl flex flex-col overflow-hidden">
    ...
    style={{ paddingLeft: "12px", paddingRight: "12px" }} onClick={() => setExpandedId(isExpanded ? null : cn.id)}>
         ...
         {isExpanded && <div className="border-b border-app-border/30 animate-in slide-in-from-top-1 duration-200 px-4 py-3" style={{ background: 'color-mix(in srgb, var(--app-surface) 40%, var(--app-bg))' }}><div className="grid grid-cols-2 md:grid-cols-4 gap-3"><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Supplier</div><div className="font-bold text-app-foreground text-sm">{supplier}</div></div><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Source</div><div className="font-bold text-app-foreground text-sm">{source}</div></div><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Reason</div><div className="font-bold text-app-foreground text-sm">{cn.reason || '—'}</div></div><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Amount</div><div className="font-bold text-sm" style={{ color: 'var(--app-error)' }}>{fmt(Number(cn.amount || cn.total_amount || 0))}</div></div></div></div>}
        </div>)
