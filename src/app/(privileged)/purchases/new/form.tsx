...
import {
    ...
    ListFilter, BookOpen, Plus, ArrowRight, Package,
...
export default function PurchaseForm({
    ...
    return (
        <form action={formAction} className="flex flex-col h-full animate-in fade-in duration-300">
            ...
            {/* ── Page Header (Dajingo Pro V2 standard) ── */}
            <div className="flex-shrink-0 px-4 md:px-6 pt-4 pb-3">
                <div className="flex items-center justify-between gap-4">

                    {/* Left: back + icon box + title + subtitle */}
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="text-[10px] md:text-[11px] font-bold text-app-muted-foreground uppercase tracking-widest mt-0.5">
                                {selectedSupplier
                                    ...
                                    : `Draft · ${scope} · Configure via ⚙`}
                            </p>
                        </div>
                    </div>

                    {/* Right: scope toggle + customize + submit */}
                    <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <button type="button" onClick={() => setSidebarOpen(!sidebarOpen)}
                            className={`flex items-center gap-1.5 text-[11px] font-bold border px-2.5 py-1.5 rounded-xl transition-all ${
                                sidebarOpen
                                    ? 'bg-app-primary/10 border-app-primary/40 text-app-primary'
                                    : 'text-app-muted-foreground hover:text-app-foreground border-app-border hover:bg-app-surface'
                            }`}>
                            <Settings2 size={13} />
            ...
            <div className="flex-1 flex min-h-0 gap-0">
                ...
                <div className="flex-1 flex flex-col min-h-0 px-4 md:px-6 pb-4">
                    ...
                    <div className="flex items-center gap-3 mb-3 flex-shrink-0">
                        ...
                        <div className="flex-1 relative">
                            <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-app-muted-foreground" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
                            <input
                                ref={searchRef}
                                type="text"
                                ...
                                onFocus={(e) => {
                                    // forward focus to the ProductSearch — we use a hidden ProductSearch for logic
                                    searchRef.current?.blur()
                                }}
                                style={{ display: 'none' }}
                                readOnly
                            />
                            {/* Actual search with dropdown */}
                            ...
                            {lines.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
                                    ...
                                    <p className="text-[11px] text-app-muted-foreground mt-1">
                                        Search above or browse the catalogue to add products.
                                    </p>
                                </div>
                            ) : (
                                lines.map((line, idx) => (
                ...
                {sidebarOpen && (
                    <div className="animate-in slide-in-from-right duration-200 flex-shrink-0">
                        <AdminSidebar
