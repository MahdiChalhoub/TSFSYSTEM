...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
 ...
 RECEIVED: { label: 'Received', color: '#14b8a6' }, ACCEPTED: { label: 'Accepted', color: '#22c55e' },
 REJECTED: { label: 'Rejected', color: '#ef4444' }, EXPIRED: { label: 'Expired', color: 'var(--app-muted-foreground)' },
  ...
  <div className="flex flex-col h-[calc(100vh-8rem)] animate-in fade-in duration-300 transition-all">
   <div className={`flex-shrink-0 space-y-4 transition-all duration-300 ${focusMode ? 'pb-2' : 'pb-4'}`}>
    {focusMode ? (
     <div className="flex items-center gap-2">
      <div className="flex items-center gap-2 flex-shrink-0"><div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: '#06b6d4' }}><Send size={14} className="text-white" /></div><span className="text-[12px] font-black text-app-foreground hidden sm:inline">Quotations</span><span className="text-[10px] font-bold text-app-muted-foreground">{filtered.length}/{quotations.length}</span></div>
     ...
     <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '8px' }}>
       {[{ label: 'Total Quoted', value: fmt(totalValue), icon: <DollarSign size={11} />, color: '#06b6d4' },{ label: 'Active', value: activeCount, icon: <Clock size={11} />, color: '#f59e0b' },{ label: 'Accepted', value: acceptedCount, icon: <CheckCircle size={11} />, color: '#22c55e' }].map(s => (
   ...
   <div className="flex-1 min-h-0 bg-app-surface/30 border border-app-border/50 rounded-2xl flex flex-col overflow-hidden">
    ...
    style={{ paddingLeft: "12px", paddingRight: "12px" }} onClick={() => setExpandedId(isExpanded ? null : q.id)}>
         ...
         {isExpanded && <div className="border-b border-app-border/30 animate-in slide-in-from-top-1 duration-200 px-4 py-3" style={{ background: 'color-mix(in srgb, var(--app-surface) 40%, var(--app-bg))' }}><div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3"><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Supplier</div><div className="font-bold text-app-foreground text-sm">{supplier}</div></div><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Valid Until</div><div className="font-bold text-app-foreground text-sm">{q.valid_until || '—'}</div></div><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Amount</div><div className="font-bold text-sm" style={{ color: '#06b6d4' }}>{fmt(Number(q.total_amount || 0))}</div></div><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Status</div><div className="font-bold text-sm" style={{ color: sc.color }}>{sc.label}</div></div></div>{q.notes && <div className="text-xs text-app-muted-foreground p-2 rounded-lg border border-app-border/30 bg-app-surface/50"><FileText size={10} className="inline mr-1" />{q.notes}</div>}</div>}
        </div>)
