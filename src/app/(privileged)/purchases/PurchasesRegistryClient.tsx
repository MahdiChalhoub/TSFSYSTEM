...
const PRIORITY_MAP: Record<string, { label: string; color: string }> = {
 LOW: { label: 'Low', color: 'text-app-text-faint' },
...
const PO_SUB_TYPE_CONFIG: Record<string, { label: string; color: string }> = {
 STANDARD: { label: 'Standard', color: 'bg-app-surface-2 text-app-text-muted' },
...
export function PurchasesRegistryClient({ orders, currency, tradeSubTypesEnabled }: PurchasesRegistryClientProps) {
 ...
 <span className="font-bold text-app-text group-hover:text-emerald-600 transition-colors uppercase tracking-tight">
 ...
 sortable: true,
 render: (po) => {
 const priorityInfo = PRIORITY_MAP[po.priority] || { label: po.priority, color: 'text-app-text-muted' }
 return (
 ...
 extra: (po) => (
 <Link href={`/purchases/${po.id}${po.is_legacy ? '?type=legacy' : ''}`} className="p-1.5 text-app-text-faint hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all">
 <Clock size={16} />
 </Link>
 )
 }}
 className="rounded-2xl shadow-sm bg-app-surface overflow-hidden border-0"
