...
type CS = { id: number; settlement_number?: string; ref_code?: string; supplier?: { id: number; name: string }; supplier_name?: string; contact_name?: string; status: string; total_amount?: number; settlement_amount?: number; settlement_date?: string; created_at?: string; notes?: string; lines?: any[] }

const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
 DRAFT: { label: 'Draft', color: 'var(--app-muted-foreground)' }, PENDING: { label: 'Pending', color: '#f59e0b' },
 SETTLED: { label: 'Settled', color: '#22c55e' }, CANCELLED: { label: 'Cancelled', color: '#ef4444' },
}
const ALL_COLUMNS = [
  ...
  <div className="flex flex-col h-[calc(100vh-8rem)] animate-in fade-in duration-300 transition-all">
    ...
    {focusMode ? (
     ...
     <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '8px' }}>
       {[{ label: 'Total Settled', value: fmt(totalValue), icon: <DollarSign size={11} />, color: '#8b5cf6' },{ label: 'Pending', value: pendingCount, icon: <Clock size={11} />, color: '#f59e0b' },{ label: 'Settled', value: settledCount, icon: <CheckCircle size={11} />, color: '#22c55e' }].map(s => (
   ...
   <div className="flex-1 min-h-0 bg-app-surface/30 border border-app-border/50 rounded-2xl flex flex-col overflow-hidden">
    ...
    style={{ paddingLeft: "12px", paddingRight: "12px" }} onClick={() => setExpandedId(isExpanded ? null : cs.id)}>
         ...
         {isExpanded && <div className="border-b border-app-border/30 animate-in slide-in-from-top-1 duration-200 px-4 py-3" style={{ background: 'color-mix(in srgb, var(--app-surface) 40%, var(--app-bg))' }}><div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3"><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Supplier</div><div className="font-bold text-app-foreground text-sm">{supplier}</div></div><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Date</div><div className="font-bold text-app-foreground text-sm">{cs.settlement_date || '—'}</div></div><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Amount</div><div className="font-bold text-sm" style={{ color: '#8b5cf6' }}>{fmt(Number(cs.settlement_amount || cs.total_amount || 0))}</div></div><div><div className="text-[8px] font-black text-app-muted-foreground uppercase tracking-widest">Status</div><div className="font-bold text-sm" style={{ color: sc.color }}>{sc.label}</div></div></div>{cs.notes && <div className="text-xs text-app-muted-foreground p-2 rounded-lg border border-app-border/30 bg-app-surface/50"><FileText size={10} className="inline mr-1" />{cs.notes}</div>}</div>}
        </div>)
