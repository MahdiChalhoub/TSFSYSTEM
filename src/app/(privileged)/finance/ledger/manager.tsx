...
function renderLedgerCell(key: string, e: JournalEntry, fmt: (n: number) => string): React.ReactNode {
  ...
  switch (key) {
    case 'reference': return <span className="font-mono text-tp-xs text-app-muted-foreground">{e.reference || '—'}</span>
    case 'journalType': return <span className="text-tp-xs text-app-foreground">{e.journal_type || e.journalType || '—'}</span>
    case 'scope':
      return e.scope === 'OFFICIAL'
        ? <span className="text-tp-xxs font-bold uppercase px-1.5 py-0.5 rounded" style={{ color: 'var(--app-success)', background: 'color-mix(in srgb, var(--app-success) 10%, transparent)' }}>OFF</span>
        : <span className="text-tp-xxs font-bold uppercase px-1.5 py-0.5 rounded" style={{ color: 'var(--app-info)', background: 'color-mix(in srgb, var(--app-info) 10%, transparent)' }}>INT</span>
    case 'sourceModule': return <span className="text-tp-xs text-app-muted-foreground">{e.source_module || '—'}</span>
    ...
    case 'currency': return <span className="text-tp-xs text-app-muted-foreground">{e.currency || '—'}</span>
    case 'exchangeRate': return <span className="text-tp-xs font-mono text-app-muted-foreground">{e.exchange_rate ? Number(e.exchange_rate).toFixed(4) : '—'}</span>
    ...
    case 'date': return <><span className="text-tp-sm text-app-muted-foreground">{e.transactionDate ? new Date(e.transactionDate).toLocaleDateString('en-GB') : '—'}</span>{e.fiscalYear && <div className="text-tp-xxs text-app-muted-foreground">{e.fiscalYear.name}</div>}</>
    case 'fiscalYear': return <span className="text-tp-xs text-app-muted-foreground">{e.fiscal_year?.name || e.fiscalYear?.name || '—'}</span>
    case 'fiscalPeriod': return <span className="text-tp-xs text-app-muted-foreground">{e.fiscal_period?.name || '—'}</span>
    case 'postedAt': return <span className="text-tp-xs text-app-muted-foreground">{e.posted_at ? new Date(e.posted_at).toLocaleDateString('en-GB') : '—'}</span>
    case 'createdAt': return <span className="text-tp-xs text-app-muted-foreground">{e.created_at ? new Date(e.created_at).toLocaleDateString('en-GB') : '—'}</span>
    case 'updatedAt': return <span className="text-tp-xs text-app-muted-foreground">{e.updated_at ? new Date(e.updated_at).toLocaleDateString('en-GB') : '—'}</span>
    case 'createdBy': return <span className="text-tp-xs text-app-muted-foreground truncate">{e.created_by?.first_name || e.created_by?.username || '—'}</span>
    case 'postedBy': return <span className="text-tp-xs text-app-muted-foreground truncate">{e.posted_by?.first_name || e.posted_by?.username || '—'}</span>
    case 'entryHash': return e.entry_hash ? <span className="font-mono text-tp-xxs truncate opacity-60">{e.entry_hash.slice(0, 12)}…</span> : <span>—</span>
    case 'site': return <span className="text-tp-xs text-app-muted-foreground truncate">{e.site?.name || '—'}</span>
    case 'status':
      return (
    ...
    default: return <span className="text-tp-xs text-app-muted-foreground">—</span>
  }
}
        ...
        {focusMode ? (
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="text-tp-md font-bold text-app-foreground hidden sm:inline">Ledger</span>
              <span className="text-tp-xs font-bold text-app-muted-foreground">{filtered.length}/{stats.total}</span>
            </div>
            ...
            <button onClick={() => setShowFilters(!showFilters)}
              className={`flex items-center gap-1 text-tp-sm font-bold px-2 py-1.5 rounded-lg border transition-all flex-shrink-0 ${showFilters ? 'border-app-primary text-app-primary bg-app-primary/5' : 'border-app-border text-app-muted-foreground'}`}>
              <SlidersHorizontal size={13} />
              {activeFilterCount > 0 && <span className="text-tp-xxs font-bold bg-app-primary text-white px-1.5 rounded-full">{activeFilterCount}</span>}
            </button>
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg md:text-xl font-bold text-app-foreground tracking-tight">General Ledger</h1>
            ...
            <KPIStrip stats={[
              ...
              { label: 'Automated', value: stats.auto, icon: <Zap size={11} />, color: 'var(--app-info)' },
            ]} />
      ...
      <DajingoListView<JournalEntry>
        ...
        renderRowTitle={entry => (
          <div className="flex-1 min-w-0">
            <div className="truncate text-tp-md font-bold text-app-foreground">JV #{entry.id}</div>
            <div className="text-tp-xs text-app-muted-foreground truncate">{entry.description || entry.reference || '—'}</div>
          </div>
        )}
