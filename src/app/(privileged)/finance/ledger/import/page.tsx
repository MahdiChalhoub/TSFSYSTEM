                        ...
                        <div className="grid grid-cols-2 gap-x-8">
                            <div className="space-y-0.5 text-xs font-mono">
                                {['date (YYYY-MM-DD)', 'description', 'debit_account_code', 'credit_account_code', 'amount'].map(c => (
                                    <div key={c} className="text-blue-700">• {c}</div>
                                ))}
                            </div>
                            <div className="space-y-0.5 text-xs font-mono text-blue-600/70">
                                <div>Optional:</div>
                                {['reference', 'currency'].map(c => <div key={c}>• {c}</div>)}
                            </div>
                        </div>
                        <button onClick={downloadSample} className="mt-3 flex items-center gap-1.5 text-xs font-bold text-blue-700 hover:text-blue-900 underline underline-offset-2">
                ...
                <div className="space-y-6 max-w-lg mx-auto text-center">
                    ...
                    {result.errors?.length > 0 && (
                        <div className="text-left border border-rose-200 rounded-xl overflow-hidden">
                            <div className="bg-rose-50 border-b border-rose-200 px-4 py-2 text-xs font-bold text-rose-700 uppercase tracking-wider">Failed Rows</div>
...
function OpeningBalanceImport() {
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-800">
                        ...
                        <div className="grid grid-cols-2 gap-x-8">
                            <div>
                                ...
                                <div className="space-y-0.5 text-xs font-mono">
                                    <div className="text-blue-700">• account_code</div>
                                    <div className="text-blue-700">• balance (positive = normal side)</div>
                                </div>
                            </div>
                            <div className="text-xs space-y-1">
                                ...
                                <div className="text-blue-600/70 mt-1">Negative balance reverses the side.</div>
                                <div className="text-blue-600/70">Imbalance → auto-adjusts via Opening Balance Equity account.</div>
                            </div>
                        </div>
                        <button onClick={downloadSample} className="mt-3 flex items-center gap-1.5 text-xs font-bold text-blue-700 hover:text-blue-900 underline underline-offset-2">
                            <Download className="h-3 w-3" /> Download sample CSV
                    ...
                    <div className="grid grid-cols-4 gap-4">
                        {[
                            ...
                            { label: 'Total Credits', value: preview.total_credit.toLocaleString('en-US', { minimumFractionDigits: 2 }), style: 'bg-app-surface border-app-border text-app-foreground' },
                            {
                                ...
                                style: isBalanced ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-amber-50 border-amber-200 text-amber-700',
                            },
                        ].map(({ label, value, style }) => (
                            <div key={label} className={`border rounded-xl p-4 text-center ${style}`}>
            ...
            {step === 'done' && result && (
                <div className="space-y-6 max-w-lg mx-auto text-center">
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto ${result.created_entry_id ? 'bg-emerald-100' : 'bg-rose-100'}`}>
