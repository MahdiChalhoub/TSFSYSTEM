...
export default function BarcodeSettingsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/settings/barcode/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/settings/barcode/${id}/`, {
        method: 'DELETE'
      })
      router.push('/finance/settings/barcode')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `BarcodeSettings #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage settings/barcode details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/settings/barcode/${id}/edit`)}
            >
