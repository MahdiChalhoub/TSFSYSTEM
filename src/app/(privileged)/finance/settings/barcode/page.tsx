...
type BarcodeSettings = Record<string, any>

const ALL_COLUMNS: ColumnDef<BarcodeSettings>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function BarcodeSettingsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<BarcodeSettings[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('finance_settings/barcode', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/settings/barcode/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load settings/barcode:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<BarcodeSettings>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/settings/barcode/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/settings/barcode/${r.id}`),
          onEdit: (r) => router.push(`/finance/settings/barcode/${r.id}/edit`),
        }}
      >
        <TypicalFilter
