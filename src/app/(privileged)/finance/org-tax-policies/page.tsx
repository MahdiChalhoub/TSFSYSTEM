...
import {
    Plus, Search, ShieldCheck, Loader2,
    Maximize2, Minimize2, Globe, Activity, Sparkles
} from 'lucide-react'
import { TemplateBanner } from '../_components/TemplateBanner'
import type { TemplateData } from '../_components/TemplateBanner'
import { OrgTaxPolicyPresetCard } from './_components/PresetCard'
import { PolicyRow } from './_components/PolicyRow'
        ...
        <div className={`flex flex-col h-full p-4 md:p-6 animate-in fade-in duration-300 transition-all ${focusMode ? 'max-h-[calc(100vh-4rem)]' : 'max-h-[calc(100vh-8rem)]'}`}>
            ...
            {!focusMode && !templateLoading && templateData && templateData.presets && templateData.presets.length > 0 && (
                <TemplateBanner
                    ...
                    entityLabel="Tax Templates"
                    renderPresetCard={(preset, importing) => (
                        <OrgTaxPolicyPresetCard key={preset.name} preset={preset} onImport={handleImportPreset} importing={importing} />
                    )}
                />
            )}
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
                            ...
                            <p className="text-[11px] text-app-muted-foreground mt-1">
                                {searchQuery ? 'Try a different search term.' : 'Create your first organization tax policy.'}
                            </p>
                        </div>
                    )}
                </div>
