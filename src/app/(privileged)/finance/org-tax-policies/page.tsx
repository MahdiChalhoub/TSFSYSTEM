...
import {
    Plus, Search, ShieldCheck, Check, X, Loader2,
    Maximize2, Minimize2, ChevronRight, Eye, Pencil,
    Globe, Layers, Activity, Percent, Download,
    CheckCircle2, ArrowDownCircle, Sparkles, FileText
} from 'lucide-react'
...
type TemplatePreset = {
    ...
    vat_input_recoverability?: string
    airsi_treatment?: string
    official_vat_treatment?: string
 ...
 *  ROW COMPONENT
 * ═══════════════════════════════════════════════════════════ */
function PolicyRow({ item, onView }: { item: OrgTaxPolicy; onView: (id: number) => void }) {
    return (
        <div
        ...
        >
            ...
            <div className="flex items-center gap-0.5 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={e => { e.stopPropagation(); onView(item.id) }}
                    className="p-1.5 hover:bg-app-border/50 rounded-lg text-app-muted-foreground hover:text-app-foreground transition-colors" title="View">
                    <Eye size={12} />
                </button>
                <button onClick={e => { e.stopPropagation(); onView(item.id) }}
                    className="p-1.5 hover:bg-app-border/50 rounded-lg text-app-muted-foreground hover:text-app-foreground transition-colors" title="Edit">
                    <Pencil size={12} />
                </button>
            </div>
        </div>
...
export default function OrgTaxPoliciesListPage() {
    ...
    async function loadTemplates() {
        try {
            ...
            const data = await erpFetch('finance/org-tax-policies/available-templates/')
    ...
    async function handleImportPreset(presetName: string) {
        ...
        try {
            await erpFetch('finance/org-tax-policies/import-from-template/', {
                method: 'POST',
                body: JSON.stringify({
                    country_code: templateData.country_code,
    ...
    const { filtered, stats } = useMemo(() => {
        ...
        if (searchQuery.trim()) {
            ...
            f = f.filter(i =>
                ...
                i.airsi_treatment?.toLowerCase().includes(q)
            )
        }
        const defaultPolicy = items.find(i => i.is_default)?.name || 'None'
        const countries = new Set(items.map(i => i.country_code).filter(Boolean))
        return { filtered: f, stats: { total: items.length, filtered: f.length, defaultPolicy, countries: countries.size } }
    }, [items, searchQuery])
    ...
    const kpis = [
        { label: 'Policies', value: stats.total, icon: <ShieldCheck size={11} />, color: 'var(--app-primary)' },
        { label: 'Countries', value: stats.countries, icon: <Globe size={11} />, color: 'var(--app-info, #3b82f6)' },
        { label: 'Default', value: stats.defaultPolicy, icon: <Activity size={11} />, color: 'var(--app-success, #22c55e)' },
        ...
        <div className={`flex flex-col h-full p-4 md:p-6 animate-in fade-in duration-300 transition-all ${focusMode ? 'max-h-[calc(100vh-4rem)]' : 'max-h-[calc(100vh-8rem)]'}`}>
                ...
                {focusMode ? (
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <span className="text-[12px] font-black text-app-foreground hidden sm:inline">Tax Policies</span>
                        ...
                        <button onClick={() => router.push('/finance/org-tax-policies/new')}
                    ...
                    <>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            ...
                            <div className="flex items-center gap-1.5 flex-shrink-0 flex-wrap justify-end">
                                <button onClick={() => router.push('/finance/org-tax-policies/new')}
                                    ...
                                    <Plus size={14} /><span className="hidden sm:inline">New Policy</span>
                                </button>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="flex-1 relative">
                                ...
                                <input ref={searchRef} type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                                    placeholder="Search by name, country, treatment... (Ctrl+K)"
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 className="text-[12px] font-black text-app-foreground">
                                    {templateData.country_name} Tax Templates
                                </h3>
                                <p className="text-[10px] font-bold text-app-muted-foreground">
                                    {allImported
                                        ? `All ${templateData.presets.length} templates synced`
                                        : `${pendingImports} of ${templateData.presets.length} templates available · ${templateData.currency_code}`}
                                </p>
                            </div>
                        </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '8px' }}>
                                {templateData.presets.map((preset) => (
                                    <PresetCard
                                        ...
                                        importing={importingPreset === preset.name}
                                    />
                                ))}
                            </div>
                        </div>
            ...
            <div className="flex-1 min-h-0 bg-app-surface/30 border border-app-border/50 rounded-2xl overflow-hidden flex flex-col">
                ...
                <div className="flex-shrink-0 flex items-center gap-2 md:gap-3 px-3 py-2 bg-app-surface/60 border-b border-app-border/50 text-[10px] font-black text-app-muted-foreground uppercase tracking-wider">
                    ...
                    <div className="flex-1 min-w-0">Policy</div>
                    <div className="hidden sm:block w-16 flex-shrink-0">Currency</div>
                    <div className="hidden sm:block w-20 flex-shrink-0">VAT</div>
                    <div className="hidden md:block w-16 text-right flex-shrink-0">Recovery</div>
                    <div className="hidden md:block w-20 flex-shrink-0">AIRSI</div>
                    <div className="hidden lg:block w-28 flex-shrink-0">Scopes</div>
                ...
                <div className="flex-1 overflow-y-auto overflow-x-hidden overscroll-contain custom-scrollbar">
                    {loading ? (
                        ...
                        filtered.map(item => (
                            <PolicyRow key={item.id} item={item} onView={id => router.push(`/finance/org-tax-policies/${id}`)} />
                        ))
                    ) : (
                        <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
                            <ShieldCheck size={36} className="text-app-muted-foreground mb-3 opacity-40" />
                            <p className="text-sm font-bold text-app-muted-foreground">No policies found</p>
                            <p className="text-[11px] text-app-muted-foreground mt-1">
                                {searchQuery
                                    ? 'Try a different search term.'
                                    : templateData?.presets?.some(p => !p.already_imported)
                                        ? 'Import templates from above to get started.'
                                        : 'Create your first organization tax policy.'}
                            </p>
                        </div>
                    )}
                </div>
