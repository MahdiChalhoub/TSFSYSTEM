...
export default function OrgTaxPoliciesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/org-tax-policies/${id}/`)
      setItem(data)
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Org-Tax-Policies #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage org tax policies details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/org-tax-policies/${id}/edit`)}
            >
