...
export default function CoaDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/coa/${id}/`)
      setItem(data)
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Coa #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage coa details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/coa/${id}/edit`)}
            >
