...
type Coa = Record<string, any>

const ALL_COLUMNS: ColumnDef<Coa>[] = [
...
export default function CoaListPage() {
  ...
  const settings = useListViewSettings('finance_coa', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/coa/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Coa>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/coa/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/coa/${r.id}`),
          onEdit: (r) => router.push(`/finance/coa/${r.id}/edit`),
        }}
      >
        <TypicalFilter
