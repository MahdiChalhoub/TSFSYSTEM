...
type CustomerBalances = Record<string, any>

const ALL_COLUMNS: ColumnDef<CustomerBalances>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function CustomerBalancesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<CustomerBalances[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('finance_customer-balances', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/customer-balances/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<CustomerBalances>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/customer-balances/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/customer-balances/${r.id}`),
          onEdit: (r) => router.push(`/finance/customer-balances/${r.id}/edit`),
        }}
      >
        <TypicalFilter
