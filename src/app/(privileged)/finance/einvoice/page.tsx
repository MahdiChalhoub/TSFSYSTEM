...
type Einvoice = Record<string, any>

const ALL_COLUMNS: ColumnDef<Einvoice>[] = [
...
export default function EinvoiceListPage() {
  ...
  const settings = useListViewSettings('finance_einvoice', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/einvoice/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load einvoice:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Einvoice>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/einvoice/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/einvoice/${r.id}`),
          onEdit: (r) => router.push(`/finance/einvoice/${r.id}/edit`),
        }}
      >
        <TypicalFilter
