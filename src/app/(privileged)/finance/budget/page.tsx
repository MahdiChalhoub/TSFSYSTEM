  ...
  const columns: ColumnDef<any>[] = useMemo(() => [
    {
      ...
      label: 'Code',
      sortable: true,
      render: (a) => <span className="font-mono text-xs text-app-muted-foreground">{a.code}</span>
    },
    {
      key: 'name',
      label: 'Account Name',
    ...
    },
    {
      ...
      render: (a) => {
        ...
        return (
          <span className={`font-bold ${a.type === 'INCOME' ? 'text-app-primary' : 'text-rose-600'}`}>
            {fmt(Math.abs(bal))}
          </span>
        )
      }
    },
    {
      ...
      render: (a) => {
        ...
        const color = a.type === 'INCOME' ? 'bg-app-success/10' : 'bg-rose-400'
        return (
          <div className="app-page flex items-center gap-2 justify-end">
            <div className="w-16 h-1.5 bg-app-surface-2 rounded-full overflow-hidden hidden sm:block">
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-4xl font-black tracking-tight text-app-foreground italic">
              Budget <span className="text-app-primary">Center</span>
            </h1>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="rounded-2xl border-0 shadow-sm bg-gradient-to-br from-emerald-50 to-emerald-100/50">
          <CardContent className="pt-5 pb-4 px-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-bold text-app-primary uppercase tracking-widest">Total Income</p>
                <p className="text-2xl font-bold text-app-success mt-0.5">{fmt(Math.abs(totalIncome))}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className={`rounded-2xl border-0 shadow-sm ${netResult >= 0 ? 'bg-gradient-to-br from-green-50 to-green-100/50' : 'bg-gradient-to-br from-red-50 to-red-100/50'}`}>
          <CardContent className="pt-5 pb-4 px-5">
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-2xl ${netResult >= 0 ? 'bg-app-success/10/60' : 'bg-app-error/10/60'} flex items-center justify-center`}>
                <DollarSign size={22} className={netResult >= 0 ? 'text-app-success' : 'text-app-error'} />
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-widest">Net Result</p>
                <p className={`text-2xl font-bold mt-0.5 ${netResult >= 0 ? 'text-app-success' : 'text-app-error'}`}>
      ...
      <Card className="rounded-2xl border-0 shadow-sm overflow-hidden">
        <CardContent className="py-6 px-8">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-black uppercase tracking-tighter text-app-primary">Assets / Income</span>
            <span className="text-xs font-black uppercase tracking-tighter text-rose-600">Liabilities / Expenses</span>
          </div>
          <div className="h-6 bg-app-surface-2 rounded-2xl overflow-hidden flex shadow-inner">
            {Math.abs(totalIncome) + Math.abs(totalExpense) > 0 ? (
              <>
                <div
                  className="h-full bg-app-success/10 flex items-center justify-center text-[10px] font-bold text-app-success"
              ...
              <div className="flex-1 flex items-center justify-center text-[10px] text-app-muted-foreground uppercase font-bold">No Data Available</div>
            )}
          </div>
        </CardContent>
      </Card>
