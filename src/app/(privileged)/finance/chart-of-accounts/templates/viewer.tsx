...
export default function CoaTemplatesLibrary({ templates }: { templates: Record<string, any> }) {
 ...
 <Undo2 size={20} className="text-app-text-muted" />
 </button>
 <div>
 <h1 className="text-xl font-bold text-app-text font-serif">Accounting Standards Library</h1>
 <p className="text-[10px] font-bold text-app-text-faint uppercase tracking-widest">Compare and select your operational layout</p>
 </div>
 </div>
 <div className="flex items-center gap-4">
 <div className="bg-app-surface-2 p-1 rounded-lg flex gap-1">
 <button className="p-2 bg-app-surface shadow-sm rounded-md text-app-text">
 <Columns size={18} />
 </button>
 <button disabled className="p-2 text-app-text-faint hover:text-app-text-muted">
 <LayoutGrid size={18} />
 </button>
 </div>
 </div>
 ...
 <div className="p-6 border-b border-app-border flex justify-between items-start bg-stone-50/50">
 <div>
 <h3 className="font-bold text-app-text text-lg">{key.replace('_', ' ')}</h3>
 <p className="text-[10px] font-bold text-app-text-faint uppercase mt-1 tracking-tight">Official Standard Structure</p>
 </div>
 <button
 disabled={isPending}
 ...
 <span className="text-[10px] font-bold text-app-text-faint uppercase tracking-widest">
...
function TemplateComparisonNode({ item, level = 0 }: Record<string, any>) {
 ...
 className={`flex items-center gap-2 py-1.5 hover:bg-app-bg rounded px-2 transition-colors group ${level === 0 ? 'bg-stone-50/80 mb-1' : ''}`}
 style={{ paddingLeft: `${level === 0 ? 8 : level * 16 + 8}px` }}
 >
 <div className="w-4 flex justify-center">
 {hasChildren && (
 open ? <ChevronDown size={12} className="text-app-text-faint" /> : <ChevronRight size={12} className="text-stone-300" />
 )}
 </div>
 <span className={`text-[10px] font-mono font-bold w-12 text-left ${level === 0 ? 'text-app-text' : 'text-app-text-faint'}`}>
 {item.code}
 </span>
 <span className={`text-xs text-left flex-1 truncate ${level === 0 ? 'font-bold text-app-text' : 'font-medium text-app-text-muted'}`}>
 {item.name}
 </span>
 ...
 item.type === 'EQUITY' ? 'bg-app-surface-2 text-app-text-muted' :
