...
type AuditLogs = Record<string, any>

const ALL_COLUMNS: ColumnDef<AuditLogs>[] = [
...
export default function AuditLogsListPage() {
  ...
  const settings = useListViewSettings('finance_audit-logs', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/audit-logs/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<AuditLogs>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/audit-logs/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/audit-logs/${r.id}`),
          onEdit: (r) => router.push(`/finance/audit-logs/${r.id}/edit`),
        }}
      >
        <TypicalFilter
