...
export default function DeferredExpensesPage() {
 ...
 <span className="font-bold text-app-text text-sm">{exp.name}</span>
 <span className="text-[10px] text-app-text-faint font-black uppercase tracking-widest">{exp.category?.replace(/_/g, " ")}</span>
 </div>
 )
 },
 {
 ...
 align: 'right',
 sortable: true,
 render: (exp) => <span className="font-mono text-sm font-bold text-app-text">{fmt(Number(exp.total_amount))}</span>
 },
 {
 key: 'progress',
 ...
 <span className="text-[9px] font-black text-app-text-faint uppercase tracking-tighter">
 ...
 <p className="text-sm font-medium text-app-text-faint mt-2 uppercase tracking-widest">Amortization & Prepaid Management</p>
 </div>
 ...
 <DialogHeader>
 <DialogTitle className="text-2xl font-black tracking-tight text-app-text flex items-center gap-2">
 <CalendarClock size={24} className="text-indigo-600" />
 Create Amortization Schedule
 </DialogTitle>
 <DialogDescription className="text-app-text-faint font-medium tracking-tight">Set up a prepaid expense to be recognized over time.</DialogDescription>
 </DialogHeader>
 <form onSubmit={handleCreate} className="grid grid-cols-2 gap-5 pt-4">
 <div className="space-y-1.5">
 <label className="text-[10px] font-black text-app-text-faint uppercase tracking-widest ml-1">Expense Name</label>
 <Input name="name" required placeholder="Annual License" className="rounded-xl bg-app-bg border-app-border focus:bg-app-surface transition-all" />
 </div>
 <div className="space-y-1.5">
 <label className="text-[10px] font-black text-app-text-faint uppercase tracking-widest ml-1">Category</label>
 <select name="category" required className="w-full h-10 px-3 border border-app-border rounded-xl bg-app-bg text-sm focus:bg-app-surface transition-all outline-none">
 {["SUBSCRIPTION", "RENOVATION", "ADVERTISING", "INSURANCE", "RENT_ADVANCE", "OTHER"].map(c => <option key={c} value={c}>{c.replace(/_/g, " ")}</option>)}
 </select>
 </div>
 <div className="space-y-1.5">
 <label className="text-[10px] font-black text-app-text-faint uppercase tracking-widest ml-1">Total Amount</label>
 <Input name="total_amount" type="number" step="0.01" min="0.01" required placeholder="0.00" className="rounded-xl bg-app-bg border-app-border" />
 </div>
 <div className="space-y-1.5">
 <label className="text-[10px] font-black text-app-text-faint uppercase tracking-widest ml-1">Duration (Months)</label>
 <Input name="duration_months" type="number" min="1" max="120" required placeholder="12" className="rounded-xl bg-app-bg border-app-border" />
 </div>
 <div className="space-y-1.5">
 <label className="text-[10px] font-black text-app-text-faint uppercase tracking-widest ml-1">Activation Date</label>
 <Input name="start_date" type="date" required className="rounded-xl bg-app-bg border-app-border" />
 </div>
 <div className="space-y-1.5">
 <label className="text-[10px] font-black text-app-text-faint uppercase tracking-widest ml-1">Funding Account</label>
 <select name="source_account_id" required className="w-full h-10 px-3 border border-app-border rounded-xl bg-app-bg text-sm focus:bg-app-surface transition-all outline-none">
 <option value="">Select account...</option>
 {accounts.map((a: Record<string, any>) => <option key={a.id} value={a.id}>{a.name} ({a.type})</option>)}
 </select>
 </div>
 <div className="col-span-2 space-y-1.5">
 <label className="text-[10px] font-black text-app-text-faint uppercase tracking-widest ml-1">Internal Notes</label>
 <textarea name="description" rows={2} className="w-full px-3 py-2 border border-app-border rounded-xl bg-app-bg text-sm resize-none focus:bg-app-surface transition-all outline-none" placeholder="Optional audit notes..." />
 </div>
 ...
 <div>
 <p className="text-[10px] font-black uppercase tracking-widest text-app-text-faint">Active Schedules</p>
 ...
 <div>
 <p className="text-[10px] font-black uppercase tracking-widest text-app-text-faint">Total Commitment</p>
 <p className="text-3xl font-black mt-1 tracking-tighter text-app-text">{fmt(stats.total)}</p>
 ...
 <div>
 <p className="text-[10px] font-black uppercase tracking-widest text-app-text-faint">Unrecognized Balance</p>
 <p className="text-3xl font-black mt-1 tracking-tighter text-amber-600">{fmt(stats.remaining)}</p>
 <p className="text-[10px] text-app-text-faint font-bold uppercase mt-1">Pending Amortization</p>
 </div>
 </CardContent>
 </Card>
 </div>
