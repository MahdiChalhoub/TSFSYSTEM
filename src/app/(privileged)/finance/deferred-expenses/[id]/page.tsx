...
export default function DeferredExpensesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/deferred-expenses/${id}/`)
      setItem(data)
    } catch (error) {
      console.error('Failed to load deferred-expenses:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/deferred-expenses/${id}/`, {
        method: 'DELETE'
      })
      router.push('/finance/deferred-expenses')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Deferred-Expenses #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage deferred expenses details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/deferred-expenses/${id}/edit`)}
            >
