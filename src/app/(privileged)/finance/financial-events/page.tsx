...
type FinancialEvents = Record<string, any>

const ALL_COLUMNS: ColumnDef<FinancialEvents>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function FinancialEventsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<FinancialEvents[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('finance_financial-events', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/financial-events/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load financial-events:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<FinancialEvents>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/financial-events/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/financial-events/${r.id}`),
          onEdit: (r) => router.push(`/finance/financial-events/${r.id}/edit`),
        }}
      >
        <TypicalFilter
