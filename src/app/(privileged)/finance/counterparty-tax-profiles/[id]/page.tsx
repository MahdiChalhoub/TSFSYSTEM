...
export default function CounterpartyTaxProfilesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/counterparty-tax-profiles/${id}/`)
      setItem(data)
    } catch (error) {
      console.error('Failed to load counterparty-tax-profiles:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/counterparty-tax-profiles/${id}/`, {
        method: 'DELETE'
      })
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Counterparty-Tax-Profiles #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage counterparty tax profiles details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/counterparty-tax-profiles/${id}/edit`)}
            >
