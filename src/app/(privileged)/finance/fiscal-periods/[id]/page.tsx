...
export default function FiscalPeriodsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/fiscal-periods/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/fiscal-periods/${id}/`, {
        method: 'DELETE'
      })
      router.push('/finance/fiscal-periods')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Fiscal-Periods #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage fiscal periods details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/fiscal-periods/${id}/edit`)}
            >
