...
type FiscalPeriods = Record<string, any>

const ALL_COLUMNS: ColumnDef<FiscalPeriods>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function FiscalPeriodsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<FiscalPeriods[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('finance_fiscal-periods', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/fiscal-periods/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<FiscalPeriods>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/fiscal-periods/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/fiscal-periods/${r.id}`),
          onEdit: (r) => router.push(`/finance/fiscal-periods/${r.id}/edit`),
        }}
      >
        <TypicalFilter
