...
export default function PeriodicTaxDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/periodic-tax/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/periodic-tax/${id}/`, {
        method: 'DELETE'
      })
      router.push('/finance/periodic-tax')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Periodic-Tax #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage periodic tax details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/periodic-tax/${id}/edit`)}
            >
