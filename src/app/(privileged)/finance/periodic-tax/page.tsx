...
const ALL_COLUMNS: ColumnDef<PeriodicTax>[] = [
...
export default function PeriodicTaxListPage() {
  ...
  const settings = useListViewSettings('finance_periodic-tax', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/periodic-tax/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load periodic-tax:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<PeriodicTax>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/periodic-tax/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/periodic-tax/${r.id}`),
          onEdit: (r) => router.push(`/finance/periodic-tax/${r.id}/edit`),
        }}
      >
        <TypicalFilter
