...
export default function VatReturnReportPage() {
 ...
 <p className="text-[11px] text-app-text-muted uppercase font-semibold tracking-wide">{k.label}</p>
 <p className={`text-xl font-black text-${k.color}-700 truncate`}>{k.value}</p>
 <p className="text-[10px] text-app-text-faint mt-0.5 truncate">{k.sub}</p>
 </div>
 </div>
 </CardContent>
 </Card>
 ...
 <Percent size={16} className="text-app-text-faint" /> Breakdown by VAT Rate
 </CardTitle>
 </CardHeader>
 ...
 <tr>
 <th className="text-left px-4 py-2 font-semibold text-app-text-muted">Rate</th>
 <th className="text-right px-4 py-2 font-semibold text-app-text-muted">Sales HT</th>
 <th className="text-right px-4 py-2 font-semibold text-app-text-muted">TVA Collectée</th>
 <th className="text-right px-4 py-2 font-semibold text-app-text-muted">Purchases HT</th>
 <th className="text-right px-4 py-2 font-semibold text-app-text-muted">TVA Récupérable</th>
 </tr>
 </thead>
 <tbody>
 {Object.entries(report.by_rate).map(([rate, data]) => (
 <tr key={rate} className="border-b hover:bg-app-bg">
 ...
 <span className="text-sm text-app-text-muted">Total Base</span>
 <span className="font-semibold">{fmt(n(report?.airsi?.total_base))}</span>
 </div>
 <div className="flex justify-between">
 <span className="text-sm text-app-text-muted">Total Withheld</span>
 <span className="font-semibold text-violet-700">{fmt(n(report?.airsi?.total_withheld))}</span>
 </div>
 <div className="pt-2 border-t text-[11px] text-app-text-faint">
 ...
 <CardContent className="space-y-3">
 <div className="flex justify-between">
 <span className="text-sm text-app-text-muted">Taxable Base</span>
 <span className="font-semibold">{fmt(n(report?.purchase_tax?.base))}</span>
 </div>
 <div className="flex justify-between">
 <span className="text-sm text-app-text-muted">Total Purchase Tax</span>
 ...
 <span className="text-sm text-app-text-muted">Reverse Charge Amount</span>
 <span className="text-lg font-black text-amber-700">{fmt(n(report?.reverse_charge_vat))}</span>
 </div>
 <p className="text-[11px] text-app-text-faint mt-2">
