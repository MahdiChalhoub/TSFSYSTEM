...
import {
  ...
  ChevronDown, User, Calendar, ArrowLeft
} from 'lucide-react';
...
export default function AccountBookManagerPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      const accountFilter = accountId ? `?account_id=${accountId}` : '';
      const sumRes = await erpFetch(`manager-address-book/all-entries/${accountFilter}`);
      if (sumRes.summary) {
        setSummary(sumRes.summary);
      }

      const snapRes = await erpFetch('manager-address-book/snapshots/');
    ...
    <div className="flex-1 flex flex-col h-full bg-app-background relative">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 fade-in-up">
        <div className="flex items-center gap-4">
          {accountId && (
            <Link href="/finance/accounts" className="p-2 rounded-xl hover:bg-app-surface transition-colors text-app-muted-foreground hover:text-app-foreground">
              <ArrowLeft size={20} />
            </Link>
          )}
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center shrink-0 bg-app-primary/10 border border-app-primary/20">
            <BookKey size={32} className="text-app-primary" />
          </div>
          <div>
                ...
                <div className="bg-app-warning-bg border border-app-warning rounded-2xl p-6 flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-app-warning/80 text-sm mt-1 max-w-2xl font-medium">
                      {accountName
                        ? `There are pending money movements in "${decodeURIComponent(accountName)}". Until you approve them, they are not posted to the general ledger.`
                ...
                <div className="bg-app-primary-light border border-app-success/30 rounded-2xl p-8 text-center flex flex-col items-center">
                  ...
                  <p className="text-app-success/80 mt-2 font-medium">
                    {accountName
                      ...
                      : 'There are no pending Account Book entries to review.'
                    }
                  </p>
                  <button
                    onClick={() => setShowReviewModal(true)}
