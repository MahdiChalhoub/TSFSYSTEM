...
export default function AccountBookManagerPage() {
    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-slate-50 relative">
            {/* Header */}
            <header className="flex-none bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between sticky top-0 z-20">
                <div>
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            {summary.pendingCount > 0 ? (
                                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 flex items-start gap-4">
                                    <div className="p-3 bg-white rounded-xl shadow-sm text-amber-500 mt-1">
                                ...
                                <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-8 text-center flex flex-col items-center">
                                    ...
                                    <button
                                        onClick={() => setShowReviewModal(true)}
                                        className="mt-6 border border-emerald-200 bg-white text-emerald-700 px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-emerald-100 transition-colors"
                                    >
                        ...
                        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-300">
                            <div className="p-4 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                                ...
                                <div className="flex items-center gap-2">
                                    <button className="text-gray-400 hover:text-gray-600 p-2"><ListFilter size={16} /></button>
                                </div>
                            </div>

                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="text-[10px] uppercase font-black tracking-wider text-gray-400 border-b border-gray-100 bg-white">
                                ...
                                <tbody className="text-sm">
                                    {snapshots.length === 0 ? (
                                        <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-400 italic">No snapshots available yet.</td></tr>
                                    ) : (
                                        snapshots.map(snap => (
                                            <tr key={snap.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors group">
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="font-bold text-gray-900">{snap.date}</span>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className="font-bold text-gray-900">{snap.registerName}</div>
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <span className={clsx("font-black px-3 py-1 rounded-lg text-xs",
                                                        snap.balance === 0 ? 'bg-gray-100 text-gray-600' :
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <button className="text-gray-400 hover:text-emerald-600 transition-colors p-2 rounded-lg hover:bg-emerald-50 tooltip-trigger" title="Download Immutable PDF">
                                                        <Download size={16} />
                                                    </button>
                                                </td>
                                            </tr>
