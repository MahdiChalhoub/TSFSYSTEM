...
export default function TrialBalanceViewer({ initialAccounts, fiscalYears }: { initialAccounts: Record<string, any>[], fiscalYears: Record<string, any>[] }) {
 ...
 <tfoot>
 <tr className="bg-app-bg font-bold border-t-2 border-stone-900">
 <td colSpan={2} className="p-4 text-right uppercase tracking-widest text-xs text-app-text-muted">Statement Totals</td>
 <td className="p-4 text-right font-mono text-lg border-double border-b-4 border-stone-900">
 {formatAmount(totals.debit)}
 </td>
 <td className="p-4 text-right font-mono text-lg border-double border-b-4 border-stone-900 text-app-text-faint">
 ...
 <div className="text-[10px] text-app-text-faint text-center font-medium uppercase tracking-widest py-8">
...
function GroupRows({ group, allAccounts, formatAmount, level = 0 }: { group: Record<string, any>, allAccounts: Record<string, any>[], formatAmount: Record<string, any>, level: number }) {
 ...
 <td colSpan={4} className="p-3 text-[11px] font-black text-app-text-faint uppercase tracking-widest border-l-4 border-stone-900">
 {group.type}s
 </td>
 </tr>
...
function AccountRow({ account, level, allAccounts, formatAmount }: { account: Record<string, any>, level: number, allAccounts: Record<string, any>[], formatAmount: Record<string, any> }) {
 ...
 <td className="p-3 font-mono text-app-text-faint text-xs pl-4">
 {account.code}
 </td>
 <td className="p-3" style={{ paddingLeft: `${level * 24 + 12}px` }}>
 <div className="flex items-center gap-2">
 {isParent && (
 <button onClick={() => setExpanded(!expanded)} className="text-stone-300 hover:text-app-text">
 ...
 <td className="p-3 text-right font-mono font-medium text-app-text-muted">
