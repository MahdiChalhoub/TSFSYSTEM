...
export default function VatReturnDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/vat-return/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/vat-return/${id}/`, {
        method: 'DELETE'
      })
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Vat-Return #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage vat return details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/vat-return/${id}/edit`)}
            >
