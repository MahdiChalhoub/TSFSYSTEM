...
const ALL_COLUMNS: ColumnDef<VatReturn>[] = [
...
export default function VatReturnListPage() {
  ...
  const settings = useListViewSettings('finance_vat-return', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/vat-return/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<VatReturn>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/vat-return/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/vat-return/${r.id}`),
          onEdit: (r) => router.push(`/finance/vat-return/${r.id}/edit`),
        }}
      >
        <TypicalFilter
