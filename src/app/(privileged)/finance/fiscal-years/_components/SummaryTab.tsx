...
export function SummaryTab({ summary }: SummaryTabProps) {
    ...
    return (
        <div className="p-4 space-y-3">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                    {[
                        ...
                        <div key={v.label} className="text-center">
                            <div className="text-[14px] font-bold tabular-nums" style={{ color: v.color }}>{v.value.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
                            <div className="text-tp-xxs font-bold uppercase" style={{ color: 'var(--app-muted-foreground)' }}>{v.label}</div>
                        </div>
                    ))}
                </div>
            </div>
            {/* Balance Sheet */}
            <div className="rounded-xl p-3" style={{ background: 'var(--app-surface)', border: '1px solid var(--app-border)' }}>
                <div className="text-tp-xxs font-bold uppercase tracking-wide mb-2" style={{ color: 'var(--app-muted-foreground)' }}>Balance Sheet</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                    {[
                        ...
                        { label: 'Equity', value: s.balance_sheet.equity, color: 'var(--app-info)' },
                    ].map(v => (
                        <div key={v.label} className="text-center">
                            <div className="text-[14px] font-bold tabular-nums" style={{ color: v.color }}>{v.value.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
                            <div className="text-tp-xxs font-bold uppercase" style={{ color: 'var(--app-muted-foreground)' }}>{v.label}</div>
            ...
            <div className="rounded-xl p-3" style={{ background: 'var(--app-surface)', border: '1px solid var(--app-border)' }}>
                ...
                <div className="flex gap-4">
                    <span className="text-tp-md font-bold" style={{ color: 'var(--app-foreground)' }}>{s.journal_entries.total} total</span>
                    <span className="text-tp-md font-bold" style={{ color: 'var(--app-success, #22c55e)' }}>{s.journal_entries.posted} posted</span>
                    {s.journal_entries.draft > 0 && <span className="text-tp-md font-bold" style={{ color: 'var(--app-warning, #f59e0b)' }}>{s.journal_entries.draft} draft</span>}
                </div>
            </div>
            {/* Closing Entry */}
            {s.closing_entry && (
                <div className="rounded-xl p-3" style={{ background: 'var(--app-surface)', border: '1px solid var(--app-border)' }}>
                    ...
                    <div className="max-h-[120px] overflow-y-auto custom-scrollbar space-y-0.5">
                        {s.closing_entry.lines.map((l, i) => (
                            <div key={i} className="flex items-center justify-between text-tp-xs py-0.5" style={{ borderBottom: '1px solid var(--app-border)' }}>
