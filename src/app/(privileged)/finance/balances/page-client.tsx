...
export default function BalancesPage() {
    ...
    const columns: ColumnDef<Balance>[] = useMemo(() => [
            ...
            render: (b) => (
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        <span className="font-bold text-gray-900 text-sm">{b.contact?.name || b.contact_name || '—'}</span>
                        {b.contact?.email && (
            ...
            render: (b) => {
                ...
                return (
                    <div className="flex flex-col items-end">
                        <span className={`font-mono font-black text-sm ${bal > 0 ? 'text-emerald-600' : bal < 0 ? 'text-blue-600' : 'text-gray-400'}`}>
                            {fmt(bal)}
                        </span>
                        <Badge className={`mt-1 border-none shadow-none text-[8px] font-black uppercase px-1.5 py-0 h-4 ${bal > 0 ? 'bg-emerald-50 text-emerald-600' : bal < 0 ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-400'}`}>
        ...
        },
        {
            ...
            align: 'right',
            sortable: true,
            render: (b) => <span className="font-mono text-xs text-stone-400">{b.credit_limit != null ? fmt(b.credit_limit) : '—'}</span>
        },
        {
            key: 'last_transaction_date',
            label: 'Last Transaction',
            sortable: true,
            render: (b) => <span className="text-xs text-gray-500 font-medium">{b.last_transaction_date || '—'}</span>
        }
    ], [fmt, tab])
        ...
        <div className="p-6 space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500 min-h-screen bg-stone-50/30">
            ...
            <header className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm font-medium text-gray-400 mt-2 uppercase tracking-widest">Real-time Financial Positions</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="rounded-3xl border-0 shadow-sm bg-white overflow-hidden group">
                    <CardContent className="p-6 flex items-center gap-5">
                        ...
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">Net Exposure</p>
                ...
                <Card className="rounded-3xl border-0 shadow-sm bg-white overflow-hidden group">
                    <CardContent className="p-6 flex items-center gap-5">
                        ...
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">Active Accounts</p>
                ...
                <Card className="rounded-3xl border-0 shadow-sm bg-white overflow-hidden group">
                    <CardContent className="p-6 flex items-center gap-5">
                        ...
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">Total Credit Limit</p>
                            <p className="text-3xl font-black mt-1 tracking-tighter text-stone-900">{fmt(stats.totalCredit)}</p>
                        </div>
                    </CardContent>
                </Card>
            </div>
            ...
            <TypicalListView
                ...
                headerExtra={
                    <button
                        onClick={loadAll}
                        className="flex items-center gap-2 px-4 py-2 text-[10px] font-black uppercase text-stone-500 hover:text-stone-900 transition-colors"
                    >
