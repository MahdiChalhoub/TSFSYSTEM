...
type InvoiceLines = Record<string, any>

const ALL_COLUMNS: ColumnDef<InvoiceLines>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function InvoiceLinesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<InvoiceLines[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('finance_invoice-lines', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/invoice-lines/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<InvoiceLines>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/invoice-lines/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/invoice-lines/${r.id}`),
          onEdit: (r) => router.push(`/finance/invoice-lines/${r.id}/edit`),
        }}
      >
        <TypicalFilter
