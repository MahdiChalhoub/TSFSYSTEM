...
export default function CustomTaxRulesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/custom-tax-rules/${id}/`)
      setItem(data)
    } catch (error) {
      console.error('Failed to load custom-tax-rules:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/custom-tax-rules/${id}/`, {
        method: 'DELETE'
      })
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Custom-Tax-Rules #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage custom tax rules details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/custom-tax-rules/${id}/edit`)}
            >
