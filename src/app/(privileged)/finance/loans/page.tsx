...
const STATUS_CONFIG: Record<string, { icon: any; color: string; bg: string }> = {
    ...
    DRAFT: { icon: FileText, color: "text-stone-700", bg: "bg-stone-50 border-stone-200" },
}
    ...
    const columns: ColumnDef<any>[] = useMemo(() => [
        {
            ...
            render: (loan) => (
                <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-gray-400 font-black uppercase tracking-widest">Effective Date</span>
                </div>
            )
        },
        {
            ...
            render: (loan) => (
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        <span className="font-bold text-gray-900 text-sm">{loan.contact?.name || 'Unknown Partner'}</span>
                        <span className="text-[10px] text-gray-400 font-black uppercase tracking-widest">ID: #{loan.id}</span>
                    </div>
                </div>
            )
        },
        {
            ...
            render: (loan) => (
                <div className="flex flex-col items-end">
                    <span className="font-mono text-sm font-black text-stone-900">{fmt(Number(loan.principalAmount))}</span>
                    <span className="text-[10px] text-stone-400 font-bold uppercase tracking-tighter">{Number(loan.interestRate)}% Int. Rate</span>
                </div>
            )
        },
        {
            ...
            sortable: true,
            render: (loan) => (
                <Badge variant="outline" className="rounded-xl px-3 py-1 bg-stone-50 border-stone-200 text-stone-600 font-bold text-[11px]">
            ...
            <header className="flex justify-between items-center">
                <div>
                    <h1 className="text-4xl font-black tracking-tighter text-gray-900 flex items-center gap-4">
                        ...
                        Partner <span className="text-stone-600">Loans</span>
                    </h1>
                    <p className="text-sm font-medium text-gray-400 mt-2 uppercase tracking-widest">Financial Contracts & Agreements</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="rounded-3xl border-0 shadow-sm bg-white overflow-hidden group">
                    <CardContent className="p-6 flex items-center gap-5">
                        ...
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">Active Facilities</p>
                ...
                <Card className="rounded-3xl border-0 shadow-sm bg-white overflow-hidden group">
                    <CardContent className="p-6 flex items-center gap-5">
                        ...
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">Total Portfolio</p>
                            <p className="text-3xl font-black mt-1 tracking-tighter text-stone-900">{fmt(stats.totalPrincipal)}</p>
                ...
                <Card className="rounded-3xl border-0 shadow-sm bg-white overflow-hidden group">
                    <CardContent className="p-6 flex items-center gap-5">
                        ...
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">Average Yield</p>
                            <p className="text-3xl font-black mt-1 tracking-tighter text-purple-600">{stats.avgInterest.toFixed(2)}%</p>
                            <p className="text-[10px] text-stone-400 font-bold uppercase mt-1">Avg. Return Rate</p>
                        </div>
                    </CardContent>
                </Card>
            </div>
