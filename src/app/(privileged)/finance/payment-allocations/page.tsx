...
type PaymentAllocations = Record<string, any>

const ALL_COLUMNS: ColumnDef<PaymentAllocations>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function PaymentAllocationsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<PaymentAllocations[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('finance_payment-allocations', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/payment-allocations/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<PaymentAllocations>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/payment-allocations/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/payment-allocations/${r.id}`),
          onEdit: (r) => router.push(`/finance/payment-allocations/${r.id}/edit`),
        }}
      >
        <TypicalFilter
