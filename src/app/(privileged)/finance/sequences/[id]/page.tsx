...
export default function SequencesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/sequences/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/sequences/${id}/`, {
        method: 'DELETE'
      })
      router.push('/finance/sequences')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Sequences #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage sequences details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/sequences/${id}/edit`)}
            >
