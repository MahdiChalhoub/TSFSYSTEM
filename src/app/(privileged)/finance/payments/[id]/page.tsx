...
export default function PaymentsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/payments/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/payments/${id}/`, {
        method: 'DELETE'
      })
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `Payments #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage payments details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/payments/${id}/edit`)}
            >
