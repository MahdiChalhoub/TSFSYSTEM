...
export default function PaymentsPage() {
 ...
 return <div className="flex items-center gap-1.5 text-sm text-app-text-muted"><Icon size={14} /> {(p.method || '').replace(/_/g, ' ')}</div>
 }
 },
 { key: 'reference', label: 'Reference', render: (p) => <span className="font-mono text-xs">{p.reference || "—"}</span> },
 { key: 'amount', label: 'Amount', align: 'right', sortable: true, render: (p) => <span className="font-semibold text-app-text">{fmt(p.amount)}</span> },
 ]
 }, [activeView, fmt])
 ...
 <p className="text-sm font-medium text-app-text-faint mt-2 uppercase tracking-widest">Supplier & Customer</p>
 <p className="text-app-text-muted font-medium mt-1">Supplier payments, customer receipts, and aged reports</p>
 </div>
 ...
 ? `bg-emerald-50 text-emerald-700 border-emerald-200 shadow-sm` : "bg-app-bg text-app-text-faint border-app-border hover:bg-app-surface-2"}`}>
 ...
 <div className="space-y-1.5">
 <label className="text-xs font-bold text-app-text-muted uppercase">Method *</label>
 ...
 <label className="text-xs font-bold text-app-text-muted uppercase">Reference</label>
 <Input name="reference" placeholder="Optional reference..." className="rounded-xl" />
 </div>
 <div className="col-span-2 space-y-1.5">
 <label className="text-xs font-bold text-app-text-muted uppercase">Payment Account</label>
 ...
 <p className="text-3xl font-bold text-app-text mt-1">{payments.length}</p>
 </div>
 <div className="w-12 h-12 rounded-2xl bg-stone-200/60 flex items-center justify-center">
 <Wallet size={22} className="text-app-text-muted" />
 </div>
 </div>
 </CardContent>
 </Card>
 ...
 ? "bg-app-surface shadow-sm font-semibold text-app-text" : "text-app-text-faint hover:text-app-text-muted"}`}>
