        ...
        <div className="min-h-screen bg-[#070D1B] text-gray-100 p-6 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-3 gap-4">
                {GATEWAYS.map(g => (
                    <div key={g.type} className="bg-[#0F1729] rounded-2xl border border-gray-800 p-4">
                        ...
                        <div className={`mt-2 inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold ${configs.some(c => c.gateway_type === g.type) ? 'bg-emerald-900/40 text-emerald-400 border border-emerald-800' : 'bg-gray-800 text-gray-500'}`}>
            ...
            {showNew && (
                <div className="bg-[#0F1729] rounded-2xl border border-emerald-800/50 p-6 flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div>
                            <label className="text-xs text-gray-400 mb-1 block">Gateway Type</label>
            ...
            <div className="flex flex-col gap-3">
                <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Configured Gateways</h3>
                {loading ? Array.from({ length: 2 }).map((_, i) => <div key={i} className="h-24 bg-gray-800/50 rounded-xl animate-pulse" />) :
                    configs.length === 0 ? (
                        <div className="bg-[#0F1729] rounded-2xl border border-gray-800 p-8 text-center text-gray-500 text-sm">No gateways configured yet.</div>
                    ) : configs.map(cfg => (
                        <div key={cfg.id} className="bg-[#0F1729] rounded-2xl border border-gray-800 p-5">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => handleDelete(cfg.id)} className="p-1.5 rounded-xl hover:bg-red-900/40 text-gray-600 hover:text-red-400 transition-colors">
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                            </div>
                            ...
                            {keyForm.id === cfg.id && (
                                <div className="mt-4 pt-4 border-t border-gray-800 flex flex-col gap-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <label className="text-xs text-gray-400 mb-1 block">API Key (sk_...)</label>
                                        ...
                                        <div>
                                            <label className="text-xs text-gray-400 mb-1 block">Webhook Secret (whsec_...)</label>
                                            <input
                                                type="password"
