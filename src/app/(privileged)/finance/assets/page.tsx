...
export default function AssetsPage() {
    ...
    async function handlePrintTag(asset: Asset) {
        ...
        try {
            // Dynamic imports — jspdf (~200KB) & html2canvas (~200KB) load only on demand
    ...
    const statusConfig: Record<string, { icon: React.ElementType; color: string; bg: string }> = {
        ACTIVE: { icon: CheckCircle2, color: "text-app-success", bg: "bg-app-primary-light border-app-success" },
        ...
        <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
            ...
            <div className="flex justify-between items-center">
                <div>
                    <div className="flex items-center gap-3">
                        <h1 className="page-header-title text-app-foreground font-serif tracking-tight">Fixed Assets</h1>
                        {settings?.assetTrackingMode && (
                            <Badge className={`rounded-xl px-3 py-1 text-[10px] font-black uppercase tracking-widest border-2 ${settings.assetTrackingMode === 'ENTERPRISE' ? 'bg-app-primary-light text-app-success border-app-success' :
                                settings.assetTrackingMode === 'PROFESSIONAL' ? 'bg-app-primary/5 text-app-primary border-app-primary/30' :
                ...
                <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                    ...
                    <DialogContent className="sm:max-w-4xl bg-app-background border-app-border">
                        <DialogHeader>
                            ...
                            <form onSubmit={handleCreate} className="lg:col-span-3 grid grid-cols-1 gap-4 bg-app-surface p-5 rounded-3xl border border-app-border shadow-sm overflow-y-auto max-h-[65vh] custom-scrollbar">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-app-muted-foreground uppercase tracking-tighter ml-1">Asset Name *</label>
                                        <Input name="name" required placeholder="Delivery Truck" className="rounded-xl bg-app-surface/50 border-app-border" />
                                    </div>
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-app-muted-foreground uppercase tracking-tighter ml-1">Asset Category *</label>
                                        <select name="category" required className="w-full px-3 py-2 border rounded-xl bg-app-surface/50 border-app-border text-sm outline-none focus:ring-2 focus:ring-black/5">
                                            {categories.map(c => <option key={c} value={c}>{c.replace(/_/g, " ")}</option>)}
                                        </select>
                                    </div>
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-app-muted-foreground uppercase tracking-tighter ml-1">Depreciation Method</label>
                                        <select
                                            name="depreciation_method"
                                            onChange={(e) => setSelectedMethod(e.target.value)}
                                            className="w-full px-3 py-2 border rounded-xl bg-app-surface/50 border-app-border text-sm outline-none focus:ring-2 focus:ring-black/5"
                                        >
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-app-muted-foreground uppercase tracking-tighter ml-1">Purchase Value *</label>
                                        <Input
                                            name="purchase_value"
                                            type="number" step="0.01" min="0.01" required
                                            onChange={(e) => setPreviewCost(Number(e.target.value))}
                                            placeholder="50,000.00"
                                            className="rounded-xl border-app-success bg-app-primary-light/10 focus:ring-app-primary/10"
                                        />
                                    </div>
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-app-muted-foreground uppercase tracking-tighter ml-1">Purchase Date *</label>
                                        <Input name="purchase_date" type="date" required className="rounded-xl bg-app-surface/50 border-app-border" />
                                    </div>
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-app-muted-foreground uppercase tracking-tighter ml-1">Useful Life (Years) *</label>
                                        <Input
                                            ...
                                            placeholder="5"
                                            className="rounded-xl bg-app-surface/50 border-app-border"
                                        />
                                    </div>
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-app-muted-foreground uppercase tracking-tighter ml-1">Residual Value</label>
                                        <Input
                                            name="residual_value"
                                    ...
                                    {selectedMethod === 'PRODUCTION' && (
                                        <>
                                            <div className="space-y-1.5 animate-in slide-in-from-top-2">
                                                <label className="text-xs font-bold text-app-primary uppercase tracking-tighter ml-1 italic">Total Capacity *</label>
                                                <Input
                                                    name="total_production_capacity"
                                                    type="number" min="1" required
                                                    onChange={(e) => setPreviewCapacity(Number(e.target.value))}
                                                    placeholder="100,000"
                                                    className="rounded-xl border-app-primary/30 bg-app-primary/5/20"
                                                />
                                            </div>
                                            <div className="space-y-1.5 animate-in slide-in-from-top-2">
                                                <label className="text-xs font-bold text-app-primary uppercase tracking-tighter ml-1 italic">Unit Name *</label>
                                                <Input name="production_unit_name" required placeholder="Miles / Hours" className="rounded-xl border-app-primary/30 bg-app-primary/5/20" />
                                            </div>
                                        </>
                                    )}

                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-app-muted-foreground uppercase tracking-tighter ml-1">Source Account</label>
                                        <select name="source_account_id" className="w-full px-3 py-2 border rounded-xl bg-app-surface/50 border-app-border text-sm outline-none focus:ring-2 focus:ring-black/5">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-xs font-bold text-app-muted-foreground uppercase tracking-tighter ml-1">Acquisition Description</label>
                                    <Input name="description" placeholder="Optional description..." className="rounded-xl bg-app-surface/50 border-app-border" />
                                </div>
                                <div className="flex justify-end gap-2 pt-3 border-t mt-2">
                                    <Button type="button" variant="outline" onClick={() => setDialogOpen(false)} className="rounded-xl px-6 h-11 text-app-muted-foreground font-bold uppercase text-[10px] tracking-widest border-app-border hover:bg-app-background">Cancel</Button>
                                    <Button type="submit" disabled={isPending} className="rounded-xl px-8 h-11 gap-2 shadow-lg shadow-black/10 bg-app-background hover:bg-app-surface-2 transition-all text-[10px] font-black uppercase tracking-widest text-app-foreground">
                                ...
                                <div className="bg-app-surface p-6 rounded-3xl border border-app-border shadow-sm h-full flex flex-col">
                                    <div className="flex items-center justify-between mb-6">
                                        <div className="flex items-center gap-2">
                                            <div className="p-1.5 bg-app-primary/5 rounded-lg text-app-primary">
                                                <TrendingDown size={14} />
                                            </div>
                                            <h4 className="text-[10px] font-black text-app-foreground uppercase tracking-[0.2em] leading-none">Book Value Projection</h4>
                                        </div>
                                        <Badge variant="outline" className="text-[9px] font-black uppercase tracking-widest bg-app-primary-light text-app-success border-app-success/30">Dynamic UI</Badge>
                                    </div>
                                    ...
                                    {previewCost > 0 ? (
                                        <div className="flex-1 flex flex-col justify-between">
                                                        ...
                                                        <Tooltip
                                                            content={({ active, payload }) => {
                                                                if (active && payload && payload.length) {
                                                                    return (
                                                                        <div className="bg-app-background/90 text-app-foreground p-3 rounded-2xl text-[10px] shadow-2xl backdrop-blur-md border border-app-text/10 scale-110">
                                                                            ...
                                                                            <p className="font-black flex justify-between gap-6 text-sm">
                                                                                Value: <span className="text-app-primary font-serif">${Number(payload[0].value).toLocaleString()}</span>
                                                                            </p>
                                                                        </div>
                                                                    );
                                                                }
                                            ...
                                            <div className="grid grid-cols-2 gap-4 mt-6">
                                                <div className="p-4 bg-app-primary-light/30 rounded-2xl border border-app-success/30 flex flex-col">
                                                    <span className="text-[9px] font-black text-app-primary uppercase tracking-widest mb-1">Total Savings</span>
                                                    <span className="text-lg font-black text-app-foreground tracking-tighter">
                                                        ${(previewCost - previewResidual).toLocaleString()}
                                                    </span>
                                                </div>
                                                <div className="p-4 bg-app-surface/50 rounded-2xl border border-app-border flex flex-col">
                                                    <span className="text-[9px] font-black text-app-muted-foreground uppercase tracking-widest mb-1">Floor Value</span>
                                                    <span className="text-lg font-black text-app-foreground tracking-tighter">
                                        ...
                                        <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-app-surface/50 rounded-[2.5rem] border-4 border-dashed border-app-border">
                                            ...
                                            <p className="text-[10px] font-black text-app-muted-foreground uppercase tracking-[0.3em]">Initialize Tracking</p>
                                            <p className="text-[9px] text-app-muted-foreground mt-2 uppercase font-bold tracking-tight leading-relaxed max-w-[15ch]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <Card className="rounded-2xl border-0 shadow-sm bg-gradient-to-br from-indigo-50 to-indigo-100">
                    <CardContent className="pt-5 pb-4 px-5">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs font-bold text-app-primary uppercase tracking-wider">Purchase Value</p>
                                <p className="text-3xl font-bold text-app-primary mt-1">{totalPurchase.toLocaleString()}</p>
                            </div>
                            <div className="w-12 h-12 rounded-2xl bg-app-primary/10/60 flex items-center justify-center">
                                <Landmark size={22} className="text-app-primary" />
                            </div>
                        </div>
                    </CardContent>
                </Card>
                <Card className="rounded-2xl border-0 shadow-sm bg-gradient-to-br from-emerald-50 to-emerald-100">
                    <CardContent className="pt-5 pb-4 px-5">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs font-bold text-app-primary uppercase tracking-wider">Book Value</p>
                                <p className="text-3xl font-bold text-app-success mt-1">{totalBook.toLocaleString()}</p>
                            </div>
                            <div className="w-12 h-12 rounded-2xl bg-app-success/10/60 flex items-center justify-center">
                                <TrendingDown size={22} className="text-app-primary" />
                            </div>
                        </div>
                    </CardContent>
                </Card>
                <Card className="rounded-2xl border-0 shadow-sm bg-gradient-to-br from-amber-50 to-amber-100">
                    <CardContent className="pt-5 pb-4 px-5">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs font-bold text-app-warning uppercase tracking-wider">Fully Depreciated</p>
                                <p className="text-3xl font-bold text-app-warning mt-1">{fullyDepreciated}</p>
                            </div>
                                    ...
                                    <TableCell>
                                        <div className="flex items-center gap-2 justify-center">
                                            <div className="w-20 h-2.5 bg-app-surface-2 rounded-full overflow-hidden">
                                                <div
                                                    className={`h-full rounded-full transition-all ${depPct >= 100 ? "bg-app-surface-2" : depPct >= 70 ? "bg-app-warning/10" : "bg-app-success/10"}`}
                                    ...
                                    <TableCell className="text-right space-x-2">
                                        {settings?.enableAssetQR && (
                                            <Button
                                                ...
                                                className="rounded-xl gap-1 h-8 text-xs font-semibold border-app-primary/30 text-app-primary hover:bg-app-primary/5"
                                            >
                                                <QrCode size={12} /> QR
                                            </Button>
                                        )}
                                        {settings?.enableAssetMaintenance && (
                                            <Button
                                                size="sm"
                                                variant="outline"
                                                className="rounded-xl gap-1 h-8 text-xs font-semibold border-app-warning text-app-warning hover:bg-app-warning-bg"
                                            >
                                ...
                                <TableCell colSpan={8} className="py-16 text-center">
                                    <div className="flex flex-col items-center gap-3">
                                        <div className="w-16 h-16 rounded-full bg-app-surface-2 flex items-center justify-center">
                                            <Landmark size={28} className="text-app-muted-foreground" />
                                        </div>
                                        <div>
                            ...
                            <TableBody>
                                {schedule.map((line: Record<string, any>) => (
                                    <TableRow key={line.id} className="hover:bg-app-surface/50 transition-colors">
                                        ...
                                        <TableCell className="text-center">
                                            <Badge variant="outline" className={`gap-1 rounded-lg border font-semibold text-[11px] ${line.is_posted ? "bg-app-primary-light border-app-success text-app-success" : "bg-app-warning-bg border-app-warning text-app-warning"}`}>
            ...
            <Dialog open={!!tagPreviewAsset} onOpenChange={(open) => !open && setTagPreviewAsset(null)}>
                <DialogContent className="sm:max-w-md bg-app-background border-app-border">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2"><QrCode size={20} className="text-app-primary" /> Physical Asset Tag</DialogTitle>
                    ...
                    <div className="flex flex-col items-center py-6">
                        ...
                        <div className="mt-8 grid grid-cols-2 gap-4 w-full">
                            <div className="p-4 bg-app-surface rounded-2xl border border-app-border shadow-sm flex flex-col items-center text-center">
                                <div className="p-2 bg-app-primary/5 rounded-lg text-app-primary mb-2">
                                    <ShieldCheck size={18} />
                                </div>
                                <h4 className="text-[10px] font-black uppercase text-app-foreground">High-Res PDF</h4>
                                <p className="text-[8px] text-app-muted-foreground font-bold uppercase mt-1">Sticker Sheet Optimized</p>
                            </div>
                            <div className="p-4 bg-app-surface rounded-2xl border border-app-border shadow-sm flex flex-col items-center text-center">
                                <div className="p-2 bg-app-primary-light rounded-lg text-app-primary mb-2">
                                    <QrCode size={18} />
                                </div>
                                <h4 className="text-[10px] font-black uppercase text-app-foreground">Audit Ready</h4>
                                <p className="text-[8px] text-app-muted-foreground font-bold uppercase mt-1">Global Mobile Sync</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-3 pt-4 border-t border-app-border">
                        ...
                        <Button
                            ...
                            disabled={printing}
                            className="flex-1 rounded-xl bg-app-primary hover:bg-app-primary gap-2"
                        >
