...
const ALL_COLUMNS: ColumnDef<Audit>[] = [
...
export default function AuditListPage() {
  ...
  const settings = useListViewSettings('finance_audit', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/audit/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Audit>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/audit/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/audit/${r.id}`),
          onEdit: (r) => router.push(`/finance/audit/${r.id}/edit`),
        }}
      >
        <TypicalFilter
