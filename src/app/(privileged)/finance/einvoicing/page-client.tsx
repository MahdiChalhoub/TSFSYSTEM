...
export default function EInvoicingPage() {
 ...
 if (!s) return <span className="px-2 py-0.5 rounded-full text-xs bg-gray-800 text-app-text-faint">—</span>
 ...
 <div className="text-[10px] font-black text-app-text-faint uppercase tracking-widest">{b.label}</div>
 <div className="text-[9px] font-bold text-app-text-muted uppercase tracking-tighter line-clamp-1">{b.country}</div>
 </div>
 ))}
 </div>
 ...
 <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-app-text-muted group-focus-within:text-indigo-500 transition-colors" />
 <input
 value={search}
 ...
 <div className="flex-1 flex flex-col items-center justify-center text-app-text-muted gap-4 opacity-50">
 <Activity size={48} />
 ...
 <div className="text-xs font-semibold text-app-text-faint uppercase tracking-wider">E-Invoice Status</div>
 ...
 {detail.invoice_hash && (
 <div>
 <div className="text-xs text-app-text-muted mb-1">Invoice Hash</div>
 <div className="font-mono text-xs text-app-text-faint break-all bg-gray-900 rounded-lg p-2">{detail.invoice_hash}</div>
 </div>
 )}
 {detail.fne_error && (
 ...
 <div className="text-xs font-semibold text-app-text-faint uppercase tracking-wider mb-2">QR Code Data</div>
