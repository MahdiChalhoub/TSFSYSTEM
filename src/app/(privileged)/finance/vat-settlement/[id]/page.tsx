...
export default function VatSettlementDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/vat-settlement/${id}/`)
      setItem(data)
    } catch (error) {
      console.error('Failed to load vat-settlement:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/vat-settlement/${id}/`, {
        method: 'DELETE'
      })
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Vat-Settlement #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage vat settlement details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/vat-settlement/${id}/edit`)}
            >
