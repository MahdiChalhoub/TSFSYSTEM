...
export default function GatewayConfigsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/gateway-configs/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/gateway-configs/${id}/`, {
        method: 'DELETE'
      })
      router.push('/finance/gateway-configs')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Gateway-Configs #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage gateway configs details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/gateway-configs/${id}/edit`)}
            >
