...
type GatewayConfigs = Record<string, any>

const ALL_COLUMNS: ColumnDef<GatewayConfigs>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function GatewayConfigsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<GatewayConfigs[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('finance_gateway-configs', {
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<GatewayConfigs>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/gateway-configs/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/gateway-configs/${r.id}`),
          onEdit: (r) => router.push(`/finance/gateway-configs/${r.id}/edit`),
        }}
      >
        <TypicalFilter
