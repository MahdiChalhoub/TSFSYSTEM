...
export default function TaxGroupsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/tax-groups/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/tax-groups/${id}/`, {
        method: 'DELETE'
      })
      router.push('/finance/tax-groups')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Tax-Groups #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage tax groups details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/tax-groups/${id}/edit`)}
            >
