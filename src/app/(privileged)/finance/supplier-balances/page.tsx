...
type SupplierBalances = Record<string, any>

const ALL_COLUMNS: ColumnDef<SupplierBalances>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function SupplierBalancesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<SupplierBalances[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('finance_supplier-balances', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('finance/supplier-balances/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<SupplierBalances>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/finance/supplier-balances/new')}
        ...
        actions={{
          onView: (r) => router.push(`/finance/supplier-balances/${r.id}`),
          onEdit: (r) => router.push(`/finance/supplier-balances/${r.id}/edit`),
        }}
      >
        <TypicalFilter
