...
export default function SupplierBalancesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/supplier-balances/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/supplier-balances/${id}/`, {
        method: 'DELETE'
      })
      router.push('/finance/supplier-balances')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Supplier-Balances #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage supplier balances details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/supplier-balances/${id}/edit`)}
            >
