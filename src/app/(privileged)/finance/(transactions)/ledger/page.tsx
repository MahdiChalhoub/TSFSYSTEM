...
import { Search, Filter, Calendar, BookOpen, ChevronDown, FileText, Zap, X, Lock, ShieldCheck, CheckCircle2, Clock, History } from 'lucide-react'
...
function getStatusStyle(status: string) {
    switch (status) {
        ...
        case 'LOCKED': return 'bg-purple-100 text-purple-700 border border-purple-200'
...
export default function GeneralLedgerPage() {
    ...
    const loadEntries = useCallback(async () => {
        ...
        try {
            const data = await getLedgerEntries('INTERNAL', {
                ...
                entry_type: activeTab === 'MANUAL' ? 'MANUAL' : undefined,
                source_type: sourceType || undefined,
                q: search || undefined,
                is_auto: activeTab === 'AUTO' ? 'true' : undefined,
            })
            setEntries(data)
        } catch (e) {
            console.error(e)
    ...
    }, [status, fiscalYear, dateFrom, dateTo, sourceType, search, activeTab])
    ...
    const activeFilterCount = [status, fiscalYear, dateFrom, dateTo, sourceType, search].filter(Boolean).length

    const clearFilters = () => {
        setStatus(''); setFiscalYear(''); setDateFrom(''); setDateTo(''); setSourceType(''); setSearch('')
    }
        ...
        <div className="p-6 max-w-6xl mx-auto">
            ...
            <div className="flex gap-1 mb-4 bg-stone-100 p-1 rounded-xl w-fit">
                <button
                    ...
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'MANUAL'
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'AUTO'
                        ? 'bg-white text-stone-900 shadow-sm'
                        : 'text-stone-500 hover:text-stone-700'
                        }`}
                >
            ...
            <div className="mb-6 bg-white border border-stone-200 rounded-xl shadow-sm overflow-hidden">
                ...
                {showFilters && (
                    <div className="p-3 grid grid-cols-4 gap-3 bg-stone-50/50 animate-in slide-in-from-top-2 duration-200">
                        ...
                        <div className="space-y-1">
                            <label className="text-[10px] font-bold text-stone-500 uppercase tracking-wider">Source Type</label>
                            <select value={sourceType} onChange={e => setSourceType(e.target.value)} className="w-full px-3 py-2 text-sm border border-stone-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
                                {SOURCE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                            </select>
                        </div>
                        {activeFilterCount > 0 && (
                            <div className="col-span-4 flex justify-end">
                                <button onClick={clearFilters} className="text-xs text-stone-500 hover:text-stone-900 font-bold uppercase tracking-wider flex items-center gap-1">
                                    <X className="h-3 w-3" />
                                    Clear All Filters
                                </button>
                            </div>
                            ...
                            <div key={entry.id} className={`bg-white border rounded-xl overflow-hidden shadow-sm transition-all border-stone-200 ${entry.status === 'REVERSED' ? 'opacity-75 grayscale-[0.5]' : ''}`}>
                                ...
                                <div className="p-4 bg-stone-50 border-b border-stone-100 flex justify-between items-center">
                                    <div className="flex items-center gap-3">
                                        ...
                                        {entry.lifecycle_status && entry.lifecycle_status !== 'OPEN' && (
                                            <Badge variant="outline" className={`gap-1 rounded-lg border ${LIFECYCLE_CONFIG[entry.lifecycle_status]?.bg} ${LIFECYCLE_CONFIG[entry.lifecycle_status]?.color} font-semibold text-[10px]`}>
                                        ...
                                        {entry.entry_type && entry.entry_type !== 'MANUAL' && (
                                            <div className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-indigo-100 text-indigo-700 border border-indigo-200 flex items-center gap-1">
                                    ...
                                    <LedgerEntryActions
                                        entryId={entry.id}
                                        status={entry.status}
                                        lifecycleStatus={entry.lifecycle_status || 'OPEN'}
                                        isLocked={isLocked}
                                        onActionComplete={loadEntries}
                                    />
                                </div>
                                ...
                                <div className="px-4 py-2 bg-white flex items-center gap-6 text-[11px] text-stone-500 border-b border-stone-50 font-medium">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span>{new Date(entry.transactionDate || entry.date).toLocaleDateString('en-GB')}</span>
                                    </div>
                                    {entry.reference && (
                                        <div className="flex items-center gap-1.5">
                                    ...
                                    {entry.createdBy && (
                                        <div className="flex items-center gap-1.5">
                                            <span className="text-stone-300">By:</span> <span className="text-stone-700">{entry.createdBy}</span>
                                        </div>
                                    )}
                                    {entry.reversalOf && (
                    ...
                    {entries.length === 0 && (
                        <div className="text-center py-16 bg-stone-50 rounded-2xl border-2 border-dashed border-stone-200">
                            <div className="w-12 h-12 bg-stone-100 rounded-full flex items-center justify-center mx-auto mb-4">
                                {activeTab === 'MANUAL' ? <FileText className="text-stone-300 h-6 w-6" /> : <Zap className="text-stone-300 h-6 w-6" />}
                            </div>
                            <h3 className="text-stone-900 font-bold mb-1">
                                {activeFilterCount > 0 ? 'No Entries Match Filters' : activeTab === 'MANUAL' ? 'No Manual Entries' : 'No Auto-Generated Entries'}
                            </h3>
                            <p className="text-stone-500 text-sm">
                                {activeFilterCount > 0 ? 'Try adjusting your filters to see more results.' : activeTab === 'MANUAL' ? 'Create your first journal voucher to get started.' : 'Auto entries are generated from invoices, sales, payroll, and other transaction sources.'}
                            </p>
                        </div>
                    )}
                </div>
