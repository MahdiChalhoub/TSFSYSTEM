...
import { lockInvoice, unlockInvoice, verifyInvoice, confirmInvoice, getInvoiceHistory } from "@/app/actions/finance/invoices"
...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; icon: any }> = {
    DRAFT: { label: 'Draft', color: 'text-stone-600', bg: 'bg-stone-50 border-stone-200', icon: FileText },
...
export default function InvoicesPage() {
    ...
    async function handleLock(id: number) {
        startTransition(async () => {
            try { await lockInvoice(id); toast.success("Invoice locked"); loadData() }
            catch (err: any) { toast.error(err.message || "Failed to lock") }
        })
    }
    async function handleUnlock(id: number, comment: string) {
        startTransition(async () => {
            try { await unlockInvoice(id, comment); toast.success("Invoice unlocked"); setCommentOpen(false); loadData() }
            catch (err: any) { toast.error(err.message || "Failed to unlock") }
        })
    }
    async function handleVerify(id: number) {
        startTransition(async () => {
            try { await verifyInvoice(id); toast.success("Invoice verified"); loadData() }
            catch (err: any) { toast.error(err.message || "Failed to verify") }
        })
    }
    async function handleConfirm(id: number) {
        startTransition(async () => {
            try { await confirmInvoice(id); toast.success("Invoice confirmed"); loadData() }
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
                                ...
                                <TableRow key={inv.id} className="hover:bg-stone-50/50 transition-colors">
                                    ...
                                    <TableCell className="text-center">
                                        <div className="flex flex-col items-center gap-1">
                                            ...
                                            {inv.lifecycle_status && inv.lifecycle_status !== 'OPEN' && (
                                                <Badge variant="outline" className={`gap-1 rounded-lg border ${LIFECYCLE_CONFIG[inv.lifecycle_status]?.bg} ${LIFECYCLE_CONFIG[inv.lifecycle_status]?.color} font-semibold text-[9px]`}>
                                    ...
                                    <TableCell className="text-center">
                                        <div className="flex items-center justify-center gap-1">
                                            {/* Standard Actions */}
                                            {inv.status === 'DRAFT' && inv.lifecycle_status === 'OPEN' && (
                                                <>
                                                    <Button size="sm" variant="ghost" className="h-7 px-2 text-blue-600 hover:bg-blue-50"
                                                        onClick={() => handleSend(inv)} disabled={isPending} title="Send">
                                                        <Send size={13} />
                                                    </Button>
                                                </>
                                            )}
                                            {['SENT', 'PARTIAL_PAID', 'OVERDUE'].includes(inv.status) && inv.lifecycle_status === 'OPEN' && (
                                                <Button size="sm" variant="ghost" className="h-7 px-2 text-emerald-600 hover:bg-emerald-50"
                                                    onClick={() => { setSelectedInvoice(inv); setPaymentOpen(true) }} disabled={isPending} title="Record Payment">
                                                    <CreditCard size={13} />
                                                </Button>
                                            )}

                                            {/* Lifecycle Actions */}
                                            {inv.lifecycle_status === 'OPEN' && (
                                                <Button size="sm" variant="ghost" className="h-7 px-2 text-amber-600 hover:bg-amber-50"
                                                    onClick={() => handleLock(inv.id)} disabled={isPending} title="Lock">
                                                    <Lock size={13} />
                                                </Button>
                                            )}
                                            {inv.lifecycle_status === 'LOCKED' && (
                                                <>
                                                    <Button size="sm" variant="ghost" className="h-7 px-2 text-stone-500 hover:bg-stone-50"
                                                        onClick={() => { setSelectedInvoice(inv); setCommentOpen(true) }} disabled={isPending} title="Unlock">
                                                        <Unlock size={13} />
                                                    </Button>
                                                    <Button size="sm" variant="ghost" className="h-7 px-2 text-purple-600 hover:bg-purple-50"
                                                        onClick={() => handleVerify(inv.id)} disabled={isPending} title="Verify">
                                                        <ShieldCheck size={13} />
                                                    </Button>
                                                </>
                                            )}
                                            {inv.lifecycle_status === 'VERIFIED' && (
                                                <Button size="sm" variant="ghost" className="h-7 px-2 text-emerald-600 hover:bg-emerald-50"
                                                    onClick={() => handleConfirm(inv.id)} disabled={isPending} title="Confirm">
                                                    <CheckCircle2 size={13} />
                                                </Button>
                                            )}

                                            <Button size="sm" variant="ghost" className="h-7 px-2 text-stone-400 hover:bg-stone-50"
                                                onClick={() => showHistory(inv.id)} title="History">
                                                <History size={13} />
                                            </Button>
                                        </div>
                                    </TableCell>
            ...
            <Dialog open={commentOpen} onOpenChange={setCommentOpen}>
                <DialogContent className="sm:max-w-sm">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2"><Unlock size={18} /> Unlock Invoice</DialogTitle>
                        <DialogDescription>Provide a reason for unlocking this invoice.</DialogDescription>
                    </DialogHeader>
                    <form onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); handleUnlock(selectedInvoice.id, fd.get('comment') as string) }} className="space-y-4">
