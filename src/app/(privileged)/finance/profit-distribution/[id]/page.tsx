...
export default function ProfitDistributionDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/profit-distribution/${id}/`)
      setItem(data)
    } catch (error) {
      console.error('Failed to load profit-distribution:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/profit-distribution/${id}/`, {
        method: 'DELETE'
      })
      router.push('/finance/profit-distribution')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Profit-Distribution #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage profit distribution details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/profit-distribution/${id}/edit`)}
            >
