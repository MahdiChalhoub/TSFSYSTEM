...
export default function PriceListManager({ priceLists }: { priceLists: Record<string, any>[] }) {
 ...
 <h2 className="text-lg font-bold text-app-text mb-2">Create Price List</h2>
 ...
 <button type="submit" className="bg-black text-app-text px-4 py-2 rounded shrink-0" suppressHydrationWarning={true}>Create</button>
 </form>
 </div>
 ...
 <thead className="bg-app-bg text-app-text-muted font-medium">
 <tr>
 ...
 {priceLists.map(list => (
 <tr key={list.id} className="hover:bg-app-bg">
 <td className="px-4 py-3 font-medium text-app-text">{list.name}</td>
 ...
 {priceLists.length === 0 && (
 <div className="text-center py-8 text-app-text-faint">No price lists defined.</div>
 )}
 </div>
 </div>
 )
