    ...
    const columns: ColumnDef<any>[] = [
        {
            ...
            render: (r) => (
                <div className="flex flex-col">
                    <span className="font-bold text-gray-900 line-clamp-1">{r.product_name || `Product #${r.product}`}</span>
                    <span className="text-[10px] text-gray-400 font-mono">ID: {r.product}</span>
                </div>
            )
        },
        {
        ...
        },
        {
            ...
            render: (r) => (
                <div className="max-w-md">
                    <p className="font-bold text-gray-800 text-xs truncate">{r.title}</p>
                    <p className="text-gray-500 text-[11px] line-clamp-1 italic">"{r.content}"</p>
                </div>
            )
        },
        {
        ...
        },
        {
            key: 'date',
            label: 'Date',
            render: (r) => <span className="text-gray-400 text-[11px]">{new Date(r.created_at).toLocaleDateString()}</span>
        },
        {
            key: 'actions',
            label: '',
            ...
            render: (r) => (
                <div className="flex items-center justify-end gap-1">
                    <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg" onClick={() => toggleVisibility(r)}>
                        {r.is_visible ? <EyeOff size={14} className="text-gray-400" /> : <Eye size={14} className="text-indigo-500" />}
                    </Button>
                ...
                <CardContent className="p-4">
                    <div className="relative">
                        ...
                        <Input
                            ...
                            className="pl-9 h-11 rounded-2xl bg-gray-50 border-0 focus-visible:ring-indigo-500/30"
                        />
                    </div>
                </CardContent>
            </Card>
