...
export default function SectionBuilder({ configId, initialLayout }: SectionBuilderProps) {
 ...
 <p className="text-sm font-medium text-app-text-faint mt-1">Customize the sections and layout of your homepage.</p>
 </div>
 ...
 {sections.length === 0 ? (
 <div className="py-20 flex flex-col items-center justify-center bg-gray-50/50 rounded-[2rem] border-2 border-dashed border-app-border">
 <div className="w-16 h-16 bg-app-surface rounded-2xl shadow-sm flex items-center justify-center text-gray-300 mb-4">
 <Layout size={32} />
 </div>
 <p className="text-app-text-faint font-bold">Your homepage is empty</p>
 ...
 <div key={section.id}
 className="group bg-app-surface border border-app-border rounded-2xl p-4 flex items-center gap-4 hover:border-indigo-200 hover:shadow-md transition-all animate-in fade-in slide-in-from-top-2"
 >
 ...
 <h4 className="font-bold text-app-text uppercase text-[10px] tracking-widest text-indigo-500">
 Section Type
 </h4>
 <p className="font-black text-lg text-app-text">
 ...
 className="w-12 h-12 rounded-xl text-app-text-faint hover:text-red-500 hover:bg-red-50 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
 >
 <Trash2 size={18} />
 </button>
 </div>
 ...
 <div className="mt-12">
 <h3 className="text-[10px] font-black text-app-text-faint uppercase tracking-[0.2em] mb-4">Add New Section</h3>
 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
 {AVAILABLE_SECTIONS.map(item => (
 <button
 key={item.type}
 onClick={() => addSection(item.type)}
 className="p-6 bg-app-bg border border-transparent rounded-[1.5rem] text-left hover:border-indigo-500 hover:bg-app-surface hover:shadow-xl transition-all group"
 >
 <div className="w-12 h-12 rounded-xl bg-app-surface shadow-sm flex items-center justify-center text-app-text-faint group-hover:text-indigo-600 group-hover:bg-indigo-50 mb-4 transition-all">
 <Plus size={24} />
 </div>
 <h4 className="font-black text-app-text text-lg uppercase tracking-tight">{item.name}</h4>
 <p className="text-xs text-app-text-muted font-medium mt-1 leading-relaxed">{item.description}</p>
 </button>
 ))}
 </div>
 </div>
