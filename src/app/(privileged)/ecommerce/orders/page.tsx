...
export default function DigitalCommerceStreamPage() {
 ...
 date: r => <span className="text-app-text-muted font-medium">{new Date(r.created_at).toLocaleDateString()}</span>,
 reference: r => <span className="font-mono font-bold text-app-text">{r.order_number || `#${r.id}`}</span>,
 client: r => (
 <div className="flex flex-col">
 <span className="font-bold text-app-text">{r.client?.name || r.client_name || 'Anonymous Guest'}</span>
 <span className="text-[10px] text-app-text-faint font-bold uppercase tracking-widest leading-none">External Link Node</span>
 </div>
 ),
 items: r => <Badge variant="secondary" className="bg-app-surface-2 text-app-text-muted border-0 font-black text-[10px]">{r.items_count || 0} ITEMS</Badge>,
 amount: r => <span className="font-black text-app-text">{fmt(r.total_amount)}</span>,
 }
 ...
 label: 'Quantity',
 align: 'center',
 render: (l: any) => <span className="font-mono text-app-text-faint font-black">×{l.quantity}</span>
 },
 {
 key: 'price',
 label: 'Unit Value',
 align: 'right',
 render: (l: any) => <span className="font-bold text-app-text-muted">{fmt(Number(l.unit_price))}</span>
 },
 {
 key: 'total',
 ...
 <p className="text-sm font-medium text-app-text-faint mt-2 uppercase tracking-widest">E-Commerce Lifecycle & Transaction Engine</p>
 </div>
 ...
 <div>
 <p className="text-[10px] font-black text-app-text-faint uppercase tracking-wider">Total Orders</p>
 <h2 className="text-3xl font-black text-app-text mt-0.5">{throughput} <span className="text-sm text-gray-300">orders</span></h2>
 </div>
 </CardContent>
 </Card>
 ...
 <div>
 <p className="text-[10px] font-black text-app-text-faint uppercase tracking-wider">Pending Fulfillment</p>
 <h2 className="text-3xl font-black text-app-text mt-0.5">{pendingFulfillment}</h2>
 </div>
 </CardContent>
 </Card>
 ...
 <div>
 <p className="text-[10px] font-black text-app-text-faint uppercase tracking-wider">Revenue Velocity</p>
 <h2 className="text-3xl font-black text-app-text mt-0.5">{fmt(revenueVelocity)}</h2>
 </div>
 </CardContent>
 </Card>
 </div>
 ...
 headerExtra={
 <Button onClick={loadOrders} variant="ghost" className="h-8 w-8 p-0 text-app-text-faint hover:text-violet-600">
