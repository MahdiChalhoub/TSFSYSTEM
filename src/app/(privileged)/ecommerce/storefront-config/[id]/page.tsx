...
export default function StorefrontConfigDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`ecommerce/storefront-config/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      router.push('/ecommerce/storefront-config')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Storefront-Config #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage storefront config details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/ecommerce/storefront-config/${id}/edit`)}
            >
