...
type StorefrontConfig = Record<string, any>

const ALL_COLUMNS: ColumnDef<StorefrontConfig>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function StorefrontConfigListPage() {
  const router = useRouter()
  const [items, setItems] = useState<StorefrontConfig[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('ecommerce_storefront-config', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('ecommerce/storefront-config/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<StorefrontConfig>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/ecommerce/storefront-config/new')}
        ...
        actions={{
          onView: (r) => router.push(`/ecommerce/storefront-config/${r.id}`),
          onEdit: (r) => router.push(`/ecommerce/storefront-config/${r.id}/edit`),
        }}
      >
        <TypicalFilter
