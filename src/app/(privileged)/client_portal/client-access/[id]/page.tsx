...
export default function ClientAccessDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`client_portal/client-access/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`client_portal/client-access/${id}/`, {
        method: 'DELETE'
      })
      router.push('/client_portal/client-access')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Client-Access #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage client access details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/client_portal/client-access/${id}/edit`)}
            >
