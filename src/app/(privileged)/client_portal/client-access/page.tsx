...
type ClientAccess = Record<string, any>

const ALL_COLUMNS: ColumnDef<ClientAccess>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function ClientAccessListPage() {
  const router = useRouter()
  const [items, setItems] = useState<ClientAccess[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('client_portal_client-access', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/client-access/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<ClientAccess>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/client-access/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/client-access/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/client-access/${r.id}/edit`),
        }}
      >
        <TypicalFilter
