...
type Reviews = Record<string, any>

const ALL_COLUMNS: ColumnDef<Reviews>[] = [
...
export default function ReviewsListPage() {
  ...
  const settings = useListViewSettings('client_portal_reviews', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/reviews/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Reviews>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/reviews/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/reviews/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/reviews/${r.id}/edit`),
        }}
      >
        <TypicalFilter
