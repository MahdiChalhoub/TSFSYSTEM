...
export default function ReviewsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`client_portal/reviews/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`client_portal/reviews/${id}/`, {
        method: 'DELETE'
      })
      router.push('/client_portal/reviews')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Reviews #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage reviews details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/client_portal/reviews/${id}/edit`)}
            >
