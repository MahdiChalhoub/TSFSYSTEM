...
type MyWallet = Record<string, any>

const ALL_COLUMNS: ColumnDef<MyWallet>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function MyWalletListPage() {
  const router = useRouter()
  const [items, setItems] = useState<MyWallet[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('client_portal_my-wallet', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/my-wallet/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load my-wallet:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<MyWallet>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/my-wallet/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/my-wallet/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/my-wallet/${r.id}/edit`),
        }}
      >
        <TypicalFilter
