...
export default function MyWalletDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`client_portal/my-wallet/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`client_portal/my-wallet/${id}/`, {
        method: 'DELETE'
      })
      router.push('/client_portal/my-wallet')
    } catch (error) {
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `My-Wallet #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage my wallet details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/client_portal/my-wallet/${id}/edit`)}
            >
