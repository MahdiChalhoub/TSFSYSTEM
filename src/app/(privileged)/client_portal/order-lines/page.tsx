...
type OrderLines = Record<string, any>

const ALL_COLUMNS: ColumnDef<OrderLines>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function OrderLinesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<OrderLines[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('client_portal_order-lines', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/order-lines/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<OrderLines>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/order-lines/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/order-lines/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/order-lines/${r.id}/edit`),
        }}
      >
        <TypicalFilter
