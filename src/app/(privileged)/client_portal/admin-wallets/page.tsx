...
type AdminWallets = Record<string, any>

const ALL_COLUMNS: ColumnDef<AdminWallets>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function AdminWalletsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<AdminWallets[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('client_portal_admin-wallets', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/admin-wallets/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<AdminWallets>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/admin-wallets/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/admin-wallets/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/admin-wallets/${r.id}/edit`),
        }}
      >
        <TypicalFilter
