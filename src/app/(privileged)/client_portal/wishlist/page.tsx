...
const ALL_COLUMNS: ColumnDef<Wishlist>[] = [
...
export default function WishlistListPage() {
  ...
  const settings = useListViewSettings('client_portal_wishlist', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/wishlist/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Wishlist>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/wishlist/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/wishlist/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/wishlist/${r.id}/edit`),
        }}
      >
        <TypicalFilter
