...
export default function WishlistDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`client_portal/wishlist/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`client_portal/wishlist/${id}/`, {
        method: 'DELETE'
      })
      router.push('/client_portal/wishlist')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Wishlist #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage wishlist details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/client_portal/wishlist/${id}/edit`)}
            >
