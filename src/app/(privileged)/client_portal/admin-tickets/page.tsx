...
type AdminTickets = Record<string, any>

const ALL_COLUMNS: ColumnDef<AdminTickets>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function AdminTicketsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<AdminTickets[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('client_portal_admin-tickets', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/admin-tickets/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<AdminTickets>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/admin-tickets/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/admin-tickets/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/admin-tickets/${r.id}/edit`),
        }}
      >
        <TypicalFilter
