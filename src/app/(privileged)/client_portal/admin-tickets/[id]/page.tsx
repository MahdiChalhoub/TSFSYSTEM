...
export default function AdminTicketsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`client_portal/admin-tickets/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`client_portal/admin-tickets/${id}/`, {
        method: 'DELETE'
      })
      router.push('/client_portal/admin-tickets')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="theme-text-muted mt-1">
              View and manage admin tickets details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/client_portal/admin-tickets/${id}/edit`)}
            >
