...
export default function ShippingRatesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`client_portal/shipping-rates/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`client_portal/shipping-rates/${id}/`, {
        method: 'DELETE'
      })
      router.push('/client_portal/shipping-rates')
    } catch (error) {
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `Shipping-Rates #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage shipping rates details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/client_portal/shipping-rates/${id}/edit`)}
            >
