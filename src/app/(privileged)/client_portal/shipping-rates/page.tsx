...
type ShippingRates = Record<string, any>

const ALL_COLUMNS: ColumnDef<ShippingRates>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function ShippingRatesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<ShippingRates[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('client_portal_shipping-rates', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/shipping-rates/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<ShippingRates>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/shipping-rates/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/shipping-rates/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/shipping-rates/${r.id}/edit`),
        }}
      >
        <TypicalFilter
