...
export default function MyOrdersListPage() {
  ...
  const settings = useListViewSettings('client_portal_my-orders', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/my-orders/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<MyOrders>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/my-orders/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/my-orders/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/my-orders/${r.id}/edit`),
        }}
      >
        <TypicalFilter
