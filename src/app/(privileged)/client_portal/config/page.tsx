...
export default function ConfigListPage() {
  ...
  const settings = useListViewSettings('client_portal_config', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/config/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Config>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/config/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/config/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/config/${r.id}/edit`),
        }}
      >
        <TypicalFilter
