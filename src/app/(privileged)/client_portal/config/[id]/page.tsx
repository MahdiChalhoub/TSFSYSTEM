...
export default function ConfigDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`client_portal/config/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`client_portal/config/${id}/`, {
        method: 'DELETE'
      })
      router.push('/client_portal/config')
    } catch (error) {
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/client_portal/config/${id}/edit`)}
            >
