...
type QuoteRequests = Record<string, any>

const ALL_COLUMNS: ColumnDef<QuoteRequests>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function QuoteRequestsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<QuoteRequests[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('client_portal_quote-requests', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/quote-requests/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<QuoteRequests>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/quote-requests/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/quote-requests/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/quote-requests/${r.id}/edit`),
        }}
      >
        <TypicalFilter
