...
type AdminOrders = Record<string, any>

const ALL_COLUMNS: ColumnDef<AdminOrders>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function AdminOrdersListPage() {
  const router = useRouter()
  const [items, setItems] = useState<AdminOrders[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('client_portal_admin-orders', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/admin-orders/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<AdminOrders>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/admin-orders/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/admin-orders/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/admin-orders/${r.id}/edit`),
        }}
      >
        <TypicalFilter
