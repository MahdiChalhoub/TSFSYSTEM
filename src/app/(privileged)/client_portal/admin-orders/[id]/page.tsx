...
export default function AdminOrdersDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`client_portal/admin-orders/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`client_portal/admin-orders/${id}/`, {
        method: 'DELETE'
      })
      router.push('/client_portal/admin-orders')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Admin-Orders #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage admin orders details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/client_portal/admin-orders/${id}/edit`)}
            >
