...
export default function DashboardListPage() {
  ...
  const settings = useListViewSettings('client_portal_dashboard', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('client_portal/dashboard/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Dashboard>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/client_portal/dashboard/new')}
        ...
        actions={{
          onView: (r) => router.push(`/client_portal/dashboard/${r.id}`),
          onEdit: (r) => router.push(`/client_portal/dashboard/${r.id}/edit`),
        }}
      >
        <TypicalFilter
