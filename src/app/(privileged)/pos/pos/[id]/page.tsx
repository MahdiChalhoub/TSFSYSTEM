...
export default function PosDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`pos/pos/${id}/`)
      setItem(data)
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Pos #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage pos details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/pos/pos/${id}/edit`)}
            >
