...
const ALL_COLUMNS: ColumnDef<Pos>[] = [
...
export default function PosListPage() {
  ...
  const settings = useListViewSettings('pos_pos', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('pos/pos/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Pos>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/pos/pos/new')}
        ...
        actions={{
          onView: (r) => router.push(`/pos/pos/${r.id}`),
          onEdit: (r) => router.push(`/pos/pos/${r.id}/edit`),
        }}
      >
        <TypicalFilter
