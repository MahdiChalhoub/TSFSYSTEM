...
export default function PurchaseReturnsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`pos/purchase-returns/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`pos/purchase-returns/${id}/`, {
        method: 'DELETE'
      })
      router.push('/pos/purchase-returns')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Purchase-Returns #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage purchase returns details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/pos/purchase-returns/${id}/edit`)}
            >
