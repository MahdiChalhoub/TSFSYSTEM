...
type PosTickets = Record<string, any>

const ALL_COLUMNS: ColumnDef<PosTickets>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function PosTicketsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<PosTickets[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('pos_pos-tickets', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('pos/pos-tickets/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load pos-tickets:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<PosTickets>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/pos/pos-tickets/new')}
        ...
        actions={{
          onView: (r) => router.push(`/pos/pos-tickets/${r.id}`),
          onEdit: (r) => router.push(`/pos/pos-tickets/${r.id}/edit`),
        }}
      >
        <TypicalFilter
