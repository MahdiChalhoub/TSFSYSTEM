...
export default function CreditNotesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`pos/credit-notes/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`pos/credit-notes/${id}/`, {
        method: 'DELETE'
      })
      router.push('/pos/credit-notes')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Credit-Notes #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage credit notes details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/pos/credit-notes/${id}/edit`)}
            >
