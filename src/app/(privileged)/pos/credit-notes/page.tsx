...
type CreditNotes = Record<string, any>

const ALL_COLUMNS: ColumnDef<CreditNotes>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function CreditNotesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<CreditNotes[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('pos_credit-notes', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('pos/credit-notes/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<CreditNotes>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/pos/credit-notes/new')}
        ...
        actions={{
          onView: (r) => router.push(`/pos/credit-notes/${r.id}`),
          onEdit: (r) => router.push(`/pos/credit-notes/${r.id}/edit`),
        }}
      >
        <TypicalFilter
