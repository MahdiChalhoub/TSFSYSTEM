...
type Sourcing = Record<string, any>

const ALL_COLUMNS: ColumnDef<Sourcing>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function SourcingListPage() {
  const router = useRouter()
  const [items, setItems] = useState<Sourcing[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('pos_sourcing', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('pos/sourcing/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Sourcing>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/pos/sourcing/new')}
        ...
        actions={{
          onView: (r) => router.push(`/pos/sourcing/${r.id}`),
          onEdit: (r) => router.push(`/pos/sourcing/${r.id}/edit`),
        }}
      >
        <TypicalFilter
