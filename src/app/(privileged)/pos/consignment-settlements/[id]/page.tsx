...
export default function ConsignmentSettlementsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`pos/consignment-settlements/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`pos/consignment-settlements/${id}/`, {
        method: 'DELETE'
      })
      router.push('/pos/consignment-settlements')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Consignment-Settlements #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage consignment settlements details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/pos/consignment-settlements/${id}/edit`)}
            >
