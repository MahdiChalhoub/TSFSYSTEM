...
type ConsignmentSettlements = Record<string, any>

const ALL_COLUMNS: ColumnDef<ConsignmentSettlements>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function ConsignmentSettlementsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<ConsignmentSettlements[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('pos_consignment-settlements', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('pos/consignment-settlements/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<ConsignmentSettlements>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/pos/consignment-settlements/new')}
        ...
        actions={{
          onView: (r) => router.push(`/pos/consignment-settlements/${r.id}`),
          onEdit: (r) => router.push(`/pos/consignment-settlements/${r.id}/edit`),
        }}
      >
        <TypicalFilter
