...
const ALL_COLUMNS: ColumnDef<PosSettings>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function PosSettingsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<PosSettings[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('pos_pos-settings', {
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<PosSettings>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/pos/pos-settings/new')}
        ...
        actions={{
          onView: (r) => router.push(`/pos/pos-settings/${r.id}`),
          onEdit: (r) => router.push(`/pos/pos-settings/${r.id}/edit`),
        }}
      >
        <TypicalFilter
