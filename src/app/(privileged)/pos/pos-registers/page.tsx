...
type PosRegisters = Record<string, any>

const ALL_COLUMNS: ColumnDef<PosRegisters>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function PosRegistersListPage() {
  const router = useRouter()
  const [items, setItems] = useState<PosRegisters[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('pos_pos-registers', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('pos/pos-registers/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load pos-registers:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<PosRegisters>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/pos/pos-registers/new')}
        ...
        actions={{
          onView: (r) => router.push(`/pos/pos-registers/${r.id}`),
          onEdit: (r) => router.push(`/pos/pos-registers/${r.id}/edit`),
        }}
      >
        <TypicalFilter
