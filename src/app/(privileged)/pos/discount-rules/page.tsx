...
type DiscountRules = Record<string, any>

const ALL_COLUMNS: ColumnDef<DiscountRules>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function DiscountRulesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<DiscountRules[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('pos_discount-rules', {
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<DiscountRules>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/pos/discount-rules/new')}
        ...
        actions={{
          onView: (r) => router.push(`/pos/discount-rules/${r.id}`),
          onEdit: (r) => router.push(`/pos/discount-rules/${r.id}/edit`),
        }}
      >
        <TypicalFilter
