...
export default function PosAuditEventsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`pos/pos-audit-events/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`pos/pos-audit-events/${id}/`, {
        method: 'DELETE'
      })
      router.push('/pos/pos-audit-events')
    } catch (error) {
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `Pos-Audit-Events #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage pos audit events details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/pos/pos-audit-events/${id}/edit`)}
            >
