...
export default function DeliveryZonesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`pos/delivery-zones/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`pos/delivery-zones/${id}/`, {
        method: 'DELETE'
      })
      router.push('/pos/delivery-zones')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Delivery-Zones #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage delivery zones details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/pos/delivery-zones/${id}/edit`)}
            >
