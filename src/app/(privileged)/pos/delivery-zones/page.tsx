...
type DeliveryZones = Record<string, any>

const ALL_COLUMNS: ColumnDef<DeliveryZones>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function DeliveryZonesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<DeliveryZones[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('pos_delivery-zones', {
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<DeliveryZones>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/pos/delivery-zones/new')}
        ...
        actions={{
          onView: (r) => router.push(`/pos/delivery-zones/${r.id}`),
          onEdit: (r) => router.push(`/pos/delivery-zones/${r.id}/edit`),
        }}
      >
        <TypicalFilter
