...
export default function ManagerAddressBookDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`pos/manager-address-book/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`pos/manager-address-book/${id}/`, {
        method: 'DELETE'
      })
      router.push('/pos/manager-address-book')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Manager-Address-Book #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage manager address book details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/pos/manager-address-book/${id}/edit`)}
            >
