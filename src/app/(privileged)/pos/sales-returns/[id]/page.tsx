...
export default function SalesReturnsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`pos/sales-returns/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`pos/sales-returns/${id}/`, {
        method: 'DELETE'
      })
      router.push('/pos/sales-returns')
    } catch (error) {
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `Sales-Returns #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage sales returns details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/pos/sales-returns/${id}/edit`)}
            >
