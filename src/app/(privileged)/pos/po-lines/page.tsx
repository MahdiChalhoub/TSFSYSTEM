...
type PoLines = Record<string, any>

const ALL_COLUMNS: ColumnDef<PoLines>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function PoLinesListPage() {
  const router = useRouter()
  const [items, setItems] = useState<PoLines[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('pos_po-lines', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('pos/po-lines/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<PoLines>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/pos/po-lines/new')}
        ...
        actions={{
          onView: (r) => router.push(`/pos/po-lines/${r.id}`),
          onEdit: (r) => router.push(`/pos/po-lines/${r.id}/edit`),
        }}
      >
        <TypicalFilter
