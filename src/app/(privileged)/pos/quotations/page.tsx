...
type Quotations = Record<string, any>

const ALL_COLUMNS: ColumnDef<Quotations>[] = [
...
export default function QuotationsListPage() {
  ...
  const settings = useListViewSettings('pos_quotations', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('pos/quotations/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Quotations>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/pos/quotations/new')}
        ...
        actions={{
          onView: (r) => router.push(`/pos/quotations/${r.id}`),
          onEdit: (r) => router.push(`/pos/quotations/${r.id}/edit`),
        }}
      >
        <TypicalFilter
