...
export default function DeliveriesDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`pos/deliveries/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`pos/deliveries/${id}/`, {
        method: 'DELETE'
      })
      router.push('/pos/deliveries')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `Deliveries #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage deliveries details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/pos/deliveries/${id}/edit`)}
            >
