...
function UsageMeter({ label, icon: Icon, current, limit, percent, unit = '' }: {
 ...
 <Icon size={16} className={isDanger ? 'text-red-500' : isWarning ? 'text-amber-500' : 'text-app-text-faint'} />
 <span className="text-xs font-bold uppercase tracking-wider text-app-text-muted">{label}</span>
 </div>
 <span className="text-sm font-black text-app-text">
 {current}{unit} <span className="text-app-text-faint font-medium">/ {limit}{unit}</span>
 </span>
 </div>
...
function ModuleCard({ module, onToggle, toggling, onFeatureToggle }: {
 ...
 : isInstalled ? 'bg-emerald-50 text-emerald-500' : 'bg-app-surface-2 text-app-text-faint'
 }`}>
 {isCore ? <Crown size={18} /> : <Package size={18} />}
 </div>
 <div>
 <h4 className="font-bold text-app-text text-sm">{module.name}</h4>
 <p className="text-[10px] text-app-text-faint font-mono uppercase tracking-widest">{module.code}</p>
 {module.description && (
 <p className="text-[10px] text-app-text-muted mt-0.5 line-clamp-1">{module.description}</p>
 )}
 </div>
 </div>
 <div className="flex items-center gap-3">
 ...
 : "bg-app-surface-2 text-app-text-muted border-app-border text-[10px]"
 }>
 ...
 <Loader2 size={24} className="animate-spin text-app-text-faint" />
 ) : isInstalled ? (
 ...
 <p className="text-[9px] text-app-text-faint font-bold uppercase tracking-widest mb-2">Capabilities</p>
 ...
 : 'bg-app-bg text-app-text-faint border border-app-border hover:bg-app-surface-2'
 }`}
 >
...
export default function OrganizationDetailPage() {
 ...
 <p className="text-app-text-faint font-medium text-sm">Loading organization...</p>
 </div>
 </div>
 )
 if (!org) return (
 <div className="p-12 text-center">
 <h2 className="text-xl font-bold text-app-text">Organization Not Found</h2>
 ...
 <Button variant="ghost" size="sm" onClick={() => router.push('/organizations')} className="text-app-text-faint hover:text-app-text rounded-xl">
 <ArrowLeft size={18} />
 </Button>
 <div>
 <div className="flex items-center gap-3">
 ...
 <Badge variant="outline" className="px-4 py-2 text-sm font-mono border-app-border text-app-text-muted">
 ...
 ? 'bg-app-surface text-app-text shadow-sm' : 'text-app-text-muted hover:text-gray-700'}`}
 >
 ...
 <div className="py-8 text-center text-app-text-faint italic">Usage data unavailable</div>
 )}
 </CardContent>
 </Card>
 <div className="space-y-6">
 ...
 <p className="text-xs text-app-text-muted mt-1">${usage.plan.annual_price}/yr</p>
 )}
 {usage?.plan?.expiry && (
 <p className="text-xs text-app-text-muted mt-2">Renews: {new Date(usage.plan.expiry).toLocaleDateString()}</p>
 )}
 </div>
 ...
 <CardContent>
 <div className="text-center py-4">
 <div className="text-3xl font-black text-app-text">{activeModules}</div>
 <p className="text-xs text-app-text-muted">of {modules.length} active</p>
 </div>
 <Button variant="outline" className="w-full border-app-border text-gray-700 hover:bg-app-bg rounded-xl font-bold"
 ...
 <UserCircle size={16} className="text-app-text-faint" /> Account Owner
 </CardTitle>
 </CardHeader>
 <CardContent>
 {usage?.client ? (
 <div className="space-y-2">
 <p className="font-black text-app-text">{usage.client.full_name}</p>
 {usage.client.company_name && (
 <p className="text-xs text-app-text-muted">{usage.client.company_name}</p>
 )}
 <p className="text-xs text-app-text-faint">{usage.client.email}</p>
 {usage.client.phone && (
 <p className="text-xs text-app-text-faint">{usage.client.phone}</p>
 )}
 <Button variant="outline" size="sm"
 className="w-full border-app-border text-app-text-muted hover:bg-app-bg rounded-xl text-xs mt-2"
 onClick={async () => {
 ...
 : <ShieldOff size={16} className="text-app-text-faint" />}
 AES-256 Encryption
 </CardTitle>
 </CardHeader>
 <CardContent>
 <div className="text-center py-2">
 <div className={`text-sm font-bold ${encryptionStatus?.encryption_enabled ? 'text-emerald-700' : 'text-app-text-muted'}`}>
 {encryptionStatus === null ? 'Loading...' : encryptionStatus?.encryption_enabled ? 'Active' : 'Disabled'}
 </div>
 {encryptionStatus?.activated_at && (
 <p className="text-[10px] text-app-text-faint mt-1">Since {new Date(encryptionStatus.activated_at).toLocaleDateString()}</p>
 )}
 </div>
 <Button
 variant="outline"
 ...
 {users.length === 0 ? (
 <Card className="border-app-border shadow-sm">
 <CardContent className="py-12 text-center text-app-text-faint italic">No users found for this organization.</CardContent>
 </Card>
 ) : (
 <div className="space-y-2">
 {users.map(user => (
 ...
 <div>
 <div className="flex items-center gap-2">
 <span className="font-bold text-app-text text-sm">{user.username}</span>
 {user.is_superuser && (
 ...
 <p className="text-xs text-app-text-faint">{user.email || 'No email'} {user.role ? `· ${user.role}` : ''}</p>
 </div>
 </div>
 <div className="flex items-center gap-2">
 ...
 className="rounded-xl border-app-border text-app-text-muted hover:text-app-text text-xs font-bold">
 ...
 {sites.length === 0 ? (
 <Card className="border-app-border shadow-sm">
 <CardContent className="py-12 text-center text-app-text-faint italic">No sites found. Create the first site for this organization.</CardContent>
 </Card>
 ) : (
 <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
 {sites.map(site => (
 ...
 ? 'bg-indigo-50 text-indigo-600 border border-indigo-100'
 : 'bg-app-surface-2 text-app-text-faint border border-app-border'
 }`}>
 <Building2 size={18} />
 </div>
 <div>
 <div className="flex items-center gap-2">
 <span className="font-bold text-app-text text-sm">{site.name}</span>
 {site.code && (
 ...
 {site.city && <span className="text-xs text-app-text-faint flex items-center gap-1"><MapPin size={10} />{site.city}</span>}
 {site.phone && <span className="text-xs text-app-text-faint">{site.phone}</span>}
 </div>
 </div>
 </div>
 ...
 <div className="mt-3 pt-3 border-t border-app-border flex justify-between text-xs text-app-text-faint">
 ...
 <div className="p-5 bg-app-bg rounded-2xl border border-app-border mt-6">
 <div className="flex items-center justify-between">
 <div>
 <p className="text-xs font-bold text-app-text-muted uppercase tracking-wider">Active Modules</p>
 <p className="text-2xl font-black text-app-text mt-1">
 {usage.modules.current} <span className="text-sm font-medium text-app-text-faint">/ {usage.modules.total_available} available</span>
 </p>
 </div>
 ...
 <div className="py-12 text-center text-app-text-faint italic">Usage data unavailable</div>
 )}
 </CardContent>
 </Card>
 </div>
 ...
 <div>
 <p className="text-sm font-bold text-app-text-muted uppercase tracking-wider mb-1">Status</p>
 {org.is_active ? (
 ...
 <p className="text-sm font-bold text-app-text-muted uppercase tracking-wider mb-1">Monthly</p>
 <p className="text-2xl font-black text-app-text">${usage?.plan?.monthly_price || '0.00'}</p>
 </div>
 </div>
 </CardContent>
 </Card>
 ...
 <UserCircle size={18} className="text-app-text-faint" /> Account Owner
 </CardTitle>
 </CardHeader>
 <CardContent className="space-y-4">
 {billing.client ? (
 <div className="p-4 bg-app-bg rounded-2xl border border-app-border space-y-2">
 <p className="font-black text-app-text text-lg">{billing.client.full_name}</p>
 {billing.client.company_name && (
 <p className="text-sm text-app-text-muted">{billing.client.company_name}</p>
 )}
 <div className="flex items-center gap-4 text-xs text-app-text-faint">
 ...
 <div className="p-3 bg-app-bg rounded-xl border border-app-border text-center">
 <p className="text-[9px] text-app-text-faint font-bold uppercase tracking-wider">Net Balance</p>
 <p className="text-lg font-black text-app-text">${billing.balance.net_balance}</p>
 </div>
 </div>
 {/* CRM Profile Link */}
 {billing.client && (
 ...
 <Badge className="bg-app-surface-2 text-app-text-muted text-[10px]">{usageAny.available_plans.length} plans</Badge>
 </div>
 <CardDescription className="text-xs text-app-text-faint">Select a plan to assign to this organization. Plans are managed from the <a href="/subscription-plans" className="text-emerald-600 underline hover:text-emerald-700 font-bold">Subscription Plans</a> page.</CardDescription>
 </CardHeader>
 <CardContent>
 ...
 <div>
 <p className="font-black text-app-text">{p.name}</p>
 {p.category && <span className="text-[10px] text-app-text-faint font-medium">{p.category}</span>}
 </div>
 ...
 <>
 <span className="text-2xl font-black text-app-text">${parseFloat(p.monthly_price).toFixed(0)}</span>
 <span className="text-xs text-app-text-faint font-bold ml-1">/mo</span>
 </>
 )}
 </div>
 {p.description && <p className="text-[11px] text-app-text-faint mb-3 line-clamp-2">{p.description}</p>}
 {p.modules?.length > 0 && (
 <div className="flex flex-wrap gap-1 mb-3">
 {p.modules.slice(0, 4).map((m: string) => (
 <Badge key={m} className="bg-app-bg text-app-text-muted text-[9px] border border-app-border uppercase">{m}</Badge>
 ))}
 {p.modules.length > 4 && (
 <Badge className="bg-app-bg text-app-text-faint text-[9px] border border-app-border">+{p.modules.length - 4}</Badge>
 )}
 </div>
 )}
 {Object.keys(limits).length > 0 && (
 <div className="grid grid-cols-2 gap-1 mb-3 text-[10px] text-app-text-faint">
 ...
 <Badge className="bg-app-surface-2 text-app-text-faint text-[9px]">Subscription Payments</Badge>
 </div>
 </CardHeader>
 <CardContent>
 {billing.history.length === 0 ? (
 <div className="text-center py-12 text-app-text-faint text-sm italic">No billing records found for this organization.</div>
 ) : (
 <div className="space-y-2">
 {billing.history.map((p: Record<string, any>) => (
 <div key={p.id} className="flex items-center justify-between p-4 bg-app-bg rounded-xl border border-app-border hover:border-app-border transition-all">
 <div className="flex-1">
 ...
 }[p.type as string] || 'bg-app-bg text-app-text-muted'}
 >{p.type === 'CREDIT_NOTE' ? 'Credit Note' : p.type === 'PURCHASE' ? 'Purchase' : p.type || 'Invoice'}</Badge>
 <p className="font-bold text-app-text text-sm">{p.plan_name}</p>
 {p.previous_plan_name && (
 <span className="text-[10px] text-app-text-faint">← from {p.previous_plan_name}</span>
 )}
 </div>
 {p.notes && <p className="text-[11px] text-app-text-faint line-clamp-1">{p.notes}</p>}
 <p className="text-xs text-app-text-faint mt-0.5">{new Date(p.created_at).toLocaleDateString()} {new Date(p.created_at).toLocaleTimeString()}</p>
 </div>
 <div className="flex items-center gap-3">
 <span className={`font-black ${p.type === 'CREDIT_NOTE' ? 'text-amber-600' : 'text-app-text'}`}>
 ...
 <div>
 <p className="font-bold text-app-text">{p.addon_name}</p>
 <div className="flex items-center gap-2 mt-0.5">
 <Badge className="bg-emerald-50 text-emerald-600 border-emerald-100 text-[10px]">{p.addon_type}</Badge>
 <span className="text-xs text-app-text-faint">×{p.quantity}</span>
 <span className="text-xs text-app-text-faint">• {p.billing_cycle}</span>
 </div>
 <p className="text-[10px] text-app-text-faint mt-1">Purchased {p.purchased_at ? new Date(p.purchased_at).toLocaleDateString() : '—'}</p>
 </div>
 </div>
 <div className="flex items-center gap-3">
 <div className="text-right">
 <p className="font-black text-app-text">${p.effective_price}</p>
 <p className="text-[10px] text-app-text-faint">{p.billing_cycle === 'ANNUAL' ? '/yr' : '/mo'}</p>
 </div>
 <Button
 size="sm"
 variant="ghost"
 ...
 <Card className="border-app-border shadow-sm">
 <CardHeader>
 <CardTitle className="text-sm font-bold text-app-text-muted">History</CardTitle>
 </CardHeader>
 <CardContent>
 <div className="space-y-2">
 {addons.purchased.filter((p: Record<string, any>) => p.status !== 'active').map((p: Record<string, any>) => (
 <div key={p.id} className="flex items-center justify-between p-3 bg-app-bg rounded-xl border border-app-border opacity-60">
 <div>
 <p className="font-medium text-app-text-muted text-sm">{p.addon_name}</p>
 ...
 <Badge className="bg-app-surface-2 text-app-text-faint border-app-border text-[10px]">{p.status}</Badge>
 </div>
 ))}
 </div>
 </CardContent>
 ...
 <div className="flex items-start justify-between">
 <div>
 <p className="font-bold text-app-text">{a.name}</p>
 <div className="flex items-center gap-2 mt-1">
 <Badge variant="outline" className="text-[10px]">{a.addon_type}</Badge>
 <span className="text-xs text-app-text-faint">+{a.quantity} units</span>
 </div>
 </div>
 <div className="text-right">
 <p className="font-black text-app-text">${a.monthly_price}</p>
 <p className="text-[10px] text-app-text-faint">/month</p>
 {a.annual_price !== '0.00' && (
 <p className="text-[10px] text-app-text-faint">${a.annual_price}/yr</p>
 )}
 </div>
 </div>
 <Button
 ...
 <div>
 <label className="text-xs font-bold text-app-text-muted mb-1 block">First Name</label>
 ...
 <div>
 <label className="text-xs font-bold text-app-text-muted mb-1 block">Last Name</label>
 ...
 <div>
 <label className="text-xs font-bold text-app-text-muted mb-1 block">Username *</label>
 ...
 <div>
 <label className="text-xs font-bold text-app-text-muted mb-1 block">Email</label>
 ...
 <div>
 <label className="text-xs font-bold text-app-text-muted mb-1 block">Password *</label>
 ...
 <div>
 <p className="text-sm font-bold text-app-text">Superuser Access</p>
 <p className="text-[10px] text-app-text-muted">Full admin access to this organization</p>
 </div>
 </div>
 </div>
 <DialogFooter>
 ...
 Set a new password for <strong className="text-app-text">{String(resetTarget.username ?? '')}</strong>
 </p>
 <div className="relative">
 <Input type={showPass ? 'text' : 'password'} value={newPassword}
 onChange={e => setNewPassword(e.target.value)} placeholder="New password (min 6 chars)" className="rounded-xl pr-10" />
 <button onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-app-text-faint">
 ...
 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="text-xs font-bold text-app-text-muted mb-1 block">Site Name *</label>
 <Input value={newSite.name} onChange={e => setNewSite({ ...newSite, name: e.target.value })} placeholder="Main Branch" className="rounded-xl" />
 </div>
 <div>
 <label className="text-xs font-bold text-app-text-muted mb-1 block">Code</label>
 ...
 <div>
 <label className="text-xs font-bold text-app-text-muted mb-1 block">Address</label>
 ...
 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="text-xs font-bold text-app-text-muted mb-1 block">City</label>
 <Input value={newSite.city} onChange={e => setNewSite({ ...newSite, city: e.target.value })} placeholder="Beirut" className="rounded-xl" />
 </div>
 <div>
 <label className="text-xs font-bold text-app-text-muted mb-1 block">Phone</label>
 ...
 <div>
 <label className="text-xs font-bold text-app-text-muted mb-1 block">VAT Number</label>
 ...
 <div className="flex justify-between items-center">
 <span className="text-xs text-app-text-faint font-bold uppercase">Current Plan</span>
 <span className="font-bold text-app-text">{usage?.plan?.name || 'Free Tier'}</span>
 </div>
 <div className="border-t border-dashed border-app-border" />
 <div className="flex justify-between items-center">
 <span className="text-xs text-app-text-faint font-bold uppercase">New Plan</span>
 <span className="font-black text-app-text">{planSwitchTarget.name}</span>
 </div>
 </div>
 ...
 <div>
 <Badge className={isUpgrade ? 'bg-emerald-100 text-emerald-700' : isDowngrade ? 'bg-amber-100 text-amber-700' : 'bg-app-surface-2 text-app-text-muted'}>
 {isUpgrade ? '⬆ Upgrade' : isDowngrade ? '⬇ Downgrade' : '↔ Switch'}
 </Badge>
 </div>
 <div className="text-right">
 <p className="text-xs text-app-text-faint font-bold">Price {isUpgrade ? 'Increase' : isDowngrade ? 'Decrease' : 'Change'}</p>
 <p className={`text-lg font-black ${isUpgrade ? 'text-emerald-600' : isDowngrade ? 'text-amber-600' : 'text-app-text-muted'}`}>
 {isUpgrade ? '+' : isDowngrade ? '-' : ''}${diff.toFixed(2)}/mo
 </p>
 </div>
 </div>
 {isUpgrade && (
 <p className="text-[11px] text-app-text-faint">A <strong>Purchase Invoice</strong> of ${diff.toFixed(2)}/mo will be generated for the price difference.</p>
 )}
 {isDowngrade && (
 <p className="text-[11px] text-app-text-faint">A <strong>Credit Note</strong> of ${diff.toFixed(2)}/mo will be issued, plus a new <strong>Purchase Invoice</strong> for ${targetPrice.toFixed(2)}/mo.</p>
 )}
 {Array.isArray((planSwitchTarget as any).modules) && (planSwitchTarget as any).modules.length > 0 && (
 <div>
 <p className="text-[10px] text-app-text-faint font-bold uppercase mb-1">Modules in new plan:</p>
 ...
 <div>
 <p className="font-bold text-sm text-app-text">{c.full_name}</p>
 <p className="text-[10px] text-app-text-faint">{c.email}{c.company_name ? ` · ${c.company_name}` : ''}</p>
 </div>
 <div className="flex items-center gap-2">
 <Badge className="bg-app-bg text-app-text-faint border-app-border text-[9px]">{c.org_count} orgs</Badge>
 {usage?.client?.id === c.id && <Check size={14} className="text-emerald-600" />}
 </div>
 </button>
 ))}
 {allClients.length === 0 && (
 <p className="text-center text-xs text-app-text-faint italic py-6">No clients found</p>
 )}
 </div>
