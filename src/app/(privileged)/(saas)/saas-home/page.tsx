        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {quickStats.map((stat, i) => {
                    if (stat.label.includes("Module")) {
                        return (
                            <Link href="/modules" key={i} className="block group">
                                <Card className="bg-app-surface border-app-border rounded-[2rem] overflow-hidden group hover:border-emerald-500/30 transition-all shadow-xl h-full cursor-pointer">
                                    <CardContent className="p-8 space-y-4">
                                        ...
                                        <div>
                                            <div className="text-sm font-bold text-app-muted-foreground uppercase tracking-widest flex justify-between items-center group-hover:text-emerald-400 transition-colors">
                                                {stat.label}
                    ...
                    return (
                        <Card key={i} className="bg-app-surface border-app-border rounded-[2rem] overflow-hidden group hover:border-emerald-500/30 transition-all shadow-xl h-full">
                            <CardContent className="p-8 space-y-4">
                                <div className={`p-4 bg-${stat.color}-500/10 rounded-2xl w-fit text-${stat.color}-400 group-hover:scale-110 transition-transform`}>
                                    <stat.icon size={24} />
                                </div>
                                ...
                                <Link key={i} href={action.href} className="block group">
                                    <div className="flex items-center gap-4 p-4 rounded-2xl hover:bg-app-surface transition-all border border-transparent hover:border-app-border cursor-pointer">
                                        ...
                                        <div className="flex-1">
                                            <div className="text-sm font-bold text-app-foreground group-hover:text-emerald-600 transition-colors">{action.title}</div>
                                            <p className="text-xs text-app-muted-foreground mt-0.5 font-medium">{action.desc}</p>
                                        </div>
                                        <TrendingUp size={14} className="text-app-faint group-hover:text-emerald-400 transition-colors" />
                                    </div>
                                </Link>
                            ))}
                        </div>
