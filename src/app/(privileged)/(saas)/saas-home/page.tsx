                    ...
                    const CardComponent = (
                        <Card className="card-premium group hover:shadow-2xl hover:shadow-emerald-700/5 transition-all duration-500 overflow-hidden relative h-full">
                            ...
                            <CardContent className="p-8 space-y-6">
                                <div className="w-14 h-14 rounded-2xl bg-slate-50 text-emerald-600 flex items-center justify-center shadow-inner shadow-slate-200 group-hover:rotate-12 transition-transform duration-500">
                                    <stat.icon size={28} />
                                </div>
                                <div>
                                    <div className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex justify-between items-center group-hover:text-emerald-500 transition-colors">
                                        {stat.label}
                                        {stat.label.includes("Module") && <Zap size={14} className="text-amber-500 animate-pulse" />}
                                    </div>
                                    <div className="text-4xl font-black text-slate-800 mt-1.5 tracking-tighter">{stat.value}</div>
                                </div>
                                <div className="flex items-center gap-2.5 text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 w-fit px-3 py-1.5 rounded-lg border border-slate-100">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <Card className="lg:col-span-2 card-premium overflow-hidden bg-white">
                    <CardHeader className="p-10 pb-4 flex flex-row items-center justify-between">
                        <div>
                            <CardTitle className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Platform Genesis Feed</CardTitle>
                            <h3 className="text-2xl font-black text-slate-800 tracking-tight">Recent Deployments</h3>
                        </div>
                        <Link href="/organizations">
                    ...
                    <CardContent className="px-6 pb-10">
                        <div className="space-y-3">
                            {stats?.latestTenants?.map((ten: Record<string, any>) => (
                                <div key={ten.id} className="flex items-center justify-between p-6 rounded-[2rem] hover:bg-slate-50/50 transition-all border border-transparent hover:border-slate-100 group/ten">
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div>
                                            <div className="text-[15px] font-black text-slate-800 uppercase tracking-tight">{ten.name}</div>
                                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.1em] mt-1">{ten.slug}.localhost</div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-10">
                                        <Badge variant="outline" className={`min-w-[100px] py-1.5 font-black text-[10px] uppercase text-center tracking-widest rounded-full border-0 ${ten.is_active ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                ...
                {/* Quick Actions Panel */}
                <Card className="card-premium overflow-hidden bg-white border-emerald-100 shadow-emerald-700/5">
                                ...
                                <Link key={i} href={action.href} className="block group/act">
                                    <div className="flex items-center gap-5 p-5 rounded-[1.8rem] hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100 shadow-sm hover:shadow-lg hover:shadow-slate-200/20 active:scale-[0.98]">
                                        ...
                                        <div className="flex-1">
                                            <div className="text-[13px] font-black text-slate-800 uppercase tracking-tight group-hover/act:text-emerald-700 transition-colors">{action.title}</div>
                                            <p className="text-[10px] font-bold text-slate-400 mt-1 italic uppercase tracking-tighter">{action.desc}</p>
                                        </div>
