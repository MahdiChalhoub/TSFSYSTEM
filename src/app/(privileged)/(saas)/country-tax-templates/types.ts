...
export type Template = {
  ...
  org_policy_defaults: any; document_requirements: DocReq[]
  counterparty_presets: any[]; custom_tax_rule_presets?: any[]
  tax_catalog?: TaxDef[]
  is_active: boolean; created_at: string; updated_at: string
  bad_debt_recovery_months?: number; self_supply_vat_threshold?: any
  gift_vat_threshold?: any; vat_on_advance_payment?: boolean
}
...
export function migrateFromLegacy(form: Record<string, any>): { catalog: TaxDef[]; orgPresets: OrgPreset[]; cpPresets: CounterpartyPreset[] } {
  // If already migrated
  if (form.tax_catalog?.length) {
    const orgP = Array.isArray(form.org_policy_defaults) ? form.org_policy_defaults : []
    const cpP = Array.isArray(form.counterparty_presets) ? form.counterparty_presets : []
    return { catalog: form.tax_catalog, orgPresets: orgP, cpPresets: cpP }
  }

  const catalog: TaxDef[] = []
  // Migrate custom_tax_rule_presets
  const customTaxes = form.custom_tax_rule_presets || []
  ...
  const rawPresets = Array.isArray(form.org_policy_defaults) ? form.org_policy_defaults
    : (form.org_policy_defaults && typeof form.org_policy_defaults === 'object' ? [form.org_policy_defaults] : [])
  ...
  const cpPresets: CounterpartyPreset[] = (form.counterparty_presets || []).map((p: any) => ({
