...
function TerminalLog({ logs, visible }: { logs: string[], visible: boolean }) {
 ...
 <span className="text-app-text-muted font-bold uppercase tracking-widest text-[9px]">Update Console</span>
 </div>
 <div className="space-y-1 max-h-[200px] overflow-y-auto custom-scrollbar">
 {logs.map((log, i) => (
 <div key={i} className="flex gap-3">
 <span className="text-app-text-muted shrink-0">[{new Date().toLocaleTimeString([], { hour12: false })}]</span>
...
export default function SystemUpdatesPage() {
 ...
 <p className="text-app-text-muted mt-2 font-medium text-sm md:text-base max-w-md">Platform update manager. Ensuring seamless upgrades.</p>
 </div>
 ...
 disabled={syncing || loading}
 variant="outline"
 className="flex-1 sm:flex-none justify-center bg-app-surface border-app-border hover:bg-app-bg text-gray-700 px-6 py-7 rounded-2xl flex gap-2 font-bold transition-all shadow-sm"
 >
 ...
 <CardDescription className="text-app-text-muted font-medium tracking-tight">Platform status monitoring</CardDescription>
 </CardHeader>
 ...
 <div className="flex justify-between items-center py-4 border-b border-gray-50">
 <span className="text-[10px] font-black text-app-text-faint uppercase tracking-[0.2em]">Integrity Check</span>
 ...
 <div className="flex justify-between items-center py-4 border-b border-gray-50">
 <span className="text-[10px] font-black text-app-text-faint uppercase tracking-[0.2em]">Environment</span>
 ...
 <CardDescription className="text-app-text-muted font-medium">Update history</CardDescription>
 </div>
 ...
 <p className="text-sm font-black text-app-text-faint uppercase tracking-widest">Scanning Registry...</p>
 </div>
 ) : history.length === 0 ? (
 <div className="py-24 border-2 border-dashed border-app-border rounded-[2.5rem] flex flex-col items-center justify-center text-app-text-faint gap-4">
 <div className="p-6 bg-app-bg rounded-full">
 <Info size={40} />
 </div>
 ...
 ? 'bg-gray-50/50 border-app-border'
 : 'bg-app-surface border-indigo-100 hover:border-indigo-400 shadow-lg shadow-indigo-50 hover:scale-[1.01]'
 }`}
 >
 <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
 <div className="flex gap-4">
 <div className={`p-4 rounded-2xl ${update.is_applied ? 'bg-gray-200 text-app-text-muted' : 'bg-indigo-100 text-indigo-600'}`}>
 <Cpu size={24} />
 </div>
 <div className="space-y-1">
 <div className="flex items-center gap-3">
 <h3 className="text-xl font-black text-app-text tracking-tight">v{update.version}</h3>
 {update.is_applied ? (
 ...
 <p className="text-sm text-app-text-muted font-bold max-w-md">{update.changelog || "Stabilization patches and infrastructure hardening."}</p>
 </div>
 </div>
 {!update.is_applied && (
 <Button
 ...
 <div className="mt-6 flex flex-wrap items-center gap-6 text-[9px] font-black tracking-[0.1em] text-app-text-faint border-t border-gray-50 pt-4">
