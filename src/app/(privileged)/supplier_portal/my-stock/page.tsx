...
type MyStock = Record<string, any>

const ALL_COLUMNS: ColumnDef<MyStock>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function MyStockListPage() {
  const router = useRouter()
  const [items, setItems] = useState<MyStock[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('supplier_portal_my-stock', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('supplier_portal/my-stock/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load my-stock:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<MyStock>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/supplier_portal/my-stock/new')}
        ...
        actions={{
          onView: (r) => router.push(`/supplier_portal/my-stock/${r.id}`),
          onEdit: (r) => router.push(`/supplier_portal/my-stock/${r.id}/edit`),
        }}
      >
        <TypicalFilter
