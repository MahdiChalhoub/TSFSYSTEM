...
type MyNotifications = Record<string, any>

const ALL_COLUMNS: ColumnDef<MyNotifications>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function MyNotificationsListPage() {
  const router = useRouter()
  const [items, setItems] = useState<MyNotifications[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('supplier_portal_my-notifications', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('supplier_portal/my-notifications/')
      setItems(Array.isArray(data) ? data : (data?.results || []))
    } catch (error) {
      console.error('Failed to load my-notifications:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<MyNotifications>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/supplier_portal/my-notifications/new')}
        ...
        actions={{
          onView: (r) => router.push(`/supplier_portal/my-notifications/${r.id}`),
          onEdit: (r) => router.push(`/supplier_portal/my-notifications/${r.id}/edit`),
        }}
      >
        <TypicalFilter
