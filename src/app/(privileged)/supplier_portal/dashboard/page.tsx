...
const ALL_COLUMNS: ColumnDef<Dashboard>[] = [
...
export default function DashboardListPage() {
  ...
  const settings = useListViewSettings('supplier_portal_dashboard', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('supplier_portal/dashboard/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Dashboard>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/supplier_portal/dashboard/new')}
        ...
        actions={{
          onView: (r) => router.push(`/supplier_portal/dashboard/${r.id}`),
          onEdit: (r) => router.push(`/supplier_portal/dashboard/${r.id}/edit`),
        }}
      >
        <TypicalFilter
