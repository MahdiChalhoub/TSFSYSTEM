...
const ALL_COLUMNS: ColumnDef<Config>[] = [
...
export default function ConfigListPage() {
  ...
  const settings = useListViewSettings('supplier_portal_config', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('supplier_portal/config/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<Config>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/supplier_portal/config/new')}
        ...
        actions={{
          onView: (r) => router.push(`/supplier_portal/config/${r.id}`),
          onEdit: (r) => router.push(`/supplier_portal/config/${r.id}/edit`),
        }}
      >
        <TypicalFilter
