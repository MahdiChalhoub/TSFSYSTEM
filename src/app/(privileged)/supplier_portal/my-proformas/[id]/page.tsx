...
export default function MyProformasDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`supplier_portal/my-proformas/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`supplier_portal/my-proformas/${id}/`, {
        method: 'DELETE'
      })
      router.push('/supplier_portal/my-proformas')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black text-app-foreground">
              {item.name || item.title || `My-Proformas #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage my proformas details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/supplier_portal/my-proformas/${id}/edit`)}
            >
