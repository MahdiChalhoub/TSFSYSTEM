...
export default function PortalAccessDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`supplier_portal/portal-access/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`supplier_portal/portal-access/${id}/`, {
        method: 'DELETE'
      })
      router.push('/supplier_portal/portal-access')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Portal-Access #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage portal access details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/supplier_portal/portal-access/${id}/edit`)}
            >
