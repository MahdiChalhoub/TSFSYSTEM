...
type PortalAccess = Record<string, any>

const ALL_COLUMNS: ColumnDef<PortalAccess>[] = [
  { key: 'id', label: 'ID', sortable: true }
]

export default function PortalAccessListPage() {
  const router = useRouter()
  const [items, setItems] = useState<PortalAccess[]>([])
  const [loading, setLoading] = useState(true)
  const [search, setSearch] = useState('')

  const settings = useListViewSettings('supplier_portal_portal-access', {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch('supplier_portal/portal-access/')
  ...
  return (
    <div className="space-y-4">
      <TypicalListView<PortalAccess>
        ...
        headerExtra={
          <Button
            onClick={() => router.push('/supplier_portal/portal-access/new')}
        ...
        actions={{
          onView: (r) => router.push(`/supplier_portal/portal-access/${r.id}`),
          onEdit: (r) => router.push(`/supplier_portal/portal-access/${r.id}/edit`),
        }}
      >
        <TypicalFilter
