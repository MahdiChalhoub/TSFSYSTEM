...
export default function MyOrdersDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`supplier_portal/my-orders/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`supplier_portal/my-orders/${id}/`, {
        method: 'DELETE'
      })
      router.push('/supplier_portal/my-orders')
    } catch (error) {
    ...
    <div className="min-h-screen layout-container-padding bg-app-bg">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1>
              {item.name || item.title || `My-Orders #${item.id}`}
            </h1>
            <p className="text-app-muted-foreground mt-1">
              View and manage my orders details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/supplier_portal/my-orders/${id}/edit`)}
            >
