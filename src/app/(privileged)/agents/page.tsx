    ...
    return (
        <div className="flex flex-col gap-8 p-1 sm:p-2 animate-in fade-in duration-500">
            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    <Button variant="outline" className="border-2 border-gray-100 font-bold" onClick={fetchData}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {loading && agents.length === 0 ? (
                            Array.from({ length: 4 }).map((_, i) => (
                                <div key={i} className="h-[200px] rounded-3xl bg-gray-100 animate-pulse" />
                            ))
                        ) : (
                            agents.map(agent => {
                                const Icon = getAgentIcon(agent.role)
                                return (
                                    <Card key={agent.id} className="relative overflow-hidden group border-2 border-gray-100 hover:border-purple-200 transition-all duration-300 rounded-3xl shadow-sm hover:shadow-xl">
                                        ...
                                        <CardHeader className="pb-2">
                                            <div className="flex items-start gap-4">
                                                <div className="w-12 h-12 rounded-2xl bg-gray-50 group-hover:bg-purple-50 flex items-center justify-center transition-colors">
                                                    <Icon className="w-6 h-6 text-gray-400 group-hover:text-purple-600" />
                                                </div>
                                                <div>
                                                    <CardTitle className="text-lg font-black text-gray-900">{agent.name}</CardTitle>
                                                    <Badge variant="secondary" className="bg-gray-100 text-gray-500 font-bold text-[10px] uppercase tracking-wider">{agent.role_display}</Badge>
                                                </div>
                                            </div>
                                        </CardHeader>
                        ...
                        <CardContent className="p-0">
                            <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y divide-gray-100">
                                {[
                                    ...
                                    <div key={item.label} className="p-6 flex flex-col items-center gap-2 hover:bg-gray-50 transition-colors">
