...
export default function SupplierNotificationsPage() {
    ...
    useEffect(() => {
        ...
        portalFetch('/notifications/', token)
            .then(data => {
    ...
    const markAsRead = (id: string) => {
        ...
        const token = getToken(slug)
        if (!token) return
        portalFetch(`/notifications/${id}/read/`, token, { method: 'POST' }).catch(() => { })
    }
    ...
    const markAllRead = () => {
        ...
        const token = getToken(slug)
        if (!token) return
        portalFetch('/notifications/mark-all-read/', token, { method: 'POST' }).catch(() => { })
    }
