    ...
    if (orderPlaced) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-6"
                ...
                <div className="w-20 h-20 rounded-full bg-app-success-soft flex items-center justify-center mb-6">
                    <CheckCircle2 size={40} className="text-app-success" />
                </div>
                <h2 className="text-3xl font-bold text-indigo-950 mb-3"
                    style={{ fontFamily: "'Playfair Display', serif" }}>
                    Thank You!
                </h2>
                <p className="text-app-muted-foreground text-sm mb-1 max-w-sm">
                    Your order has been placed successfully. We&apos;ll send you a confirmation soon.
                </p>
                {orderId && <p className="text-xs text-app-muted-foreground font-mono mb-6">Order #{orderId}</p>}
                <Link href={base}
            ...
            <div className="max-w-xl mx-auto px-6 py-10" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                <div className="bg-white rounded-3xl border border-violet-100 p-8 shadow-xl space-y-6">
                    ...
                    <button
                        ...
                        className="w-full text-app-muted-foreground text-xs font-bold hover:text-indigo-950 transition-colors py-2"
                    >
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-5 gap-10">
                ...
                <div className="lg:col-span-3 space-y-8">
                    ...
                    <section className="bg-white rounded-2xl border border-violet-100 p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {[
                                ...
                                <div key={f.k}>
                                    <label className="text-xs font-semibold text-app-muted-foreground uppercase tracking-wider mb-1.5 block">
                                        {f.label} {f.req && <span className="text-app-error">*</span>}
                                    </label>
                                    <input
                                        type={f.type}
                                        required={f.req}
                                        ...
                                        className="w-full px-4 py-3 rounded-xl border border-violet-200 bg-violet-50/30 text-indigo-950 placeholder:text-app-muted-foreground focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 text-sm"
                                    />
                                </div>
                            ))}
                            <div className="sm:col-span-2">
                                <label className="text-xs font-semibold text-app-muted-foreground uppercase tracking-wider mb-1.5 block">Address</label>
                                <input
                                    type="text"
                                    value={form.address}
                            ...
                            <div className="sm:col-span-2">
                                <label className="text-xs font-semibold text-app-muted-foreground uppercase tracking-wider mb-1.5 block">Notes</label>
                                <textarea
                                    rows={3}
                                    value={form.notes}
                                    onChange={e => update('notes', e.target.value)}
                                    className="w-full px-4 py-3 rounded-xl border border-violet-200 bg-violet-50/30 text-indigo-950 placeholder:text-app-muted-foreground focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 text-sm resize-none"
                                />
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="bg-white rounded-2xl border border-violet-100 p-6 shadow-sm">
                        ...
                        {error && (
                            <div className="mb-4 p-3 bg-app-error-soft border border-red-100 rounded-xl text-app-error text-xs font-medium">
                                {error}
                            </div>
                        )}
                            ...
                            {[
                                ...
                                <button key={m.id} type="button"
                                    ...
                                    className={`p-4 rounded-xl border-2 text-sm font-semibold transition flex items-center gap-2 justify-center ${form.paymentMethod === m.id
                                        ...
                                        : 'border-violet-100 text-app-muted-foreground hover:border-violet-300'
                                        }`}>
                ...
                <div className="lg:col-span-2">
                    <div className="bg-white rounded-2xl border border-violet-100 p-6 shadow-sm sticky top-24">
                        ...
                        <div className="space-y-3 max-h-60 overflow-y-auto">
                            {cart.map(item => (
                                <div key={item.product_id} className="flex justify-between text-sm">
                                    <span className="text-app-muted-foreground truncate max-w-[60%]">
                        ...
                        <p className="text-[10px] text-app-muted-foreground text-center mt-3">
