    ...
    if (orderPlaced) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-6"
                ...
                <p className="text-app-text-faint text-sm mb-1 max-w-sm">
                    Your order has been placed successfully. We&apos;ll send you a confirmation soon.
                </p>
                {orderId && <p className="text-xs text-app-text-faint font-mono mb-6">Order #{orderId}</p>}
                <Link href={base}
    ...
    if (stripeClientSecret) {
        return (
            <div className="max-w-xl mx-auto px-6 py-10" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                <div className="bg-app-surface rounded-3xl border border-violet-100 p-8 shadow-xl space-y-6">
                    ...
                    <button
                        ...
                        className="w-full text-app-text-faint text-xs font-bold hover:text-indigo-950 transition-colors py-2"
                    >
        ...
        <div className="max-w-5xl mx-auto px-6 py-10" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-5 gap-10">
                ...
                <div className="lg:col-span-3 space-y-8">
                    {/* Contact */}
                    <section className="bg-app-surface rounded-2xl border border-violet-100 p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {[
                                ...
                                <div key={f.k}>
                                    <label className="text-xs font-semibold text-app-text-faint uppercase tracking-wider mb-1.5 block">
                                    ...
                                    <input
                                        ...
                                        className="w-full px-4 py-3 rounded-xl border border-violet-200 bg-violet-50/30 text-indigo-950 placeholder:text-app-text-faint focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 text-sm"
                                    />
                                </div>
                            ))}
                            <div className="sm:col-span-2">
                                <label className="text-xs font-semibold text-app-text-faint uppercase tracking-wider mb-1.5 block">Address</label>
                                <input
                                    type="text"
                                    value={form.address}
                            ...
                            <div className="sm:col-span-2">
                                <label className="text-xs font-semibold text-app-text-faint uppercase tracking-wider mb-1.5 block">Notes</label>
                                <textarea
                                    rows={3}
                                    value={form.notes}
                                    onChange={e => update('notes', e.target.value)}
                                    className="w-full px-4 py-3 rounded-xl border border-violet-200 bg-violet-50/30 text-indigo-950 placeholder:text-app-text-faint focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 text-sm resize-none"
                                />
                            </div>
                        </div>
                    </section>
                            ...
                            {[
                                ...
                                <button key={m.id} type="button"
                                    ...
                                    className={`p-4 rounded-xl border-2 text-sm font-semibold transition flex items-center gap-2 justify-center ${form.paymentMethod === m.id
                                        ...
                                        : 'border-violet-100 text-app-text-faint hover:border-violet-300'
                                        }`}>
                ...
                <div className="lg:col-span-2">
                    <div className="bg-app-surface rounded-2xl border border-violet-100 p-6 shadow-sm sticky top-24">
                        <h3 className="font-bold text-indigo-950 mb-4">Order Summary</h3>
                        <div className="space-y-3 max-h-60 overflow-y-auto">
                            {cart.map(item => (
                                <div key={item.product_id} className="flex justify-between text-sm">
                                    <span className="text-app-text-muted truncate max-w-[60%]">
                        ...
                        <p className="text-[10px] text-app-text-faint text-center mt-3">
