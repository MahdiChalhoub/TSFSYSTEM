        ...
        <div className="min-h-screen bg-gradient-to-b from-violet-50/30 to-white"
            ...
            <div className="max-w-7xl mx-auto px-6 py-10">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-8">
                    <div className="flex-1 relative">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-10 py-3.5 rounded-xl border border-violet-200 bg-white text-indigo-950 placeholder:text-app-muted-foreground focus:outline-none focus:ring-2 focus:ring-violet-300 focus:border-violet-400 shadow-sm text-sm"
                        />
                        {query && (
                            <button onClick={() => setQuery('')}
                                className="absolute right-3 top-1/2 -translate-y-1/2 text-app-muted-foreground hover:text-violet-600">
                                <X size={16} />
                            </button>
                        )}
                    </div>
                    ...
                    <div className="flex gap-2 flex-wrap mb-8">
                        <button onClick={() => setSelectedCat(null)}
                            className={`px-4 py-2 rounded-full text-xs font-semibold transition border ${!selectedCat
                                    ...
                                    : 'bg-white text-app-muted-foreground border-violet-200 hover:border-violet-400'
                                }`}>
                            All ({products.length})
                        </button>
                        {categories.map(cat => (
                ...
                {loading ? (
                    ...
                    <>
                        <p className="text-sm text-app-muted-foreground mb-6">{results.length} {results.length === 1 ? 'result' : 'results'}</p>
