...
export default function EmporiumHomePage({ products = [], categories = [] }: HomePageProps) {
    ...
    return (
        <div className="min-h-screen bg-app-bg font-sans text-app-foreground">
            {/* Value Props Bar */}
                        ...
                        <div className="bg-app-surface rounded-2xl border border-app-border shadow-sm overflow-hidden">
                            ...
                            <nav className="p-2">
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center justify-between group ${!selectedCategory ? 'bg-yellow-400 text-app-foreground shadow-sm' : 'hover:bg-app-surface-2'}`}
                                >
                                ...
                                {categories.map(cat => (
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center justify-between group ${selectedCategory === cat.name ? 'bg-yellow-400 text-app-foreground shadow-sm' : 'hover:bg-app-surface-2 text-app-muted-foreground'}`}
                                    >
                                        {cat.name}
                    ...
                    <div className="flex-1 space-y-6">
                        ...
                        <div className="bg-app-surface p-4 rounded-2xl border border-app-border shadow-sm space-y-4 lg:space-y-0 lg:flex lg:items-center lg:gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button className="flex items-center gap-2 px-4 py-2.5 bg-app-surface-2 rounded-xl text-sm font-bold text-app-muted-foreground hover:bg-app-border transition-all">
                                    <ArrowUpDown size={16} /> Sort
                                </button>
                                <button className="lg:hidden flex items-center gap-2 px-4 py-2.5 bg-app-surface-2 rounded-xl text-sm font-bold text-app-muted-foreground hover:bg-app-border transition-all">
                        ...
                        {filtered.length === 0 ? (
                            <div className="bg-app-surface rounded-3xl border border-app-border p-20 text-center space-y-4">
                                ...
                                <p className="text-app-muted-foreground max-w-xs mx-auto text-sm">We couldn't find what you're looking for. Try a different category or search term.</p>
                                <button
                                    onClick={() => { setSearch(''); setSelectedCategory(null) }}
                                    className="px-6 py-2.5 bg-yellow-400 text-app-foreground rounded-xl text-sm font-black shadow-lg shadow-yellow-200/50"
                                >
                                    CLEAR ALL FILTERS
                                </button>
                            </div>
