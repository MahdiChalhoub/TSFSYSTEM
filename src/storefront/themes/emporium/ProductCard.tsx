    ...
    if (layout === 'compact') {
        return (
            <div className="group bg-white rounded-2xl border border-slate-200 p-4 flex gap-6 hover:shadow-xl hover:shadow-slate-200/50 hover:border-yellow-400/50 transition-all">
                <div className="w-32 h-32 bg-slate-50 rounded-xl overflow-hidden shrink-0 border border-slate-100 flex items-center justify-center relative">
                    {product.image_url ? (
                ...
                <div className="flex-1 flex flex-col justify-between py-1">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-[10px] font-black bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full uppercase">{product.category_name || 'General'}</span>
                        </div>
                        <Link href={`/tenant/${slug}/product/${product.id}`}>
                            <h3 className="text-lg font-black text-slate-900 leading-tight hover:text-indigo-600 transition-colors">{product.name}</h3>
                        </Link>
                    ...
                    <div className="flex items-center justify-between mt-4">
                        ...
                        <button
                            ...
                            className="bg-yellow-400 hover:bg-yellow-500 text-slate-950 px-6 py-2.5 rounded-xl font-black text-xs flex items-center gap-2 shadow-lg shadow-yellow-200 active:scale-95 transition-all"
                        >
        ...
        <div className="group bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-2xl hover:shadow-slate-200/50 hover:border-yellow-400/50 transition-all flex flex-col">
            <div className="aspect-square bg-slate-50 relative overflow-hidden flex items-center justify-center border-b border-slate-100">
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-2">
                    <div className="bg-white/90 backdrop-blur-sm border border-slate-200 rounded-lg p-1.5 shadow-sm">
                        <ShieldCheck size={14} className="text-emerald-500" />
                    </div>
                </div>
                <button
                    onClick={handleAddToCart}
                    className="absolute bottom-3 right-3 w-10 h-10 bg-yellow-400 text-slate-900 rounded-xl flex items-center justify-center shadow-lg transform translate-y-12 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 active:scale-90"
                >
            ...
            <div className="p-4 flex-1 flex flex-col">
                <div className="flex-1 space-y-2">
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">SKU: {product.sku || 'N/A'}</span>
                        <div className="flex items-center gap-1 text-yellow-400">
                            <Star size={10} fill="currentColor" />
                            <span className="text-[10px] font-black text-slate-600">{rating}</span>
                        </div>
                    </div>
                    <Link href={`/tenant/${slug}/product/${product.id}`}>
                        <h3 className="text-sm font-black text-slate-900 leading-snug hover:text-indigo-600 transition-colors line-clamp-2">
                            {product.name}
                        </h3>
                    </Link>
                </div>
                <div className="mt-4 space-y-1">
                    <div className="flex items-baseline gap-1">
                        ...
                        {price > 100 && (
                            <span className="text-xs text-slate-400 line-through font-bold">${(price * 1.25).toFixed(2)}</span>
                        )}
                    </div>
