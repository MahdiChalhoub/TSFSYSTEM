            ...
            <div className="bg-app-bg border-b border-app-border py-1.5 px-4 lg:px-6 hidden md:block">
                <div className="max-w-7xl mx-auto flex justify-between items-center text-[10px] font-bold text-app-muted-foreground uppercase tracking-widest">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex items-center gap-1.5 cursor-pointer hover:text-app-foreground transition-colors">USD <ChevronDown size={10} /></div>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 lg:px-6 h-20 flex items-center gap-6 lg:gap-12">
                ...
                <div className="flex items-center gap-6">
                    <Link href={`/tenant/${slug}`} className="flex items-center gap-3 shrink-0">
                        ...
                        <span className="font-black text-lg tracking-tighter text-app-foreground hidden lg:block italic lowercase">
                ...
                <div className="flex-1 hidden md:flex items-center relative">
                    <button className="h-12 px-4 bg-app-surface-2 border-y border-l border-app-border rounded-l-xl text-xs font-black text-app-muted-foreground flex items-center gap-2 hover:bg-app-border transition-colors">
                ...
                <div className="flex items-center gap-2 lg:gap-4 shrink-0">
                    {isAuthenticated ? (
                        <Link href={`/tenant/${slug}/account`} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-app-bg transition-all group">
                            ...
                            <div className="hidden lg:block text-left">
                                <p className="text-[10px] font-black text-app-muted-foreground uppercase leading-none mb-1 tracking-widest">Hi, {user?.name?.split(' ')[0]}</p>
                                <p className="text-sm font-black text-app-foreground leading-none">Account</p>
                            </div>
                        </Link>
                    ) : (
                        <Link href={`/tenant/${slug}/register`} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-app-bg transition-all group">
                            ...
                            <div className="hidden lg:block text-left">
                                <p className="text-[10px] font-black text-app-muted-foreground uppercase leading-none mb-1 tracking-widest">Guest</p>
                                <p className="text-sm font-black text-app-foreground leading-none">Sign In</p>
                            </div>
                        </Link>
                    )}
                    ...
                    >
                        ...
                        {cartCount > 0 && (
                            <span className="absolute top-1 right-1 min-w-[20px] h-[20px] bg-yellow-400 border-2 border-white text-[10px] font-black text-app-foreground rounded-full flex items-center justify-center shadow-sm">
                                {cartCount}
                            </span>
                        )}
                    </button>

                    <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden w-12 h-12 flex items-center justify-center text-app-foreground">
