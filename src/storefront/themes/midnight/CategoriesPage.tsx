    ...
    if (categories.length === 0) {
        return (
            <div className="max-w-6xl mx-auto px-4 py-16 text-center">
                <Grid3x3 size={48} className="mx-auto text-slate-700 mb-4" />
                <h2 className="text-xl font-bold text-white">No categories yet</h2>
                <p className="text-slate-500 mt-2 text-sm">Categories will appear here once products are added</p>
            </div>
        )
    }
                        ...
                        <Link key={cat.id} href={path(`/?category=${cat.name}`)}
                            ...
                            <div className="p-6 flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500 mt-1">{cat.product_count} product{cat.product_count !== 1 ? 's' : ''}</p>
                                </div>
