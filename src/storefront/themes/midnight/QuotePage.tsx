        ...
        return (
            <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 relative overflow-hidden">
                ...
                <div className="max-w-md w-full text-center space-y-10 relative z-10">
                    <div className="w-24 h-24 bg-emerald-500/10 border border-emerald-500/20 rounded-[2rem] flex items-center justify-center mx-auto text-emerald-400 shadow-2xl shadow-emerald-500/20 rotate-12">
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                                ...
                                <div key={i} className="flex gap-3 items-start">
                                    <div className="flex-1 space-y-3">
                                        <input type="text" placeholder="Product name or description" value={line.product_name} onChange={e => updateLine(i, 'product_name', e.target.value)}
                                            className="w-full bg-slate-950/60 border border-white/5 p-4 rounded-2xl text-white outline-none focus:border-blue-500 transition-all placeholder:text-app-foreground font-medium" />
                                        <div className="flex gap-3">
                                            ...
                                            <input type="text" placeholder="Notes (optional)" value={line.notes} onChange={e => updateLine(i, 'notes', e.target.value)}
                                                className="flex-1 bg-slate-950/60 border border-white/5 p-4 rounded-2xl text-white outline-none focus:border-blue-500 transition-all placeholder:text-app-foreground font-medium" />
                                        </div>
                                    </div>
                                    {lineItems.length > 1 && (
                    ...
                    <div className="p-10 bg-slate-900/40 border border-white/5 rounded-[3rem] space-y-6">
                        ...
                        <textarea placeholder="Any additional requirements or notes..." value={message} onChange={e => setMessage(e.target.value)} rows={4}
                            className="w-full bg-slate-950/60 border border-white/5 p-5 rounded-2xl text-white outline-none focus:border-blue-500 transition-all placeholder:text-app-foreground resize-none font-medium" />
                    </div>
