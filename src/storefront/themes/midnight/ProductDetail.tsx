        ...
        <div className="min-h-screen bg-slate-950 py-8">
            <div className="max-w-5xl mx-auto px-4">
                ...
                <button onClick={() => router.back()}
                    className="flex items-center gap-2 text-sm text-app-muted-foreground hover:text-white mb-8 transition-colors">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden aspect-square">
                        {detail.image_url ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-900 to-slate-950">
                                <Package size={64} className="text-app-foreground" />
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-6">
                        {detail.category_name && (
                            <span className="inline-flex items-center gap-2 px-3 py-1 bg-app-success/10 text-emerald-400 rounded-full text-[10px] font-black uppercase tracking-wider border border-app-success/20">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs text-app-muted-foreground">Premium Quality</span>
                        </div>

                        <p className="text-[10px] text-app-muted-foreground font-mono tracking-widest">SKU: {detail.sku}</p>

                        {detail.description && (
                            <p className="text-app-muted-foreground leading-relaxed">{detail.description}</p>
                        )}
                        ...
                        {showPrice ? (
                            <div className="space-y-1">
                                <div className="text-4xl font-black text-white">
                                    <span className="text-app-success mr-1">$</span>{detail.selling_price_ttc}
                                </div>
                                {detail.selling_price_ht && detail.selling_price_ht !== detail.selling_price_ttc && (
                                    <p className="text-xs text-app-muted-foreground">Before tax: ${detail.selling_price_ht}</p>
                                )}
                            </div>
                        ) : (
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {detail.barcode && (
                                <div className="bg-white/5 border border-white/5 rounded-xl p-3">
                                    <p className="text-[10px] text-app-muted-foreground mb-1">Barcode</p>
                                    <p className="text-xs text-white font-mono">{detail.barcode}</p>
                                </div>
                            )}
                            {detail.unit_of_measure && (
                                <div className="bg-white/5 border border-white/5 rounded-xl p-3">
                                    <p className="text-[10px] text-app-muted-foreground mb-1">Unit</p>
                        ...
                        {!isQuoteMode && (
                            <div className="space-y-4 pt-4">
                                <div className="flex items-center gap-4">
                                    <span className="text-xs text-app-muted-foreground font-bold uppercase tracking-wider">Qty</span>
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 py-4 rounded-2xl font-bold text-sm uppercase tracking-wider transition-all flex items-center justify-center gap-2
                                            ${added
                                                ...
                                                : 'bg-app-success hover:bg-app-success text-white shadow-lg shadow-emerald-900/30'
                                    ...
                                    <button
                                        ...
                                        className={`w-14 h-14 rounded-2xl border flex items-center justify-center transition-all
                                            ${isInWishlist(product.id)
                                                ? 'bg-app-error/20 text-rose-400 border-app-error/30'
                                                : 'bg-white/5 text-app-muted-foreground border-white/10 hover:text-rose-400'
                                            }`}>
                        ...
                        {isQuoteMode && (
                            <Link href={`/tenant/${slug}/quote`}
                                className="block w-full py-4 bg-app-success/10 text-teal-400 border border-app-success/20 rounded-2xl font-bold text-sm uppercase tracking-wider text-center hover:bg-app-success/20 transition-all">
