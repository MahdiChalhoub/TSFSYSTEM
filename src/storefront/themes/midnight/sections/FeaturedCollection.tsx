            ...
            <div className="max-w-6xl mx-auto px-4">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
                    ...
                    <div className="flex items-center gap-2 bg-white/5 p-1 rounded-xl border border-white/10">
                        <button
                            ...
                            className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
                        >
                            <Grid size={18} />
                        </button>
                        <button
                            onClick={() => setViewMode('list')}
                            className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
                        >
                            <List size={18} />
                        </button>
                    </div>
                    ...
                    <div className="lg:col-span-3">
                        <div className="relative group">
                            ...
                            <input
                                ...
                                className="w-full bg-slate-900/50 border border-white/10 pl-14 pr-12 py-5 rounded-[2rem] text-white outline-none focus:border-emerald-500/30 transition-all placeholder:text-slate-600 font-medium"
                            />
                            {search && (
                ...
                {filtered.length > 0 ? (
                    ...
                    <div className="py-32 text-center bg-white/5 rounded-[3rem] border border-dashed border-white/10">
                        <div className="w-20 h-20 bg-slate-900 border border-white/10 rounded-3xl flex items-center justify-center mx-auto mb-6 text-slate-600">
                            <Search size={32} />
                        </div>
                        <h3 className="text-xl font-black text-white italic">No matches found</h3>
                        <p className="text-slate-500 mt-2 font-medium">Try broadening your search or selection.</p>
                    </div>
                )}
            </div>
        </section>
