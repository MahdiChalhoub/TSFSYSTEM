            ...
            <div className="max-w-5xl mx-auto relative z-10 space-y-10">
                    ...
                    <div className="p-10 bg-slate-900/60 border border-purple-500/20 rounded-[3rem] space-y-8 animate-in fade-in duration-300 relative overflow-hidden">
                        ...
                        <form onSubmit={handleCreate} className="space-y-5 relative z-10">
                            ...
                            <input type="text" placeholder="Subject" value={subject} onChange={e => setSubject(e.target.value)} required
                                className="w-full bg-slate-950/60 border border-white/5 p-5 rounded-2xl text-white outline-none focus:border-purple-500 transition-all placeholder:text-app-foreground font-medium" />
                            <textarea placeholder="Describe your issue..." value={description} onChange={e => setDescription(e.target.value)} required rows={4}
                                className="w-full bg-slate-950/60 border border-white/5 p-5 rounded-2xl text-white outline-none focus:border-purple-500 transition-all placeholder:text-app-foreground resize-none font-medium" />
                ...
                {loading ? (
                    ...
                    <div className="py-24 text-center space-y-8 bg-slate-900/20 border border-white/5 rounded-[3.5rem]">
                        <div className="w-24 h-24 bg-white/5 rounded-[2rem] flex items-center justify-center mx-auto text-app-muted-foreground"><MessageSquare size={48} /></div>
                        <h2 className="text-2xl font-black text-white italic">No Active Threads</h2>
                        <p className="text-app-muted-foreground text-sm">Create a support thread above if you need assistance.</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {tickets.map(ticket => {
                                ...
                                <div key={ticket.id} className="p-8 bg-slate-900/40 border border-white/5 rounded-[2.5rem] hover:border-purple-500/20 transition-all group">
                                    <div className="flex items-start justify-between gap-6">
                                        <div className="flex-1 min-w-0 space-y-3">
                                            ...
                                            <p className="text-app-muted-foreground text-sm line-clamp-2">{ticket.description}</p>
                                            <div className="flex items-center gap-6 text-[10px] text-app-muted-foreground font-black uppercase tracking-widest">
