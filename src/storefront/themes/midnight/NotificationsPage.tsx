...
const TYPE_MAP: Record<string, { icon: any; color: string; bg: string }> = {
    ...
    INFO: { icon: Info, color: 'text-app-text-faint', bg: 'bg-slate-500/10' },
}
            ...
            <div className="max-w-4xl mx-auto relative z-10 space-y-10">
                <div className="space-y-4">
                    ...
                    <div className="flex items-start justify-between flex-wrap gap-6">
                        <div>
                            ...
                            {unreadCount > 0 && <p className="text-app-text-faint text-xs font-black uppercase tracking-widest mt-2">{unreadCount} unread signals</p>}
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="flex bg-slate-900/60 border border-white/5 rounded-2xl overflow-hidden">
                                <button onClick={() => setFilter('all')} className={`px-5 py-3 text-[10px] font-black uppercase tracking-widest transition-all ${filter === 'all' ? 'bg-white/10 text-white' : 'text-app-text-muted hover:text-white'}`}>All</button>
                                <button onClick={() => setFilter('unread')} className={`px-5 py-3 text-[10px] font-black uppercase tracking-widest transition-all ${filter === 'unread' ? 'bg-white/10 text-white' : 'text-app-text-muted hover:text-white'}`}>Unread</button>
                            </div>
                            {unreadCount > 0 && (
                                <button onClick={markAllRead} className="px-5 py-3 bg-white/5 border border-white/10 text-app-text-faint rounded-2xl text-[10px] font-black uppercase tracking-widest hover:text-white transition-all flex items-center gap-2">
                ...
                {loading ? (
                    ...
                    <div className="py-24 text-center space-y-8 bg-slate-900/20 border border-white/5 rounded-[3.5rem]">
                        <div className="w-24 h-24 bg-white/5 rounded-[2rem] flex items-center justify-center mx-auto text-app-text-muted"><Bell size={48} /></div>
                        <h2 className="text-2xl font-black text-white italic">{filter === 'unread' ? 'All Signals Cleared' : 'Zero Active Signals'}</h2>
                        <p className="text-app-text-faint text-sm">{filter === 'unread' ? 'All signal feeds have been acknowledged' : 'Signal notifications will appear here'}</p>
                    </div>
                ) : (
                            ...
                            return (
                                <div key={notif.id} onClick={() => !notif.is_read && markRead(notif.id)}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-3">
                                            <p className={`font-black italic text-sm ${notif.is_read ? 'text-app-text-faint' : 'text-white'}`}>{notif.title}</p>
                                            {!notif.is_read && (
                                        ...
                                        <p className={`text-sm mt-2 ${notif.is_read ? 'text-app-text-muted' : 'text-app-text-faint'}`}>{notif.message}</p>
                                        <p className="text-app-text-muted text-[10px] mt-3 flex items-center gap-2 font-black uppercase tracking-widest">
