        ...
        return (
            <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-center">
                <div className="space-y-6">
                    ...
                    <p className="text-app-muted-foreground text-sm">The requested order object does not exist in the active ledger.</p>
        ...
        <div className="min-h-screen bg-slate-950 p-6 lg:p-12 relative overflow-hidden">
            ...
            <div className="max-w-5xl mx-auto relative z-10 space-y-10">
                <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-6">
                        <div className="space-y-1">
                            ...
                            <p className="text-app-muted-foreground text-xs font-bold uppercase tracking-[0.2em]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <SummaryCard label="Review Score" value={order.delivery_rating ? `${order.delivery_rating}.0` : 'Pending'} meta="Delivery Rating" icon={<Star size={18} />} color={order.delivery_rating ? 'text-amber-400' : 'text-app-muted-foreground'} />
                </div>
                                ...
                                return (
                                    <div key={step} className="flex flex-col items-center relative z-10" style={{ width: '20%' }}>
                                        ...
                                        <span className={`mt-4 text-[9px] font-black uppercase tracking-widest text-center transition-colors
                                            ${isCurrent ? 'text-emerald-400' : isCompleted ? 'text-app-muted-foreground' : 'text-app-foreground'}`}>
                                            {step}
                                        </span>
                                    </div>
                                )
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    <div className="lg:col-span-2 space-y-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="px-3 py-1 bg-white/5 rounded-full text-[10px] font-black text-app-muted-foreground uppercase tracking-widest">{order.lines?.length || 0} Entities</div>
                        </div>
                        <div className="space-y-4">
                                ...
                                <div key={line.id} className="p-6 bg-slate-900/40 border border-white/5 rounded-[2.5rem] flex items-center gap-6 group hover:border-white/10 transition-all">
                                    <div className="w-20 h-20 bg-slate-950 rounded-[1.5rem] overflow-hidden flex-shrink-0 border border-white/5">
                                        {line.image_url ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center text-app-foreground"><Package size={28} /></div>
                                        )}
                                    </div>
                                    ...
                                    <div className="text-right flex-shrink-0 space-y-1">
                                        ...
                                        <p className="text-app-muted-foreground text-[10px] font-bold uppercase tracking-widest">{line.quantity} Unit @ ${parseFloat(line.unit_price).toFixed(0)}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="p-8 bg-slate-900/60 border border-app-success/10 rounded-[3rem] space-y-8 shadow-2xl relative overflow-hidden">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center">
                                        <span className="text-app-muted-foreground text-xs font-bold uppercase tracking-widest">Net Value</span>
                                        <span className="text-white font-black italic tracking-tight">${parseFloat(order.subtotal || '0').toFixed(2)}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-app-muted-foreground text-xs font-bold uppercase tracking-widest">Tax Provision</span>
                                    ...
                                    <div className="pt-6 border-t border-white/5 flex justify-between items-end">
                                        <span className="text-app-muted-foreground text-[10px] font-black uppercase tracking-[0.3em]">Total Ledger</span>
                            ...
                            {order.notes && (
                                <div className="p-4 bg-white/5 rounded-2xl border border-white/5 space-y-2">
                                    <div className="flex items-center gap-2 text-app-muted-foreground text-[9px] font-black uppercase tracking-widest">
                                        <FileText size={10} /> Internal Memo
                                    </div>
                                    <p className="text-app-muted-foreground text-[10px] font-medium leading-relaxed italic line-clamp-3">{order.notes}</p>
                                </div>
                            )}
                        </div>
    ...
    return (
        <div className="p-8 bg-slate-900/40 border border-white/5 rounded-[2.5rem] space-y-4 hover:bg-slate-900/60 transition-all group">
            ...
            <div>
                ...
                <p className="text-[10px] text-app-muted-foreground uppercase font-bold tracking-widest mt-1">{meta}</p>
            </div>
        </div>
    )
}
