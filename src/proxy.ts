...
export default async function middleware(req: NextRequest) {
    ...
    // ─── CANONICAL REDIRECT (Security & Session Stability) ────────
    // Redirect requests hitting the platform via raw IP or wrong hostname
    // to the preferred ROOT_DOMAIN for SSL and session stability.
    if (isLocalhost && !isRootDomain && rootDomain !== "localhost") {
    ...
    if (url.searchParams.get('clear_auth') === '1' && (url.pathname === '/login' || url.pathname === '/saas/login')) {
        ...
        cleanUrl.hostname = hostname;
        cleanUrl.port = "";
        const response = NextResponse.redirect(cleanUrl);
        response.cookies.delete('auth_token');
        response.cookies.delete('scope_access');
        return response;
    }
    ...
    if (isOnRootOrSaaS) {
        ...
        if (isAppRoute || url.pathname.startsWith('/landing')) {
            return NextResponse.next();
        }

        // Avoid infinite rewrite loop if we are already at landing internally
