    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // ⚡ FOUC Prevention: Only set theme from backend if localStorage didn't
          // already provide one. The inline script in layout.tsx already applied
          // the correct CSS vars from localStorage — overriding here causes flash.
          if (!initialState.theme) {
            ...
            if (theme) {
              console.log('🎨 [Theme] No localStorage theme — setting from backend:', theme.name)
            ...
            console.log('🎨 [Theme] localStorage theme preserved (no backend override):', initialState.theme.name)
          }
          return // Success!
        } else {
      ...
      // ⚡ Same FOUC guard as client-side path
      if (!initialState.theme) {
        ...
        if (theme) {
          console.log('🎨 [Theme] No localStorage — setting from server action:', theme.name)
          setCurrentTheme(theme)
        }
