...
export function SerialTracker() {
 ...
 <p className={`text-xs font-bold leading-tight ${selectedSerial?.id === s.id ? 'text-app-text-faint' : 'text-app-text-muted'}`}>
 {s.product_name}
 </p>
 </div>
 ...
 <h3 className="text-sm font-black text-app-text-faint uppercase tracking-widest">No Serial Data</h3>
 <p className="text-xs text-app-text-faint mt-2 font-medium">Use the search bar to locate specific units.</p>
 </div>
 )}
 </div>
 ...
 <div className="space-y-6">
 {selectedSerial ? (
 <div className="bg-app-surface p-8 rounded-[2.5rem] border border-app-border shadow-sm sticky top-24">
 <div className="flex items-center gap-3 mb-8">
 <div className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center">
 <History size={20} />
 </div>
 <div>
 <h3 className="text-sm font-black text-app-text uppercase tracking-tighter">Unit Timeline</h3>
 <p className="text-[10px] text-app-text-faint font-bold uppercase tracking-widest">History Log</p>
 </div>
 </div>

 <div className="space-y-8 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-app-bg">
 {historyLoading ? (
 ...
 <p className="text-[11px] font-black text-app-text uppercase tracking-tight">{log.action}</p>
 <span className="text-[9px] font-bold text-app-text-faint">
 {format(new Date(log.created_at), 'dd/MM HH:mm')}
 </span>
 </div>
 <p className="text-[10px] text-app-text-muted font-medium bg-app-bg p-2 rounded-lg border border-app-border flex items-center justify-between">
 <span>Ref: {log.reference || 'N/A'}</span>
 <span className="text-[8px] opacity-50">{log.warehouse_name || ''}</span>
 </p>
 <div className="flex items-center gap-1.5 text-[9px] text-app-text-faint font-bold uppercase tracking-wider pl-1">
