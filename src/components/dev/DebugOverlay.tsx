 ...
 */
 ...
 <Settings size={14} className="text-app-text-faint" />
 <span className="text-[10px] font-black text-app-text-faint uppercase tracking-wider">Active Linkages</span>
 </div>
 ...
 {/* Footer */}
 <div className="p-4 bg-app-bg border-t border-app-border flex justify-between items-center shrink-0 text-[10px] font-bold text-app-text-faint font-mono">
...
function InspectorTab({ readDetails, writeDetails }: { readDetails: string[], writeDetails: string[] }) {
 ...
 <div key={idx} className="flex items-center gap-2 text-[10px] font-bold text-app-text-muted bg-app-surface p-2 rounded-lg border border-app-border italic">
 <span>{detail}</span>
 </div>
 ))}
 </div>
...
function LedgerTab({ recentLedger }: { recentLedger: Record<string, any>[] }) {
 ...
 <span className="text-[9px] font-bold text-app-text-faint uppercase">Live from DB</span>
 </div>
 <div className="space-y-4">
 {recentLedger.length === 0 ? (
 <div className="p-8 text-center text-app-text-faint text-xs font-medium">
 No recent transactions found.
 </div>
 ) : (
 recentLedger.map((entry) => (
 <div key={entry.id} className="bg-app-bg rounded-2xl border border-app-border overflow-hidden shadow-sm hover:border-indigo-200 transition-colors">
 <div className="p-3 bg-app-surface border-b border-app-border flex justify-between items-center">
 ...
 <span className="text-app-text-faint w-8">{line.account?.code}</span>
