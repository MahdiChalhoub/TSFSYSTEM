    ...
    return (
        <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 flex-1">
                    <div className="relative flex-1 max-w-sm">
                        ...
                        <Input
                            ...
                            className="pl-10 h-11 rounded-2xl border-app-border focus:ring-app-primary/10"
                        />
                    </div>
                        ...
                        <PopoverTrigger asChild>
                            <Button variant="outline" className="h-11 rounded-2xl gap-2 border-app-border">
                                ...
                                {Object.keys(filters).length > 0 && (
                                    <Badge className="ml-1 px-1.5 h-5 min-w-[20px] bg-app-primary">{Object.keys(filters).length}</Badge>
                                )}
                            </Button>
                        </PopoverTrigger>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" className="h-11 rounded-2xl gap-2 border-app-border">
                                ...
                                Views
                                {currentViewId && <Badge variant="secondary" className="bg-app-surface-2 text-app-text-muted scale-90">Active</Badge>}
                            </Button>
                        </DropdownMenuTrigger>
                            ...
                            {savedViews.map(view => (
                                <DropdownMenuItem key={view.id} className="flex items-center justify-between group rounded-xl">
                                    ...
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <Button
                                            variant="ghost"
                                            className={`h-6 w-6 p-0 ${view.is_default ? 'text-app-warning' : 'text-gray-300'}`}
                                        ...
                                        <Button
                                            variant="ghost"
                                            className="h-6 w-6 p-0 text-app-error hover:text-app-error hover:bg-app-error-bg"
            ...
            <div className="bg-app-surface rounded-[2rem] border border-app-border shadow-sm overflow-hidden">
                <Table>
                    <TableHeader className="bg-app-surface-2/50">
                        <TableRow className="hover:bg-transparent border-app-border">
                            {visibleColumns.map(col => {
                                ...
                                return (
                                    <TableHead
                                        key={col}
                                        className="h-14 text-[10px] font-black uppercase tracking-widest text-app-text-faint cursor-pointer hover:text-app-text transition-colors"
                        ...
                        {loading && data.length === 0 ? (
                            ...
                            data.map((row, idx) => (
                                <TableRow
                                    ...
                                    className={`group cursor-pointer border-gray-50 ${onRowClick ? 'hover:bg-app-surface-2/80 transition-all' : ''}`}
                                >
                                    {visibleColumns.map(col => {
                                        const field = meta.fields.find(f => f.name === col);
                                        return (
                                            <TableCell key={col} className="text-xs font-medium text-app-text-muted py-4">
