...
export function TypicalListView<T, D = any>({
 ...
 />
 <span className={`text-sm ${c.alwaysVisible ? 'text-app-text-faint' : 'text-app-text-muted'}`}>{c.label}</span>
 </label>
 ))}
 </div>
 </PopoverContent>
 ...
 <Table>
 <TableHeader className="bg-app-surface-2">
 ...
 <TableCell colSpan={activeColumns.length + 5} className="h-12 bg-app-surface-2" />
 </TableRow>
 ))
 ...
 <TableCell className="px-4 text-center">
 {lifecycle.getVerified?.(row) ? <CheckCircle2 className="h-4 w-4 text-app-primary mx-auto" /> : <div className="h-4 w-4 rounded-full border border-app-border mx-auto" />}
 </TableCell>
 <TableCell className="px-4 text-center">
 <button
 ...
 <Button variant="ghost" size="sm" className="h-8 w-8 p-0 hover:bg-app-primary-light hover:text-app-primary">
 <Check className="h-4 w-4" />
 </Button>
 </DropdownMenuTrigger>
 <DropdownMenuContent align="end">
 {lifecycle.onApprove && !lifecycle.getApproved?.(row) && !lifecycle.getCanceled?.(row) && (
 <DropdownMenuItem onClick={() => lifecycle.onApprove?.(row)} className="text-app-primary font-semibold focus:text-app-primary focus:bg-app-primary-light">
 ...
 {actions.onView && (
 <button onClick={() => actions.onView!(row)} className="p-1.5 text-app-text-faint hover:text-app-primary hover:bg-app-primary-light rounded-lg transition-all">
 <Eye size={16} />
 </button>
 )}
 {actions.extra && actions.extra(row)}
 {actions.onEdit && (
 <button onClick={() => actions.onEdit!(row)} className="p-1.5 text-app-text-faint hover:text-app-warning hover:bg-app-warning-bg rounded-lg transition-all">
 <Pencil size={16} />
 </button>
 )}
 <DropdownMenu>
 ...
 <TableRow className="bg-app-surface-2 backdrop-blur-md border-l-4 border-app-primary hover:bg-app-surface-2 transition-colors">
 ...
 <TableHead key={ec.key} className={`text-[10px] font-bold text-app-primary uppercase tracking-widest px-4 py-3 ${ec.align === 'right' ? 'text-right' : 'text-left'}`}>
 {ec.label}
 </TableHead>
 ))}
 ...
 <TableRow>
 <TableCell colSpan={expandable.columns.length + 1} className="py-4 text-center text-xs text-app-primary font-medium">
 ...
 {' '}- <span className="text-app-text-muted font-bold">{Math.min(sortedData.length, currentPage * pageSize)}</span>
 {' '}of <span className="text-app-text-muted font-bold">{sortedData.length}</span> results
 </p>
 ...
 ? 'bg-app-surface shadow text-app-primary'
 : 'text-app-text-faint hover:text-app-primary'
 }`}
 >
 {pageNum}
 </button>
