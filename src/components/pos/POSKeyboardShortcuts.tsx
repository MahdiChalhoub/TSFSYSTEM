...
export default function POSKeyboardShortcuts({
 ...
 <div className="bg-slate-900 border border-white/10 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
 <div className="flex items-center gap-3 px-5 py-4 border-b border-white/5">
 <Keyboard size={18} className="text-indigo-400" />
 <h2 className="text-white font-black text-base flex-1">Keyboard Shortcuts</h2>
 <button onClick={() => setShowHelp(false)} className="w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 text-white/40 hover:text-white flex items-center justify-center">
 <X size={14} />
 </button>
 </div>
 ...
 {groups.map(group => (
 <div key={group}>
 <div className="text-white/25 text-[10px] uppercase tracking-widest font-black mb-2">{group}</div>
 ...
 {s.keys.map(k => (
 <kbd key={k} className="px-2 py-0.5 rounded bg-white/10 text-white/80 text-xs font-mono font-bold border border-white/10">{k}</kbd>
 ))}
 </div>
 </div>
 ))}
 ...
 <p className="text-white/20 text-xs text-center border-t border-white/5 pt-3">Press <kbd className="px-1 bg-white/10 rounded text-white/40 text-[10px]">?</kbd> to toggle this overlay</p>
 </div>
 </div>
 </div>
 );
