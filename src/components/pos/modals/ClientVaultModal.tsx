...
export function ClientVaultModal({ isOpen, onClose, clientName, currency, fidelity, loading }: ClientVaultModalProps) {
 ...
 <h2 className="text-3xl font-black text-app-text tracking-tight">{clientName}</h2>
 ...
 <div className="flex items-center gap-4 text-xs font-bold text-app-text/40">
 ...
 <button
 onClick={onClose}
 className="w-14 h-14 rounded-2xl bg-app-text/5 hover:bg-app-text/10 text-app-text/20 hover:text-app-text flex items-center justify-center border border-app-text/5 transition-all active:scale-95"
 >
 <X size={24} />
 </button>
 </div>
 ...
 {/* 🏗️ Navigation Tabs */}
 <div className="px-8 border-b border-app-text/5 flex gap-8">
 {[
 ...
 : "text-app-text/30 border-transparent hover:text-app-text/60"
 )}
 >
 <tab.icon size={14} />
 {tab.label}
 ...
 <p className="mt-6 text-xs font-black text-app-text/30 uppercase tracking-[0.3em] animate-pulse">Scanning Vault Data...</p>
 </div>
 ) : (
 <>
 {activeTab === 'overview' && (
 ...
 <div className={clsx("p-2.5 rounded-xl bg-app-text/5", stat.color)}>
 <stat.icon size={18} />
 </div>
 <ArrowUpRight size={14} className="text-app-text/10 group-hover:text-app-text/30 transition-all" />
 </div>
 <div className="text-[10px] font-black text-app-text/30 uppercase tracking-widest mb-1">{stat.label}</div>
 ...
 {/* Tier Progress Card */}
 <div className="lg:col-span-2 bg-gradient-to-br from-indigo-500/10 to-transparent rounded-[2rem] p-8 border border-app-text/10 relative overflow-hidden group">
 ...
 <div>
 <h3 className="text-xl font-black text-app-text mb-1">Tier Progression</h3>
 <p className="text-xs font-bold text-app-text/40">Keep spending to unlock premium benefits</p>
 </div>
 <div className="text-right">
 <span className="text-3xl font-black text-app-text">{Math.round(tierInfo.progress)}%</span>
 <div className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Progress to {tierInfo.next || 'MAX'}</div>
 </div>
 </div>

 <div className="relative h-4 bg-app-text/5 rounded-full overflow-hidden mb-8">
 <div
 ...
 <div className="text-sm font-black text-app-text">{item.value}</div>
 </div>
 ))}
 </div>
 </div>
 ...
 <h4 className="text-lg font-black text-app-text mb-2">Fidelity Score</h4>
 <p className="text-xs font-bold text-app-text/30 mb-8 max-w-[200px]">Base evaluation of customer retention and brand loyalty</p>
 ...
 <span className="text-app-text/20">{row.label}</span>
 ...
 <div className="text-2xl font-black text-app-text mb-2">Available for Redemption</div>
 <div className="text-sm font-bold text-app-text/30">Equiv. to <span className="text-teal-400">{currency}{pointsValue.toLocaleString()}</span> in direct discount</div>
 </div>
 ...
 <div key={i} className="flex items-start gap-4 p-5 bg-app-text/5 rounded-2xl border border-app-text/5">
 <div className="mt-1 p-2 rounded-lg bg-app-text/5 text-teal-400">
 <rule.icon size={16} />
 </div>
 <div>
 <div className="text-xs font-black text-app-text mb-1 uppercase tracking-widest">{rule.label}</div>
 <p className="text-[11px] font-medium text-app-text/40 leading-relaxed">{rule.desc}</p>
 </div>
 </div>
 ))}
 </div>
 ...
 <h3 className="text-sm font-black text-app-text uppercase tracking-widest">Favorite Products</h3>
 <span className="text-[10px] font-bold text-app-text/20">Based on quantity purchased</span>
 </div>
 ...
 <div>
 <div className="text-sm font-black text-app-text group-hover:text-indigo-400 transition-colors">{product.name}</div>
 <div className="text-[10px] font-bold text-app-text/20">{product.sku}</div>
 </div>
 </div>
 <div className="text-right">
 ...
 <div className="space-y-4">
 <div className="flex items-center justify-between">
 <h3 className="text-sm font-black text-app-text uppercase tracking-widest">Transaction Log</h3>
 ...
 <div>
 <div className="text-sm font-black text-app-text">{order.ref_code || `#${order.id}`}</div>
 <div className="text-[10px] font-bold text-app-text/20">{new Date(order.created_at).toLocaleDateString()}</div>
 </div>
 </div>
 <div className="text-right">
 ...
 <div className="px-8 py-6 bg-black/20 border-t border-app-text/5 flex items-center justify-between">
 ...
 <button className="px-6 py-2.5 rounded-xl bg-app-text/5 hover:bg-app-text/10 text-app-text/70 text-xs font-black uppercase tracking-widest border border-app-text/5 transition-all">
 Export Record
 </button>
 <button
 className="group relative px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-app-text text-xs font-black uppercase tracking-widest transition-all shadow-lg hover:shadow-indigo-500/20"
 >
