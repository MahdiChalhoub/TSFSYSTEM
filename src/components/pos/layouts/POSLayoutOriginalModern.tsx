        ...
        <div className={clsx(
            ...
            <header className="h-14 bg-white border-b border-gray-100 flex items-center justify-between px-5 shrink-0 z-50 shadow-sm">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={onOpenLayoutSelector} className="w-8 h-8 flex items-center justify-center bg-white border border-gray-100 rounded-lg text-gray-400 hover:text-violet-600 hover:border-violet-100 hover:bg-violet-50 transition-all" title="Switch Layout">
                        <Layout size={15} />
                    </button>
                </div>
            </header>
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            className="bg-transparent text-xs font-bold text-gray-800 appearance-none outline-none cursor-pointer pr-4"
                        >
                            {clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                        </select>
                    </div>
                    <div className="h-4 w-px bg-violet-200/50" />
                    <span className="text-[9px] font-bold text-gray-500">{selectedClient.phone}</span>
                    {selectedClient.address !== 'Counter Sales' && (
                        <>
                            <div className="h-4 w-px bg-violet-200/50" />
                            <span className="text-[9px] font-bold text-gray-500 truncate max-w-[200px]">{selectedClient.address}</span>
                        </>
                    )}
                </div>
                <div className="flex items-center gap-3 text-[9px] font-black text-gray-500">
                ...
                <div className="flex gap-1.5 overflow-x-auto no-scrollbar shrink-0">
                    <button
                        ...
                        className={clsx(
                            ...
                            activeCategoryId === null ? 'bg-violet-600 text-white border-violet-600' : 'bg-white text-gray-500 border-gray-200 hover:border-violet-200'
                        )}
                    >
                        All
                    </button>
                    {categories.map(cat => (
                        <button
                            ...
                            className={clsx(
                                "px-3 py-1.5 whitespace-nowrap rounded-lg text-[9px] font-black uppercase tracking-widest transition-all border",
                                activeCategoryId === cat.id ? 'bg-violet-600 text-white border-violet-600' : 'bg-white text-gray-500 border-gray-200 hover:border-violet-200'
                            )}
                        >
                            {cat.name}
                        </button>
