...
import {
    ...
    History, GripHorizontal, MessageSquare, ShieldCheck, Star, Coins, Banknote, CreditCard, Smartphone, Landmark, AlertCircle, BookOpen
} from 'lucide-react';
...
export function POSLayoutClassic(props: POSLayoutProps) {
    ...
    return (
        <div className={clsx(
            "flex flex-col bg-app-bg overflow-hidden select-none h-full font-sans transition-colors duration-700",
                ...
                <div className="flex gap-4 items-center">
                    <div className="relative flex-1 group">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-3.5 bg-app-surface border border-app-border rounded-[1.2rem] text-[13px] outline-none focus:border-emerald-500/50 focus:ring-8 focus:ring-emerald-500/5 transition-all font-black text-app-text placeholder:text-app-text-faint uppercase tracking-widest"
                            value={searchQuery}
                            onChange={(e) => onSetSearchQuery(e.target.value)}
                        />
                        {searchQuery && (
                            <button onClick={() => onSetSearchQuery('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-app-text-muted hover:text-app-text transition-colors">
                                <X size={16} />
                            </button>
                        )}
                    </div>
                ...
                <aside className="w-[420px] bg-app-surface border-l border-app-border flex flex-col shrink-0 shadow-[-20px_0_50px_rgba(0,0,0,0.5)] z-20">
                    ...
                    <div className="px-6 py-5 border-b border-app-border flex items-center justify-between bg-app-text/90 backdrop-blur-2xl sticky top-0 z-[30]">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-black text-app-text-faint bg-app-surface px-3 py-1.5 rounded-xl border border-app-border italic">{totalPieces} units</span>
                            <button
                                onClick={() => setIsAccountBookOpen(true)}
                                className="w-10 h-10 flex items-center justify-center rounded-xl bg-app-surface text-app-text-muted hover:text-amber-500 hover:bg-amber-500/10 transition-all border border-app-border"
                                title="Account Book"
                            >
                                <BookOpen size={16} />
                            </button>
                            {cart.length > 0 && (
                                <button onClick={() => onClearCart(false)} className="w-10 h-10 flex items-center justify-center rounded-xl bg-app-surface text-app-text-muted hover:text-rose-500 hover:bg-rose-500/10 transition-all border border-app-border">
                                    <Trash2 size={16} />
                                </button>
                            )}
                        </div>
                    ...
                    <div className="p-6 border-b border-app-border bg-app-bg/80 backdrop-blur-md">
                        <div className="flex gap-3 mb-4">
                            <div className="relative flex-1 group">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-11 pr-4 py-3 bg-app-surface border border-app-border rounded-2xl text-[12px] font-black text-app-text outline-none focus:border-emerald-500/50 transition-all placeholder:text-app-text-faint uppercase tracking-widest italic"
                                ...
                                {clientSearchQuery && (
                                    <div className="absolute top-full left-0 right-0 mt-2 bg-app-surface border border-app-border rounded-2xl shadow-2xl z-[100] max-h-64 overflow-y-auto custom-scrollbar backdrop-blur-3xl ring-1 ring-gray-200">
                                        {filteredClients.map((c: any) => (
                                            <button
                                                ...
                                                className="w-full text-left px-5 py-3 hover:bg-emerald-500/10 border-b border-app-border flex items-center justify-between group transition-colors"
                                            >
                                                <div>
                                                    <p className="text-[11px] font-black text-app-text group-hover:text-emerald-400 uppercase tracking-widest transition-colors italic">{c.name}</p>
                                                    <p className="text-[9px] text-app-text-muted font-mono mt-0.5">{c.phone || 'NO_COMMS_LINK'}</p>
                                                </div>
                            ...
                            <button
                                ...
                                className="w-12 h-12 flex items-center justify-center rounded-2xl bg-app-surface border border-app-border text-app-text-muted hover:border-emerald-500/50 hover:text-emerald-400 transition-all hover:bg-app-surface shadow-inner"
                        ...
                        <div className="relative group/zone">
                            <span className="absolute left-4 -top-2 px-2 bg-app-surface text-[8px] font-black text-app-text-muted uppercase tracking-[0.3em] z-10">Logistics Zone</span>
                            <div className="relative">
                                <MapPin size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-app-text-muted group-focus-within/zone:text-emerald-400 transition-colors" />
                                <select
                                    value={deliveryZone || ''}
                                    onChange={(e) => onSetDeliveryZone(e.target.value)}
                                    className="w-full pl-11 pr-10 py-3 bg-app-surface border border-app-border rounded-2xl text-[11px] font-black text-app-text outline-none appearance-none cursor-pointer focus:border-emerald-500/50 transition-all uppercase tracking-widest italic"
                                >
                                    {deliveryZones.map(z => (
                                        <option key={z.id} value={z.id} className="bg-app-surface text-app-text font-black">{z.name}</option>
                                    ))}
                                </select>
                                <ChevronDown size={14} className="absolute right-4 top-1/2 -translate-y-1/2 text-app-text-muted pointer-events-none" />
                            </div>
                        </div>
                    </div>
                    {/* Cart Items Area */}
                    <div className="flex-1 overflow-y-auto custom-scrollbar bg-app-text/20 backdrop-blur-sm">
                        {cart.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-full text-app-text gap-6 animate-in fade-in zoom-in duration-700">
                                <div className="w-32 h-32 rounded-full bg-app-surface flex items-center justify-center border-2 border-dashed border-app-border relative group">
                                    <ShoppingCart size={48} strokeWidth={1} className="text-app-text group-hover:text-emerald-500/50 transition-colors" />
                                ...
                                <div className="text-center space-y-2">
                                    <p className="text-[11px] font-black uppercase tracking-[0.3em] text-app-text-muted">Cart Empty</p>
                                    <p className="text-[10px] font-bold text-app-text uppercase tracking-widest">Add items to get started</p>
                                </div>
                            </div>
                        ) : (
                                                ...
                                                highlightedItemId === item.productId
                                                    ...
                                                    : isOverselling
                                                        ...
                                                        : lastAddedItemId === item.productId
                                                            ...
                                                            : "bg-app-bg/80 border-app-border text-app-text hover:bg-app-text/60 hover:border-app-border"
                                            )}
                                        >
                                            <div className="flex items-center gap-4 relative z-10">
                                                ...
                                                <div className="flex items-center gap-3 bg-app-text/80 p-1.5 rounded-2xl border border-app-border">
                                                    ...
                                                    <button onClick={(e) => { e.stopPropagation(); onUpdateQuantity(item.productId, 1); }} className="w-8 h-8 rounded-xl bg-app-surface hover:bg-emerald-500/20 hover:text-emerald-400 flex items-center justify-center text-app-text-muted transition-all active:scale-90 border border-app-border">
                                                ...
                                                <div className="text-right min-w-[100px]">
                                                    <p
                                                        ...
                                                        className="text-[15px] font-black text-app-text cursor-pointer hover:text-emerald-400 transition-colors italic tabular-nums"
                                                    >
                                                        {currency}{formatNumber(Number(item.price) * item.quantity)}
                                                    </p>
                                                    <p className="text-[9px] font-black text-app-text-muted uppercase tracking-tighter mt-0.5">
                                            ...
                                            {openNoteId === item.productId && (
                                                <div className="mt-4 pt-4 border-t border-app-border animate-in slide-in-from-top-2 duration-300">
                                                    <textarea
                                                        autoFocus
                                                        defaultValue={item.note || ''}
                                                        placeholder="DATA LOG ENTRY..."
                                                        rows={2}
                                                        className="w-full bg-app-surface border border-app-border rounded-xl px-4 py-3 text-[11px] font-black text-app-text outline-none focus:border-amber-500/50 resize-none placeholder:text-app-text-faint uppercase tracking-widest italic"
                                                    ...
                                                    <div className="flex justify-between items-center mt-2 px-1">
                                                        <p className="text-[8px] font-black text-app-text uppercase tracking-widest italic">ENTER: COMMIT // ESC: ABORT</p>
                    ...
                    <div className="p-6 border-t border-app-border bg-app-surface shrink-0 space-y-5">
                        <div className="flex items-center justify-between">
                            <div className="flex flex-col">
                                <span className="text-[10px] font-black text-app-text-muted uppercase tracking-[0.3em] mb-1 italic">Order Summary</span>
                                <div className="flex items-center gap-3">
                                    <span className="px-3 py-1 bg-app-surface border border-app-border rounded-lg text-[10px] font-black text-app-text-faint italic uppercase">
                                        {totalPieces} Units
                                    </span>
                                    {discount > 0 && (
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="relative group/input">
                                <span className="absolute left-4 -top-2 px-2 bg-app-surface text-[8px] font-black text-app-text-muted uppercase tracking-[0.3em] z-10 transition-colors group-focus-within/input:text-emerald-400">Captured Fund</span>
                                <input
                                    type="text"
                                    ...
                                    className="w-full px-4 py-3 bg-app-surface border border-app-border rounded-2xl text-[14px] font-black text-app-text outline-none focus:border-emerald-500/50 transition-all font-mono italic text-right placeholder:text-app-text-faint"
                                />
                            </div>
                            <div className="relative group/select">
                                <span className="absolute left-4 -top-2 px-2 bg-app-surface text-[8px] font-black text-app-text-muted uppercase tracking-[0.3em] z-10 transition-colors group-focus-within/select:text-emerald-400">Payment Vector</span>
                                <div className="relative flex items-center">
                                    <select
                                        value={paymentMethod}
                                        onChange={(e) => onSetPaymentMethod(e.target.value)}
                                        className="w-full pl-4 pr-10 py-3 bg-app-surface border border-app-border rounded-2xl text-[11px] font-black text-app-text outline-none appearance-none cursor-pointer focus:border-emerald-500/50 transition-all uppercase tracking-widest italic"
                                    >
                                        {paymentMethods
                                            .filter((m: any) => {
                                            ...
                                            .map((m: any) => {
                                                ...
                                                return <option key={key} value={key} className="bg-app-surface text-app-text">{label}</option>;
                                            })
                                        }
                                    </select>
                                    <ChevronDown size={14} className="absolute right-12 top-1/2 -translate-y-1/2 text-app-text-muted pointer-events-none" />
                                    <button
                                        onClick={() => setIsMultiPayMode(true)}
                                        className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center bg-app-surface border border-app-border text-app-text-faint rounded-xl hover:text-emerald-400 hover:border-emerald-500/50 transition-all"
                                        title="Split Payment"
                                    >
                                        <Calculator size={14} />
                                    </button>
                        ...
                        <button
                            ...
                            className={clsx(
                                ...
                                cart.length > 0 && !isProcessing
                                    ...
                                    : "bg-app-surface grayscale opacity-50 cursor-not-allowed border border-app-border shadow-none"
                            )}
                        >
                            ...
                            <div className="relative z-10 flex items-center justify-between px-10">
                                <div className="flex items-center gap-5">
                                    ...
                                    <div className="flex flex-col items-start">
                                        ...
                                        <span className="text-xl font-black text-app-text uppercase tracking-[0.1em] leading-none italic">
                                ...
                                <div className="text-right flex flex-col items-end">
                                    ...
                                    <span className="text-2xl font-black text-app-text tabular-nums tracking-tighter leading-none italic shadow-emerald-900/10 shadow-sm">
            ...
            {showNumpad && (
                ...
                >
                    <div
                        ...
                        className="flex items-center justify-between px-4 mb-3 cursor-grab active:cursor-grabbing hover:bg-app-bg rounded-2xl p-3 transition-colors group/handle border border-transparent hover:border-app-border"
                    >
                        <div className="flex items-center gap-3">
