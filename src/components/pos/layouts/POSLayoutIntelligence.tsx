...
export function POSLayoutIntelligence(props: POSLayoutProps) {
 ...
 return (
 <div className={clsx(
 "flex flex-col overflow-hidden select-none h-full font-sans transition-colors duration-700 bg-app-bg text-app-text",
 ...
 {isMultiPayMode && (
 <div className="absolute top-0 right-0 w-[42%] h-full bg-app-text/80 backdrop-blur-[8px] z-[100] flex flex-col items-center justify-center p-8 text-center pointer-events-auto">
 <div className="bg-app-text/95 p-8 rounded-[2rem] shadow-[0_0_100px_rgba(0,0,0,0.5)] border border-app-border flex flex-col items-center gap-6 animate-in zoom-in-95 duration-500 backdrop-blur-2xl">
 ...
 <h3 className="text-xl font-black text-app-text uppercase tracking-widest">Cart Locked</h3>
 <p className="text-[11px] text-app-text-faint max-w-[240px] uppercase font-bold tracking-tighter leading-relaxed">Payment in progress. Return to cart to modify items.</p>
 </div>
 <button
 ...
 <aside className="w-[58%] flex flex-col bg-app-surface border-r border-app-border shrink-0 overflow-hidden relative shadow-[40px_0_80px_rgba(0,0,0,0.4)] z-20">
 {/* Ambient Glow */}
 ...
 <div className="px-10 py-6 border-b border-app-border bg-app-text/80 backdrop-blur-2xl sticky top-0 z-[20] space-y-6 shrink-0">
 <div className="flex items-center gap-4">
 <div className="relative flex-1 group">
 <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-app-text-muted group-focus-within:text-emerald-500 transition-colors" size={18} />
 <input
 id="pos-product-search"
 type="text"
 placeholder="Search by barcode or name..."
 className="w-full pl-14 pr-14 py-4.5 bg-app-surface border border-app-border rounded-2xl text-[14px] outline-none focus:bg-app-bg focus:border-emerald-500/50 focus:ring-8 focus:ring-emerald-500/5 transition-all font-black text-app-text placeholder:text-app-text-faint placeholder:uppercase placeholder:tracking-[0.2em]"
 value={searchQuery}
 ...
 className="absolute right-4 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center bg-app-surface-2 text-app-text-faint hover:bg-rose-500 hover:text-app-text rounded-xl transition-all"
 >
 <X size={16} />
 </button>
 )}
 ...
 showNumpad
 ? "bg-amber-gradient border-amber-400 text-app-text shadow-amber-500/20 rotate-6"
 : "bg-app-surface border-app-border text-app-text-faint hover:border-amber-500/30 hover:text-amber-500"
 )}
 >
 ...
 >All Products</button>
 <div className="w-px h-6 bg-app-text/5 shrink-0" />
 ...
 ? 'bg-emerald-gradient border-emerald-400 text-app-text shadow-2xl shadow-emerald-500/20 scale-105'
 : 'bg-app-bg border-app-border text-app-text-faint hover:border-emerald-500/30 hover:text-emerald-400'
 )}
 >{cat.name}</button>
 ))}
 ...
 className="h-11 px-6 bg-app-surface border-2 border-emerald-500/30 text-emerald-400 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all shrink-0 flex items-center gap-3 shadow-2xl"
 >
 <ArrowLeft size={16} className="text-emerald-500 stroke-[3]" />
 {currentParentName}
 </button>
 <div className="w-px h-6 bg-app-text/10 mx-2 shrink-0" />
 ...
 <div className="px-8 py-3 bg-app-bg border-b border-app-border flex items-center justify-between shrink-0">
 <span className="text-[10px] font-black text-app-text-muted flex items-center gap-3 uppercase tracking-[0.3em]">
 <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
 Active Stream: <span className="text-app-text-faint italic font-black">{leftExpanded ? 'Grid View' : 'List View'}</span>
 </span>
 <button
 onClick={() => setLeftExpanded(!leftExpanded)}
 className="text-[10px] font-black text-emerald-400 hover:text-app-text bg-emerald-500/5 hover:bg-emerald-500 border border-emerald-500/20 px-4 py-1.5 rounded-full flex items-center gap-2 transition-all hover:shadow-[0_0_20px_rgba(16,185,129,0.2)]"
 >
 ...
 className="p-10 rounded-3xl bg-app-surface border border-app-border text-app-text-faint text-center hover:bg-app-surface-2 transition-all flex flex-col items-center justify-center font-black uppercase text-[12px] tracking-[0.2em] gap-4 shadow-2xl group"
 >
 ...
 className="p-8 rounded-3xl bg-emerald-gradient text-app-text text-center group hover:shadow-[0_0_50px_rgba(16,185,129,0.3)] transition-all border-none relative overflow-hidden"
 >
 <div className="absolute top-0 right-0 w-32 h-32 bg-app-text/10 blur-[40px] -rotate-45" />
 ...
 (activeCategoryId === cat.id || currentParentId === cat.id)
 ? 'bg-emerald-gradient text-app-text rotate-6 scale-110 shadow-emerald-500/20'
 : 'bg-app-surface text-app-text-muted group-hover:bg-app-surface-2 group-hover:text-emerald-400 group-hover:rotate-3'
 )}>
 ...
 className={clsx(
 "z-[100] w-[300px] p-3 bg-app-text/95 backdrop-blur-3xl rounded-3xl border border-app-border shadow-2xl animate-in zoom-in-95 ring-1 ring-gray-200",
 !isDragging && "transition-transform duration-200 ease-out"
 )}
 >
 <div
 onMouseDown={startDragging}
 className="flex items-center justify-between px-3 mb-3 cursor-grab active:cursor-grabbing hover:bg-app-bg rounded-2xl p-2 transition-colors group/handle border border-transparent hover:border-app-border"
 >
 ...
 <button onClick={() => setShowNumpad(false)} className="w-8 h-8 flex items-center justify-center rounded-xl bg-app-surface-2 text-app-text-faint hover:bg-rose-500 hover:text-app-text transition-all shadow-inner border border-app-border">
 <X size={14} />
 </button>
 </div>
 <POSNumpad
 ...
 !isLinked
 ? "bg-app-bg border-transparent text-app-text cursor-not-allowed opacity-30 shadow-inner"
 : isActive || (isMultiPayMode && key.includes('MULTI'))
 ? "bg-emerald-gradient border-emerald-400 text-app-text shadow-[0_0_40px_rgba(16,185,129,0.3)] scale-110 z-10"
 : "bg-app-surface border-app-border text-app-text-faint hover:border-emerald-500/50 hover:text-emerald-400 hover:shadow-2xl hover:bg-app-surface-2"
 )}
 >
 ...
 <AlertCircle size={10} className="text-app-text" />
 </div>
 )}
 </button>
 );
 ...
 <div className="mt-auto w-full flex flex-col items-center gap-6 pt-8 border-t border-app-border">
 ...
 ? "bg-blue-600 border-blue-400 text-app-text shadow-[0_0_40px_rgba(59,130,246,0.3)] scale-110"
 : "bg-app-surface border-app-border text-app-text-faint hover:border-blue-500/50 hover:text-blue-400 hover:bg-app-surface-2"
 )}
 >
 ...
 className="group flex flex-col items-center justify-center p-3 rounded-2xl transition-all w-20 h-20 border-2 bg-app-surface border-app-border text-app-text-faint hover:border-indigo-500/50 hover:text-indigo-400 hover:bg-app-surface-2 hover:shadow-2xl"
 >
 ...
 className="group flex flex-col items-center justify-center p-3 rounded-2xl transition-all w-20 h-20 border-2 bg-app-surface border-app-border text-app-text-faint hover:border-emerald-500/50 hover:text-emerald-400 hover:bg-app-surface-2"
 >
 ...
 {/* ═══ MAIN CART AREA ═══ */}
 <main className="flex-1 flex flex-col bg-app-bg overflow-hidden relative shadow-2xl">
 <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-emerald-500/5 blur-[150px] pointer-events-none" />

 <div className="flex flex-col h-full overflow-hidden relative z-10">
 <div className="px-6 py-4 border-b border-app-border bg-app-text/80 backdrop-blur-xl flex items-center justify-between shrink-0">
 <div className="flex items-center gap-3">
 <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
 <h2 className="text-sm font-black text-app-text uppercase tracking-[0.2em]">Active Shipment</h2>
 <span className="text-[10px] text-app-text-faint font-bold uppercase tracking-widest">{uniqueItems} Lines · {totalPieces} Pcs</span>
 </div>
 {cart.length > 0 && (
 <button onClick={() => {
 ...
 {cart.length === 0 ? (
 <div className="flex flex-col items-center justify-center h-full text-app-text gap-6 opacity-40">
 <div className="w-24 h-24 rounded-[2.5rem] bg-app-surface flex items-center justify-center border-2 border-dashed border-app-border">
 ...
 <p className="text-[15px] font-black text-app-text truncate leading-tight group-hover:text-emerald-400 italic transition-colors">{item.name}</p>
 ...
 className="text-[13px] font-black text-app-text-faint shrink-0 hover:text-emerald-400 transition-colors uppercase tracking-tight tabular-nums"
 >
 {currency}{Number(item.price).toFixed(2)}
 </span>
 </div>
 <div className="flex items-center gap-1 shrink-0 bg-app-text/90 p-1 rounded-xl border border-app-border">
 <button
 onClick={(e) => { e.stopPropagation(); handleProtectedQuantity(item.productId, -1); }}
 className="w-8 h-8 rounded-lg bg-app-surface-2 hover:bg-rose-600 hover:text-app-text flex items-center justify-center text-app-text-faint transition-all border border-app-border active:scale-90"
 >
 <Minus size={14} className="stroke-[3]" />
 </button>
 <span
 onClick={(e) => { e.stopPropagation(); setSelectedCartIdx(idx); setNumpadMode('qty'); setShowNumpad(true); }}
 className="w-10 text-center text-[15px] font-black tabular-nums text-app-text hover:text-amber-400 transition-colors"
 >
 {item.quantity}
 </span>
 <button
 onClick={(e) => { e.stopPropagation(); onUpdateQuantity(item.productId, 1); }}
 className="w-8 h-8 rounded-lg bg-app-surface-2 hover:bg-emerald-500 hover:text-app-text text-emerald-500 flex items-center justify-center transition-all border border-app-border active:scale-90"
 >
 ...
 className="ml-3 w-10 h-10 rounded-xl bg-app-surface border border-app-border text-rose-500 hover:bg-rose-600 hover:text-app-text transition-all shrink-0 flex items-center justify-center shadow-inner"
 >
 <Trash2 size={16} />
 </button>
 </div>
 ...
 <div className="flex items-center gap-3 bg-app-surface-2 border border-app-border px-4 py-2 rounded-xl backdrop-blur-md">
 <Edit3 size={12} className="text-app-text-muted" />
 <input
 type="text"
 ...
 <div className="border-t border-app-border bg-app-text/90 backdrop-blur-2xl px-6 py-6 shrink-0 space-y-5 rounded-t-[3rem] shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
 <div className="grid grid-cols-3 gap-4">
 <div className="bg-app-bg rounded-2xl p-4 border border-app-border shadow-inner group">
 <span className="text-[10px] font-black text-app-text-muted uppercase tracking-[0.3em] block mb-2 group-hover:text-app-text-faint transition-colors">Subtotal</span>
 <span className="text-xl font-black text-app-text tabular-nums tracking-tighter italic">{currency}{formatNumber(total)}</span>
 </div>
 <div
 onClick={() => { setNumpadMode('disc'); setShowNumpad(true); }}
 className="bg-app-bg rounded-2xl p-4 border border-app-border text-center cursor-pointer hover:bg-app-surface transition-all group relative overflow-hidden"
 >
 ...
 <span className={clsx("text-2xl font-black tabular-nums tracking-tighter", discount > 0 ? "text-amber-500 italic" : "text-app-text")}>
 {discount > 0 ? `-${formatNumber(discountType === 'percentage' ? total * discount / 100 : discount)}` : '0.00'}
 </span>
 </div>
 <div className="bg-emerald-gradient rounded-3xl p-4 text-center shadow-[0_10px_40px_rgba(16,185,129,0.2)] border border-app-border relative overflow-hidden group">
 <div className="absolute top-0 right-0 w-24 h-24 bg-app-text/10 blur-2xl rounded-full" />
 <span className="text-[10px] font-black text-app-text-muted uppercase tracking-[0.3em] block mb-2 relative z-10">Grand Aggregate</span>
 <span className="text-3xl font-black text-app-text tabular-nums tracking-tighter leading-none relative z-10 italic">{currency}{formatNumber(totalAmount)}</span>
 </div>
 </div>

 {selectedClient && selectedClientId > 1 && (
 <div className="flex items-center gap-3 flex-wrap bg-app-bg p-2 rounded-2xl border border-app-border">
 ...
 className="flex items-center gap-3 px-4 py-2 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[11px] font-black uppercase tracking-widest hover:bg-blue-500 hover:text-app-text transition-all shadow-lg"
 >
 <Wallet size={14} />
 <span>Vault: {currency}{((selectedClient as any).balance || 0).toFixed(2)}</span>
 <span className="bg-blue-500 text-app-text px-2 py-0.5 rounded text-[9px] font-black">EXTRACT</span>
 </button>
 )}
 ...
 <span className="absolute left-4 top-2 text-[8px] font-black text-app-text-muted uppercase tracking-[0.4em] group-focus-within:text-emerald-500 transition-colors">Credits Received</span>
 <input
 type="text"
 ...
 className="w-full pt-7 pb-3 px-4 text-right bg-app-surface border-2 border-app-border rounded-2xl text-2xl font-black outline-none focus:border-emerald-500/50 focus:ring-8 focus:ring-emerald-500/5 transition-all font-mono tabular-nums text-app-text placeholder:text-app-text-faint"
 />
