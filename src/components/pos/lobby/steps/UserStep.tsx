            ...
            {register.authorizedUsers.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {register.authorizedUsers.map(u => (
                        ...
                        >
                            <div className="w-16 h-16 rounded-full bg-[var(--app-info-bg)] text-[var(--app-info)] flex items-center justify-center mx-auto mb-3 font-black text-xl group-hover:bg-[var(--app-info)] group-hover:text-[var(--app-foreground)] transition-all shadow-lg">
                                {u.name?.substring(0, 2).toUpperCase()}
                            </div>
                            <p className="text-sm font-black text-[var(--app-foreground)]">{u.name}</p>
                            {!u.hasPin && (
                ...
                <div className="text-center py-12 border border-[var(--app-border)]/50 rounded-2xl bg-[var(--app-surface-2)]">
                    <AlertCircle size={36} className="text-[var(--app-muted-foreground)] mx-auto mb-3" />
                    <p className="text-[var(--app-muted-foreground)] font-bold">No cashiers assigned</p>
                    <p className="text-[var(--app-muted-foreground)] text-sm mt-1">Assign users in POS Configuration</p>
                </div>
            )}
        </div>
    );
