                ...
                <div className="mb-5 bg-[var(--app-warning-bg)] border border-[var(--app-warning)]/30 rounded-2xl p-5 space-y-4">
                    ...
                    <div className="flex gap-2">
                        <input type="password" inputMode="numeric" value={forceClosePin} onChange={e => setForceClosePin(e.target.value.replace(/\D/g, '').slice(0, 6))}
                            ...
                            className="flex-1 px-4 py-2.5 bg-[var(--app-surface-hover)] border border-[var(--app-warning)]/30 rounded-xl text-sm font-mono font-bold text-[var(--app-foreground)] outline-none focus:ring-2 focus:ring-[var(--app-warning)]/30 tracking-[0.3em] text-center" />
                ...
                <>
                    {openingMode === 'standard' ? (
                        <div className="bg-[var(--app-surface)] rounded-2xl p-6 border border-[var(--app-border)] space-y-4">
                            <div>
                                <label className="text-[10px] text-[var(--app-muted-foreground)] uppercase tracking-widest font-black block mb-1.5">Opening Cash ({currency})</label>
                                <div className="relative">
                                    <Banknote size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--app-muted-foreground)]" />
                                    <input type="number" value={openingBalance} onChange={e => setOpeningBalance(e.target.value)}
                                        className="w-full pl-10 pr-4 py-3.5 bg-[var(--app-surface-hover)] border border-[var(--app-border)] rounded-xl text-[var(--app-foreground)] text-2xl font-black outline-none focus:border-[var(--app-primary-strong)]/50 focus:ring-4 focus:ring-[var(--app-primary)]/10 transition-all" placeholder="0.00" />
                                </div>
                            </div>
                            <div>
                                <label className="text-[10px] text-[var(--app-muted-foreground)] uppercase tracking-widest font-black block mb-1.5">Shift Notes</label>
                                <textarea value={notes} onChange={e => setNotes(e.target.value)}
                                    className="w-full px-4 py-2.5 bg-[var(--app-surface-hover)] border border-[var(--app-border)] rounded-xl text-[var(--app-foreground)] text-sm outline-none focus:border-[var(--app-primary-strong)]/50 resize-none h-16" placeholder="Optional notes..." />
                            </div>
                        ...
                        <div className="bg-[var(--app-surface)] rounded-2xl p-6 border border-violet-500/20 space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                {!managerUnlocked && (
                                    <button onClick={() => setManagerUnlocked(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[var(--app-surface-hover)] border border-[var(--app-border)] text-xs font-bold text-[var(--app-muted-foreground)] hover:text-[var(--app-warning)] hover:border-[var(--app-warning)]/30 transition-all">
                            ...
                            <div className={clsx('grid gap-2 items-center p-1.5 rounded-xl', managerUnlocked ? 'grid-cols-[1fr_90px_90px_70px_80px] bg-[var(--app-success)]/5 border border-app-success/15' : 'grid-cols-[1fr_110px]')}>
                                ...
                                {managerUnlocked && (() => {
                                    ...
                                    return <span className={clsx('text-xs font-black text-center', diff > 0 ? 'text-[var(--app-success)]' : diff < 0 ? 'text-[var(--app-error)]' : 'text-[var(--app-muted-foreground)]')}>{diff > 0 ? '+' : ''}{diff !== 0 ? diff.toFixed(0) : '—'}</span>;
                                })()}
                                {managerUnlocked && (() => {
                                    const diff = (parseFloat(cashReal) || 0) - (parseFloat(cashSoftware) || 0);
                                    return diff !== 0 ? (
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        {accountBookLive !== null
                                            ...
                                            : <span className="ml-1 text-[9px] px-1.5 py-0.5 rounded-full bg-[var(--app-surface-hover)] text-[var(--app-muted-foreground)] font-bold">No session</span>
                                        }
                                    </div>
                                </div>
                                {managerUnlocked && <span className="text-[var(--app-muted-foreground)] text-xs text-center">—</span>}
                            ...
                            {reconEntries.length > 0 && (
                                <div className="flex items-center gap-2 pt-1">
                                    ...
                                    <span className="text-[9px] text-[var(--app-muted-foreground)] uppercase tracking-widest font-bold">Payment Accounts</span>
                                    <div className="flex-1 h-px bg-[var(--app-surface-hover)]" />
                                </div>
                            )}
                                ...
                                return (
                                    <div key={entry.account_id} className={clsx('grid gap-2 items-center', managerUnlocked ? 'grid-cols-[1fr_90px_90px_70px_80px]' : 'grid-cols-[1fr_110px]')}>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-bold text-[var(--app-foreground)] truncate">{entry.name}</span>
                                        </div>
                                        {managerUnlocked && <input type="number" value={entry.software} onChange={e => { const c = [...reconEntries]; c[idx] = { ...c[idx], software: e.target.value }; setReconEntries(c); }} className="w-full px-2 py-1.5 bg-[var(--app-surface-hover)] border border-[var(--app-border)] rounded-lg text-[var(--app-foreground)] text-xs font-bold text-center outline-none focus:border-[var(--app-primary-strong)]" />}
                                        ...
                                        {managerUnlocked && <span className={clsx('text-xs font-black text-center', cal > 0 ? 'text-[var(--app-success)]' : cal < 0 ? 'text-[var(--app-error)]' : 'text-[var(--app-muted-foreground)]')}>{cal !== 0 ? (cal > 0 ? '+' : '') + cal.toFixed(0) : '—'}</span>}
                            ...
                            {managerUnlocked ? (
                                <div className="bg-[var(--app-surface-2)] rounded-xl p-4 space-y-1.5 border border-[var(--app-border)]/50">
                                    <p className="text-[10px] text-[var(--app-muted-foreground)] uppercase tracking-widest font-bold mb-2">Calibration Summary</p>
                                    {[['Cash (software)', `${currency}${cashSW.toFixed(0)}`, ''], ['Calibration', `${totalCalibration > 0 ? '+' : ''}${(-totalCalibration).toFixed(0)}`, totalCalibration > 0 ? 'text-[var(--app-error)]' : totalCalibration < 0 ? 'text-[var(--app-success)]' : 'text-[var(--app-muted-foreground)]'], ['Cash expected', `${currency}${cashExpected.toFixed(0)}`, 'text-[var(--app-foreground)] font-black'], ['Counted (real)', `${currency}${cashRL.toFixed(0)}`, ''], ['FINAL GAP', `${finalGap > 0 ? '+' : ''}${currency}${finalGap.toFixed(0)}${finalGap > 0 ? ' EXCESS' : finalGap < 0 ? ' SHORT' : ''}`, finalGap > 0 ? 'text-[var(--app-success)] font-black text-sm' : finalGap < 0 ? 'text-[var(--app-error)] font-black text-sm' : 'text-[var(--app-muted-foreground)]']].map(([l, v, c]) => (
                                        <div key={l as string} className="flex justify-between text-xs"><span className="text-[var(--app-muted-foreground)]">{l as string}</span><span className={c as string || 'text-[var(--app-foreground)] font-bold'}>{v as string}</span></div>
                                    ))}
                                </div>
                            ) : (
                                <div className="bg-[var(--app-surface-2)] rounded-xl p-4 text-center border border-[var(--app-border)]/50">
                                    ...
                                    {finalGap !== 0 && <p className="text-xs text-[var(--app-muted-foreground)] mt-2">Manager has been notified</p>}
                                </div>
                            )}

                            <textarea value={notes} onChange={e => setNotes(e.target.value)} className="w-full px-4 py-2.5 bg-[var(--app-surface-hover)] border border-[var(--app-border)] rounded-xl text-[var(--app-foreground)] text-sm outline-none focus:border-violet-400 resize-none h-14" placeholder="Reconciliation notes..." />
                            <button onClick={handleOpen} disabled={loading} className="w-full py-4 rounded-xl bg-gradient-to-r from-violet-500 via-indigo-500 to-violet-600 text-[var(--app-foreground)] font-black text-lg shadow-2xl shadow-violet-500/30 hover:scale-[1.01] active:scale-[0.99] transition-all flex items-center justify-center gap-3 disabled:opacity-50">
