...
export default function ReturnOrderModal({ currency, onClose }: ReturnOrderModalProps) {
 ...
 const [result, setResult] = useState<{ return_ref: string; total_returned: number } | null>(null);
 ...
 if (res?.return_ref) {
 setResult({ return_ref: res.return_ref, total_returned: res.total_returned });
 setStep('done');
 toast.success(`Return ${res.return_ref} processed`);
 } else {
