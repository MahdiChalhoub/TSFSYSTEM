...
export default function POSQuickHold({
 ...
 <div
 className="absolute top-9 right-4 w-72 bg-app-surface border border-app-foreground/10 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-top-2 duration-200"
 ...
 {holds.length === 0 ? (
 <div className="py-10 text-center text-app-foreground/20 text-xs flex flex-col items-center gap-2">
 <ShoppingBag size={24} strokeWidth={1} />
 <span>No held carts yet</span>
 <span className="text-app-foreground/15">Press Hold Current or use Ctrl+H</span>
 </div>
 ) : (
 ...
 <p className="text-app-foreground text-xs font-bold truncate">{hold.label}</p>
 <div className="flex items-center gap-2 mt-0.5">
 <span className="text-app-foreground/40 text-[10px] flex items-center gap-1"><Clock size={8} />{fmtTime(hold.savedAt)}</span>
 <span className="text-app-foreground/40 text-[10px]">·</span>
 <span className="text-app-foreground/40 text-[10px] truncate">{hold.clientName}</span>
 </div>
 </div>
 ...
 <div className="px-4 py-2 border-t border-app-foreground/5 text-app-foreground/20 text-[10px] text-center">
...
export function QuickHoldButton({ orgKey, currency, cart, totalAmount, selectedClientId, selectedClientName, onRestoreHold, onCreateNewSession }: POSQuickHoldProps) {
 ...
 <div className="fixed inset-0 z-[150]" onClick={() => setIsOpen(false)}>
 <div
 className="absolute mt-1 w-72 bg-app-surface border border-app-foreground/10 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-top-2 duration-200"
 ...
 {holds.length === 0 ? (
 <div className="py-8 text-center text-app-foreground/20 text-xs flex flex-col items-center gap-1">
 ...
 {holds.map(hold => (
 <button key={hold.id} onClick={(e) => handleRestore(hold, e)} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-app-foreground/5 text-left group transition-all">
 <ShoppingBag size={14} className="text-indigo-400 shrink-0" />
 <div className="flex-1 min-w-0">
 <p className="text-app-foreground text-xs font-bold">{hold.label}</p>
 <p className="text-app-foreground/30 text-[10px]">{hold.clientName} · {fmtTime(hold.savedAt)}</p>
 </div>
 <div className="flex flex-col items-end gap-1">
