...
export function AccountBook({ isOpen, onClose, sessionId, cashierId, currency, isManager = false }: AccountBookProps) {
 ...
 <span className={clsx(
 "text-sm font-black",
 summary.pendingBalance >= 0 ? 'text-app-success' : 'text-rose-700'
 )}>
 ...
 <span className={clsx(
 "text-sm font-black",
 summary.approvedBalance >= 0 ? 'text-app-success' : 'text-rose-700'
 )}>
 ...
 <span className="text-[8px] font-bold text-app-text-faint">
 ...
 <div className="space-y-2">
 <label className="text-[9px] font-black text-app-text-muted uppercase tracking-widest block mb-1.5">
 Expense Category
 </label>
 <input
 ...
 <span className="text-sm font-bold text-app-text">{form.partnerName}</span>
 <button
