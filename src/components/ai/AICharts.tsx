...
export function AIChart({ chart }: AIChartProps) {
 ...
 default:
 return <p className="text-app-text-muted">Unsupported chart type: {type}</p>
 }
 }
...
export function AIStrategyCard({ strategy }: { strategy: StrategyRecommendation }) {
 ...
 <span className="text-sm font-medium text-app-text-muted">{strategy.category}</span>
 </div>
 <h4 className="font-bold text-app-text mb-2">{strategy.action}</h4>
 <p className="text-sm text-app-text-muted">{strategy.impact}</p>
 {strategy.timeline && (
 <p className="text-xs text-app-text-faint mt-2">Timeline: {strategy.timeline}</p>
 )}
 </div>
 )
}
