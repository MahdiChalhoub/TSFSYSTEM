...
export function GroupedProductForm({ brands, categories, units, countries, initialGroup }: Props) {
    ...
    useEffect(() => {
        const filterBrands = async () => {
            ...
            try {
                ...
                if (master.brandId && !filtered.find((b: Record<string, any>) => b.id === Number(master.brandId))) {
    ...
    const updateVariant = (id: number, field: keyof VariantRow, value: string | number) => {
