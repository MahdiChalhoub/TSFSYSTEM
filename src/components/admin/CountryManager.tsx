...
export function CountryManager({ countries, categories = [] }: CountryManagerProps) {
 ...
 <div>
 <h1 className="text-4xl font-bold text-app-text tracking-tight mb-2">Countries</h1>
 <p className="text-app-text-muted">Manage manufacturing origins.</p>
 </div>
 <button
 ...
 <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 pb-4 border-b border-app-border">
 <div className="flex flex-1 items-center gap-3 w-full">
 <div className="relative flex-1 max-w-md">
 <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-app-text-faint" size={18} />
 <input
 type="text"
 ...
 <Filter className="absolute left-3 top-1/2 -translate-y-1/2 text-app-text-faint" size={14} />
 </div>
...
function CountryCard({ country, onEdit }: Record<string, any>) {
 return (
 <div className="group border border-app-border rounded-2xl p-6 hover:shadow-lg transition-all bg-app-surface relative overflow-hidden flex flex-col cursor-pointer hover:border-app-success" onClick={() => onEdit(country)}>
 ...
 <div className="flex items-center gap-1 text-xs font-semibold bg-app-surface-2 text-app-text-muted px-2 py-1 rounded-lg">
 <Package size={12} />
 {country.product_count || 0} products
 </div>
 </div>
 <h3 className="text-lg font-bold text-app-text mb-1">{country.name}</h3>
 </div>
 );
}
...
function CountryRow({ country, onEdit }: Record<string, any>) {
 ...
 <div>
 <h3 className="font-bold text-app-text text-lg">{country.name}</h3>
 <span className="text-xs font-mono text-app-text-faint">{country.product_count || 0} products</span>
 </div>
 </div>
 <button onClick={(e) => { e.stopPropagation(); onEdit(country) }} className="p-2 text-app-text-faint hover:text-app-info hover:bg-app-info-bg rounded-lg transition-colors"><Edit2 size={16} /></button>
 </div>
 ...
 <div key={brand.id} className="bg-app-surface border border-app-border rounded-xl overflow-hidden shadow-sm">
 <div className="px-4 py-2 bg-gradient-to-r from-gray-50 to-white border-b border-app-border flex justify-between items-center">
 <div className="flex items-center gap-2">
 <Factory size={14} className="text-purple-500" />
 <span className="font-bold text-sm text-app-text">{brand.name}</span>
 </div>
 <span className="text-xs font-bold text-app-success bg-app-primary-light px-2 py-0.5 rounded-full">Stock: {brand.totalStock}</span>
 </div>
 ...
 <span className="text-app-text-muted font-medium">{p.name} {p.size && `- ${p.size}${p.unit ? p.unit.name : ''}`}</span>
 {p.sku && <span className="text-[10px] text-app-text-faint font-mono">{p.sku}</span>}
 </div>
 </div>
 ...
 {data.length === 0 && <p className="text-app-text-faint italic text-sm p-2">No inventory found.</p>}
 </div>
 )}
 </div>
 )}
...
function CountryFormModal({ isOpen, onClose, country }: { isOpen: boolean, onClose: () => void, country?: Record<string, any> }) {
 ...
 <div className="bg-app-surface rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200">
 ...
 <label className="text-xs font-bold text-app-text-muted uppercase tracking-wide">Name</label>
 <input name="name" defaultValue={country?.name} className="input-field w-full" placeholder="Turkey" required />
 </div>
 <div className="space-y-1">
 <label className="text-xs font-bold text-app-text-muted uppercase tracking-wide">Code</label>
 ...
 <button type="submit" disabled={pending} className="flex-1 py-2 rounded-xl bg-app-primary text-app-text font-semibold flex items-center justify-center gap-2">
