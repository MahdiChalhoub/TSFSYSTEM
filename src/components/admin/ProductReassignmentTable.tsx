...
export function ProductReassignmentTable({ products, categories, currentCategoryId }: Props) {
 ...
 return (
 <div className="flex flex-col items-center justify-center h-full text-app-text-faint p-12">
 ...
 {/* Toolbar */}
 <div className="p-4 bg-app-surface border-b border-app-border flex items-center justify-between sticky top-0 z-10 shadow-sm">
 ...
 <div className="w-10 h-10 bg-app-surface-2 rounded-lg flex items-center justify-center text-xs font-bold text-app-text-faint">
 IMG
 </div>

 <div className="flex-1 min-w-0">
 <h4 className="font-medium text-app-text truncate">{product.name}</h4>
 <div className="text-xs text-app-text-muted flex gap-2">
 {product.brand_name && <span className="bg-app-surface-2 px-1.5 rounded">{product.brand_name}</span>}
 {product.unit_name && <span className="text-app-text-faint">{product.unit_name}</span>}
 </div>
 </div>
 </div>
 ))}
