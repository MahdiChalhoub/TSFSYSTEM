...
export function CategoryTree({ categories, allCategories = [], authToken }: { categories: CategoryNode[], allCategories?: Record<string, any>[], authToken?: string }) {
 ...
 <h3 className="text-xl font-bold text-app-text-faint">No categories defined</h3>
 <p className="text-app-text-faint mt-1 max-w-xs mx-auto">Create a main category to organize your catalog.</p>
 </div>
 );
 }
...
const CategoryTreeNode = memo(function CategoryTreeNode({
 ...
 isExpanded ? "bg-orange-50 text-orange-600 rotate-0" : "bg-app-bg text-app-text-faint -rotate-90",
 ...
 : "bg-app-surface-2 text-app-text-faint ring-4 ring-gray-100/30"
 )}>
 ...
 <h4 className="font-extrabold text-app-text text-lg tracking-tight group-hover/title:text-orange-600 transition-colors flex items-center gap-2">
 {category.name}
 ...
 <p className="text-sm text-app-text-muted mt-1 flex items-center gap-3">
