...
export function AttributeManager({ attributes = [], categories = [] }: AttributeManagerProps) {
 ...
 <div>
 <h1 className="text-4xl font-bold text-app-text tracking-tight mb-2">Attributes</h1>
 <p className="text-app-text-muted">Manage scents, flavors, and other variant attributes.</p>
 </div>
 <button
...
function AttributeCard({ attribute, onEdit }: Record<string, any>) {
 ...
 <div className="text-xs font-semibold bg-app-surface-2 text-app-text-muted px-2 py-1 rounded-lg">
 {attribute.product_count || 0} products
 </div>
 </div>
 <h3 className="text-lg font-bold text-app-text mb-1">{attribute.name}</h3>
 {attribute.short_name && <p className="text-sm text-app-text-muted">{attribute.short_name}</p>}
...
function AttributeRow({ attribute, onEdit }: Record<string, any>) {
 ...
 <div>
 <h3 className="font-bold text-app-text text-lg">{attribute.name}</h3>
 <div className="flex gap-2 text-xs text-app-text-muted">
 {attribute.short_name && <span className="font-mono bg-app-surface-2 px-1 rounded">{attribute.short_name}</span>}
 ...
 {data.length === 0 && <p className="text-app-text-faint italic text-sm p-2">No items found with this attribute.</p>}
 </div>
 )}
 </div>
 )}
