...
export function BrandManager({ brands, countries, categories }: BrandManagerProps) {
 ...
 <div>
 <h1 className="text-4xl font-bold text-app-text tracking-tight mb-2">Brands</h1>
 <p className="text-app-text-muted">Manage manufacturers and product brands.</p>
 </div>
 ...
 <Globe className="absolute left-3 top-1/2 -translate-y-1/2 text-app-text-faint" size={14} />
 </div>
 </div>
...
function BrandCard({ brand, onEdit }: Record<string, any>) {
 ...
 <div key={c.id} className="flex items-center gap-1 text-xs bg-app-bg border border-app-border px-1.5 py-0.5 rounded text-app-text-muted">
 <Globe size={10} className="text-app-text-faint" />
 <span>{c.code}</span>
 </div>
 ))
 ) : (
 <span className="text-xs text-app-text-faint italic">No Origin</span>
 )}
 </div>
 </div>
 ...
 <span className="text-[10px] text-app-text-faint italic">Universal</span>
 )}
 {brand.categories && brand.categories.length > 3 && <span className="text-[10px] text-app-text-faint">+{brand.categories.length - 3}</span>}
 </div>
 </div>
 </div>
 );
...
function BrandRow({ brand, onEdit }: Record<string, any>) {
 ...
 <div>
 <div className="flex items-center gap-2">
 <h3 className="font-bold text-app-text text-lg">{brand.name}</h3>
 <span className="text-xs font-mono text-app-text-faint">({brand.product_count || 0})</span>
 </div>
 <div className="flex gap-2">
 {brand.countries?.map((c: Record<string, any>) => <span key={c.id} className="text-[10px] bg-app-surface-2 text-app-text-muted px-1.5 py-0.5 rounded">{c.code}</span>)}
 </div>
 </div>
 </div>
 ...
 <div>
 <div className="flex items-center gap-2">
 <span className="font-bold text-app-text">{group.name}</span>
 {group.products.length > 0 && <span className="text-[10px] font-bold text-app-text-faint border border-app-border px-1.5 rounded-full bg-app-surface">{group.products.length} VARS</span>}
 </div>
 </div>
 </div>
 <div className="flex items-center gap-3">
 ...
 <span className="text-sm font-medium text-app-text">{p.name} {p.size && `- ${p.size}${p.unitName}`}</span>
 {p.sku && <span className="text-[10px] text-app-text-faint font-mono">SKU: {p.sku}</span>}
 </div>
 </div>
 ...
 <div className="bg-app-surface border border-app-border shadow-sm rounded-xl overflow-hidden mt-4">
 <div className="px-4 py-2 bg-app-bg border-b border-app-border flex items-center gap-2">
 <Package size={14} className="text-app-text-faint" />
 <span className="text-xs font-bold text-app-text-muted uppercase tracking-wider">Ungrouped Items</span>
 </div>
 <div className="divide-y divide-gray-50">
 {data.looseProducts.map((p: Record<string, any>) => (
 <div key={p.id} className="px-4 py-3 flex justify-between items-center hover:bg-app-bg transition-colors">
 <div className="flex items-center gap-3">
 ...
 <p className="text-sm text-app-text-faint">No products found for this brand.</p>
 </div>
 )}
 </div>
