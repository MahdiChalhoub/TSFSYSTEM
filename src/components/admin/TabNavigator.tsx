...
export function TabNavigator() {
 ...
 return (
 <div className="flex items-end px-3 bg-gray-50/80 border-b border-app-border shrink-0 relative h-[42px]">
 ...
 className={clsx(
 "h-8 w-8 flex items-center justify-center rounded-lg bg-app-surface border border-app-border text-app-text-faint hover:text-emerald-600 hover:border-emerald-200 transition-all shadow-sm hover:shadow-md",
 ...
 <button
 onClick={clearTabs}
 className="pb-3.5 ml-4 text-[10px] font-black text-app-text-faint hover:text-red-500 uppercase tracking-widest transition-colors"
 >
 Clear All
 </button>
 )}
...
const TabItem = React.memo(({ tab, isActive, onOpen, onClose }: Record<string, any>) => {
 ...
 : "hover:bg-gray-200 text-app-text-faint hover:text-app-text-muted"
 )}
 >
 <X size={14} strokeWidth={3} />
 </button>
