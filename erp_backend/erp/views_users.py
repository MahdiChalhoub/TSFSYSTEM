...
class UserViewSet(TenantModelViewSet):
    ...
    # ── Safe delete: soft-deactivate if user has transactions ──
    def destroy(self, request, *args, **kwargs):
        """
        Delete a user safely.
        If the user has related Orders, JournalEntryLines, or POS sessions,
        soft-deactivate (set account_status=BLOCKED) instead of hard-deleting.
        """
        user = self.get_object()
        ...
        try:
            ...
            order_count = Order.objects.filter(
                Q(created_by=user) | Q(cashier=user),
                organization=user.organization
            ).count()
            if order_count:
                has_transactions = True
                reasons.append(f"{order_count} order(s)")
        except (ImportError, Exception):
            pass

        # Check journal entries (finance)
        try:
            if JournalEntryLine:
                jel_count = JournalEntryLine.objects.filter(
                    entry__created_by=user,
                    entry__organization=user.organization
                ).count()
                if jel_count:
                    has_transactions = True
                    reasons.append(f"{jel_count} journal entry line(s)")
        except Exception:
            pass
        ...
        try:
            ...
            session_count = POSSession.objects.filter(
                user=user, organization=user.organization
            ).count()
            if session_count:
                has_transactions = True
                reasons.append(f"{session_count} POS session(s)")
        except (ImportError, Exception):
            pass
        ...
        if has_transactions:
            ...
            return Response({
                "message": f"User deactivated instead of deleted (has {reason_str})",
                "soft_deleted": True,
                "reasons": reasons,
    ...
    @action(detail=True, methods=['get'], url_path='linked-contacts')
    def linked_contacts(self, request, pk=None):
        """Get CRM contacts linked to this user via IAM portal access."""
        user = self.get_object()
        try:
            from apps.iam.models import ContactPortalAccess
            accesses = ContactPortalAccess.objects.filter(
                user=user
            ).select_related('contact')
            data = [{
                'access_id': a.id,
                'contact_id': a.contact_id,
                'contact_name': str(a.contact) if a.contact else None,
                'portal_type': a.portal_type,
                'status': a.status,
                'relationship_role': a.relationship_role,
            } for a in accesses]
            return Response(data)
        except (ImportError, Exception):
            return Response([])
...
class PermissionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    pagination_class = None  # Permissions are a fixed catalogue — return all at once
