...
class GlobalCurrency(models.Model):
    ...
    symbol = models.CharField(max_length=10) # $, Γé¼
...
class Organization(models.Model):
    ...
    current_plan = models.ForeignKey('SubscriptionPlan', on_delete=models.SET_NULL, null=True, blank=True, related_name='main_organizations')
...
class PlanCategory(models.Model):
    ...
    parent = models.ForeignKey('self', on_delete=models.CASCADE, null=True, blank=True, related_name='children_main')
    country = models.ForeignKey('Country', on_delete=models.SET_NULL, null=True, blank=True)
    allowed_organizations = models.ManyToManyField('Organization', blank=True, related_name='available_plan_categories_main')

    class Meta:
        db_table = 'PlanCategory_main' # Temporary or distinct? I'll use same if possible but let's stick to the names.
...
class SubscriptionPlan(models.Model):
    category = models.ForeignKey(PlanCategory, on_delete=models.CASCADE, related_name='plans_main')
    name = models.CharField(max_length=255)
    description = models.TextField(null=True, blank=True)
    ...
    # Hierarchy
    upgrade_path = models.ManyToManyField('self', symmetrical=False, blank=True, related_name='downgrade_options_main')
    ...
    class Meta:
        db_table = 'SubscriptionPlan_main'
...
class SubscriptionPayment(models.Model):
    ...
    organization = models.ForeignKey('Organization', on_delete=models.CASCADE, related_name='subscription_payments_main')
    ...
    class Meta:
        db_table = 'SubscriptionPayment_main'
