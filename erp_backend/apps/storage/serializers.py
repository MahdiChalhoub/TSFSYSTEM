"""
Storage Module — serializers.py
"""
from rest_framework import serializers
from .models import StorageProvider, StoredFile, FILE_CATEGORIES


class StorageProviderSerializer(serializers.ModelSerializer):
    """Admin-facing serializer for storage provider configuration."""
    class Meta:
...
class StorageProviderReadSerializer(serializers.ModelSerializer):
    """Read-only serializer that masks credentials."""
...
class StoredFileSerializer(serializers.ModelSerializer):
    """Read serializer for stored files."""
...
class FileUploadSerializer(serializers.Serializer):
    """Serializer for validating file upload requests."""
    ...
    def validate_file(self, value):
        """Validate file size and extension against provider config."""
        ...
        if provider:
            ...
            if value.size > max_bytes:
                raise serializers.ValidationError(
                    f"File too large. Maximum size is {provider.max_file_size_mb} MB."
                )
            # Check extension
