...
class RegisterPaymentMethod(TenantModel):
    """
    Links a PaymentMethod to a specific register with an explicit financial account.
    Replaces the old JSONField approach for per-register method config.
    """
    register = models.ForeignKey(POSRegister, on_delete=models.CASCADE, related_name='register_methods')
    payment_method = models.ForeignKey(
        'finance.PaymentMethod', on_delete=models.CASCADE, related_name='register_links',
    )
    financial_account = models.ForeignKey(
        'finance.FinancialAccount', on_delete=models.SET_NULL,
        null=True, blank=True, related_name='register_method_links',
        help_text='Financial account for this payment method on this register'
    )
    is_active = models.BooleanField(default=True)
    ...
    def __str__(self):
        return f"{self.register.name} — {self.payment_method.code}"
