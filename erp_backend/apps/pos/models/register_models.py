...
class CashierAddressBook(TenantModel):
    """
    Manual offline ledger for transactions the cashier can't enter in the POS
    (e.g. security restrictions). Each entry is an in/out with description.
    Manager must review and approve before it counts in reconciliation.
    If entries are wrong, the cashier who logged them is responsible.
    """
    STATUS_CHOICES = (
    ...
    rejection_notes = models.TextField(null=True, blank=True,
        help_text='Reason for rejection by manager')
    ...
    def __str__(self):
        return f"{self.description}: in={self.amount_in} out={self.amount_out} [{self.status}]"
