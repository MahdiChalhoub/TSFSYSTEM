...
class Order(TenantModel):
    ...
    # ── FNE E-Invoicing (Côte d'Ivoire DGI Certification) ────────────
    fne_status = models.CharField(
        ...
        help_text='FNE certification status'
    )
    fne_reference = models.CharField(
        max_length=100, null=True, blank=True,
        help_text='FNE-assigned invoice reference (e.g. 2026TK00000001)'
    )
    fne_token = models.TextField(
        null=True, blank=True,
        help_text='FNE QR code token URL for printing on invoices'
    )
    fne_error = models.TextField(
        null=True, blank=True,
        help_text='Error message from last failed FNE certification attempt'
    )
    fne_invoice_id = models.CharField(
        max_length=200, null=True, blank=True,
        help_text='FNE platform invoice UUID (required for /refund credit note endpoint)'
    )
    fne_raw_response = models.JSONField(
        null=True, blank=True,
        help_text='Full FNE certification response including line item IDs (needed for credit note mapping)'
    )
...
class OrderLine(TenantModel):
    ...
    # ── Product Snapshot (frozen at transaction time) ──────────────────
