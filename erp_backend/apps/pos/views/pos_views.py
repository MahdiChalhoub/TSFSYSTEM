...
class POSViewSet(viewsets.ViewSet):
    ...
    def checkout(self, request):
        ...
        try:
            ...
            return Response({
                ...
                # FNE E-Invoice certification
                "fne_status": order.fne_status,
                "fne_reference": order.fne_reference,
                "fne_token": order.fne_token,
    ...
    def retry_fne(self, request, pk=None):
        ...
        try:
            organization = Organization.objects.get(id=organization_id)
            order = Order.objects.select_related('contact').prefetch_related('lines').get(
            ...
            for line in order.lines.all():
                items.append(FNELineItem(
                    description=getattr(line, 'product_name', '') or str(getattr(line, 'product_id', '')),
                    quantity=float(getattr(line, 'quantity', 1)),
                    amount=float(getattr(line, 'unit_price', 0)),
                    taxes=['TVA'],
                ))
            if not items:
                return Response({"error": "Order has no line items"}, status=400)

            # Determine template
            template = 'B2C'
            contact = order.contact
            if contact and getattr(contact, 'is_company', False):
                template = 'B2B'

            PAYMENT_MAP = {'CASH': 'cash', 'CARD': 'credit-card', 'CREDIT_CARD': 'credit-card',
                           'MOBILE_MONEY': 'mobile-money', 'WAVE': 'mobile-money', 'OM': 'mobile-money',
                           'BANK_TRANSFER': 'transfer', 'CHEQUE': 'cheque'}
            fne_payment = PAYMENT_MAP.get(order.payment_method or 'CASH', 'cash')

            fne_request = FNEInvoiceRequest(
                invoice_type='sale', payment_method=fne_payment, template=template, items=items,
                client_company_name=getattr(contact, 'name', '') if contact else '',
            ...
            response = service.sign_invoice(fne_request)

            if response.success:
                order.fne_status = 'CERTIFIED'
                order.fne_reference = response.reference
                order.fne_token = response.token
                order.fne_error = ''
                order.fne_invoice_id = response.invoice_id or ''
                order.fne_raw_response = response.raw_response or {}
                order.save(update_fields=[
                    'fne_status', 'fne_reference', 'fne_token', 'fne_error',
                    'fne_invoice_id', 'fne_raw_response'
                ])
                return Response({
                    "success": True, "fne_status": "CERTIFIED",
                    "fne_reference": response.reference, "fne_token": response.token,
                })
            else:
                order.fne_status = 'FAILED'
                order.fne_error = response.error_message
                order.save(update_fields=['fne_status', 'fne_error'])
                return Response({
                    "success": False, "fne_status": "FAILED",
