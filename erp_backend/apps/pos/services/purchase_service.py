...
class PurchaseService:
    ...
    def quick_purchase(organization, supplier_id, warehouse_id, site_id, scope, invoice_price_type, vat_recoverable, lines, notes=None, ref_code=None, user=None, **kwargs):
        ...
        with transaction.atomic():
            ...
            custom_liabilities_map = {}      # {liability_account_id: Decimal amount}
                        ...
                        if tl.get('type') == 'CUSTOM' and tl.get('custom_tax_rule_id'):
                            try:
                                ...
                                if rule.liability_account_id:
                                    c_amt = Decimal(str(tl['amount']))
                                    custom_liabilities_map[rule.liability_account_id] = \
                                        custom_liabilities_map.get(rule.liability_account_id, Decimal('0')) + c_amt
                            except Exception:
                                pass
                except Exception:
