    ...
    def _validate_preset_data(self):
        ...
        for key in required_keys:
            if key not in self.preset_data:
                raise ValueError(f"preset_data must contain '{key}'")

        # Validate colors have dark and light variants
        if 'dark' not in self.preset_data['colors'] or 'light' not in self.preset_data['colors']:
            raise ValueError("preset_data.colors must have 'dark' and 'light' variants")
