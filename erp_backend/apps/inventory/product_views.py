...
class ProductViewSet(ProductBulkMixin, ProductAnalyticsMixin, ProductComboMixin, ProductStorefrontMixin, UDLEViewSetMixin, TenantModelViewSet):
    ...
    filterset_fields = ['category', 'brand', 'product_type', 'is_active', 'tracks_serials',
                        'data_completeness_level', 'completeness_status']
    ...
    def completeness_summary(self, request):
        """Returns count of products per completeness level for the current org."""
        ...
        summary = (
            ...
            .values('data_completeness_level', 'completeness_status')
            .annotate(count=Count('id'))
            .order_by('data_completeness_level')
        )

        total = Product.objects.filter(organization=org, is_active=True).count()
        levels = {row['data_completeness_level']: {
            'count': row['count'],
        ...
        return Response({
            'total': total,
            'levels': levels,
            'level_labels': dict(Product.DATA_COMPLETENESS_LEVELS),
        })
