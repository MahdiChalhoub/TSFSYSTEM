...
class ContactViewSet(TenantModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Optional result count limit — apply ordering BEFORE slicing
    ...
    def create(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            if not parent_account_id:
                ...
                ChartOfAccount = connector.require('finance.accounts.get_model', org_id=organization.id, source='crm.contacts')
                if ChartOfAccount:
            ...
            if parent_account_id:
                ChartOfAccount = ChartOfAccount or connector.require('finance.accounts.get_model', org_id=organization.id, source='crm.contacts')
                LedgerService = connector.require('finance.services.get_ledger_service', org_id=organization.id, source='crm.contacts')
    ...
    def detail_summary(self, request, pk=None):
        ...
        try:
            ...
            try:
                Order = connector.require('pos.orders.get_model', org_id=organization_id, source='crm.contacts')
            except (ImportError, Exception):
                Order = None
            if not Order:
                return Response({"error": "POS module not available"}, status=503)
            ...
            try:
                Payment = connector.require('finance.payments.get_model', org_id=organization_id, source='crm.contacts')
            except (ImportError, Exception):
                Payment = None
            if not Payment:
            ...
            if contact.type == 'CUSTOMER':
                CustomerBalance = connector.require('finance.payments.get_customer_balance_model', org_id=organization_id, source='crm.contacts')
                if CustomerBalance:
                    bal_obj = CustomerBalance.objects.filter(
                        organization_id=organization_id, contact=contact
                    ).first()
            else:
                SupplierBalance = connector.require('finance.payments.get_supplier_balance_model', org_id=organization_id, source='crm.contacts')
                if SupplierBalance:
            ...
            if contact.linked_account:
                JournalEntryLine = connector.require('finance.journal.get_line_model', org_id=organization_id, source='crm.contacts')
                if JournalEntryLine:
    ...
    def _build_analytics(self, contact, orders, order_stats):
        ...
        try:
            Order = connector.require('pos.orders.get_model', org_id=organization_id, source='crm.contacts')
            OrderLine = connector.require('pos.order_lines.get_model', org_id=organization_id, source='crm.contacts')
            if OrderLine:
...
class ContactTagViewSet(TenantModelViewSet):
    """CRUD for contact tags (labels used to categorize contacts)."""
...
class ContactPersonViewSet(TenantModelViewSet):
    """CRUD for contact persons (individuals associated with a business contact)."""
