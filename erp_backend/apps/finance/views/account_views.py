...
class ChartOfAccountViewSet(UDLEViewSetMixin, TenantModelViewSet):
    ...
    def apply_template(self, request):
        ...
        try:
            ...
            # ── Auto-sync posting rules from the same template ──
            posting_synced = 0
            try:
                from apps.finance.models.coa_template import COATemplate, COATemplatePostingRule
                ...
                if tpl:
                    ...
                    org_accts = {a.code: a for a in ChartOfAccount.objects.filter(organization=organization)}
                ...
                import logging
                logging.getLogger(__name__).warning("Posting rules auto-sync warning: %s", exc)

            return Response({
                "message": f"Template applied successfully. {posting_synced} posting rules synced.",
