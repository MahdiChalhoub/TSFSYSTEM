        ...
        for org in org_qs:
            ...
            for period in periods:
                ...
                created_here, skipped_here = self._seed_one_period(
                    ...
                    kinds=KINDS, n=n_per_period, dry=dry,
                )
                created_total += created_here
                skipped_total += skipped_here

            msg = (f'[{org}] {"DRY-RUN: would create" if dry else "Created"} '
                   f'{created_total} JE(s); skipped {skipped_total} (duplicate ref or no valid account pair).')
            self.stdout.write(self.style.SUCCESS(msg))

    # ── One period ───────────────────────────────────────────────────────
    def _seed_one_period(self, *, org, period, accounts, kinds, n, dry):
        ...
        for i in range(n):
            ...
            ref = f'SEED-{org.id}-P{period.id}-{i+1:02d}'
            ...
            if dry:
                self.stdout.write(
                    f'  · would create {ref}  {tx_date_d.isoformat()}  '
            ...
            try:
                with db_tx.atomic():
                    ...
                    je = JournalEntry.objects.create(
                        organization=org,
                        description=f'{base_desc} — {period.name}',
                        reference=ref,
        ...
        if created or skipped:
            self.stdout.write(
                f'  [{period.fiscal_year.name} / {period.name}]  '
