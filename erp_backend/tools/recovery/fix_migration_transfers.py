...
def run_transfers():
    ...
    for row in rows:
        ...
        try:
            ...
            from_wh = id_map.get(loc_id)
            # Find destination Site ID
            # In UPOS, sell_transfer location_id is From, transfer_parent_id transaction has To
            to_wh = None
            if parent_id:
                # We need to find the location_id of the parent transaction from the SQL
                # For simplicity, if we are in the same business, we might find it in loc_rows
                pass
                
            if not from_wh: continue
            ...
            transfer = StockTransferOrder.objects.create(
                ...
                to_warehouse_id=to_wh or from_wh, # Fallback to same site if destination unknown
            ...
            for l in tx_lines:
                ...
                if not target_prod: continue
                
                StockTransferLine.objects.create(
                    organization_id=org_id,
                    transfer_id=transfer.id,
                    product_id=target_prod,
                    quantity=safe_decimal(l.get('quantity')),
                    cost_price=safe_decimal(l.get('unit_price')),
                )
                line_count += 1
