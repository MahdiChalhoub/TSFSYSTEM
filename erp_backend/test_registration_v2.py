import requests
import json
import uuid
import os

# Create a dummy logo file
with open("dummy_logo.jpg", "wb") as f:
    f.write(b"")

def test_business_registration_v2():
    url = "http://localhost:8000/api/auth/register/business/"
    
    unique_suffix = str(uuid.uuid4())[:8]
    safe_name = f"Vantage{unique_suffix}"
    slug = f"vantage-{unique_suffix}"
    
    # We use files for Multipart upload
    files = {
        'logo': ('logo.jpg', open('dummy_logo.jpg', 'rb'), 'image/jpeg')
    }
    
    data = {
        "business_name": safe_name,
        "slug": slug,
        "business_type_id": 1, 
        "currency_id": 1,      
        "email": f"ceo@{safe_name}.com",
        "phone": "+1555000999",
        "website": f"https://{slug}.com",
        "address": "123 Innovation Blvd",
        "city": "Metropolis",
        "state": "NY",
        "zip_code": "10001",
        "country": "USA",
        "timezone": "UTC",
        
        # Admin
        "admin_first_name": "Alice",
        "admin_last_name": "Director",
        "admin_username": f"admin_{unique_suffix}",
        "admin_password": "StrongPassword!23",
        "admin_email": f"admin@{safe_name}.com"
    }
    
    print(f"TEST: Registering Business '{safe_name}' with Logo...")
    try:
        # Note: 'data' is passed to 'data' in requests.post for multipart/form-data (fields), 'files' for files
        response = requests.post(url, data=data, files=files) 
        
        if response.status_code == 201:
            print("[PASS] Registration Successful")
            print(json.dumps(response.json(), indent=2))
        else:
            print(f"[FAIL] Status Code: {response.status_code}")
            try:
                print(json.dumps(response.json(), indent=2))
            except:
                print(response.text)
                
    except Exception as e:
        print(f"[ERROR] Connection or Script Error: {e}")
    finally:
        # Clean up
        files['logo'][1].close()
        if os.path.exists("dummy_logo.jpg"):
            os.remove("dummy_logo.jpg")

if __name__ == "__main__":
    test_business_registration_v2()
