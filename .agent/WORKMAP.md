...
### [PARTIAL DONE 2026-05-01] Maintainability Phase 5 — Frontend Type Safety
- **Discovered**: 2026-04-30
- **Impact (cumulative across three sessions)**: Repo-wide `any` count: 2,812 → **2,329** (−483 net). `// @ts-nocheck` directives: 194 → **165** (−29 cleared this session). Session 1 cleared `src/types/erp.ts` (-93). Session 2 cleared 24 high-density consumer files (-322). Session 3 (2026-05-01) dropped `@ts-nocheck` on 28 files in `crm/`, `hr/`, `inventory/categories/`, `inventory/combo/`, `ecommerce/themes/`, `actions/plm-governance.ts`, `actions/commercial/payment-terms.ts` and added proper types.
...
- **Verification**: `npx tsc --noEmit` clean baseline before sweep (exit 0), 0 new errors after Sessions 3, 4, 5, 6, 7, 8, 9, and 10 (exit 0). Sweep itself is byte-symmetric: pure class-name swaps + atomic css-utility additions.
...
- **Precursors documented**: missing `ring-app-error`, **`--app-accent` precursor RESOLVED Session 4**, **gradient utilities (`bg-app-gradient-*` × 14, both `-bold` and `-soft` tiers) RESOLVED Session 10**. Missing `--app-accent` cyan variant for `(auth)/register` + `tenant/[slug]/quote` + `tenant/[slug]/account/page` still pending. **Session 7 nuance**: `app-{success,info,warning,error}-strong` tokens do NOT exist (only `app-accent-strong` and `app-primary-dark` exist as the "darker" variant) — Session 7 used `hover:opacity-90` as the standard hover-darken pattern for status-tinted CTAs (as in `bg-app-info hover:opacity-90`). ~2,900 hex/rgb literals in inline styles + ~21 opacity-modified gradient stops still pending separate phase. Aware tokens for opacity-modified colors (`bg-rose-500/10` toast bgs, `bg-blue-500/10` icon-tile glows) would unblock more sweeping.
- **Remaining**: All originally-enumerated `(privileged)/*` and `(auth)/*` and `tenant/*` and `supplier-portal/*` subdirs covered. **Plain (non-opacity) gradients are exhausted in all in-scope subdirs**. Hex/rgb literal phase (the 4 module `accentColor` props + ~2,900 inline-style hex values across the codebase) + opacity-modified gradient stops (~21 across in-scope subdirs) remain as future phases.
