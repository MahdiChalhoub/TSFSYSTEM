...
export default function FiscalYearsDetailPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const data = await erpFetch(`finance/fiscal-years/${id}/`)
      setItem(data)
    } catch (error) {
  ...
  async function handleDelete() {
    ...
    try {
      await erpFetch(`finance/fiscal-years/${id}/`, {
        method: 'DELETE'
      })
      router.push('/finance/fiscal-years')
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-black theme-text">
              {item.name || item.title || `Fiscal-Years #${item.id}`}
            </h1>
            <p className="theme-text-muted mt-1">
              View and manage fiscal years details
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/finance/fiscal-years/${id}/edit`)}
            >
