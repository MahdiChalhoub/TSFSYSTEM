...
const getBadgeStyle = (badgeRaw?: string) => {
    if (!badgeRaw) return null;
    const badge = badgeRaw.toUpperCase();

    let colorClass = "bg-emerald-500 text-white"; // default
    if (badge === 'NEW') colorClass = "bg-blue-500 text-white";
    else if (badge === 'PENDING') colorClass = "bg-amber-500 text-amber-50";
    else if (badge === 'REVIEW') colorClass = "bg-purple-600 text-white";
    else if (badge === 'LOCKED') colorClass = "bg-rose-600 text-white";
    else if (badge === 'FINAL') colorClass = "bg-slate-800 text-emerald-400 border border-emerald-500/30 shadow-inner";

    return { text: badge, colorClass };
};
    ...
    return (
        <div className={level > 0 ? "mt-1" : "mt-2"}>
            ...
            >
                ...
                {item.badge && getBadgeStyle(item.badge) && (
                    <span className={clsx("text-[9px] font-black px-1.5 py-0.5 rounded-md tracking-wider uppercase shadow-sm", getBadgeStyle(item.badge)?.colorClass)}>
                        {getBadgeStyle(item.badge)?.text}
                    </span>
                )}
